	...
	fetch: (event) => {
		...
		if (url.host == HOST) {
			if (pathname == "/_") {
				event.respondWith(
					(async () => {
						const hostUrl = (suffix) => (host) => `${https}//${host}/${suffix}`,
							cdn_v = split("{cdn.v}"),
							cdn_v_len = cdn_v.length,
							retryFetch = async (func) => {
								let n = 1,
									url = func(cdn_v[0])
								...
								while (true) {
									try {
										const r = await get(now, req, new URL(url))
										if (r.status == 200) {
											return await r.text()
										}
									} catch (e) {
							...
							org_i = "@{pkg.i}/",
							site = "site",
							mdv = retryFetch(hostUrl(pkg_md)),
							[sitev, _18x, _18x_v] = split(
								await retryFetch(hostUrl(org_i + site)),
							),
							pkg_site = site + at + sitev,
							pkg_18x = _18x + at + _18x_v,
							_V = `${NPM}${pkg_md}`,
							fStr = async (url) =>
								(await get(now, new Request(url), new URL(url))).text(),
		...
		event.respondWith(
			caches.match(req).then((res) => {
				const go = () =>
					get(now, req, url).catch((err) => logerr(url + "", err))
