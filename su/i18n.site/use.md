...
Saatos ngajalankeun, tarjamahan sareng file cache bakal dibangkitkeun Mangga émut pikeun `git add . ` kana gudang dina diréktori `md` .
...
`dev` nyaéta singketan tina `development` , nuduhkeun lingkungan pangwangunan, dipaké pikeun sawangan lokal, sarta ogé mangrupa file konfigurasi standar.
`ol` nyaéta singketan tina `online` , `-n` lingkungan online, anu dianggo pikeun pelepasan `npm` .

Anjeun ogé tiasa nyiptakeun file konfigurasi anu sanés Anggo `--htm_conf` dina baris paréntah pikeun nangtukeun nami file konfigurasi anu dianggo:
...
Nyebarkeun eusi ka [npmjs.com](//npmjs.com) nyaéta solusi standar anu disarankeun (tingali [Kasadiaan Luhur Front-end](/i18n.site/feature#ha) ).
...
Upami anjeun nganggo lingkungan integrasi kontinyu pikeun nyebarkeun, henteu kedah dipasang `nodejs` , ngan ukur nyalin ijin anu lebet sareng nyebarkeun `~/.npmrc` ka lingkungan.
...
Pikeun eusi timer host, `CDN` ti ngagunakeun server anjeun sorangan, pilihan umum séjén nyaéta ngagunakeun `S3` +
...
Klik `i18n-demo` [di dieu github pikeun nyieun organisasi](https://github.com/account/organizations/new?plan=free) .
...
Dibandingkeun [cloudflare page](//pages.cloudflare.com) `github page` , éta nyayogikeun panulisan jalur sareng langkung ramah ka daratan Cina sareng langkung diaksés.
...
Upami anjeun hoyong kéngingkeun kamampuan aksés anu langkung saé di lingkungan jaringan daratan Cina, mangga [ngadaptarkeun nami domain](//beian.aliyun.com) heula.

Teras, pake panyimpen obyék ngical paralatan awan di daratan Cina + Nyebarkeun eusi di handap ieu `CDN` `out/ol/htm`
...
Tambahkeun nami `CNAME` `*.xxx.com` Alibaba `CDN` [CDN](https://cdn.console.aliyun.com/domain/list)
...
Punten tambahkeun konfigurasi `out/ol/htm` sami sareng di handap `/root/i18n/md/out/ol/htm` dina paragraf `server` nginx
