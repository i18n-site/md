...
windows , mangga [klik di dieu pikeun ngundeur tur masang `git bash`](https://git-scm.com/download/win) munggaran.
...
Anjeun tiasa ngonpigurasikeun `fromTo` anu béda pikeun subdirektori : béda /
...
Dina tabel konfigurasi ieu, basa sumber tarjamahan katalog `blog` nyaéta `zh` , sareng basa sumber tarjamahan `blog/your_file_name.md` nyaéta `ja` .
...
`ignore` ngagunakeun [globset](https://docs.rs/globset/latest/globset/#syntax) anu sami sareng file `.gitignore` .
