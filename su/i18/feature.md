...
Ngadukung tarjamahan HTML Markdown , eusi dina tag `<pre>` sareng `<code>` tina HTML anu diselapkeun dina `MarkDown` henteu ditarjamahkeun
...
## Kamampuhan Pikeun Narjamahkeun Komentar Dina Snippét Kode

Inline kode jeung kode snippét teu ditarjamahkeun, tapi komentar dina kode bisa ditarjamahkeun.

` ```rust ` tarjamahan kedah nunjukkeun basa saatos ` ``` ` , sapertos :
...
Ngarojong konfigurasi lulugu tina blog statik tipe [HOGO](https://github.com/gohugoio/hugo) , nu ngan narjamahkeun widang `title` , `summary` jeung `description` .
...
Cina, Jepang, sareng Korea teu gaduh hurup ageung sareng hurup leutik, tapi konvénsi pikeun judul Inggris nyaéta hurup ageung hurup kahiji.
...
Ditarjamahkeun kana basa `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` :

![](//p.3ti.site/1720199391.avif)

Ditarjamahkeun kana basa `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :
...
Ditarjamahkeun kana basa `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` :
