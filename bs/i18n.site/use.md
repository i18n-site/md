...
Ime direktorija klona baze koda `demo.i18n.site` mora biti `md` da bi se olakšao lokalni pregled sa `docker` .
...
Nakon pokretanja, prijevod i keš fajlovi će biti generisani. Molimo zapamtite da `git add . ` dodate u spremište u `md` direktorijumu.
...
Kada objavljujete na osnovu paketa `npm` , **obavezno koristite `//unpkg.com/` za** prefiks od `v:` `i18n.site` je posebno optimizirao vrijeme keširanja od `/.v` pod ovom putanjom prefiksa kako bi se postiglo pravovremeno gledanje novih izdanja.
...
Za samostalni hosting sadržaja, osim korištenja vlastitog servera, još jedna `CDN` opcija je korištenje `S3` +
...
Ulaznu stranicu na web stranicu potrebno je postaviti samo jednom i nema potrebe za ponovnom raspoređivanjem stranice za unos web stranice za naknadna ažuriranja sadržaja.
...
Zatim kreirajte skladište `i18n-demo.github.io` pod ovom organizacijom (molimo zamijenite `i18n-demo` imenom organizacije koju ste kreirali):
...
U poređenju sa `github page` , omogućava prepisivanje putanje i više je prijateljski [cloudflare page](//pages.cloudflare.com) prema kontinentalnoj Kini i preporučuje se za korišćenje.
...
Ako ste vezali skladište druge organizacije, možda ćete morati dvaput kliknuti `Add Account` za autorizaciju prije nego što se nova organizacija prikaže.
...
Zatim koristite skladište objekata dobavljača u oblaku u kontinentalnoj Kini `CDN` Postavite sljedeći sadržaj `out/ol/htm` +
...
Budući da zapis `MX` i zapis `CNAME` ne mogu koegzistirati, ako želite istovremeno primati e-poruke sa imenom domene, morate sarađivati sa [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten) `CNAME` u zapis `A` .
...
Dodajte naziv domene u [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) i pokažite naziv domene `*.xxx.com` do `CNAME` u Alibaba Cloud `CDN` !
