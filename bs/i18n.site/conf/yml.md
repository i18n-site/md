...
Gornji način pisanja je ekvivalentan `url: xxx` , a odgovarajući fajl je `en/xxx/TOC` .
...
Na primjer, ako se pristupi `/test` , a `nav:` je konfigurisan bez prefiksa ove staze, a trenutni jezik pregledavanja je engleski (kod `en` ), `/en/test.md` će biti učitano po defaultu i prikazano pomoću šablona `Md` .
