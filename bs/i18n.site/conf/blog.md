...
Demo fajl je konfigurisan na sledeći način:
...
Podaci o autoru mogu se napisati u metainformacijama članka, kao što su:
...
`name` , `url` i `title` su sve opcione. Ako `name` nije postavljeno, naziv ključa (ovdje `marlowe` ) će se koristiti kao `name` .
