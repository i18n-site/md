...
Za programere koji su upoznati sa `git` , `vim` i `vscode` , korištenje ovih alata za uređivanje dokumenata i upravljanje verzijama će nesumnjivo povećati troškove učenja.
...
Podržava konfiguraciju [HOGO](https://github.com/gohugoio/hugo) statičnog bloga, koji prevodi samo polja `title` , `summary` i `description` .
...
Prijevod gore navedenog na DeepL (također prepisivanjem vlastitih imena i dodavanjem neparnog `... ` ):
