...
ჯერ შეიყვანეთ `md` დირექტორიაში და გაუშვით `i18n.site` , რომელიც ითარგმნება `en` დან `zh` მდე.

<img src="https://p.3ti.site/1721114619.avif" style="width:350px">

გაშვების შემდეგ წარმოიქმნება თარგმანი და ქეში ფაილები, გთხოვთ, გახსოვდეთ, რომ დაამატოთ `git add . ` საცავში `md` დირექტორიაში.
...
`ol` არის `online` -ის აბრევიატურა, `-n` მიუთითებს ონლაინ გარემოზე, რომელიც გამოიყენება `npm` გამოშვებისთვის.
...
პაკეტის `npm` -ზე დაფუძნებული გამოქვეყნებისას, **დარწმუნდით, რომ გამოიყენეთ `//unpkg.com/`** `i18n.site` `/.v` `v:` .
...
თუ მატერიკზე ჩინეთში მომხმარებლები აწყდებიან ქსელის პრობლემებს და არ შეუძლიათ `npm` პაკეტის გამოქვეყნება, მათ შეუძლიათ დააყენონ გარემოს ცვლადი `https_proxy` პროქსი სერვერის კონფიგურაციისთვის.
...
არ დაგავიწყდეთ `CDN` ის კონფიგურაცია ისე, რომ `/.v` ით დამთავრებული გზის ქეშის დრო იყოს `1s` , წინააღმდეგ შემთხვევაში ახლად გამოშვებულ კონტენტზე დაუყოვნებლივ წვდომა შეუძლებელია.
...
იმის გამო, რომ ვებსაიტი იყენებს [ერთგვერდიანი აპლიკაციის](https://developer.mozilla.org/docs/Glossary/SPA) არქიტექტურას, გახსოვდეთ, რომ გადაწეროთ URL ბილიკები, რომლებიც არ შეიცავს `. ` დან `index.html` მდე.
...
#### Განლაგება github Გვერდზე

ჯერ დააწკაპუნეთ აქ `i18n-demo` [github შექმნათ ორგანიზაცია](https://github.com/account/organizations/new?plan=free) .
...
შემდეგ, გამოიყენეთ ღრუბლოვანი `out/ol/htm` ობიექტების საცავი მატერიკზე ჩინეთში + განათავსეთ შემდეგი შინაარსი `CDN` .
...
თუ თქვენ იყენებთ `i18n.site` ვებსაიტის გენერირებისთვის, როგორც თქვენს მთავარ ვებსაიტად, ჩვეულებრივ უნდა დააკონფიგურიროთ პან-დომენის გადამისამართება, ანუ გადამისამართოთ წვდომა `*.xxx.com` -ზე ( `www.xxx.com` ის ჩათვლით) `xxx.com` -ზე.
...
დაამატეთ დომენის სახელი [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) ში და მიუთითეთ დომენის სახელი `*.xxx.com` დან `CNAME` მდე Alibaba Cloud `CDN` -ში.
...
გთხოვთ, დაამატოთ `/root/i18n/md/out/ol/htm` კონფიგურაცია `server` `out/ol/htm` nginx
