...
windows , გთხოვთ, [დააწკაპუნოთ აქ, რომ ჩამოტვირთოთ და დააინსტალიროთ `git bash`](https://git-scm.com/download/win) ჯერ.
...
გთხოვთ, მიმართოთ დემო პროექტს `i18` თარგმანის კონფიგურაციის გასაგებად [github.com/i18n-site/demo.i18](//github.com/i18n-site/demo.i18)
...
მათ შორის `.i18n/cache/.gitignore` ის შინაარსი ასეთია :
...
ეს ნიშნავს, რომ ყველა ფაილი დირექტორიაში `.i18n/cache/` (გარდა `.i18n/cache/.gitignore` ) იგნორირებულია.

თუ თქვენი ვერსიის კონტროლის პროგრამული უზრუნველყოფა არ არის `git` , გთხოვთ, უგულებელყოთ იგი ამ კონფიგურაციის მიხედვით.
...
ამ კონფიგურაციის ცხრილში, კატალოგი `blog` თარგმანის საწყისი ენა არის `zh` , ხოლო `blog/your_file_name.md` თარგმანის საწყისი ენა არის `ja` .

### Ფაილის Იგნორირება

ნაგულისხმევად, ყველა ფაილი, რომელიც იწყება `.md` და `.yml` ით, საწყისი ენის დირექტორიაში ითარგმნება.
...
მაგრამ გთხოვთ, გაითვალისწინოთ, რომ თარგმანისა და ორიგინალური ტექსტის სტრიქონები უნდა შეესაბამებოდეს ერთს. ანუ თარგმანის შედგენისას ნუ დაამატებთ ან არ წაშლით ხაზებს. წინააღმდეგ შემთხვევაში, ეს გამოიწვევს დაბნეულობას თარგმანის რედაქტირების ქეშში.
