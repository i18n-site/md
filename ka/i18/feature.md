...
თარგმანის კომენტარები უნდა მიუთითებდეს ენაზე ` ``` ` შემდეგ, : ` ```rust ` .
...
პროგრამისტებისთვის, რომლებიც იცნობენ `git` , `vim` ს და `vscode` , ამ ინსტრუმენტების გამოყენება დოკუმენტების რედაქტირებისთვის და ვერსიების სამართავად, უდავოდ გაზრდის სწავლის ღირებულებას.
...
მხარს უჭერს სტატიკური ბლოგის სათაურის კონფიგურაციას [HOGO](https://github.com/gohugoio/hugo) რომელიც თარგმნის მხოლოდ ველებს `title` , `summary` და `description` .
...
თარგმნილია `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :
...
ზემოაღნიშნულის თარგმნა DeepL (ასევე გადაწერე შესაბამისი სახელები და დაამატე კენტი `... ` ):
