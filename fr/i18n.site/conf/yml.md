...
### `use: Md` , Modèle De Fichier Unique Sans Contour
...
### `use: Doc` , Modèle De Document Multi-Fichiers
...
La méthode d'écriture ci-dessus équivaut à `url: xxx` et son fichier correspondant est `en/xxx/TOC` .

#### Index De La Table Des Matières De La Table Des Matières

`i18n.site` exécutera `js` plug-in `.i18n/hook/after.tran/TOC.js` dans l'entrepôt de démonstration pour lire le fichier d'index de `TOC` répertoires correspondant à la configuration `doc` modèles afin de générer le plan de `json` répertoires.

Si vous utilisez `doc` modèle, vous devez disposer de ce plug-in.

Si vous initialisez le projet `i18n.site` à partir d'un dossier vide, n'oubliez pas de copier le projet démo `.i18n` dans votre répertoire.

Le modèle `Doc` affichera le plan de la table des matières en fonction du `json` généré.
...
Pour un certain chemin d'accès, si le préfixe du chemin n'est pas configuré en `nav:` , le `MarkDown` fichier correspondant au chemin sera chargé par défaut et rendu à l'aide du modèle `Md` .

Par exemple, si `/test` est accédé et `nav:` est configuré sans le préfixe de ce chemin et que la langue de navigation actuelle est l'anglais (code `en` ), `/en/test.md` sera chargé par défaut et rendu à l'aide du modèle `Md` .
