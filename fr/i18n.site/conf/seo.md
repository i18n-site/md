...
Prenons l'exemple du fichier de configuration `.i18n/htm/ol.yml` dans le projet de démonstration
...
Dans la configuration, veuillez remplacer `i18n.site` par la valeur `host:` in `.i18n/htm/ol.yml` , plusieurs magasins d'objets peuvent être configurés sous `s3` et le champ `region` est facultatif (de nombreux magasins d'objets n'ont pas besoin de définir ce champ).
