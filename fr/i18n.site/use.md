...
Si vous utilisez un environnement d'intégration continue pour publier, il n'est pas nécessaire d'installer `nodejs` , copiez simplement l'autorisation de connexion et de publication `~/.npmrc` dans l'environnement.
...
Par exemple, la configuration de redirection de nom de domaine pan-domaine de `*.i18n.site` dans l'image ci-dessus est la suivante :
