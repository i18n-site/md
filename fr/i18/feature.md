...
Prend en charge la traduction de Mixed HTML Markdown , le contenu des balises `<pre>` et `<code>` de HTML intégré dans `MarkDown` n'est pas traduit
...
DeepL Traduisez ce qui précède en français (en réécrivant également les noms propres et en ajoutant le `... ` impair) :
...
Le client est entièrement open source, et le côté 90 % du code est open source [Cliquez ici pour voir le code source](/i18n.site/src) .
