...
ድሕሪ ምጉያይ ትርጉምን ካሸን ፋይላት ክፍጠሩ እዮም በጃኹም ኣብቲ ኣብ `md` ማህደር ዘሎ መኽዘን `git add . ` ኣይትረስዑ።
...
ንኣብነት `.i18n/htm/ol.yml` `out/ol` ማህደር ክፈጥር እዩ።
...
ኣብ መሬት ቻይና ዝርከቡ ተጠቀምቲ ናይ መርበብ ጸገም እንተጋጢሞምን `npm` ፓኬጃት ክዝርግሑ እንተዘይክኢሎምን፡ ነቲ ፕሮክሲ ሰርቨር ንምውቃር ነቲ ናይ ከባቢ ተለዋዋጢ `https_proxy` ከቐምጥዎ ይኽእሉ።
...
ትሕዝቶ ባዕልኻ ንምእንጋድ፡ ብዘይካ `CDN` ገዛእ ርእስኻ ሰርቨር ምጥቃም፡ ካልእ ልሙድ ኣማራጺ `S3` +

`S3` `S3` ኣገልጋሊ ንምእታው [rclone](https://rclone.org)
...
እቲ መርበብ ሓበሬታ [ናይ ሓደ ገጽ መተግበሪ](https://developer.mozilla.org/docs/Glossary/SPA) ስነ ህንጻ ስለ ዝጥቀም፡ ካብ `. ` ክሳብ `index.html` ዘይሓዙ መንገድታት URL ዳግማይ ምጽሓፍ ኣይትረስዕ።

እቲ ናይ ዌብሳይት መእተዊ ገጽ ሓንሳብ ጥራይ እዩ ክዝርጋሕ ዘለዎ፣ ንቐጻሊ ትሕዝቶ ምዕባለታት ድማ ነቲ ናይ ዌብሳይት መእተዊ ገጽ ዳግማይ ምዝርጋሕ ኣየድልን።

#### ኣብ ገጽ github ምዝርጋሕ

መጀመርታ [github ጠውቑ ትካል ንምፍጣር](https://github.com/account/organizations/new?plan=free) እዚ ዝስዕብ ስም ትካል `i18n-demo` ከም ኣብነት እዩ።
...
ኣብዚ ኣብ ላዕሊ ዘሎ ስእሊ `i18n.site` ሕግታት ከምዚ ዝስዕብ እዩ።
...
ድሕሪኡ፡ ኣብ መሬት ቻይና + ሸየጥቲ ደበና መኽዘን ነገራት ተጠቐም `CDN` ነዚ ዝስዕብ ትሕዝቶ ኣዋፍር `out/ol/htm` .
...
ኣብ [ዓሊባባ CDN](https://cdn.console.aliyun.com/domain/list) ስም ዶሜይን ወሲኽና ኣብ ዓሊባባ ክላውድ `CDN` ካብ `*.xxx.com` ክሳብ `CNAME` ዝብል ስም ዶሜይን ኣመልክት !
...
`/root/i18n/md/out/ol/htm` ኣብ `server` ሕጡበ- `out/ol/htm` ናይ nginx
