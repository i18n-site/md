...
Kirish so'rovining `User-Agent` qiymati qidiruv tizimi brauzeri tomonidan foydalanilganda, so'rov `302` orqali statik sahifaga yo'naltiriladi.
...
`R2` moslashtirilgan domen nomi orqali kirish `cloudflare` sababli, statik fayllarni joylashtirish uchun uchinchi tomon ob'ekt xotirasidan foydalanish kerak.

Bu erda biz `cloudflare` da saqlanadigan uchinchi tomon ob'ektlarini qanday bog'lashni ko'rsatish uchun misol [backblaze.com](https://www.backblaze.com) .
...
Domen nomini `cloudflare` da `CNAME` dan `f003.backblazeb2.com` ga o'zgartiring va proksi-serverni yoqing.

![](//p.3ti.site/1725440896.avif)

`SSL` `cloudflare` o‘zgartiring → shifrlash rejimi, `Full` ga o‘rnating
...
`Rewrite to` dinamikni tanlang va paqir nomiga `your_bucketname` dan `concat("/file/your_bucketname",http.request.uri.path)` ni o'zgartiring.
