...
* `i18n.addon/mouse` : effektlari

## Fayl Nomi Konventsiyasi

Plaginlar hammasi `npm` paketdan iborat.
...
O'rnatilgan `js` ish vaqti ning ikkinchi darajali [boa](https://github.com/boa-dev/boa) asoslanadi va o'rnatilgan funktsiyalar quyidagilardan iborat :

* `wPath(path, txt)` : yozing
* `rTxt(path)` : Matn faylini o'qing
* `rBin(path)` : Ikkilik faylni o'qing
* `rDir(dirpath)` : Katalogni o'qing, qaytariladigan qiymat massiv : ro'yxati, fayllar ro'yxati
