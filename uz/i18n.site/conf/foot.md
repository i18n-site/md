...
**`foot.pug` ga `css` va `js` ni yozmang** , aks holda xato bo'ladi.
...
Altbilgi belgisi iconfont.cn `F` shrift yaratish orqali hosil bo'ladi ( [inglizcha versiya](https://www.iconfont.cn/?lang=en-us) / [shínín](https://www.iconfont.cn/?lang=zh) ).
...
`foot.pug` dan `js` yozib bo‘lmaydi. Agar o‘zaro aloqa zarur bo‘lsa, veb-komponentni sozlang.
...
`i18n.yml` bilan birlashtirilgan ushbu yozish usuliga o'xshash `${I18N.xxx}` dan foydalanib, siz kolontiterni ko'p tilli xalqarolashtirishga erishishingiz mumkin.

Havolaga `class="a"` qo'shish havolani `MarkDown` ga o'tkazishning oldini olishdir. Qarang :
 [➔ `YAML` : `HTML` `Markdown` ga aylantirishni qanday oldini olish mumkin](/i18/qa#H2) .