...
Rasmiy nashr qilish uchun o'zgartirilgan fayllar yangi versiya raqamlari katalogiga ko'chiriladi.
...
Proksi-server portingiz `7890` deb hisoblasangiz, quyidagilarni yozishingiz mumkin:
...
Tarkibni o'z-o'zidan joylashtirish uchun, o'z serveringizdan foydalanishdan tashqari, yana `CDN` keng tarqalgan variant `S3` dan foydalanishdir +
...
Kodni bosgandan so'ng, unga kirishdan oldin `github page` ning muvaffaqiyatli ishga tushirilishini kuting (quyida ko'rsatilgandek).
...
Keyin, materik Xitoyda bulutli sotuvchilarning ob'ekt saqlashidan foydalaning `CDN` Quyidagi tarkibni joylashtiring `out/ol/htm` +
...
Iltimos, `server` `/root/i18n/md/out/ol/htm` quyidagiga o'xshash konfiguratsiyani `out/ol/htm` nginx
