...
An'anaviy `MarkDown` langar nuqtalari matn mazmuni asosida yaratilgan. Ko'p tilli bo'lsa, bu yechim amalga oshirilmaydi.
...
## `MarkDown` Dan `HTML` Ni Yozing

`pug` kodiga `HTML` kiritilishi mumkin.

`<pre>` elementdagi kontent tarjima qilinmaydi.

Ushbu ikki nuqtani birlashtirib, turli xil displey effektlariga erishish uchun `HTML` `MarkDown` yozishingiz mumkin.
