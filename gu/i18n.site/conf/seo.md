...
`i18n.site` નોન-રિફ્રેશ સિંગલ પેજ આર્કિટેક્ચરને અપનાવે છે, જે શોધ ઈન્ડેક્સીંગને સરળ બનાવવા માટે, ક્રોલર્સ માટે એક અલગ સ્ટેટિક પેજ અને `sitemap.xml` જનરેટ કરવામાં આવશે.
...
રૂપરેખાંકનમાં, કૃપા કરીને `i18n.site` `.i18n/htm/ol.yml` માં `host:` ના મૂલ્યમાં બદલો, બહુવિધ ઑબ્જેક્ટ સ્ટોર્સ `s3` હેઠળ ગોઠવી શકાય છે, અને `region` ફીલ્ડ વૈકલ્પિક છે (ઘણા ઑબ્જેક્ટ સ્ટોર્સને આ ક્ષેત્ર સેટ કરવાની જરૂર નથી).

પછી પ્રોજેક્ટ પુનઃપ્રકાશિત કરવા માટે `i18n.site -n` ચલાવો.

જો તમે `~/.config/i18n.site.yml` માં ફેરફાર કર્યો હોય અને ફરીથી અપલોડ કરવા માંગો છો, તો કૃપા કરીને અપલોડ કેશ સાફ કરવા માટે પ્રોજેક્ટ રૂટ ડિરેક્ટરીમાં નીચેના આદેશનો ઉપયોગ કરો :
...
`URL redirect` ડાયનેમિક રીડાયરેક્શન પસંદ કરો, કૃપા કરીને રીડાયરેકશન પાથ `concat("/en",http.request.uri.path,".htm")` માં `/en` ડિફૉલ્ટ ભાષામાં સંશોધિત કરો જેને તમે શોધ એંજીન શામેલ કરવા માંગો છો.

## Baidu બુદ્ધિશાળી મેઘ ગોઠવણી
...
કારણ કે `R2` કસ્ટમાઇઝ્ડ ડોમેન નામ દ્વારા ઍક્સેસ `cloudflare` , સ્થિર ફાઇલો મૂકવા માટે તૃતીય-પક્ષ ઑબ્જેક્ટ સ્ટોરેજનો ઉપયોગ કરવાની જરૂર છે.
...
`SSL` માંથી `cloudflare` ને સંશોધિત કરો → એન્ક્રિપ્શન મોડ, `Full` પર સેટ કરો
...
`Rewrite to` ડાયનેમિક પસંદ કરો અને તમારા બકેટના નામમાં `your_bucketname` `concat("/file/your_bucketname",http.request.uri.path)` કરો.
