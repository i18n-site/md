...
Yn dangos CPU Model `AMD EPYC 9224 24-Core Processor` , sgôr perfformiad un craidd 1285
, sgôr aml-graidd 4277 Gweld mwy o ganlyniadau [vpsbenchmarks.com](https://www.vpsbenchmarks.com/yabs/contabo-6c-16gb-3fb7d8)

Prynais dri gweinydd ar yr un pryd a'u actifadu ar yr un pryd Mae hyn yn sicrhau bod y gweinyddwyr yn yr un ystafell gyfrifiaduron, gan ei gwneud hi'n haws adeiladu argaeledd uchel yn y dyfodol.
...
Y peth cyntaf IP golygu `~/.ssh/config` y peiriant lleol i osod alias, ac yna defnyddio `ssh-copy-id root@c2` i ffurfweddu mewngofnodi heb gyfrinair.
...
Mae'r cam hwn yn cymryd amser hir ar ôl aros am hanner awr, bydd uwchraddio a gosod meddalwedd a ddefnyddir yn gyffredin yn llwyddiannus. Gallwch gadarnhau hyn gyda'r gorchymyn canlynol:
...
Nid yw'r rhwydwaith o Ewrop i Tsieina yn `ssh` [mosh](//mosh.org)

Creu `c0` sgript yn y cyfeiriadur lleol `~/.bin` (wedi'i ychwanegu'n bennaf at newidyn amgylchedd `PATH` ) fel a ganlyn:
...
Ar ôl creu, rhowch ganiatâd gweithredadwy a dolen feddal `c1` ac `c2` .
...
Yn y dyfodol, gallwch chi fynd i mewn yn uniongyrchol `c0` ac `c1` i ddefnyddio `mosh` gysylltiad uniongyrchol cyflym a mynd i mewn [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
ssh newidiadau olion bysedd, defnyddiwch `ssh-keygen -R ip` i'w lanhau
...
Gall `/etc/snapper/configs/mnt` weld gosodiadau wrth gefn penodol yr is-gyfrol hon
...
Cofiwch fod y gweithrediadau uchod yn cael eu perfformio ar system achub, felly ni fydd y newidiadau hynny'n dod i rym nes bod y system wedi'i hailgychwyn ac na fydd yn y modd achub mwyach.

Mae hyn yn golygu na fyddwch yn gweld unrhyw newidiadau ar unwaith i logiau presennol sy'n dod yn barhaus, ond bydd yr holl logiau a gynhyrchir ar ôl hynny yn parhau.
...
Defnyddiais github action i wneud sgript casglu a defnyddio awtomatig o'r blaen, ond darganfyddais na allai'r gwasanaeth lleoli fod yn ssh.
...
Ar ôl ymchwiliadau dro ar ôl tro, y rheswm am hynny yw github action `1001` yw Defnyddiwr `uid` tar

tar Rhaid dadbacio os nad oes paramedr `--no-same-owner` , bydd yn cael ei ddatgywasgu i `uid` ar gyfer `1001` ddefnyddiwr
