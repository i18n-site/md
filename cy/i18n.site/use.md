...
Ar ôl rhedeg, bydd ffeiliau cyfieithu a storfa yn cael eu cynhyrchu. Cofiwch `git add . ` hychwanegu at y storfa yn y cyfeiriadur `md` .
...
Wrth gyhoeddi yn seiliedig ar becyn `npm` , **gofalwch eich bod yn defnyddio `//unpkg.com/` ar gyfer** `i18n.site` `/.v` o `v:` gwerth.
...
I hunangynnal cynnwys, `CDN` ogystal â defnyddio'ch gweinydd eich hun, opsiwn cyffredin arall yw defnyddio `S3` +
...
Cliciwch yma yn gyntaf i `i18n-demo` [github](https://github.com/account/organizations/new?plan=free) .

Yna creu warws `i18n-demo.github.io` o dan y sefydliad hwn (rhowch enw'r sefydliad a grewyd gennych yn lle `i18n-demo` ):

![](https://p.3ti.site/1721098657.avif)

Wrth gyhoeddi'r cynnwys yn yr erthygl flaenorol, mae `out/ol/htm` wedi'i greu Rhowch y cyfeiriadur hwn a rhedeg :
...
O'i gymharu [cloudflare page](//pages.cloudflare.com) `github page` , mae'n darparu ailysgrifennu llwybrau ac mae'n fwy cyfeillgar i dir mawr Tsieina ac mae'n fwy hygyrch.

Mae lleoli `cloudflare page` fel arfer yn seiliedig ar ddefnyddio `github page` uchod.
...
Mae'r `i18n.site` yn y llun uchod fel a ganlyn.
...
Yna, defnyddiwch storfa gwrthrychau gwerthwyr cwmwl ar `out/ol/htm` mawr Tsieina + Defnyddio'r cynnwys canlynol `CDN` .
...
Gan `CNAME` all cofnod `MX` a record `CNAME` gydfodoli, os ydych am dderbyn e-byst enw parth `A` yr un pryd, mae angen i chi gydweithredu â'r sgript [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten)
...
Ychwanegwch yr enw parth yn [Alibaba Cloud CDN](https://cdn.console.aliyun.com/domain/list) a phwyntiwch yr enw parth `*.xxx.com` i `CNAME` yn Alibaba Cloud `CDN` .
