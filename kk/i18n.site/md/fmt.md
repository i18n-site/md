...
`i18n.site` мен келісілген бекіту нүктесінің шешімі - орналасу анкерлік нүктесін қолмен жасау үшін `MarkDown` -де `<a rel=id href="#xxx" id="xxx"></a>` ге ұқсас мәтінді кірістіру.
...
Осы екі нүктені біріктіре отырып, әртүрлі дисплей әсерлеріне қол жеткізу үшін `HTML` ден `MarkDown` ге оңай жаза аласыз.
...
## Бірнеше Тілдегі Суреттер/Бейнелер/Сілтемелер

`MarkDown` `HTML` ге түрлендіруден кейін `src` және `href` атрибуттарындағы `$LANG` ағымдағы [тіл кодымен](/i18/LANG_CODE) ауыстырылады.

Бұл айнымалының көмегімен суреттердің/бейнелердің/сілтемелердің көптілділігіне қол жеткізуге болады.