...
`i18n.site` кірістірілген `i18` аударма құралы бар, [рұқсат таңбалауышын конфигурациялау үшін `i18` құжатты қараңыз](/i18/use) .
...
Іске қосылғаннан кейін аударма және кэш файлдары жасалады, `git add . ` `md` каталогындағы репозиторийге қосуды ұмытпаңыз.
...
Сондай-ақ, басқа конфигурация файлдарын жасауға болады, ол пайдаланылатын конфигурация файлының атын көрсету үшін пәрмен жолында `--htm_conf` пайдаланыңыз:
...
`github page` [cloudflare page](//pages.cloudflare.com) салыстырғанда, ол жолды қайта жазуды қамтамасыз етеді және материктік Қытайға ыңғайлы және оны пайдалану ұсынылады.
...
Содан кейін қойма `i18n-demo.github.io` тармағын таңдап, `Begin setup` түймесін басып, келесі қадамдар үшін әдепкі мәндерді пайдаланыңыз.
...
`out/ol/htm` кейін, материктік Қытайдағы бұлттық жеткізушілердің нысан қоймасын пайдаланыңыз + Келесі мазмұнды орналастырыңыз `CDN` .
...
Веб-сайтты негізгі веб-сайт ретінде жасау үшін `i18n.site` мәнін пайдалансаңыз, әдетте пандоменді қайта бағыттауды конфигурациялауыңыз керек, яғни `*.xxx.com` ге (соның ішінде `www.xxx.com` ) `xxx.com` ке қатынасты қайта бағыттау керек.
...
[Alibaba CDN](https://cdn.console.aliyun.com/domain/list) жүйесінде домен атауын қосыңыз және Alibaba Cloud `CDN` жүйесінде домен атауын `*.xxx.com` ден `CNAME` дейін көрсетіңіз.
...
`server` параграфындағы `out/ol/htm` ұқсас `/root/i18n/md/out/ol/htm` қосыңыз nginx
...
`.i18n/conf.yml` конфигурация файлының `en` / `fromTo` -ге сәйкес келетін бастапқы тіл каталогы
