...
`ol.yml` -де `v:` бума атауын өзгертсеңіз, **алдымен `.i18n/v/ol` жойып** , содан кейін оны жариялауды ұмытпаңыз.
...
Кодты басқаннан кейін, оған қол жеткізе алмас бұрын, `github page` орналастыру сәтті іске қосылғанша күтіңіз (төменде көрсетілгендей).
...
`i18n-demo` ұйымға кіру рұқсатын беру үшін `Add Account` түймесін басыңыз.
