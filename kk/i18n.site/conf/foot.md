...
Осы жазу әдісіне ұқсас `${I18N.xxx}` мәнін `i18n.yml` мен біріктіріп пайдалану арқылы төменгі деректемені көп тілді интернационалдандыруға қол жеткізуге болады.

Сілтемеге `class="a"` қосу сілтеменің `MarkDown` ге айналуын болдырмау болып табылады. Қараңыз :
 [➔ `YAML` `HTML` : `Markdown` ге айналуын қалай болдырмауға болады](/i18/qa#H2) .