...
Non anyè script baz kòd `demo.i18n.site` dwe `md` pou fasilite aperçu lokal ak `docker` .
...
Apre kouri, dosye tradiksyon ak kachèt yo pral pwodwi Tanpri sonje ajoute `git add . ` nan repozitwa a nan anyè `md` .

### Preview Lokal Yo

Enstale epi kòmanse `docker` ( `MAC` itilizatè rekòmande pou itilize [orbstack](https://orbstack.dev) kòm ègzekutabl pou `docker` ).

Lè sa a, antre nan anyè `docker` epi kouri `./up.sh` , epi ale [https://127.0.0.1](https://127.0.0.1) aperçu lokalman.
...
Aperçu lokal pa pran swen nimewo vèsyon an epi li pral kopye tout fichye yo nan anyè `out/dev/v/0.1.0` a.
...
Isit la `--save` reprezante nimewo vèsyon an lage aktyalizasyon.
...
Lè pibliye ki baze sou pake `npm` , **asire w ke ou sèvi ak `//unpkg.com/` pou** prefiks nan `v:` valè `i18n.site` te espesyalman optimize tan an kachèt `/.v` chemen prefiks sa a pou reyalize alè gade nan nouvo degaje.
...
Si ou modifye non pake a nan `v:` nan `ol.yml` , tanpri **asire w ke ou efase `.i18n/v/ol` dabò** epi pibliye li.

##### Sèvè Proxy Pibliye Pa npm
...
Sipoze ke pò sèvè proxy ou a se `7890` , ou ka ekri:
...
Finalman, **konfigirasyon tan an kachèt nan chemen an fini nan `/.v` a `1s`** , otreman kontni an ki fèk pibliye pa ka jwenn aksè imedyatman.

Tan kachèt pou lòt chemen yo ka mete sou yon ane oswa plis pou diminye demann ki pa nesesè yo.

##### Mete Kontni Nan s3

Pou pwòp tèt ou òganize kontni, anplis de itilize pwòp sèvè ou, yon lòt opsyon komen se sèvi ak `S3` + `CDN`
...
Konpare [cloudflare page](//pages.cloudflare.com) `github page` , li bay reekri chemen epi li pi zanmitay ak tè pwensipal Lachin epi li rekòmande pou itilize.
...
Paske dosye `MX` ak dosye `CNAME` pa ka viv ansanm, si ou vle resevwa imèl non domèn an menm tan, ou bezwen kolabore ak script [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten) nan nivo `CNAME` nan dosye `A` .
...
Ajoute non domèn nan [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) epi montre non domèn `*.xxx.com` a `CNAME` nan Alibaba Cloud `CDN` .
