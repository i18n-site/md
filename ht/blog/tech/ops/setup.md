...
CPU Modèl `AMD EPYC 9224 24-Core Processor` , nòt pèfòmans yon sèl-nwayo 1285
...
Premye bagay la IP modifye `~/.ssh/config` machin lokal la pou mete yon alyas, epi sèvi ak `ssh-copy-id root@c2` pou configured login san modpas.
...
Etap sa a pran yon bon bout tan Apre w fin tann yon demi èdtan, ajou ak enstalasyon lojisyèl yo souvan itilize yo pral reyisi. Ou ka konfime sa a ak lòd sa a:
...
Rezo a soti nan Ewòp nan Lachin se pa estab Li rekòmande [mosh](//mosh.org) pi vit `ssh` .

Kreye `c0` scripts nan anyè lokal `~/.bin` (sitou ajoute nan varyab anviwònman `PATH` ) jan sa a:
...
Apre kreyasyon an, bay pèmisyon ègzekutabl ak lyen mou `c1` ak `c2` .
...
Nan lavni an, ou ka dirèkteman antre `c0` ak `c1` pou itilize `mosh` gwo vitès koneksyon dirèk epi antre [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Tann sèvè a rekòmanse

Ap ssh chanjman anprent, sèvi ak `ssh-keygen -R ip` pou netwaye li
...
Si ou se yon pèfeksyonis, ou kapab tou fè yon konpresyon plen disk manyèlman.
...
Sèvi ak `snapper -c mnt list` pou wè yon snapshot yon sou-volim

`/etc/snapper/configs/mnt` ka wè anviwònman backup espesifik nan subvolume sa a
...
Kenbe nan tèt ou ke operasyon ki anwo yo fèt sou yon sistèm sekou, kidonk chanjman sa yo pa pral pran efè jiskaske sistèm nan rekòmanse epi yo pa nan mòd sekou ankò.

Sa vle di ke ou pa pral imedyatman wè okenn chanjman nan mòso bwa ki egziste deja ki vin pèsistan, men tout mòso bwa ki te pwodwi apre sa yo pral pèsiste.
...
Apre envestigasyon repete, se github action itilizatè `uid` nan tar se `1001`

Dekonpresyon dwe si pa gen paramèt `--no-same-owner` , li pral dekonprese a `uid` pou `1001` itilizatè tar
