...
## <a rel=id href="#project" id="project"></a> Projekt Beskrivelse
...
Oversæt `Markdown` og `YAML` til et kommandolinjeværktøj ( [kildekode](https://github.com/i18n-site/rust/tree/main/i18) ) på flere sprog.

Kan perfekt opretholde formatet på `Markdown` Kan identificere filændringer og kun oversætte ændrede filer.
...
Vi deler " `UNIX` Everything is a File " og kan administrere oversættelser på hundredvis af sprog uden at introducere komplekse og besværlige løsninger på virksomhedsniveau.

Du kan bruge de mest velkendte værktøjer til `Markdown` oversættelsen (men du kan ikke tilføje eller slette afsnit), og bruge den mest velkendte måde at udføre versionskontrol på.

En kodebase kan oprettes som en open source for sprogfiler, og ved hjælp af `Pull Request` 's proces kan globale brugere deltage i den løbende optimering af oversættelser. Problemfri forbindelse [github](//github.com) andre open source-fællesskaber.
...
Rent statisk, optimeret til læseoplevelse og integreret med oversættelsen af [i18](#i18) er det det bedste valg til at bygge et projektdokumentwebsted.

Den underliggende front-end-ramme anvender en fuld plug-in-arkitektur, som er let til sekundær udvikling. Om nødvendigt kan back-end-funktioner integreres.

Denne hjemmeside er udviklet baseret på denne ramme og inkluderer bruger-, betalings- og andre funktioner ( [kildekode](/i18n.site/src) ) En detaljeret vejledning vil blive skrevet senere.

[→ For brugervejledning, læs venligst projektdokumentationen](/i18n.site) .
