...
Efter kørsel vil der blive genereret oversættelses- og cachefiler. Husk at tilføje `git add . ` til depotet i `md` -mappen.
...
For selv at hoste indhold, ud `CDN` at bruge din egen server, er en anden almindelig mulighed at bruge `S3` +
...
[Klik først github for at oprette en organisation](https://github.com/account/organizations/new?plan=free) . Følgende organisationsnavn er `i18n-demo` som eksempel.
...
[cloudflare page](//pages.cloudflare.com) Sammenlignet med `github page` giver det path rewriting og er mere venligt for det kinesiske fastland og er mere tilgængeligt.
...
Brug derefter objektlageret fra cloud `out/ol/htm` leverandører på det kinesiske fastland + Implementer følgende indhold `CDN` .
...
Fordi record `MX` og record `CNAME` ikke kan eksistere side om side, hvis du ønsker at modtage domænenavne-e-mails på samme tid, skal du samarbejde med [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten) til niveau `CNAME` i record `A` .
...
Tilføj domænenavnet i [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) og peg domænenavnet `*.xxx.com` til `CNAME` i Alibaba Cloud `CDN` .
...
Kildesprogskatalog, svarende til `en` under `fromTo` i konfigurationsfilen `.i18n/conf.yml`
...
Oversættelseskonfigurationen bedes referere til [i18](/i18/use)