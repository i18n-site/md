...
[i18n.site/payBill](//i18n.site/payBill) skal du binde et kreditkort til betaling (ingen genopladning er påkrævet, hjemmesiden trækker automatisk gebyrer efter brug, [se hjemmesiden for priser](/#price) ).
...
Forskellige `fromTo` kan konfigureres til forskellige undermapper / En demonstration er skrevet som følger :
...
I denne konfigurationstabel er kildesproget for bibliotek `blog` -oversættelse `zh` , og kildesproget for oversættelse af bibliotek `blog/your_file_name.md` er `ja` .

### Ignorere Filen

Som standard vil alle filer, der starter med `.md` og `.yml` i kildesprogsbiblioteket, blive oversat.

Hvis du vil ignorere bestemte filer og ikke oversætte dem (såsom ufærdige udkast), kan du bruge `ignore` feltkonfigurationen.
