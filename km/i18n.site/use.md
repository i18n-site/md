...
ដំបូងបញ្ចូល `md` directory ហើយរត់ `i18n.site` ដែលនឹងបកប្រែ `en` ទៅ `zh` ។

<img src="https://p.3ti.site/1721114619.avif" style="width:350px">

បន្ទាប់ពីដំណើរការ ការបកប្រែ និងឯកសារឃ្លាំងសម្ងាត់នឹងត្រូវបានបង្កើត សូមចងចាំថាត្រូវបន្ថែម `git add . ` ទៅក្នុងឃ្លាំងក្នុង `md` ។
...
នៅពេលបោះពុម្ពដោយផ្អែកលើកញ្ចប់ `npm` **ត្រូវប្រាកដថាប្រើ `//unpkg.com/` សម្រាប់** បុព្វបទនៃតម្លៃ `v:` `i18n.site` បានបង្កើនប្រសិទ្ធភាពពេលវេលាឃ្លាំងសម្ងាត់នៃ `/.v` នៅក្រោមផ្លូវបុព្វបទនេះ ដើម្បីសម្រេចបានការមើលទាន់ពេលនៃការចេញផ្សាយថ្មី។
...
ប្រសិនបើអ្នកកែប្រែឈ្មោះកញ្ចប់ `v:` ក្នុង `ol.yml` សូម **ប្រាកដថាត្រូវលុប `.i18n/v/ol` ជាមុនសិន** រួចបោះផ្សាយវា។
...
ចំពោះមាតិកាដែលបង្ហោះដោយខ្លួនឯង បន្ថែមពី `CDN` ការប្រើប្រាស់ម៉ាស៊ីនមេផ្ទាល់ខ្លួនរបស់អ្នក ជម្រើសទូទៅមួយទៀតគឺត្រូវប្រើ `S3` +
...
ដោយសារគេហទំព័រប្រើស្ថាបត្យកម្ម [កម្មវិធីទំព័រតែមួយ](https://developer.mozilla.org/docs/Glossary/SPA) សូមចាំថាត្រូវសរសេរឡើងវិញនូវផ្លូវ URL ដែលមិនមាន `. ` ទៅ `index.html` ។
...
ជាដំបូង ចុចត្រង់នេះ ! ដើម្បី `i18n-demo` [github](https://github.com/account/organizations/new?plan=free) .
...
បន្ទាប់ពីចងឈ្មោះដែនផ្ទាល់ខ្លួនហើយ សូមចូលទៅកាន់ឈ្មោះដែនដើម្បីកំណត់រចនាសម្ព័ន្ធការសរសេរផ្លូវនៃកម្មវិធីទំព័រតែមួយ ដូចបង្ហាញខាងក្រោម៖
...
បន្ទាប់មក ប្រើកន្លែងផ្ទុកវត្ថុរបស់អ្នកលក់ពពកនៅក្នុងប្រទេសចិនដីគោក + `CDN` ដាក់ពង្រាយខ្លឹមសារ `out/ol/htm`
...
ដោយសារតែ record `MX` និង record `CNAME` មិនអាចរួមរស់ជាមួយគ្នា `A` `CNAME` ប្រសិនបើអ្នកចង់ទទួលបាន domain name emails ក្នុងពេលតែមួយ អ្នកត្រូវសហការជាមួយ [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten)
...
បន្ថែម `CNAME` `*.xxx.com` នៅក្នុង Alibaba `CDN` [CDN](https://cdn.console.aliyun.com/domain/list)
...
សូម `/root/i18n/md/out/ol/htm` ការកំណត់ដូចខាង `out/ol/htm` ក្នុងកថាខណ្ឌ `server` នៃ nginx
