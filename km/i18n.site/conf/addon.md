...
`file` គឺជាបញ្ជីឯកសារលទ្ធផល `path` ជាផ្លូវឯកសារ និង `txt` ជាមាតិកាឯកសារ។

## មុខងារដែលភ្ជាប់មកជាមួយ

ពេលវេលាដំណើរការ `js` ដែលភ្ជាប់មកជាមួយគឺផ្អែកលើការអភិវឌ្ឍន៍បន្ទាប់បន្សំ : [boa](https://github.com/boa-dev/boa)

* `wPath(path, txt)` :
* `rTxt(path)` : ឯកសារអត្ថបទ
* `rBin(path)` អាន : គោលពីរ
