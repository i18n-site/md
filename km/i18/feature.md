...
`i18` ជាឧបករណ៍បន្ទាត់ពាក្យបញ្ជាសម្រាប់ការបកប្រែ Markdown & Yaml
...
គាំទ្រការបកប្រែ Markdown នៅក្នុងលាយ HTML `<pre>` HTML `<code>` `MarkDown`
...
មតិ ` ```rust ` បកប្រែត្រូវបង្ហាញភាសាបន្ទាប់ : ` ``` `

បច្ចុប្បន្នគាំទ្រការបកប្រែមតិ `toml` , `yaml` , `json5` , `go` , `rust` , `c` , `cpp` , `java` `js` `coffee` , `python` , `bash` , `php` [និងភាសាផ្សេងទៀត](https://github.com/i18n-site/rust/blob/main/getc/src/style.rs#L14) ។
...
សម្រាប់អ្នកសរសេរកម្មវិធី `vim` `vscode` `git`

`KISS` ( `Keep It Simple, Stupid` ) ក្នុងចំណោមអ្នកជឿលើគោលការណ៍ ដំណោះស្រាយកម្រិតសហគ្រាសមានន័យដូចគ្នានឹងការលំបាក មិនមានប្រសិទ្ធភាព និងពិបាកប្រើ។
...
ទោះយ៉ាងណាក៏ដោយ ពួកគេមិនគាំទ្រការកំណត់អត្តសញ្ញាណការកែប្រែឯកសារ ហើយគ្រាន់តែបកប្រែឯកសារដែលបានកែប្រែប៉ុណ្ណោះ ដើម្បីកាត់បន្ថយការចំណាយ និងបង្កើនប្រសិទ្ធភាព។
...
ឧបករណ៍នេះគ្រាន់តែបកប្រែតម្លៃវចនានុក្រមនៅក្នុង `YAML`

ដោយផ្អែកលើ `YAML` ការបកប្រែ អ្នកអាចបង្កើតដំណោះស្រាយអន្តរជាតិយ៉ាងងាយស្រួលសម្រាប់ភាសាសរសេរកម្មវិធីផ្សេងៗ។

## គាំទ្រការបកប្រែ `HOGO`

គាំទ្រការកំណត់រចនាសម្ព័ន្ធបឋមកថា [HOGO](https://github.com/gohugoio/hugo) វាយប្លុកឋិតិវន្ត មានតែវាល `title` , `summary` និង `description` ដែលត្រូវបានបកប្រែ។

## កុំបកប្រែឈ្មោះអថេរ

`Markdown` ប្រើជាគំរូអ៊ីមែល `YAML` ជាការកំណត់រចនាសម្ព័ន្ធឯកសារភាសា នឹងមានប៉ារ៉ាម៉ែត្រអថេរ (ឧទាហរណ៍៖ បញ្ចូលទឹកប្រាក់ `${amount}` ត្រូវបានទទួល)។

`${varname}` ឈ្មោះអថេរប្រភេទនេះនឹងមិនចាត់ទុកជាការបកប្រែជាភាសាអង់គ្លេសទេ។
...
ចិន ជប៉ុន និងកូរ៉េ មិនមានអក្សរធំ ឬអក្សរតូចទេ ប៉ុន្តែចំណងជើងភាសាអង់គ្លេសជាទូទៅដាក់អក្សរធំ។

`i18` អាចស្គាល់ចំណងជើងក្នុង `MarkDown` ហើយនឹងសរសេរអក្សរធំដោយស្វ័យប្រវត្តិនៅពេលបកប្រែទៅជាភាសាដែលប្រកាន់អក្សរតូចធំ។

ឧទាហរណ៍ `为阅读体验而优化` នឹងត្រូវបានបកប្រែជា `Optimized for Reading Experience` ។
...
បកប្រែជា `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` :

<img alt="" src="//p.3ti.site/1720199391.avif">

បកប្រែជា `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :

`Falcon` ត្រូវបានបកប្រែជា `faucon` ហើយ `Llama` ត្រូវបានបកប្រែជា `lama` . As proper nouns, they should not be translation.

<img alt="" src="//p.3ti.site/1720199451.avif">

### ការបកប្រែ DeepL

បកប្រែជា `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` :
...
ការបកប្រែរបស់ `i18` នឹងទទួលស្គាល់ពាក្យជាភាសាអង់គ្លេសនៅក្នុងឯកសារចិន ជប៉ុន និងកូរ៉េ ហើយទុកពាក្យឱ្យនៅដដែល។
...
ឧទាហរណ៍ `Cristiano Ronaldo` `C罗`

## អាចរួមបញ្ចូលគ្នាបាន `i18n.site`

`i18` ត្រូវបានបញ្ចូលទៅក្នុងឧបករណ៍បន្ទាត់ពាក្យបញ្ជាសម្រាប់ការកសាងគេហទំព័រពហុភាសា [`i18n.site`](/i18n.site) .

សូមមើលឯកសាររបស់ [`i18n.site`](/i18n.site)

## កូដប្រភពបើកចំហ

ម៉ាស៊ីនភ្ញៀវគឺជាប្រភពបើកចំហទាំងស្រុង ហើយផ្នែកខាងម៉ាស៊ីនមេ 90 % នៃកូដគឺជាប្រភពបើកចំហ [សូមចុចទីនេះដើម្បីមើលកូដប្រភព](/i18n.site/src) ។
