...
windows ប្រព័ន្ធ សូម [ចុចទីនេះដើម្បីទាញយក និងដំឡើង `git bash`](https://git-scm.com/download/win) ជាមុនសិន។
...
ក្នុងចំណោមនោះ មាតិកា `.i18n/cache/.gitignore` មានដូចខាងក្រោម :
...
នេះមានន័យថាឯកសារទាំងអស់នៅក្នុងថត `.i18n/cache/` (លើកលែងតែ `.i18n/cache/.gitignore` ) មិនត្រូវបានអើពើ។

ប្រសិនបើកម្មវិធីគ្រប់គ្រងកំណែរបស់អ្នកមិនមែនជា `git` សូមមិនអើពើវាយោងទៅតាមការកំណត់នេះ។
...
អ្នកអាចកំណត់រចនាសម្ព័ន្ធ `fromTo` ផ្សេងគ្នាសម្រាប់ថតរងផ្សេងៗ : /
...
នៅក្នុងតារាងការកំណត់រចនាសម្ព័ន្ធនេះ ភាសាប្រភពនៃការបកប្រែកាតាឡុក `blog` គឺ `zh` ហើយភាសាប្រភពនៃការបកប្រែ `blog/your_file_name.md` គឺ `ja` ។
