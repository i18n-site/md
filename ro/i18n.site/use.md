...
Dacă utilizați un mediu de integrare continuă pentru a publica, nu este nevoie să instalați `nodejs` , doar să copiați permisiunea de autentificare și publicare `~/.npmrc` în mediu.

Dacă modificați numele pachetului `v:` în `ol.yml` , **asigurați-vă că ștergeți mai întâi `.i18n/v/ol`** și apoi îl publicați.
...
Dacă doriți să autogăzduiți conținutul, mai întâi editați `md/.i18n/htm/ol.yml` și schimbați `v: //unpkg.com/i18n.site` cu prefixul URL, cum ar fi `v: //i18n-v.xxx.com` .
...
Vă rugăm să faceți clic pe `Add Account` pentru a acorda acces la organizația `i18n-demo` .
...
Apoi, utilizați stocarea de obiecte a furnizorilor `out/ol/htm` cloud din China continentală + Implementați următorul conținut `CDN` .
...
Vă rugăm să adăugați o configurație similară cu următoarea în paragraful `server` din nginx Vă rugăm să schimbați `/root/i18n/md/out/ol/htm` în calea propriului proiect `out/ol/htm` :
