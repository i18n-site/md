...
### `use: Doc` , Șablon De Document Cu Mai Multe Fișiere
...
Metoda de scriere de mai sus este echivalentă cu a avea `url: xxx` , iar fișierul său corespunzător este `en/xxx/TOC` .

#### Indexul Cuprinsului TOC

`i18n.site` va executa `js` plug-in `.i18n/hook/after.tran/TOC.js` în depozitul demonstrativ pentru a citi fișierul index cu `TOC` directoare corespunzător configurației cu `doc` șabloane pentru a genera schița celor `json` directoare.

Dacă utilizați `doc` șablon, trebuie să aveți acest plug-in.

Dacă inițializați proiectul `i18n.site` dintr-un folder gol, nu uitați să copiați proiectul demo `.i18n` în directorul dvs.
