...
Nehmen Sie als Beispiel die `.i18n/htm/ol.yml` -Konfigurationsdatei im Demoprojekt
...
Bitte ändern Sie zunächst den Wert `host:` oben in Ihren Domainnamen, z. B. `i18n.site` .
...
Bitte ändern Sie in der Konfiguration `i18n.site` in den Wert `host:` in `.i18n/htm/ol.yml` Unter `s3` können mehrere Objektspeicher konfiguriert werden, und das Feld `region` ist optional (viele Objektspeicher müssen dieses Feld nicht festlegen).
...
Binden Sie zunächst einen beliebigen Domänennamen an `cloudflare` und verknüpfen Sie dann mit `SSL/TLS` → benutzerdefinierter Domänenname den Hauptdomänennamen mit diesem Domänennamen.
