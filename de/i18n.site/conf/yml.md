...
`nav:` Konfigurationsoptionen, entsprechend dem Navigationsmenü oben auf der Startseite.
...
### `use: Toc` , Einzelne Dateivorlage Mit Umriss
...
### `use: Md` , Einzelne Dateivorlage Ohne Gliederung
...
### `use: Doc` , Dokumentvorlage Mit Mehreren Dateien
...
Die obige Schreibmethode entspricht `url: xxx` und die entsprechende Datei ist `en/xxx/TOC` .

#### Index Des Inhaltsverzeichnisses Des Inhaltsverzeichnisses

`i18n.site` führt `js` Plug-in `.i18n/hook/after.tran/TOC.js` im Demo-Warehouse aus, um die `TOC` -Verzeichnis-Indexdatei entsprechend der `doc` Vorlagenkonfiguration zu lesen und die `json` Verzeichnis-Gliederung zu generieren.

Wenn Sie die Vorlage `doc` verwenden, müssen Sie über dieses Plug-in verfügen.

Wenn Sie Projekt `i18n.site` aus einem leeren Ordner initialisieren, denken Sie daran, Demoprojekt `.i18n` in Ihr Verzeichnis zu kopieren.

Die `Doc` -Vorlage rendert die Gliederung des Inhaltsverzeichnisses basierend auf der generierten `json` .
...
Die nächsten beiden Zeilen sind wie im Screenshot unten dargestellt.
...
Der Inhalt kann in Element `README.md` geschrieben werden, z. B. `en/demo2/README.md` .
...
Wenn beispielsweise auf `/test` zugegriffen wird und `nav:` ohne das Präfix dieses Pfads konfiguriert ist und die aktuelle Browsersprache Englisch ist (Code `en` ), wird `/en/test.md` standardmäßig geladen und mit Vorlage `Md` gerendert.
