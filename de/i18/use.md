...
Darüber [i18n.site/payBill](//i18n.site/payBill) müssen Sie eine Kreditkarte für die Zahlung binden (es ist keine Aufladung erforderlich, die Website zieht automatisch Gebühren entsprechend der Nutzung ab, [Preise finden Sie auf der Homepage](/#price) ).
...
Für verschiedene Unterverzeichnisse können unterschiedliche `fromTo` konfiguriert werden / Eine Demonstration ist wie folgt geschrieben :
...
In dieser Konfigurationstabelle ist die Quellsprache der Übersetzung von Verzeichnis `blog` `zh` und die Quellsprache der Übersetzung von Verzeichnis `blog/your_file_name.md` ist `ja` .
