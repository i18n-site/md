<h1 style="display:flex;justify-content:space-between">i18n.site ⋅ Internationale Lösungen<img src="//p.3ti.site/logo.svg" style="user-select:none;margin-top:-1px;width:42px"></h1>

Befehlszeile Markdown Yaml , hilft Ihnen beim Aufbau einer internationalen Dokumentenseite, [die Hunderte von Sprachen](/i18/LANG_CODE) unterstützt ...
...
## <a rel=id href="#project" id="project"></a> Projektbeschreibung
...
Übersetzen Sie `Markdown` und `YAML` in ein Befehlszeilentool ( [Quellcode](https://github.com/i18n-site/rust/tree/main/i18) ) in mehreren Sprachen.

Kann das Format von perfekt beibehalten `Markdown` Kann Dateiänderungen erkennen und nur geänderte Dateien übersetzen.
...
Wir teilen " Philosophie `UNIX` Alles ist eine Datei " und können Übersetzungen in Hunderten von Sprachen verwalten, ohne komplexe und umständliche Lösungen auf Unternehmensebene einführen zu müssen.

Sie können die gängigsten Tools zum `Markdown` der Übersetzung verwenden (aber Sie können keine Absätze hinzufügen oder löschen) und die gängigste Methode zur Versionskontrolle verwenden.

Eine Codebasis kann als Open Source für Sprachdateien erstellt werden, und mit Hilfe des Prozesses von `Pull Request` können globale Benutzer an der kontinuierlichen Optimierung von Übersetzungen teilnehmen. Nahtlose Verbindung [github](//github.com) Und andere Open-Source-Communities.
...
Rein statisch, für das Leseerlebnis optimiert und in die Übersetzung von [i18](#i18) ist es die beste Wahl für die Erstellung einer Projektdokument-Site.
...
Diese Website basiert auf diesem Framework und umfasst Benutzer-, Zahlungs- und andere Funktionen ( [Quellcode](/i18n.site/src) ). Ein detailliertes Tutorial wird später geschrieben.

[→ Für eine Bedienungsanleitung lesen Sie bitte die Projektdokumentation](/i18n.site) .
