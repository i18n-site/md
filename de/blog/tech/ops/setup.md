...
Anzeige CPU Modell `AMD EPYC 9224 24-Core Processor` Single-Core-Leistungsbewertung 1285
...
Der erste Schritt besteht darin, das `~/.ssh/config` -Attribut des lokalen Computers zu bearbeiten. Legen Sie einen Alias ​​für IP fest und verwenden Sie dann `ssh-copy-id root@c2` , um die passwortfreie Anmeldung zu konfigurieren.
...
Dieser Schritt dauert lange. Nach einer halben Stunde Wartezeit sind das Upgrade und die Installation häufig verwendeter Software erfolgreich. Sie können es mit dem folgenden Befehl bestätigen:
...
Das Netzwerk von Europa nach China ist nicht stabil. Es wird empfohlen [mosh](//mosh.org) es zu beschleunigen `ssh`

Erstellen Sie ein `c0` -Skript im `~/.bin` -Verzeichnis des lokalen Computers (hauptsächlich zur Umgebungsvariablen `PATH` hinzugefügt):
...
Erteilen Sie nach der Erstellung ausführbare Berechtigungen und `c2` Softlink `c1`
...
In Zukunft können Sie `c0` , `c1` direkt eingeben und Sie können `mosh` Hochgeschwindigkeits-Direktverbindung verwenden und [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Es kommt zu Veränderungen des Fingerabdrucks `ssh-keygen -R ip` wenn ssh zum Reinigen verwendet wird

Führen Sie dann das folgende Skript aus, um die Partition zu konvertieren
...
Verwenden Sie `snapper -c mnt list` um einen Schnappschuss eines Subvolumes anzuzeigen

`/etc/snapper/configs/mnt` Sie können die spezifischen Backup-Einstellungen dieses Subvolumes anzeigen
...
Starten Sie Ihr Ubuntu-System neu: Nachdem Sie die oben genannten Änderungen vorgenommen haben, müssen Sie Ihren Computer normal und in Ihrem Ubuntu-System neu starten, anstatt in das Rettungssystem.
...
Dies bedeutet, dass Änderungen an vorhandenen Protokollen nicht sofort dauerhaft werden, alle danach generierten Protokolle jedoch dauerhaft bleiben.
...
## Hinweise Zu Schwierigen Und Komplizierten Krankheiten

### Lösen Sie Das Seltsame Problem: Der Server Kann Keine SSH-Verbindung Herstellen

Ich habe zuvor die Github-Aktion verwendet, um ein automatisches Kompilierungs- und Bereitstellungsskript zu erstellen, habe jedoch festgestellt, dass der Bereitstellungsdienst kein SSH ausführen konnte.

Das Aufrufen des Rettungssystems und das Anzeigen der Protokolle ist eine Berechtigungsfrage:

`Missing privilege separation directory: /run/sshd`

Sie können dieses Verzeichnis mit dem folgenden Befehl erstellen
...
Nach wiederholten Untersuchungen liegt es daran, dass der Benutzer, der tar hochgeladen hat `uid` `1001` ist github action

tar Das Entpacken muss ohne Parameter erfolgen `--no-same-owner` es wird für `1001` user in `uid` dekomprimiert
...
Ändern Sie das Dekomprimierungsskript wie folgt:
