...
1. Unterstützt die mehrsprachige Suche und ist klein. Die Größe des Suchkernels nach dem Packen `6.9KB` `gzip` zum Vergleich: Die Größe von `lunrjs` beträgt `25KB` ).
...
Die Anti-Shake-Funktion `debounce` wird auch in der Front-End-Interaktion verwendet (wie folgt implementiert), um die Häufigkeit von Benutzereingaben zu reduzieren, die Suchen auslösen, und den Rechenaufwand zu reduzieren.
