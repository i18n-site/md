...
## `YAML` : د لینک `HTML` څخه په `Markdown` بدلیدو څخه مخنیوی وشي

د `YAML` ارزښت د ژباړې لپاره `MarkDown` ګڼل کیږي.

ځینې وختونه د `HTML` → `MarkDown` څخه تبادله هغه څه ندي چې موږ یې غواړو، لکه `<a href="/">Home</a>` په `[Home](/)` بدلیږي.
