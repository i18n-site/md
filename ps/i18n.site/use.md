...
لومړی، `md` لارښود دننه کړئ او `i18n.site` چل کړئ، کوم چې به له `en` څخه تر `zh` پورې وژباړي.

<img src="https://p.3ti.site/1721114619.avif" style="width:350px">

د چلولو وروسته، ژباړه او کیچ فایلونه به تولید شي مهرباني وکړئ په یاد ولرئ `git add . ` په `md` ډایرکټر کې ذخیره کې اضافه کړئ.

### ځايي مخکتنه

`docker` نصب او پیل کړئ ( `MAC` کارن د `docker` لپاره د چلولو وخت په توګه کارولو وړاندیز [orbstack](https://orbstack.dev) ).
...
کله چې د پیکج `npm` پراساس خپریږي، **ډاډ ترلاسه کړئ `//unpkg.com/`** د `v:` ارزښت `i18n.site` د مخکینۍ لپاره 2 وکاروئ د دې مخکینۍ لارې لاندې د `/.v` کیچ وخت په ځانګړي ډول د نوي ریلیزونو په وخت لید ترلاسه کولو لپاره غوره کړی.
...
د ځان کوربه مینځپانګې لپاره ، د خپل سرور کارولو سربیره ، بل عام اختیار `CDN` `S3` + کارول دي.

تاسو کولی [rclone](https://rclone.org) د `S3` سرور ته د ننوتلو لپاره وکاروئ، بیا لاندې سکریپټ ته مراجعه وکړئ او تعدیل کړئ، او یوازې د هر ریلیز لپاره `S3` ته زیاتیدونکي بدلونونه کاپي کړئ.
...
ځکه چې ویب پاڼه د یو [واحد پاڼې غوښتنلیک](https://developer.mozilla.org/docs/Glossary/SPA) جوړښت کاروي، په یاد ولرئ چې د URL لارې بیا ولیکئ چې له `. ` څخه تر `index.html` پورې نلري.
...
[cloudflare page](//pages.cloudflare.com) `github page` په پرتله، دا د بیا لیکلو لاره برابروي او د چین اصلي ټاټوبي ته ډیر د لاسرسي وړ دی.
...
مهرباني وکړئ د `Add Account` تنظیم `i18n-demo` ته د لاسرسي لپاره کلیک وکړئ.
...
`out/ol/htm` ، په چین کې د بادل پلورونکو څیز ذخیره وکاروئ + لاندې مینځپانګې ځای په ځای کړئ `CDN` .

تاسو کولی شئ د یوې پاڼې غوښتنلیکونو سره د موافقت لپاره لاره بیا لیکلو لپاره د څنډه کمپیوټر وکاروئ د مثال په توګه ، [Baidu Smart Cloud `CDN`](//cloud.baidu.com/product/cdn.html) پدې ډول تنظیم کیدی شي:
...
ځکه چې ریکارډ `MX` او ریکارډ `CNAME` یوځای نشي کولی، که تاسو غواړئ په ورته وخت کې د ډومین نوم بریښنالیکونه ترلاسه کړئ، نو تاسو اړتیا لرئ چې [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten) سکریپټ سره همکاري وکړئ ترڅو ریکارډ `A` ته `CNAME` کچه.
...
د مثال په توګه، په پورتني انځور کې د `*.i18n.site` د پین ډومین نوم ریډیریکیشن ترتیب په لاندې ډول دی:
...
د `.i18n` ډایرکټر لاندې `i18n.site` د ترتیب کولو فایلونه، د ژباړې زیرمه، او نور شتون لري. د توضیحاتو لپاره راتلونکی فصل ["تنظیم"](/i18n.site/conf) وګورئ.

#### `en`

د سرچینې ژبې لارښود، د `.i18n/conf.yml` ترتیب کولو فایل کې د `en` `fromTo` مطابقت لري
