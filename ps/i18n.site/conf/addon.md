...
`htmIndex.js` به د `.i18n/htm/index.js` پای ته داخل شي.
...
* `lang_li` : ژبې لیست، لومړۍ ژبه سرچینه ژبه ده
* `changed` بدل : فایلونه
* `root` : روټ لارښود
...
جوړ شوی `js` رن ټایم [boa](https://github.com/boa-dev/boa) ثانوي پراختیا پراساس دی ، او جوړ شوي افعال په لاندې ډول دي :

* `wPath(path, txt)` : ته ولیکئ
* : متن فایل `rTxt(path)`
* `rBin(path)` : بائنری فایل ولولئ
* `rDir(dirpath)` : ولولئ، د بیرته ستنیدو ارزښت یو : دی، د فایل لیست
