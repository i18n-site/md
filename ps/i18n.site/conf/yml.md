...
### `use: Toc` ، واحد دوتنې کينډۍ د خاکه سره
...
### `use: Md` ، د واحد فایل ټیمپلیټ پرته له خاکه
...
### `use: Doc` ، د څو فایلونو سند ټیمپلیټ
...
د پورته لیکلو طریقه د `url: xxx` سره مساوي ده، او د هغې اړونده فایل `en/xxx/TOC` دی.

#### TOC د منځپانګو شاخص

`i18n.site` به په ډیمو ګودام کې `js` پلگ ان `.i18n/hook/after.tran/TOC.js` اجرا کړي ترڅو د `TOC` ډایرکټر شاخص فایل لوستل شي چې د `doc` ټیمپلیټ ترتیب سره مطابقت لري ترڅو د `json` لارښود خاکه رامینځته کړي.
...
تاسو لیدلی شئ چې `Deme Two` د ډراپ-ډاون مینو لاندې د پروژې ټاګ لاین لري او د پروژې نوم `Your Project slogan` :
...
د پروژې `README.md` د لومړۍ کچې سرلیک د لومړي کولون `:` وروسته مینځپانګه به د پروژې شعار په توګه وګڼل شي.
...
د مثال په توګه، که چیرې `/test` ته لاسرسی ومومي، او `nav:` د دې لارې له مخکینۍ پرته ترتیب شوی وي، او د لټون کولو اوسنی ژبه انګلیسي ده (کوډ `en` )، `/en/test.md` به د ډیفالټ لخوا پورته شي او د ټیمپلیټ `Md` په کارولو سره وړاندې شي.
