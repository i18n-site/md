...
`i18n.site` د غیر ریفریش واحد پاڼې جوړښت غوره کوي ترڅو د لټون لیست کولو اسانتیا لپاره، یو جلا جامد پاڼه او `sitemap.xml` به د کرالرانو لپاره رامینځته شي.

کله چې د لاسرسي غوښتنې `User-Agent` د لټون انجن کرالر لخوا کارول کیږي، غوښتنه به `302` له لارې جامد پاڼې ته لیږدول کیږي.
...
په ترتیب کې، مهرباني وکړئ `i18n.site` په `.i18n/htm/ol.yml` کې د `host:` ارزښت ته بدل کړئ، ډیری اعتراض پلورنځي `s3` لاندې ترتیب کیدی شي، او `region` ساحه اختیاري ده (ډیری اعتراض پلورنځي د دې ساحې تنظیم کولو ته اړتیا نلري).
...
که تاسو د چین اصلي ځمکې ته خدمت چمتو کولو ته اړتیا لرئ، تاسو کولی شئ [د Baidu Smart Cloud](//cloud.baidu.com) څخه کار واخلئ.
...
ځکه چې `R2` د دودیز شوي ډومین نوم لخوا `cloudflare` لاسرسی کیدی، د دریمې ډلې اعتراض ذخیره باید د جامد فایلونو ځای پرځای کولو لپاره وکارول شي.

دلته موږ د مثال په توګه [backblaze.com](https://www.backblaze.com) ترڅو وښیو چې څنګه د دریمې ډلې توکي په `cloudflare` کې ساتل کیږي.
...
د `SSL` څخه `cloudflare` → د کوډ کولو حالت بدل کړئ، `Full` ته ټاکل شوی
