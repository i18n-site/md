...
پیکج `npm` کی بنیاد پر شائع کرتے وقت، `v:` قدر کے سابقہ کے لیے **`//unpkg.com/` استعمال یقینی** بنائیں `i18n.site` نئی ریلیز کو بروقت دیکھنے کے لیے اس سابقہ راستے کے تحت `/.v` کے کیش ٹائم کو خاص طور پر بہتر بنایا ہے۔

ترجمہ اور شائع کرنے کے لیے `md` ڈائریکٹری میں `i18n.site --npm` یا `i18n.site -n` چلائیں۔

اگر آپ شائع کرنے کے لیے مسلسل انضمام کا ماحول استعمال کرتے ہیں، تو `nodejs` انسٹال کرنے کی ضرورت نہیں ہے، صرف لاگ ان اور اشاعت کی اجازت `~/.npmrc` ماحول میں کاپی کریں۔
...
براہ کرم اوپر دی گئی تصویر کے دوسرے مرحلے میں مماثل ڈومین نام کو اس ڈومین نام سے تبدیل کریں جس کا آپ نے پابند کیا ہے۔
...
اس کے بعد، مینلینڈ + میں کلاؤڈ فروشوں کے آبجیکٹ اسٹوریج کا استعمال کریں `CDN` درج ذیل مواد کو تعینات کریں `out/ol/htm` !
