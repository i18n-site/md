...
### `use: Toc` ، آؤٹ لائن کے ساتھ واحد فائل ٹیمپلیٹ
...
### `use: Md` ، آؤٹ لائن کے بغیر واحد فائل ٹیمپلیٹ
...
### `use: Doc` ، ملٹی فائل دستاویز ٹیمپلیٹ
...
اوپر لکھنے کا طریقہ `url: xxx` کے برابر ہے، اور اس کی متعلقہ فائل `en/xxx/TOC` ہے۔

#### TOC مشمولات کا اشاریہ

`i18n.site` ڈیمو گودام میں `js` پلگ ان `.i18n/hook/after.tran/TOC.js` پر عمل کرے گا تاکہ `json` ڈائرکٹری کا خاکہ تیار کرنے کے لیے `doc` ٹیمپلیٹ کنفیگریشن کے مطابق `TOC` ڈائریکٹری انڈیکس فائل کو پڑھ سکے۔

اگر آپ `doc` ٹیمپلیٹ استعمال کرتے ہیں، تو آپ کے پاس یہ پلگ ان ہونا ضروری ہے۔

اگر آپ خالی فولڈر سے پروجیکٹ `i18n.site` شروع کرتے ہیں تو، ڈیمو پروجیکٹ `.i18n` اپنی ڈائرکٹری میں کاپی کرنا یاد رکھیں۔
