...
پھر، `~/.config/i18n.site.yml` ترمیم کریں اور درج ذیل ترتیب شامل کریں :
...
اگر آپ نے `~/.config/i18n.site.yml` میں ترمیم کی ہے اور دوبارہ اپ لوڈ کرنا چاہتے ہیں، تو براہ کرم پراجیکٹ روٹ ڈائرکٹری میں درج ذیل کمانڈ کو اپ لوڈ کیشے کو صاف کرنے کے لیے استعمال کریں :
...
`URL redirect` ڈائنامک ری ڈائریکشن کو منتخب کریں، براہ کرم `/en` ری ڈائریکشن پاتھ `concat("/en",http.request.uri.path,".htm")` میں اس ڈیفالٹ زبان میں تبدیل کریں جسے آپ سرچ انجنز کو شامل کرنا چاہتے ہیں۔
...
پھر اسکرپٹ کو [ایج EdgeJS](//console.bce.baidu.com/cdn/#/cdn/ejs/list) میں درج ذیل بنائیں
...
مثال کے طور پر، [Huawei DNS](https://www.huaweicloud.com) مفت علاقائی تجزیہ فراہم کرتا ہے، جس کے ساتھ مین لینڈ چینی ٹریفک Baidu Smart Cloud سے گزر سکتی ہے، اور بین الاقوامی ٹریفک `cloudflare` سے گزر سکتی ہے۔
...
چونکہ `R2` کسی حسب ضرورت ڈومین نام کے ذریعے رسائی حاصل `cloudflare` کی جاسکتی ہے، اس لیے تھرڈ پارٹی آبجیکٹ اسٹوریج کو جامد فائلوں کو رکھنے کے لیے استعمال کرنے کی ضرورت ہے۔

یہاں ہم ایک مثال کے طور پر یہ ظاہر کرتے [backblaze.com](https://www.backblaze.com) کہ تھرڈ پارٹی اشیاء کو `cloudflare` پر کیسے باندھنا ہے۔
...
اس کے علاوہ، اوپر کے `cloudflare` کنورژن اصول میں، `index.html` کو `file/your_bucketname/index.html` میں تبدیل کر دیا گیا ہے، اور دیگر کنفیگریشنز وہی رہیں گی۔
