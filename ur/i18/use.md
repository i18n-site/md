...
سسٹم، پہلے [`git bash` ڈاؤن لوڈ اور انسٹال کرنے کے لیے یہاں کلک کریں](https://git-scm.com/download/win) windows
...
ان میں، `.i18n/cache/.gitignore` کا مواد مندرجہ ذیل ہے :
...
اس کا مطلب یہ ہے کہ ڈائریکٹری `.i18n/cache/` (سوائے `.i18n/cache/.gitignore` ) میں موجود تمام فائلوں کو نظر انداز کر دیا گیا ہے۔

اگر آپ کا ورژن کنٹرول سافٹ ویئر `git` نہیں ہے، تو براہ کرم اس ترتیب کے مطابق اسے نظر انداز کریں۔
...
: مختلف ذیلی ڈائرکٹریز کے لیے مختلف `fromTo` ترتیب دے سکتے ہیں /
...
اس کنفیگریشن ٹیبل میں، کیٹلاگ `blog` ترجمہ کی ماخذ زبان `zh` ہے، اور `blog/your_file_name.md` کے ترجمہ کی ماخذ زبان `ja` ہے۔
