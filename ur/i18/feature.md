...
mixed Markdown کے ترجمہ کی حمایت کرتا ہے، `MarkDown` میں ایمبیڈڈ HTML کے `<pre>` اور `<code>` ٹیگز میں موجود مواد کا ترجمہ نہیں کیا گیا ہے HTML

## ریاضی کے فارمولوں کو پہچان سکتے ہیں اور ترجمہ چھوڑ سکتے ہیں۔

ریاضی کے فارمولے پہچانے جاتے ہیں اور ترجمہ چھوڑ دیا جاتا ہے۔
...
سٹیٹک بلاگ کی ہیڈر کنفیگریشن کو سپورٹ کرتا [HOGO](https://github.com/gohugoio/hugo) ، جو صرف فیلڈز `title` ، `summary` اور `description` کا ترجمہ کرتا ہے۔

## متغیر ناموں کا ترجمہ نہ کریں۔

`Markdown` کو ای میل ٹیمپلیٹ کے طور پر استعمال کیا جاتا ہے، `YAML` کو زبان کی فائل کنفیگریشن کے طور پر استعمال کیا جاتا ہے، اور متغیر پیرامیٹرز ہوں گے (مثال کے طور پر: ریچارج `${amount}` موصول ہو گیا ہے)۔
...
فرانسیسی میں `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :

`Falcon` کا ترجمہ `faucon` اور `Llama` کا ترجمہ `lama` کے طور پر کیا گیا ہے۔ مناسب اسم کے طور پر، ان کا ترجمہ نہیں کیا جانا چاہئے.
...
مندرجہ بالا کا DeepL میں ترجمہ (صحیح ناموں کو دوبارہ لکھنا اور odd `... ` شامل کرنا):
