...
Бул [i18n.site](//i18n.site) `i18n.site` таза толук тексттик издөө технологиясын бөлүшөт.

Код ачык булак [издөө өзөгү](//github.com/i18n-site/ie/tree/main/qy) [Интерактивдүү интерфейс](//github.com/i18n-site/plugin/tree/main/qy) /

## Серверсиз Толук Тексттик Издөө Чечимдерине Сереп Салуу

Таза статикалык документтер/жеке блогдор сыяктуу кичинекей веб-сайттар үчүн, албетте, өз алдынча толук тексттик издөө серверин түзүү өтө оор жана кызматтарсыз толук тексттик издөө, албетте, жакшыраак салмак.

Учурдагы серверсиз толук тексттүү издөө чечимдери эки кеңири категорияга бөлүнөт.

Алардын бири - [algolia.com](//algolia.com) ал алдыңкы толук тексттик издөө компоненттерин камсыз кылат.

Мындай кызматтар төлөмдү талап кылат жана веб-сайтка шайкеш келүү маселелеринен улам материктик Кытайдагы колдонуучуларга жеткиликтүү эмес.
...
Көбүрөөк белгилүү таза алдыңкы толук тексттик издөөлөр жана [ ElasticLunr.js ] [https://github.com/weixsong/elasticlunr.js](%E5%9F%BA%E4%BA%8E%60lunrjs%60%E4%BA%8C%E6%AC%A1%E5%BC%80%E5%8F%91) [lunrjs](https://lunrjs.com)

`lunrjs` Индекс түзүүнүн эки жолу бар, бирок экөөнүн тең өз көйгөйлөрү бар.
...
2. Документтерди жүктөңүз жана индекстерди тез арада түзүңүз
...
[fusejs](https://www.fusejs.io) издөө үчүн саптардын окшоштугун эсептеңиз.

Бул чечимдин иштеши өтө начар жана аны толук тексттик издөө үчүн колдонуу мүмкүн эмес (караңыз [Fuse.js Узак суроо 10 секунддан ашык убакытты талап кылат, аны кантип оптималдаштыруу керек?](https://stackoverflow.com/questions/70984437/fuse-js-takes-10-seconds-with-semi-long-queries) ).

[TinySearch](https://github.com/tinysearch/tinysearch) , издөө үчүн Блум чыпкасын колдонуңуз, префиксти издөө үчүн колдонулбайт (мисалы, `goo` , издөө `good` , `google` ) жана окшош автоматтык бүтүрүү эффектине жетише албайт.

Колдонуудагы чечимдердин кемчиликтерине нааразы болуп, `i18n.site` төмөнкү өзгөчөлүктөргө ээ болгон жаңы таза алдыңкы толук тексттик издөө чечимин иштеп чыкты :
...
1. Колдонуучу терип жатканда издөө натыйжаларын реалдуу убакытта көрсөтө турган префикс издөөнү колдойт.
...
`IndexedDB` 5 объект сактагыч таблица түзүлгөн :
...
Фронттун өз ара аракетинде, биринчи жолу жүктөөдө артта калуудан качуу үчүн индекстин жүктөлүшүнүн жүрүшү көрсөтүлүшү мүмкүн "Бирдиктүү progress + Таза css Ишке ашырууга негизделген прогресс тилкеси" [Кыргызча](https://dev.to/i18n-site/a-single-progress-uses-pure-css-to-achieve-animation-effects-2oo) / [Кытай](https://juejin.cn/post/7413586285954154522) .

### IndexedDB Жогорку Параллелдүү Жазуу

Долбоор асинхрондук инкапсуляциянын негизинде [idb](https://www.npmjs.com/package/idb) IndexedDB
...
Издөө натыйжаларын бөлүп көрсөтүү түпнуска текстти кайра жүктөөнү талап кылат жана `service worker` дал келүү кайталануучу тармак сурамдарынан качууга жардам берет.
...
Инверттелген толук тексттик издөө, тез жооп берүү менен жана сервердин кереги жок, алдыңкы бетинде гана ишке ашырылат.

Документтер жана жеке блогдор сыяктуу чакан жана орто веб-сайттар үчүн абдан ылайыктуу.