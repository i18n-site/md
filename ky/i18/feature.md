...
Markdown HTML колдойт, `MarkDown` ичинде камтылган HTML `<pre>` жана `<code>` тегдериндеги мазмун которулбайт
...
`title` , `summary` жана `description` талааларын гана которгон статикалык блогдун баш конфигурациясын колдойт [HOGO](https://github.com/gohugoio/hugo)

## Өзгөрмө Аттарын Которбоңуз

`Markdown` электрондук почта үлгүсү катары колдонулат, `YAML` тил файлынын конфигурациясы катары колдонулат жана өзгөрүлмө параметрлер болот (мисалы: `${amount}` заряды алынды).
