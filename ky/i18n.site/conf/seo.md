...
Мисал катары демо долбоордогу `.i18n/htm/ol.yml` конфигурация файлын алалы
...
Сураныч, алгач жогорудагы `host:` маанисин домен атыңызга өзгөртүңүз, мисалы `i18n.site` .
...
Конфигурацияда `i18n.site` `.i18n/htm/ol.yml` `host:` маанисине өзгөртүңүз, бир нече объект дүкөндөрү `s3` астында конфигурацияланса болот, ал эми `region` талаасы милдеттүү эмес (көп объект дүкөндөрүндө бул талааны коюунун кереги жок).
...
Эгер сиз материктик Кытайга кызмат көрсөтүү керек болсо, анда сиз [Baidu Smart Cloud](//cloud.baidu.com) колдоно аласыз.
...
Адегенде ыктыярдуу домен атын `cloudflare` менен байлаңыз, андан кийин `SSL/TLS` → колдонуучунун домендик аталышын колдонуп, негизги домендик аталышты бул домендик ат менен байланыштырыңыз.

![](https://p.3ti.site/1725438658.avif)

### `cloudflare R2` Ыңгайлаштырылган Домен Аты Аркылуу Кирүү Мүмкүн Эмес

`R2` ылайыкташтырылган домен аты менен кирүү `cloudflare` , статикалык файлдарды жайгаштыруу үчүн үчүнчү тараптын объект сактагычын колдонуу керек.

Бул жерде биз `cloudflare` сактала турган үчүнчү тараптын объекттерин кантип байланыштырууну көрсөтүү үчүн мисал [backblaze.com](https://www.backblaze.com) .
...
Домен атын `cloudflare` `CNAME` `f003.backblazeb2.com` өзгөртүп, проксиди иштетиңиз.
...
Төмөндө көрсөтүлгөндөй конверсия эрежесин кошуңуз, аны биринчи коюңуз (биринчи эң төмөнкү артыкчылыкка ээ):
