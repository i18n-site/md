...
Эгерде сиз версияны оңдоону кааласаңыз, анда `i18n.addon/toc@0.1.3` деп жазсаңыз болот.

Translation буйрук сабы `i18n.site` плагин пакетинин конвенциялык файлын орнотуп, анан аны аткарат.
...
Камтылган `js` иштөө убактысы экинчилик иштеп чыгууга негизделген жана орнотулган функциялар төмөнкүдөй : [boa](https://github.com/boa-dev/boa)

* `wPath(path, txt)` файлга :
* `rTxt(path)` : Текст файлын окуу
* `rBin(path)` : бинардык файлды окуу
* `rDir(dirpath)` Каталогду : , кайтаруу мааниси массив : тизмеси, файл тизмеси
