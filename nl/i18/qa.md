...
Als u per ongeluk regels toevoegt of verwijdert, wat verwarring veroorzaakt, herstel dan de vertaling naar de versie vóór de wijziging, voer opnieuw `i18` uit en cache opnieuw de juiste mapping
...
Een waarde in `YAML` wordt beschouwd als `Markdown` voor vertaling

Soms is de conversie van `HTML` naar `Markdown` niet gewenst, zoals bijvoorbeeld `<a href="/">Home</a>` dat wordt omgezet in `[Home](/)`
...
Verwijder de conflicterende bestanden en voer opnieuw `i18` uit voor vertaling