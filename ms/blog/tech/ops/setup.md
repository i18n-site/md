...
Paparan Model CPU `AMD EPYC 9224 24-Core Processor` skor prestasi teras tunggal 1285
...
Perkara pertama ialah mengedit mesin tempatan `~/.ssh/config` Tetapkan alias untuk IP dan kemudian gunakan `ssh-copy-id root@c2` untuk mengkonfigurasi log masuk tanpa kata laluan.
...
Rangkaian dari Eropah ke China tidak stabil Adalah disyorkan `ssh` menggunakan [mosh](//mosh.org)

Buat skrip `c0` dalam `~/.bin` mesin tempatan (terutamanya ditambahkan pada pembolehubah persekitaran `PATH` ) seperti berikut:
...
Selepas penciptaan, berikan kebenaran boleh laku dan pautan lembut `c1` `c2` .
...
Pada masa hadapan, anda boleh terus memasukkan `c0` , `c1` dan anda boleh menggunakan sambungan langsung berkelajuan tinggi `mosh` dan masukkan [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Perubahan cap jari akan berlaku apabila ssh , `ssh-keygen -R ip` untuk membersihkannya
...
Gunakan `snapper -c mnt list` untuk melihat petikan subjilid

`/etc/snapper/configs/mnt` Anda boleh melihat tetapan sandaran khusus subvolum ini
...
Perlu diingat bahawa operasi di atas dilakukan pada sistem menyelamat, jadi perubahan tersebut hanya akan berkuat kuasa sebaik sahaja sistem dibut semula dan tidak lagi dalam mod menyelamat.

Ini bermakna anda tidak akan segera melihat sebarang perubahan pada log sedia ada menjadi berterusan, tetapi semua log yang dijana selepas ini akan berterusan.
...
Selepas siasatan berulang, ia `1001` github action pengguna yang memuat naik tar `uid`

tar Membongkar mesti dilakukan tanpa parameter `--no-same-owner` ia akan dinyahmampatkan menjadi `uid` untuk `1001` pengguna
