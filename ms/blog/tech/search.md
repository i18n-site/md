...
* `/\p{P}/` ialah ungkapan biasa yang sepadan dengan simbol padanan khusus termasuk: `! " # $ % & ' ( ) * + , - . / : ; < = > ? @ [ \ ] ^ _` { | } `.</p><ul><li> `split('.')` adalah kerana `Firefox` pembahagian perkataan pelayar tidak membahagikan `. ` .</li>
...
Andaikan bahawa terdapat `N` perkataan selepas pembahagian perkataan Apabila mengembalikan hasil, hasil yang mengandungi semua kata kunci akan dikembalikan dahulu, dan kemudian hasil yang mengandungi `N-1` , `N-2` ,..., `1` kata kunci akan dikembalikan.
...
`i18n.site` Carian hadapan tulen yang dibangunkan sendiri sumber terbuka, bersaiz kecil dan tindak balas pantas, menyelesaikan kelemahan carian teks penuh bahagian hadapan tulen semasa dan memberikan pengalaman pengguna yang lebih baik.