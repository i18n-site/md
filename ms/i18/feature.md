...
Menyokong konfigurasi pengepala blog statik jenis [HOGO](https://github.com/gohugoio/hugo) , yang hanya menterjemah medan `title` , `summary` dan `description` .
...
### Apabila Diterjemahkan Ke Dalam Bahasa Inggeris, Huruf Pertama Tajuk Akan Ditulis Dengan Huruf Besar Secara Automatik.
...
Jika anda menggunakan Google atau DeepL, kedua-duanya tersalah menterjemah istilah Inggeris yang sepatutnya kekal asli (ambil bahasa Jepun dan Perancis sebagai contoh).
...
Diterjemah ke bahasa `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :
