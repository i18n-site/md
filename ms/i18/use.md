...
Sistem windows , sila [klik di sini untuk memuat turun dan memasang `git bash`](https://git-scm.com/download/win) dahulu.
...
Anda boleh mengkonfigurasi `fromTo` yang berbeza untuk / yang berbeza Satu demonstrasi ditulis seperti berikut :
...
Dalam jadual konfigurasi ini, bahasa sumber terjemahan katalog `blog` ialah `zh` dan bahasa sumber terjemahan `blog/your_file_name.md` ialah `ja` .
...
`ignore` menggunakan [globset](https://docs.rs/globset/latest/globset/#syntax) yang sama seperti `.gitignore` fail.
