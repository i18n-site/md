...
Profil untuk `i18n.site` ialah `.i18n/conf.yml` .

Kecuali untuk tetapan [`i18`](/i18) , `ignore:` dan `i18n:` , fail konfigurasi adalah seperti berikut:
...
Antaranya, `i18n: home` sepadan dengan `home: Home` dalam `en/i18n.yml` .
...
Jika laluan tertentu diakses dan awalan laluannya tidak dikonfigurasikan dalam `nav:` , `MarkDown` fail yang sepadan dengan laluan akan dimuatkan secara lalai dan diberikan menggunakan templat `Md` .

Contohnya, jika `/test` diakses, dan `nav:` dikonfigurasikan tanpa laluan ini, dan bahasa halaman ialah bahasa Inggeris (kod `en` ), `/en/test.md` akan dimuatkan secara lalai dan diberikan menggunakan templat `Md` .
...
`blog` di atas ialah mod item tunggal bagi `Doc` .
...
Konfigurasi `i18n:doc` daripada `.i18n/conf.yml` ialah mod berbilang projek.
...
`i18n.site` akan melaksanakan `js` pemalam `.i18n/hook/after.tran/TOC.js` dalam gudang demo untuk membaca `doc` fail indeks direktori yang sepadan dengan konfigurasi templat `TOC` untuk menjana garis besar `json` direktori.
...
Anda boleh melihat bahawa `Deme Two` mempunyai slogan projeknya di bawah menu lungsur dan garis besar katalog : projek `Your Project slogan` .
