...
Ginin lokacin gudu `js` yana dogara ne akan haɓaka na biyu na [boa](https://github.com/boa-dev/boa) kuma ginanniyar ayyukan sune kamar haka :

* `wPath(path, txt)` : zuwa fayil
* `rTxt(path)` Karanta fayil : rubutu
* `rBin(path)` Karanta fayil : binary
* `rDir(dirpath)` : Karanta kundin adireshi, ƙimar dawowa shine jeri : , jerin fayil

## Jagoran Ci Gaba

Ci gaban plug-in na iya zama tunani [https://github.com/i18n-site/addon](https://github.com/i18n-site/addon)