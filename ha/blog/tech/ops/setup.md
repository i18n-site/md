...
Nuni CPU Model `AMD EPYC 9224 24-Core Processor` makin wasan kwaikwayo guda ɗaya 1285
...
Sabbin sabar sa lokaci-lokaci suna ratayewa, suna tabbatar da samun damar yanar gizo ta hanyar samun dama.
...
Abu na farko shine a gyara na'urar gida ta `~/.ssh/config` IP sannan a yi amfani da `ssh-copy-id root@c2` don saita shiga mara kalmar sirri.
...
Sannan haɓaka tsarin zuwa ubuntu 24 tare da dannawa ɗaya kuma shigar da software da aka saba amfani da su.
...
Wannan matakin yana ɗaukar lokaci mai tsawo Bayan jira na rabin sa'a, haɓakawa da shigar da software da aka saba amfani da su za su yi nasara. Kuna iya tabbatar da shi tare da umarni mai zuwa:
...
Cibiyar sadarwa daga Turai zuwa China ba ta da ƙarfi Ana ba `ssh` shawarar yin amfani da sauri [mosh](//mosh.org)

Ƙirƙiri rubutun `c0` a cikin `~/.bin` na gida (wanda aka fi sani da shi zuwa madaidaicin yanayi `PATH` ) kamar haka:
...
Bayan ƙirƙira, ba da izini da za a iya aiwatarwa da hanyar haɗin gwiwa `c1` `c2` .
...
A nan gaba, zaku iya shigar da kai tsaye `c0` `c1` kuma kuna iya amfani da haɗin kai tsaye mai sauri `mosh` kuma shigar da [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Canjin sawun yatsa zai faru lokacin da ssh yi amfani da `ssh-keygen -R ip` don tsaftace shi
...
Rubutun farawa zai haifar da `~/ubuntu`
...
Yi amfani da `snapper list-configs`

<img alt="" src="https://i-01.eu.org/1718302689.avif">

Yi amfani `snapper -c mnt list` don duba hoton ƙaramin juzu'i

`/etc/snapper/configs/mnt` Kuna iya duba takamaiman saitunan wariyar ajiya na wannan ƙaramin juzu'in
...
Don mayar da hoton uwar garken, da farko danna Crontol a cikin menu na gudanarwa
...
Ba da damar dagewar log ɗin ƙarƙashin tsarin ceto yana buƙatar ka ƙirƙiri littafin da ya dace da hannu kuma ka aiwatar da wasu ƙarin matakai, saboda ƙila ba ka da sabis na tsarin aiki. Anan akwai matakan da zaku iya gwadawa:
...
Ka tuna cewa ana yin ayyukan da ke sama akan tsarin ceto, don haka waɗannan canje-canjen za su fara aiki ne kawai da zarar an sake kunna tsarin kuma baya cikin yanayin ceto.

Wannan yana nufin cewa nan da nan ba za ku ga kowane canje-canje ga rajistan ayyukan da ke akwai sun zama masu naci ba, amma duk rajistan ayyukan da aka samar bayan wannan za a dage.
...
Bayan bincike akai-akai, github action mai amfani `1001` ya loda tar `uid`

tar Dole ne a yi cirewa ba tare da sigogi ba `--no-same-owner` za a narkar da shi zuwa `uid` mai amfani da `1001`
