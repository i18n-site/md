...
Porque ao escrever documentos, os nomes dos arquivos Markdown são frequentemente ajustados, resultando em muitos arquivos antigos e abandonados no diretório de tradução.

Use este parâmetro para limpar arquivos que devem ser excluídos em diretórios de outros idiomas.

## `-D` Especifique O Diretório De Tradução

O diretório traduzido é padronizado como o diretório onde o arquivo atual está localizado.

`-d` Ou `--workdir` pode especificar o diretório de tradução, como:
...
`-h` Ou `--help` pode visualizar a ajuda da linha de comando.