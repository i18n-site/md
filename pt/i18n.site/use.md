...
`i18n.site` inclui uma ferramenta de tradução integrada `i18`, por favor, [clique aqui para consultar o documento de configuração do `i18`](/i18/use).
...
Após a execução, serão gerados arquivos de tradução e cache. Lembre-se de adicionar `git add .` ao repositório no diretório `md`.

### Visualização Local

Instale e inicie o `docker` (os usuários de `MAC` são recomendados a usar [orbstack](https://orbstack.dev) como runtime para o `docker`).
...
O `i18n.site` adota uma arquitetura [de aplicativo de página única](https://developer.mozilla.org/docs/Glossary/SPA), e a página de entrada do site e o conteúdo do site são implantados de forma independente.
...
Para o lançamento oficial, os arquivos alterados serão copiados para o diretório do novo número de versão.
...
`ol` é a abreviação de `online`, indicando o ambiente online, usado para o lançamento oficial. É também o arquivo de configuração padrão ao usar o parâmetro de linha de comando `-n` para lançamento ao `npm`.
...
Aqui, `--save` representa a atualização do número da versão do lançamento.

#### <a rel=id href="#npm" id="npm"></a> Publicar conteúdo no npmjs.com

Publicar conteúdo no [npmjs.com](//npmjs.com) é a solução padrão recomendada (veja [Alta disponibilidade de front-end](/i18n.site/feature#ha)).
...
Ao publicar com base no pacote `npm`, **certifique-se de usar `//unpkg.com/` como prefixo de `v:`**. O `i18n.site` otimizou especialmente o tempo de cache de `/.v` sob este caminho de prefixo para garantir a visualização oportuna de novos lançamentos.

Execute `i18n.site --npm` ou `i18n.site -n` no diretório `md` para traduzir e publicar.

Se você usar um ambiente de integração contínua para publicar, não há necessidade de instalar o `nodejs`. Basta copiar as permissões de login e publicação `~/.npmrc` para o ambiente.
...
Se os usuários na China continental encontrarem problemas de rede e não conseguirem publicar pacotes `npm`, podem definir a variável de ambiente `https_proxy` para configurar o servidor proxy.
...
Como o site usa uma arquitetura [de aplicativo de página única](https://developer.mozilla.org/docs/Glossary/SPA), lembre-se de reescrever os caminhos de URL que não contêm `. ` para `index.html`.
...
Primeiro [clique aqui para criar uma organização no github](https://github.com/account/organizations/new?plan=free). O nome da organização a seguir é `i18n-demo` como exemplo.
...
Quando você publicar o conteúdo do artigo anterior, `out/ol/htm` será gerado. Por favor, acesse este diretório e execute:
...
Como o `github page` não oferece suporte à reescrita de caminho de URL, `404.html` é usado em seu lugar.

Em seguida, execute o seguinte comando no diretório `htm` (substitua `i18n-demo/i18n-demo.github.io.git` pelo seu próprio endereço de repositório):
...
Depois de enviar o código, aguarde até que a implantação do `github page` seja executada com sucesso (veja a imagem abaixo) antes de poder acessá-lo.
...
Se você vinculou outro repositório de organização, pode ser necessário clicar duas vezes em `Add Account` para autorizar antes que a nova organização seja exibida.

![](https://p.3ti.site/1721118306.avif)

Em seguida, selecione o repositório `i18n-demo.github.io` e clique em `Begin setup`. Use os valores padrão para as etapas subsequentes.
...
Se você deseja obter um melhor desempenho de acessibilidade no ambiente de rede da China continental, [registe primeiro um nome de domínio](//beian.aliyun.com).

Em seguida, use o armazenamento `CDN` + o conteúdo `out/ol/htm`.
...
Adicione o nome de domínio no [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) e aponte o nome de domínio `*.xxx.com` para o `CNAME` no Alibaba Cloud `CDN`.
