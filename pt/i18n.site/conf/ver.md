...
`en/demo2/v` é o número da versão atual do projeto, que será exibido ao lado direito do nome do item no esquema da barra lateral.
...
Recomenda-se modificar o número da versão do documento apenas em caso de lançamento de atualizações significativas (como `v1`, `v2`), para evitar que uma excessiva quantidade de versões cause uma indexação confusa nas páginas dos mecanismos de busca.

## Utilize arquivos `v` vazios para dividir os índices de arquivos de diferentes projetos

No projeto de demonstração, além de `en/demo2/v`, também é possível ver que há arquivos `v` vazios nos diretórios `en/blog` e `en/demo1`.

Arquivos `v` vazios não serão exibidos na barra lateral, mas, enquanto existir um arquivo `v`, será gerado um índice independente para os arquivos no diretório e seus subdiretórios.
