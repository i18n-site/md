...
Ukuba usebenzisa indawo eqhubekayo yokudibanisa ukupapasha, akukho mfuneko yokufaka `nodejs` , khuphela nje ukungena ngemvume kunye nokushicilela imvume `~/.npmrc` kwindawo.
...
Ixesha le-cache lezinye iindlela linokumiselwa kunyaka okanye ngaphezulu ukunciphisa izicelo ezingeyomfuneko.
...
Xa upapasha umxholo kwinqaku elidlulileyo, i : `out/ol/htm` yenziwe.
...
Nceda ucofe `Add Account` ukunika imvume yokufikelela kumbutho `i18n-demo` .
...
Ukuba ufuna ukufumana ufikeleleko olungcono kwimo yenethiwekhi yelizwe laseTshayina, nceda [ubhalise igama lesizinda](//beian.aliyun.com) kuqala.

Emva koko, sebenzisa into yokugcina abathengisi bamafu kwilizwe laseChina + Sebenzisa umxholo olandelayo `CDN` `out/ol/htm`
...
`out/ol/htm` wongeze uqwalaselo olufana noku kulandelayo kumhlathi we `/root/i18n/md/out/ol/htm` `server` nginx
