...
Nceda ungabhekiseli ngqo kwifayile yefonti ye iconfont.cn kuba ayinakulayishwa kwisikhangeli se safari.

## Amacandelo Ewebhu

Awukwazi ukubhala `js` ku `foot.pug` Ukuba intsebenziswano iyafuneka, nceda wenze icandelo lewebhu.
...
Ukusebenzisa `${I18N.xxx}` efana nale ndlela yokubhala, idityaniswe no `i18n.yml` , unokufikelela kwiilwimi ezininzi ezibhalwe phantsi.

Ukongeza `class="a"` kwikhonkco kukuthintela ikhonkco ukuba iguqulwe kwi `MarkDown` Bona :
 [➔ `YAML` Indlela yokuthintela ikhonkco `HTML` : iguqulwe ibe yi `Markdown`](/i18/qa#H2) .