...
* `i18n.addon/toc` :
  Yenza isalathiso sikavimba weefayili wesi `json` esekwe ku `TOC` , senziwa ngokungagqibekanga

* `i18n.addon/mouse` iziphumo :
...
Umgca womyalelo wokuguqulela ongu `i18n.site` uza kuhlohla ifayile yendibano yeplagi-in ipakethe kwaye emva koko iphumeze.
...
Ixesha lokubaleka elakhelwe ngaphakathi `js` lisekwe kuphuhliso lwesibini lwe [boa](https://github.com/boa-dev/boa) kwaye imisebenzi eyakhelweyo yile ilandelayo :

* `wPath(path, txt)` :
* `rTxt(path)` Funda : yokubhaliweyo
* `rBin(path)` Funda ifayile :
* `rDir(dirpath)` Funda : : , ixabiso lembuyekezo luludwe lwefayile
