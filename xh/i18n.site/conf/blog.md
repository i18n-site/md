...
Ulwazi lombhali lunokubhalwa kwi-meta-information yenqaku, njenge:

```yml
author: marlowe
```

Emva koko uhlele `author.yml` kuluhlu lolwimi lwemvelaphi kwaye wongeze ulwazi lombhali, olunje :
...
`name` , `url` kunye `title` zonke zikhetho. Ukuba `name` ayimiselwanga, igama eliphambili (apha `marlowe` ) liya kusetyenziswa njenge `name` .
