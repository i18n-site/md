...
CPU `AMD EPYC 9224 24-Core Processor` , inqaku lentsebenzo engundoqo 1285
...
Into yokuqala IP umatshini wobulali `~/.ssh/config` ukuseta isiteketiso, kwaye emva koko usebenzise `ssh-copy-id root@c2` ukuqwalasela igama elingenagama lokugqitha.
...
Uthungelwano olusuka eYurophu ukuya eChina `ssh` [mosh](//mosh.org)

Yenza izikripthi `c0` kulawulo lwendawo `~/.bin` (ingakumbi idityaniswe kwimeko-bume eguquguqukayo `PATH` ) ngolu hlobo lulandelayo:
...
Emva kokudalwa, nika iimvume eziphunyeziweyo kunye nekhonkco elithambileyo `c1` kunye ne `c2` .
...
Kwixesha elizayo, ungangenisa ngokuthe ngqo `c0` kunye no `c1` ukusebenzisa `mosh` yoqhagamshelwano olukhawulezayo oluthe ngqo kwaye ungene [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Kuya ssh utshintsho lweminwe, sebenzisa `ssh-keygen -R ip` ukuyicoca
...
Sebenzisa `snapper list-configs` ukujonga imiqulu esezantsi esetyenzisiweyo izifinyezo.

![](https://i-01.eu.org/1718302689.avif)

Sebenzisa `snapper -c mnt list` ukujonga umfanekiso okhawulezayo wevolyum esezantsi

`/etc/snapper/configs/mnt` unokujonga useto oluthile logcino lwalo mqulu osezantsi
...
Nangona kunjalo, i-granularity yesi sithombe sifinyeziweyo inkulu kakhulu yakuba ibuyiselwe, idatha entsha eyongeziweyo iya kulahleka ngokupheleleyo, ngoko ke nceda uyisebenzise ngononophelo.
...
Qalisa kwakhona kwisixokelelwano sakho se-Ubuntu: Emva kokwenza olu tshintsho lungasentla, kufuneka uqalise ngokutsha umatshini wakho ngokwesiqhelo nakwinkqubo yakho ye-Ubuntu endaweni yenkqubo yokuhlangula.

Ngale ndlela, i-systemd-journald iyakuqala ngoqwalaselo olutsha kwaye iqalise ukugcina iilog ngokuzingileyo kwihard disk.

Gcina ukhumbule ukuba le misebenzi ingentla yenziwa kwinkqubo yokuhlangula, ngoko ke olo tshintsho luya kusebenza kuphela xa inkqubo iqaliswe ngokutsha kwaye ayisekho kwimodi yokuhlangula.
...
Ukungena kwinkqubo yokuhlangula kunye nokujonga iilogi ngumba wemvume:
...
Emva kophando oluphindaphindiweyo, kungenxa github action Umsebenzisi `uid` we- tar ngu `1001`

Ukungapakishi kufuneka ukuba akukho parameter `--no-same-owner` , iyakuxinzelelwa ku `uid` kubasebenzisi `1001` tar

Ngenxa yoko, i-rsync izakutshintsha iqela lomsebenzisi lolawulo lweengcambu, kwaye emva koko awuyi kukwazi ukungena kwi-ssh emva kokuqala kwakhona.
