...
windows Pūnaha, [paatohia ki konei ki te tango me te whakauru i `git bash`](https://git-scm.com/download/win) i te tuatahi.
...
I roto i te konae whirihoranga, ko te raro o `fromTo` :
...
Ma te taunoa, ka whakamaoritia nga konae katoa e timata ana i `.md` me `.yml` i te raarangi reo puna.
...
Ka kitea e te taputapu raina whakahau nga konae katoa ka mutu ki `.yml` i roto i te raarangi konae reo taketake ka whakamaoritia.
...
Ko te whakamaoritanga o `YAML` ka taea hoki te whakarereke-a-ringa (engari kaua e taapiri, e whakakore ranei i nga taviri, raina ranei i roto i te whakamaoritanga).
...
Mena karekau he whakaaetanga tuhituhi a `/usr/local/bin` ka whakauruhia ki `~/.bin` .
