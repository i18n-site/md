...
windows System, [fare clic qui per scaricare e installare prima `git bash`](https://git-scm.com/download/win) .
...
Crea un file `~/.config/i18n.site.yml` e incolla il contenuto copiato al suo interno, come segue:

```
token: YOUR_API_TOKEN
```

Inoltre, è necessario registrare una carta di credito per il pagamento su [i18n.site/payBill](//i18n.site/payBill) (nessuna ricarica necessaria, il sito addebiterà automaticamente le spese in base all'uso, [vedi i prezzi sulla pagina iniziale](/#price)).

## USO

### Progetto di Dimostrazione

Consulta il progetto dimostrativo [github.com/i18n-site/demo.i18](//github.com/i18n-site/demo.i18) per imparare la configurazione della traduzione con `i18`.

Gli utenti cinesi possono clonare [atomgit.com/i18n/demo.i18](//atomgit.com/i18n/demo.i18)

Dopo aver clonato, entra nella directory e esegui `i18` per completare la traduzione.
...
Il directory `en` contiene file dimostrativi di traduzione, che sono solo esempi e possono essere eliminati.
...
Tra questi, il contenuto di `.i18n/cache/.gitignore` è il seguente :
...
Ciò significa che tutti i file nella directory `.i18n/cache/` (eccetto `.i18n/cache/.gitignore` ) vengono ignorati.
...
Se desideri tradurre in tutte le lingue supportate, lascia vuoto dopo `:`. Ad esempio:

È possibile configurare `fromTo` diversi per diverse sottodirectory o file. Un esempio di configurazione è il seguente:
...
È possibile configurare diversi `fromTo` per diverse sottodirectory Files / Una dimostrazione è scritta come segue :
...
In questa tabella di configurazione, la lingua di origine della traduzione del catalogo `blog` è `zh` e la lingua di origine della traduzione `blog/your_file_name.md` è `ja` .
...
## Non aggiungere o eliminare righe durante la traduzione
