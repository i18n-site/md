...
Se accidentalmente si aggiunge o elimina una riga, causando una confusione, ripristina la traduzione al versione precedente, esegui di nuovo `i18` per ricreare la mappatura corretta

La mappatura tra traduzione e testo originale è legata ai token. Crea un nuovo token su [i18n.site/token](//i18n.site/token), elimina `.i18h/hash`, e traduci di nuovo per pulire la mappatura corrotta (ma questo comporterà la perdita di tutte le modifiche manuali)
...
## Conflict dei file sotto `./i18n/hash`

Elimina i file in conflitto e riavvia la traduzione con `i18`