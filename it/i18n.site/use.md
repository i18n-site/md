...
Quindi, entra nella directory `docker` e esegui `./up.sh`, poi visita [https://127.0.0.1](https://127.0.0.1) per visualizzare l'anteprima localmente.

<img src="//p.3ti.site/1721104238.avif" style="width:360px">

### Pubblicazione dei Contenuti

`i18n.site` utilizza un'architettura di [applicazione a pagina singola](https://developer.mozilla.org/docs/Glossary/SPA), con la pagina di ingresso del sito Web e il contenuto del sito Web distribuiti in modo indipendente.
...
#### Uso di -c per Specificare il File di Configurazione
...
`ol` è l'abbreviazione di `online`, che indica l'ambiente online, utilizzato per il rilascio ufficiale e è anche il file di configurazione predefinito quando si utilizza il comando `-n` di riga di comando per il rilascio a `npm`.

È possibile creare anche altri file di configurazione. Utilizza `--htm_conf` sulla riga di comando per specificare il nome del file di configurazione da utilizzare:
...
#### <a rel=id href="#npm" id="npm"></a> Pubblicazione dei Contenuti su npmjs.com
...
Installa `nodejs` e accedi con `npm login`.

Modifica `md/.i18n/htm/ol.yml` e cambia `i18n.site` in `v: //unpkg.com/i18n.site` con il nome del tuo pacchetto `npm`.

Utilizza il nome del pacchetto non occupato su [npmjs.com](//npmjs.com). Utilizzare il nome del dominio del sito web come nome del pacchetto è una buona scelta.

Quando si pubblica in base al pacchetto `npm`, **assicurati di utilizzare `//unpkg.com/`** come prefisso del valore `v:`. `i18n.site` ha ottimizzato il tempo di cache di `/.v` sotto questo prefisso per ottenere una visualizzazione tempestiva delle nuove versioni.

Esegui `i18n.site --npm` o `i18n.site -n` nella directory `md` per tradurre e pubblicare.

Se utilizzi un ambiente di integrazione continua per pubblicare, non è necessario installare `nodejs`. Basta copiare le autorizzazioni di accesso e pubblicazione `~/.npmrc` in quell'ambiente.
...
#### Distribuzione sul Cloudflare Page

[cloudflare page](//pages.cloudflare.com) rispetto a `github page`, offre la riscrittura dei percorsi e una maggiore accessibilità per la Cina continentale.
...
Se desideri ottenere prestazioni di accessibilità migliori nell'ambiente di rete della Cina continentale, [registra prima un nome di dominio](//beian.aliyun.com).
...
Questo requisito può essere soddisfatto con l'aiuto di Alibaba Cloud `CDN` `EdgeScript` ( [documento in inglese](https://www.alibabacloud.com/help/en/cdn/developer-reference/how-edgescript-works) / [documento in cinese](https://help.aliyun.com/zh/cdn/developer-reference/edgescript))
...
#### Distribuzione con nginx

Aggiungi una configurazione simile alla seguente nel paragrafo `server` di nginx. Sostituisci `/root/i18n/md/out/ol/htm` con il percorso del tuo progetto `out/ol/htm`
