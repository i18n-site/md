...
Sempre prendendo a esempio il progetto di dimostrazione, nella directory `md`, il file `.i18n/htm/foot.pug` definisce il footer del sito web.
...
* `foot.css`: Stile del piè di pagina
* `import.css`: Stile predefinito per l'intero sito `i18n.site`
* `conf.css`: Icone e font del piè di pagina

### Icon font

Le icone del piè di pagina sono generate tramite la creazione di un font chiamato `F` su iconfont.cn ([versione inglese](https://www.iconfont.cn/?lang=en-us) / [versione cinese](https://www.iconfont.cn/?lang=zh)).

Crea il tuo icon font secondo le tue esigenze e sostituisci la seguente configurazione in `conf.css`:
