...
CPU Imodeli `AMD EPYC 9224 24-Core Processor` , umphumela wokusebenza we-single-core 1285
...
## Lungiselela Ukungena Ngemvume Mahhala

Ngemuva kwemizuzu embalwa, wonke amaseva acushiwe.

IP yokuqala ukuhlela `~/.ssh/config` womshini wendawo ukuze usethe isibizo, bese usebenzisa `ssh-copy-id root@c2` ukuze ulungiselele ukungena ngemvume okungenaphasiwedi.
...
Esokuqala ukunika amandla i-ipv6 nokuhlola
...
Lesi sinyathelo sithatha isikhathi eside Ngemva kokulinda isigamu sehora, ukuthuthukiswa nokufakwa kwesofthiwe evame ukusetshenziswa kuzophumelela. Ungaqinisekisa lokhu ngomyalo olandelayo:
...
Inethiwekhi esuka e-Europe iye e- `ssh` ayizinzile [mosh](//mosh.org)

Dala imibhalo emi `c0` kumkhombandlela wendawo `~/.bin` (ikakhulukazi yengezwe kokuguquguquka kwemvelo `PATH` ) kanje:
...
Ngemva kokudala, nikeza izimvume ezisebenzisekayo nesixhumanisi esithambile esingu- `c1` no- `c2` .
...
Ngokuzayo, ungafaka ngokuqondile `c0` no `c1` ukuze usebenzise uxhumano oluqondile olu `mosh` olunesivinini esiphezulu bese ufaka [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)

## Guqula Ifomethi Yokuhlukanisa Isuke Ku-Ext4 Iye Ku-Btrfs

[btrfs](https://wikipedia.org/wiki/Btrfs) Ingathatha izifinyezo zediski ukuze uthole ikhophi yasenqolobaneni kalula.
...
ssh Kuzoba nezinguquko zezigxivizo zeminwe, sebenzisa `ssh-keygen -R ip` ukuze uzihlanze
...
Iskripthi sokuqalisa sizodala umkhombandlela `~/ubuntu` , vele usebenzise umyalo olandelayo:
...
Sebenzisa `snapper list-configs` ukuze ubuke amavolumu amancane okusethelwe wona izifinyezo.

![](https://i-01.eu.org/1718302689.avif)

Sebenzisa `snapper -c mnt list` ukuze ubuke isifinyezo sevolumu engaphansi

`/etc/snapper/configs/mnt` ingabuka izilungiselelo ezithile zokulondoloza zale volumu engaphansi
...
Khumbula ukuthi imisebenzi engenhla yenziwa ohlelweni lokuhlenga, ngakho lezo zinguquko ngeke zisebenze kuze kube yilapho uhlelo luqalwa kabusha futhi lungasekho kumodi yokuhlenga.

Lokhu kusho ukuthi ngeke ubone ngokushesha noma yiziphi izinguquko kulogi ezikhona eziqhubekayo, kodwa wonke amalogi akhiqizwa ngemva kwalokho azophikelela.
...
Ngemuva kophenyo oluphindaphindiwe, kungenxa github action Umsebenzisi ongu `uid` we- tar ngu `1001`
