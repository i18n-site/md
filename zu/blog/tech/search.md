...
* `/\p{P}/` isisho esivamile esihambisana nezimpawu zokubhala eziqondile zihlanganisa: `! " # $ % & ' ( ) * + , - . / : ; < = > ? @ [ \ ] ^ _` { | } . `.</p><ul><li> `split('.')` kungenxa yokuthi ukuhlukaniswa kwegama lesiphequluli esi `Firefox` akunayo ingxenye yesi `. ` .</li>
...
Qaphela ukuthi isikhathi ngasinye uma usesha futhi ngemva kuka `yield` , udinga ukuvula kabusha umsebenzi wombuzo ongu `IndexedDB` .
