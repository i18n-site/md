...
Ama-plug-in wonke amaphakheji angu `npm` .
...
Uma ufuna ukulungisa inguqulo, ungabhala u `i18n.addon/toc@0.1.3` .
...
Isikhathi sokusebenza esakhelwe ngaphakathi esingu- `js` sisekelwe ekuthuthukisweni kwesibili [boa](https://github.com/boa-dev/boa) , futhi imisebenzi eyakhelwe ngaphakathi imi kanje :

* `wPath(path, txt)` :
* `rTxt(path)` Funda : lombhalo
* `rBin(path)` Funda : kanambambili
* `rDir(dirpath)` Funda uhla : , inani lokubuyisela liwuhlu : , uhlu lwamafayela
