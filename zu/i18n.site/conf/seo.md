...
`i18n.site` ithatha ikhasi elilodwa elingavuseleli kabusha Ukuze kube lula ukukhonjwa kokusesha, ikhasi elimile elihlukile kanye no `sitemap.xml` kuzokwenziwa ukuze abaseshi bakhase.
...
Ekucupheni, sicela ushintshe `i18n.site` enani lika `host:` koku- `.i18n/htm/ol.yml` , izitolo eziningi zezinto zingalungiselelwa ngaphansi `s3` , futhi inkambu `region` ingokuzithandela (izitolo eziningi zezinto azidingi ukusetha le nkambu).

Bese usebenzisa `i18n.site -n` ukuze ushicilele kabusha iphrojekthi.

Uma ushintshe u `~/.config/i18n.site.yml` futhi ufuna ukuphinda ulayishe, sicela usebenzise umyalo olandelayo kumkhombandlela wempande yephrojekthi ukuze usule inqolobane yokulayisha :
...
`URL redirect` Khetha ukuqondisa kabusha okuguquguqukayo, sicela uguqule `/en` endleleni yokuqondisa kabusha `concat("/en",http.request.uri.path,".htm")` ukuya olimini olumisiwe ofuna izinjini zokusesha ziluhlanganise.
...
Bese udala iskripthi [kusevisi ye EdgeJS edge](//console.bce.baidu.com/cdn/#/cdn/ejs/list) ngendlela elandelayo
...
Uma ufuna ukuhlinzeka ngezinsizakalo ezweni laseChina futhi ufuna `cloudflare` ithrafikhi yamazwe ngamazwe yamahhala, ungasebenzisa u `DNS` ngokulungiswa kwesifunda.
...
Ngenxa yokuthi `R2` `cloudflare` ukufinyelelwa ngegama lesizinda elenziwe ngokwezifiso, isitoreji sento yenkampani yangaphandle sidinga ukusetshenziselwa ukubeka amafayela amile.
...
Shintsha okuthi `cloudflare` koku `SSL` → imodi yokubethela, isethelwe ku `Full`
...
`Rewrite to` khetha okuguquguqukayo bese ulungisa `your_bucketname` koku `concat("/file/your_bucketname",http.request.uri.path)` egameni lakho lebhakede.

Ukwengeza, kumthetho wokuguqulwa `cloudflare` ngenhla, `index.html` uguqulwa ube `file/your_bucketname/index.html` , futhi okunye ukulungiselelwa kuhlala okufanayo.
