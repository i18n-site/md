...
Igama lohla lwemibhalo lesisekelo sekhodi engu `demo.i18n.site` kufanele libe `md` ukuze kube nokuhlola kuqala kwendawo ngo `docker` .
...
`dev.yml` kanye `ol.yml` yizicucu ezizenzakalelayo.
...
Uma usebenzisa indawo eqhubekayo yokuhlanganisa ukuze ushicilele, asikho isidingo sokufaka `nodejs` , vele ukopishe ukungena ngemvume bese ushicilela imvume `~/.npmrc` endaweni.
...
Uma ufuna ukuzibamba ngokwakho okuqukethwe, qala uhlele okuthi `md/.i18n/htm/ol.yml` bese ushintsha `v: //unpkg.com/i18n.site` kusiqalo se-URL yakho, njengokuthi `v: //i18n-v.xxx.com` .

Faka uhla lwemibhalo `md` bese ugijima
...
Sicela ushintshe igama lesizinda elimedayo esinyathelweni sesibili esithombeni esingenhla siye egameni lesizinda osibophe ngalo.
...
Sicela ungeze ukucushwa `out/ol/htm` nalokhu okulandelayo endimeni engu `/root/i18n/md/out/ol/htm` `server` ye- nginx
