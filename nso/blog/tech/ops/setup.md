...
Showing Model `AMD EPYC 9224 24-Core Processor` , maemo a tshebetso ya motheo o le 'ngoe 1285 CPU
...
Selo sa pele IP go rulaganya `~/.ssh/config` ya motšhene wa selegae go beakanya leina le lengwe, gomme ka morago o šomiše `ssh-copy-id root@c2` go beakanya go tsena ka ntle le phasewete.
...
Ebe ntlafatsa tsamaiso ho ubuntu 24 ka ho tobetsa e le 'ngoe mme o kenye software e sebediswang ka tloaelo.
...
Mogato wo o tšea nako ye telele Ka morago ga go leta seripa sa iri, go kaonafatša le go tsenywa ga disoftware tšeo di šomišwago ka tlwaelo go tla atlega. O ka tiišetša se ka taelo ye e latelago:
...
Neteweke go tloga Yuropa go ya China ga e tsepame [mosh](//mosh.org) dirišwa go akgofiša `ssh` .

Hlama dingwalwa `c0` ka go tšhupetšo ya selegae `~/.bin` (kudukudu e okeditšwe go phetogo ya tikologo `PATH` ) ka tsela ye e latelago:
...
Ka morago ga tlholo, fa ditumelelo tše di phethagatšwago le kgokagano ya boleta `c1` le `c2` .
...
Nakong e tlago, o ka tsenya ka go lebanya `c0` le `c1` go diriša kgokagano ya thwii ya lebelo le legolo `mosh` gomme wa tsenya [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)

## Fetola Sebopego Sa Karoganyo Go Tšwa Go ext4 Go Ya Go btrfs

[btrfs](https://wikipedia.org/wiki/Btrfs) E ka tšea ditshwantšho tša disk bakeng sa go dira bekapo e bonolo.
...
ssh Go tla ba le diphetogo tša kgatišo ya menwana, šomiša `ssh-keygen -R ip` go e hlwekiša
...
Sengwalwa sa go thoma se tla hlola tšhupetšo ya `~/ubuntu` , e no sepetša taelo ye e latelago:
...
Šomiša `snapper list-configs` go lebelela dibolumo tše nnyane tšeo diswantšho tša go tšewa ka lebelo di beakantšwego go tšona.

![](https://i-01.eu.org/1718302689.avif)

Šomiša `snapper -c mnt list` go lebelela seswantšho sa bolumo ya ka fasana

`/etc/snapper/configs/mnt` ka sheba di-setting itseng bekapo ya subvolume ena
...
Dula o gopola gore ditiro tše di lego ka mo godimo di dirwa tshepedišong ya phološo, ka gona diphetogo tšeo di ka se šome go fihlela tshepedišo e thongwa gape gomme e se sa le mokgweng wa phološo.

Se se ra gore o ka se bone diphetogo dife goba dife ka pela go dilog tše di lego gona tšeo di fetogago tše di phegelelago, eupša dilog ka moka tšeo di tšweleditšwego ka morago ga moo di tla phegelela.
...
Ka morago ga dinyakišišo tše di boeletšwago, ke ka lebaka la gore github action Modiriši `uid` wa tar ke `1001`

tar Go phutholla e swanetše go ba ge e ba go se na paramethara `--no-same-owner` , e tla decompressed go `uid` bakeng sa badiriši `1001`
