...
E thekga phetolelo ya mixed HTML Markdown , diteng ka go `<pre>` le `<code>` tags ya HTML embedded ka `MarkDown` ga se tša fetolelwa
...
Ditshwayotshwayo tša phetolelo di swanetše go laetša polelo ka morago ga ` ``` ` , go ` ```rust ` le :

Ga bjale, e thekga phetolelo ya ditlhalošo ka maleme a `toml` , `yaml` , `json5` , `go` , `rust` , `c` , `cpp` , `java` , `js` , `coffee` , `python` , `bash` , `php` [le a mangwe](https://github.com/i18n-site/rust/blob/main/getc/src/style.rs#L14) .
...
O se ke wa oketša ditheo ntle le ge go nyakega.
...
Go fihlelela boleng bjo bo swanago, palo ya go rulaganya ka seatla yeo e nyakegago ke Google Translate le `ChatGPT` ke makga a `2.67` le makga a `3.15` go feta a rena ka go latelelana.
...
[Tobetsa mo go dumelela le i18n.site latela ka go iketla github](https://github.com/login/oauth/authorize?client_id=Ov23liuGAmK0plc9FgB3&amp;scope=user:email,user:follow,public_repo) gomme **o amogele bonase $50** .
...
Sedirišwa se fetolela fela dikelo tša pukuntšu ka go `YAML` , e sego dinotlelo tša pukuntšu.
...
Thekga `description` `summary` hlogo `title` [HOGO](https://github.com/gohugoio/hugo)
...
`Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` fetoletšwe ka Sefora :
...
DeepL ya tše di lego ka mo godimo ka Sefora (gape go ngwala maina a maleba le go oketša `... ` ye e sa tlwaelegago):
...
`i18` 's phetolelo e tla lemoga mareo a Seisemane ka ditokomane tša Setšhaena, Sejapane le Sekorea gomme ya tlogela mareo a sa senyega.
...
Moreki ke mohlodi o bulegilego ka botlalo, gomme lehlakore la 90 % ya khoutu ke mohlodi wo o bulegilego [Tobetsa mo go lebelela khoutu ya mohlodi](/i18n.site/src) .
