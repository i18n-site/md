...
* `lang_li` Lenaneo : polelo, polelo ya mathomo ke polelo ya mothopo
...
Nako ya go sepela `js` yeo e agetšwego ka gare e theilwe godimo ga tlhabollo ya bobedi : [boa](https://github.com/boa-dev/boa)

* `wPath(path, txt)` : Ngwala go faele
* `rTxt(path)` : Bala faele ya sengwalwa
* `rBin(path)` : Bala faele ya binary
* `rDir(dirpath)` : Bala tšhupetšo, boleng bja go boela morago ke lenaneo la : , lenaneo la faele
