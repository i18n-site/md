...
* `foot.css` : Setaele sa letlapa la ka tlase
* `import.css` : `i18n.site` tlwaelo sa sebaka ka moka
...
Ka go šomiša `${I18N.xxx}` ye e swanago le mokgwa wo wa go ngwala, e kopantšwe le `i18n.yml` , o ka fihlelela boditšhabatšhaba bja maleme a mantši ga letlakala la ka tlase.

Go oketša `class="a"` go kgokagano ke go thibela kgokagano go fetošetšwa go `MarkDown` Bona :
 [➔ `YAML` : ya go thibela kgokagano ya `HTML` go fetošetšwa go `Markdown`](/i18/qa#H2) .