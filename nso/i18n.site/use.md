...
Leina la tšhupetšo la clone ya motheo wa khoutu ya `demo.i18n.site` e swanetše go ba `md` go nolofatša ponelopele ya selegae ka `docker` .
...
Ka morago ga go šoma, difaele tša phetolelo le tša cache di tla tšweletšwa Hle gopola go `git add . ` oketša polokelong ka gare ga tšhupetšo ya `md` .
...
Ka morago ga go sepetša phetolelo ye e lego ka mo godimo, dipuku tša ditšhupetšo tša `htm` le `v` di tla tšweletšwa ka fase ga tšhupetšo ya `md/out/dev` .
...
Difaele tša peakanyo tše di fapanego di tla hlola dipuku tša go tshwanela ka gare ga tšhupetšo ya `out` .
...
Ge e ba badiriši ba nageng-kgolo ya China ba kopana le mathata a netweke gomme ba sa kgone go phatlalatša diphuthelwana `npm` , ba ka beakanya phetogo ya tikologo `https_proxy` go beakanya seva ya moemedi.
...
Ka morago ga moo, beakanya diteng ka go tšhupetšo ya `md/out/ol/v` go tsela ya hlogo ya URL yeo e beakantšwego go `v:` .
...
Go itshwara diteng, go `CDN` go šomiša seva ya gago, kgetho ye nngwe ye e tlwaelegilego ke go šomiša `S3` +

O ka šomiša [rclone](https://rclone.org) go tsena go seva ya `S3` , ke moka wa lebelela le go fetoša sengwalwa se se latelago, gomme wa kopiša fela diphetogo tša go oketšega go `S3` bakeng sa tokollo ye nngwe le ye nngwe.
...
Pele [tobetsa github ho bopa mokhatlo](https://github.com/account/organizations/new?plan=free) o hlophisitsoeng o latelang ke `i18n-demo` e le mohlala.
...
Ge e bapetšwa le [cloudflare page](//pages.cloudflare.com) `github page` e fana ka go ngwala gape ga tsela gomme e na le bogwera kudu go naga-kgolo ya China gomme e kgothaletšwa go e diriša.
...
Ka morago ga moo, diriša polokelo ya dilo ya barekiši ba leru nageng-kgolo + China `CDN` Deploy diteng tše di latelago `out/ol/htm` .
...
Ka lebaka la gore rekoto `MX` le rekoto `CNAME` di ka se phele mmogo, ge o nyaka go amogela di-imeile `CNAME` leina la domain ka nako e tee, o swanetše go dirišana le sengwalwa `A` [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten)
...
Oketša leina la domain ka go [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) gomme o šupe leina la domain `*.xxx.com` go ya go `CNAME` ka go Alibaba Cloud `CDN` .
...
Hle oketša peakanyo `/root/i18n/md/out/ol/htm` e swanago le ye `out/ol/htm` latelago mo temaneng ya `server` ya nginx
