...
[i18n.site/payBill](//i18n.site/payBill) ، برای پرداخت باید یک کارت اعتباری متصل کنید (بدون نیاز به شارژ مجدد، وب سایت به طور خودکار هزینه ها را با توجه به استفاده کسر می کند، [برای قیمت گذاری به صفحه اصلی مراجعه کنید](/#price) ).
...
`fromTo` مختلف را می : برای زیر شاخه های مختلف پیکربندی کرد /
...
در این جدول پیکربندی، زبان مبدأ برای ترجمه دایرکتوری `blog` `zh` و زبان مبدأ برای ترجمه دایرکتوری `blog/your_file_name.md` `ja` است.

### نادیده گرفتن فایل

به طور پیش فرض، تمام فایل هایی که با `.md` و `.yml` در فهرست زبان مبدأ شروع می شوند، ترجمه خواهند شد.

اگر می خواهید فایل های خاصی را نادیده بگیرید و آنها را ترجمه نکنید (مانند پیش نویس های ناتمام)، می توانید از پیکربندی فیلد `ignore` استفاده کنید.
