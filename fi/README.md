...
## <a rel=id href="#project" id="project"></a> Hankkeen kuvaus
...
Käännä `Markdown` ja `YAML` komentorivityökaluksi ( [lähdekoodiksi](https://github.com/i18n-site/rust/tree/main/i18) ) useilla kielillä.

Voi täydellisesti ylläpitää muotoa `Markdown` Pystyy tunnistamaan tiedostojen muutokset ja kääntämään vain muuttuneet tiedostot.
...
Jaamme Kaikki on " " `UNIX` ja voimme hallita käännöksiä sadoilla kielillä ilman monimutkaisia ja hankalia yritystason ratkaisuja.

Voit käyttää tutuimpia työkaluja käännöksen `Markdown` (mutta et voi lisätä tai poistaa kappaleita) ja käyttää tutuinta tapaa tehdä versionhallinta.

Koodikanta voidaan luoda avoimeksi lähdekoodiksi kielitiedostoille, ja `Pull Request` :n prosessin avulla globaalit käyttäjät voivat osallistua käännösten jatkuvaan optimointiin. Saumaton yhteys [github](//github.com) muut avoimen lähdekoodin yhteisöt.
...
Puhtaasti staattinen, lukukokemusta varten optimoitu ja integroitu [i18](#i18) se on paras valinta projektidokumenttisivuston rakentamiseen.
...
Tämä verkkosivusto on kehitetty tämän kehyksen pohjalta ja sisältää käyttäjä-, maksu- ja muita toimintoja ( [lähdekoodi](/i18n.site/src) ). Yksityiskohtainen opetusohjelma kirjoitetaan myöhemmin.
