...
Piştgiriya wergerandina mixed HTML dike, naveroka di `<pre>` û `<code>` tagên HTML yên Markdown di nav `MarkDown` de cih digirin nayê wergerandin
...
Pêdivî ye ku şîroveyên werger zimanê piştî ` ``` ` nîşan bidin, : ` ```rust ` .
...
Ji bo bernamenûsên ku bi `git` , `vim` , û `vscode` dizanin, karanîna van amûran ji bo sererastkirina belgeyan û rêvebirina guhertoyan bê guman dê lêçûna fêrbûnê zêde bike.
...
Hin amûrên wergera rêzika fermanê jî hene, wek [translate-shell](https://github.com/soimort/translate-shell)
...
Me nifşek nû ya teknolojiya wergerê pêşxistiye ku avantajên teknîkî yên modelên wergera makîneyî ya kevneşopî û modelên zimanên mezin di nav xwe de dihewîne da ku werger rast, sivik û xweşik bike.
...
[Li vir bikirtînin da ku hûn github bişopînin û i18n.site](https://github.com/login/oauth/authorize?client_id=Ov23liuGAmK0plc9FgB3&amp;scope=user:email,user:follow,public_repo) û **$50 bistînin** .
...
Piştgiriya veavakirina sernavê ya tîpa statîkî [HOGO](https://github.com/gohugoio/hugo) , ku tenê zeviyên `title` , `summary` û `description` werdigerîne.
...
Wergera li `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` :
...
Wergera li `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` :

![](//p.3ti.site/1720199550.avif)

Wergera ya li jor bo DeepL (herweha navên xwerû ji nû ve dinivîsin û `... ` jî lê zêde dikin):
...
Xerîdar bi tevahî çavkaniyek vekirî ye, û aliyê serverê 90 [ji bo dîtina kodê](/i18n.site/src) vekirî ye.

Em li dilxwazan digerin ku beşdarî pêşvebirina koda çavkaniya vekirî û rastnivîsandina metnên wergerandî bibin.
