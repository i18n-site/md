...
Pokud k publikování používáte kontinuální integrační prostředí, není potřeba instalovat `nodejs` , stačí zkopírovat přihlášené a publikační oprávnění `~/.npmrc` do prostředí.

Pokud změníte název balíčku `v:` v `ol.yml` , **nezapomeňte nejprve odstranit `.i18n/v/ol`** a poté je publikovat.
...
Při publikování obsahu v předchozím článku bylo vygenerováno `out/ol/htm` , zadejte tento adresář a spusťte :
...
Poté použijte úložiště objektů cloudových `out/ol/htm` v pevninské Číně + Nasaďte následující obsah `CDN` .
...
Přidejte konfiguraci podobnou následující v odstavci `server` v nginx Změňte prosím `/root/i18n/md/out/ol/htm` na cestu vašeho vlastního projektu `out/ol/htm` :
