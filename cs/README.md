...
* [i18 : MarkDown pro překlad příkazového řádku](/i18/feature)
...
Přeložte `Markdown` a `YAML` do nástroje příkazového řádku ( [zdrojový kód](https://github.com/i18n-site/rust/tree/main/i18) ) ve více jazycích.

Dokáže dokonale udržovat formát `Markdown` Dokáže identifikovat úpravy souborů a překládat pouze změněné soubory.
...
Sdílíme " `UNIX` is a File " a dokážeme spravovat překlady do stovek jazyků, aniž bychom zaváděli složitá a těžkopádná řešení na podnikové úrovni.

K `Markdown` překladu můžete použít nejznámější nástroje (ale nemůžete přidávat ani mazat odstavce) a používat nejznámější způsob správy verzí.

Kódovou základnu lze vytvořit jako otevřený zdroj pro jazykové soubory a pomocí procesu `Pull Request` se mohou globální uživatelé podílet na neustálé optimalizaci překladů. Bezproblémové připojení a [github](//github.com) open source komunity.
...
Čistě statický, optimalizovaný pro čtení a integrovaný s překladem [i18](#i18) je tou nejlepší volbou pro vytvoření stránky projektových dokumentů.
...
Tento web je vytvořen na základě tohoto rámce a obsahuje uživatelské, platební a další funkce ( [zdrojový kód](/i18n.site/src) ).

[→ Pro uživatelskou příručku si přečtěte projektovou dokumentaci](/i18n.site) .
...
>
> PÁN to viděl a řekl: "Všechny lidské bytosti mají stejný jazyk a rasu. Nyní, když je toto provedeno, bude vše hotovo."
>
