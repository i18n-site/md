...
1. Předem vytvořené indexové soubory
   ...
   Kdykoli je dokument přidán nebo upraven, musí být načten nový indexový soubor.
...
V interakci front-end lze / ukazatel [průběhu](//juejin.cn/post/7413586285954154522) načítání indexu [,](//dev.to/i18n-site/a-single-progress-uses-pure-css-to-achieve-animation-effects-2oo) aby se css zpoždění při prvním progress + .
...
Abyste se vyhnuli částečné ztrátě dat způsobené konkurenčním zápisem, můžete se podívat na níže uvedený kód `coffeescript` a přidat mezi čtení a zápis mezipaměť `ing` , abyste zachytili konkurenční zápisy.
...
Zvýraznění výsledků hledání vyžaduje opětovné načtení původního textu a shoda s `service worker` může zabránit opakovaným síťovým požadavkům.
