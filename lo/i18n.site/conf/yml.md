...
### `use: Toc` , ແມ່ແບບໄຟລ໌ດຽວທີ່ມີໂຄງຮ່າງ
...
### `use: Md` , ແມ່ແບບໄຟລ໌ດຽວໂດຍບໍ່ມີໂຄງຮ່າງ
...
ວິທີການຂຽນຂ້າງເທິງແມ່ນເທົ່າກັບ `url: xxx` , ແລະໄຟລ໌ທີ່ສອດຄ້ອງກັນຂອງມັນແມ່ນ `en/xxx/TOC` .

#### ດັດຊະນີຕາຕະລາງເນື້ອໃນ TOC

`i18n.site` ຈະດໍາເນີນການ `js` plug-in `.i18n/hook/after.tran/TOC.js` ໃນສາງສາທິດເພື່ອອ່ານໄຟລ໌ດັດສະນີ `TOC` ໄດເລກະທໍລີທີ່ສອດຄ້ອງກັບການຕັ້ງຄ່າແມ່ແບບ `doc` ເພື່ອສ້າງໂຄງຮ່າງໄດເລກະທໍລີ `json` .
...
ທ່ານສາມາດເບິ່ງວ່າ `Deme Two` ມີໂຄດຄໍາສັ່ງໂຄງການ `Your Project slogan` :
...
ຕົວຢ່າງ, ຖ້າ `/test` ຖືກເຂົ້າເຖິງ, ແລະ `nav:` ຖືກຕັ້ງຄ່າໂດຍບໍ່ມີຄໍານໍາຫນ້າຂອງເສັ້ນທາງນີ້, ແລະພາສາຜົນການຊອກຫາໃນປະຈຸບັນແມ່ນພາສາອັງກິດ (ລະຫັດ `en` ), `/en/test.md` ຈະຖືກໂຫລດໂດຍຄ່າເລີ່ມຕົ້ນແລະສະແດງໂດຍໃຊ້ແມ່ແບບ `Md` .
