...
Chương trình có tích hợp `i18` tính năng dịch thuật, vui lòng xem [➔ `i18`](/i18) để biết cách sử dụng cụ thể.
...
`i18n.site` mặc định, nội dung trang web sẽ được xuất bản lên `npmjs.com` , với sự trợ giúp của [jsdelivr.com](//jsdelivr.com) , [unpkg.com](//unpkg.com) và nhiều nội dung `CDN` được tải trên `npm` .
...
Nguyên tắc là: chặn yêu cầu với sự trợ giúp của [`service worker`](https://developer.mozilla.org/docs/Web/API/Service_Worker_API) , nếu yêu cầu không thành công, hãy thử lại trên `CDN` khác và kích hoạt thích ứng trạm gốc phản hồi nhanh nhất làm nguồn tải mặc định.
...
> Nó từ bỏ việc trang trí không cần thiết và trình bày nội dung ở dạng thuần túy nhất.
...
`404` Có một con ma nhỏ dễ thương nổi trên trang, mắt nó sẽ di chuyển theo con chuột, [➔ Bấm vào đây để xem](/404) ,

## Mã Nguồn Mở

[Mã này là mã nguồn mở](/i18n.site/src) . Nếu bạn muốn tham gia phát triển, vui lòng giới thiệu [danh sách gửi thư](//groups.google.com/u/2/g/i18n-site) của mình.
