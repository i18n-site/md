...
## Tổng Quan Về Các Giải Pháp Tìm Kiếm Toàn Văn Không Có Máy Chủ
...
1. Hỗ trợ tìm kiếm đa ngôn ngữ và có kích thước nhỏ. Kích thước của kernel tìm kiếm sau khi đóng gói `gzip` là `6.9KB` (để so sánh, kích thước của `lunrjs` là `25KB` )
...
1. Hỗ trợ tìm kiếm tiền tố, có thể hiển thị kết quả tìm kiếm theo thời gian thực trong khi người dùng đang gõ.
...
Lưu ý rằng mỗi lần bạn tìm kiếm lại sau `yield` , bạn cần mở lại giao dịch truy vấn là `IndexedDB` .
...
Hạt nhân tìm kiếm sẽ sử dụng bảng `prefix` cho từ cuối cùng sau khi phân đoạn từ để tìm tất cả các từ có tiền tố trước nó và tìm kiếm theo trình tự.
