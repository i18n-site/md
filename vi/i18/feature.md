...
`i18` là một công cụ dòng lệnh để dịch các tệp Markdown & Yaml
...
Hỗ trợ dịch Markdown trong hỗn hợp HTML `<pre>` HTML trong `MarkDown` và nội dung trong thẻ `<code>` không được dịch
...
Các nhận xét dịch cần chỉ ra ngôn ngữ sau ` ``` ` , ` ```rust ` hạn như :

Hiện hỗ trợ dịch các nhận xét `toml` , `yaml` , `json5` , `go` , `rust` , `c` , `cpp` , `java` `js` , `coffee` `python` , `bash` , `php` [và các ngôn ngữ khác](https://github.com/i18n-site/rust/blob/main/getc/src/style.rs#L14) .
...
Đối với những lập trình viên đã quen với `git` , `vim` , `vscode` , việc sử dụng các công cụ này để chỉnh sửa tài liệu và quản lý phiên bản chắc chắn sẽ làm tăng chi phí học tập.

`KISS` ( `Keep It Simple, Stupid` ) Trong số những người tin vào nguyên tắc, các giải pháp cấp doanh nghiệp đồng nghĩa với việc cồng kềnh, kém hiệu quả và khó sử dụng.
...
## Bản Dịch Chất Lượng Cao Của Xindaya

Chúng tôi đã phát triển một thế hệ công nghệ dịch thuật mới tích hợp các ưu điểm kỹ thuật của mô hình dịch máy truyền thống và mô hình ngôn ngữ lớn để giúp bản dịch chính xác, mượt mà và trang nhã.

Các thử nghiệm mù cho thấy chất lượng dịch thuật của chúng tôi tốt hơn đáng kể so với các dịch vụ tương tự.

Để đạt được chất lượng như nhau, số lượng chỉnh sửa thủ công mà Google Translate và `ChatGPT` yêu cầu lần lượt là `2.67` lần và `3.15` lần của chúng tôi.
...
Công cụ này chỉ dịch các giá trị từ điển trong `YAML` , không dịch các khóa từ điển.

Dựa trên `YAML` Translation, bạn có thể dễ dàng xây dựng các giải pháp quốc tế cho nhiều ngôn ngữ lập trình khác nhau.

## Hỗ Trợ Cấu Hình Tiêu Đề `HOGO`

Hỗ trợ cấu hình tiêu đề của blog tĩnh loại [HOGO](https://github.com/gohugoio/hugo) , chỉ các trường `title` , `summary` và `description` được dịch.

## Không Dịch Tên Biến

`Markdown` Được sử dụng làm mẫu email, `YAML` làm cấu hình tệp ngôn ngữ, sẽ có các tham số thay đổi (ví dụ: đã nhận được khoản tiền nạp `${amount}` ).

`${varname}` Loại tên biến này sẽ không được coi là bản dịch tiếng Anh.
...
Trung Quốc, Nhật Bản và Hàn Quốc không có chữ hoa hoặc chữ thường, nhưng tiêu đề tiếng Anh thường viết hoa chữ cái đầu tiên.
...
Ví dụ `为阅读体验而优化` sẽ được dịch là `Optimized for Reading Experience` .
...
Đã dịch sang `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` :

<img alt="" src="//p.3ti.site/1720199391.avif">

Đã dịch sang `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :
...
Đã dịch sang `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` :
...
Bản dịch của `i18` sẽ nhận dạng các thuật ngữ tiếng Anh trong các tài liệu tiếng Trung, tiếng Nhật và tiếng Hàn và giữ nguyên các điều khoản.
...
## Có Thể Kết Hợp Xây Dựng Trang Web Đa Ngôn Ngữ `i18n.site`

`i18` Được tích hợp vào công cụ dòng lệnh để xây dựng trang web đa ngôn ngữ [`i18n.site`](/i18n.site) .

Xem tài liệu của [`i18n.site`](/i18n.site)

## Mã Nguồn Mở

Máy khách hoàn toàn là nguồn mở 90 % phía máy chủ là nguồn mở. [Nhấp vào đây để xem mã nguồn](/i18n.site/src) .
...
/ hoan nghênh theo dõi [i18n-site.bsky.social](https://bsky.app/profile/i18n-site.bsky.social) tài khoản xã hội của chúng tôi [X.COM: @i18nSite](https://x.com/i18nSite)
