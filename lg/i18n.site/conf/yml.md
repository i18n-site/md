...
### `use: Md` , Ekifaananyi Kya Fayiro Emu Nga Tekirina Nsengeka
...
### `use: Doc` , Ekifaananyi Ky'ekiwandiiko Ekirimu Fayiro Eziwera
...
Enkola y'okuwandiika waggulu yenkana `url: xxx` , era fayiro yaayo ekwatagana nayo eri `en/xxx/TOC` .

#### TOC Omuko Gw’ebirimu

`i18n.site` ejja kukola `js` plug-in `.i18n/hook/after.tran/TOC.js` mu demo warehouse okusoma fayiro ya `TOC` directory index ekwatagana n'ensengeka ya `doc` template okukola `json` directory outline.
