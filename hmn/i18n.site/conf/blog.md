...
Cov ntaub ntawv demo tau teeb tsa raws li hauv qab no:
...
Cov ntaub ntawv sau tuaj yeem sau rau hauv cov ntaub ntawv meta-cov ntaub ntawv ntawm tsab xov xwm, xws li:
...
`name` , `url` thiab `title` yog txhua yam xaiv tau. Yog tias `name` tsis tau teeb tsa, lub npe tseem ceeb (ntawm no `marlowe` ) yuav raug siv ua `name` .
