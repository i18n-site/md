...
### `use: Doc` , Ntau Cov Ntaub Ntawv Template
...
Rau cov kev txwv saum toj no, cov ntaub ntawv sib txuas cov ntaub ntawv index yog:
...
Txoj kev sau saum toj no yog sib npaug rau `url: xxx` , thiab nws cov ntaub ntawv sib raug yog `en/xxx/TOC` .

#### TOC Table of Content Index

`i18n.site` yuav ua tiav `js` plug-in `.i18n/hook/after.tran/TOC.js` nyob rau hauv demo warehouse nyeem `TOC` directory index cov ntaub ntawv sib xws rau `doc` template configuration los tsim `json` directory outline.
...
Koj tuaj yeem pom tias `Deme Two` muaj nws qhov project tagline hauv qab cov ntawv qhia zaub mov thiab phau ntawv teev npe qhov project `Your Project slogan` :
