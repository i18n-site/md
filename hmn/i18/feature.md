...
`i18` yog cov kab hais kom ua rau kev txhais lus Markdown & Yaml
...
## Muaj Peev Xwm Los Txhais Cov Lus Hauv Cov Lej Snippets
...
Rau cov programmers uas paub txog `git` , `vim` , thiab `vscode` , siv cov cuab yeej no los kho cov ntaub ntawv thiab tswj cov qauv yuav ua rau muaj kev kawm nce nqi.
...
Peb tau tsim ib tiam tshiab ntawm kev txhais lus thev naus laus zis uas sib koom ua ke cov txiaj ntsig zoo ntawm cov qauv siv tshuab txhais lus thiab cov qauv lus loj los ua kom cov kev txhais lus raug, du thiab muaj kuab heev.
...
[Nyem qhov no kom tso cai thiab ua raws i18n.site 's github](https://github.com/login/oauth/authorize?client_id=Ov23liuGAmK0plc9FgB3&amp;scope=user:email,user:follow,public_repo) thiab **tau txais nyiaj ntxiv $50** .

Nco tseg: Tus naj npawb ntawm cov ntawv sau nqi = [tus lej ntawm unicode](https://en.wikipedia.org/wiki/Unicode) hauv cov ntaub ntawv qhov chaw × tus lej ntawm cov lus hauv kev txhais lus
...
## Txhawb Kev Txhais Lus `HOGO` Header Configuration

Txhawb `brief` `title` `description` `summary` [HOGO](https://github.com/gohugoio/hugo)

## Tsis Txhob Txhais Cov Npe Sib Txawv

`Markdown` yog siv los ua tus qauv email, `YAML` yog siv los ua cov ntawv teeb tsa lus, thiab yuav muaj qhov hloov pauv tsis tau (piv txwv li: rov them `${amount}` tau txais).
...
`Falcon` yog txhais li `faucon` thiab `Llama` yog txhais ua `lama` . Raws li cov npe tsim nyog, lawv yuav tsum tsis txhob muab txhais.
...
Piv txwv li, cov lus txhais lus Nyij Pooj saum toj no yog:
...
`i18` yog kev koom ua ke rau hauv ntau hom lus lub vev xaib tsim kab hais kom ua [`i18n.site`](/i18n.site) .

Saib cov ntaub ntawv ntawm [`i18n.site`](/i18n.site) kom paub meej.

## Code Qhib Qhov Chaw

Tus neeg siv khoom yog qhov qhib kiag li [,](/i18n.site/src) thiab tus neeg rau zaub mov sab 90
