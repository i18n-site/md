...
contabo yog tus muab VPS pheej yig hauv lub teb chaws Yelemees nrog keeb kwm ntev (ntau tshaj 20 xyoo) thiab ntau tus neeg siv (ntau tshaj 200,000), yog li tsis tas yuav txhawj xeeb txog kev khiav tawm.
...
Zaub CPU Model `AMD EPYC 9224 24-Core Processor` single-core performance score 1285
...
Kuv yuav peb lub servers ib zaug thiab qhib lawv tib lub sijhawm Qhov no ua kom cov servers nyob rau hauv tib lub computer chav, ua kom yooj yim rau kev tsim kom muaj nyob rau yav tom ntej.
...
Tom qab ob peb feeb, tag nrho cov servers tau qhib.

Thawj qhov yog los hloov kho lub `ssh-copy-id root@c2` hauv zos IP `~/.ssh/config`
...
Tom qab ntawd hloov kho lub kaw lus mus rau ubuntu 24 nrog ib qho nyem thiab nruab ib qho software uas nquag siv.
...
Cov kauj ruam no yuav siv sij hawm ntev tom qab tos ib nrab teev, kev txhim kho thiab kev teeb tsa ntawm cov software uas nquag siv yuav ua tiav. Koj tuaj yeem paub meej tias nws nrog cov lus txib hauv qab no:
...
Lub network los `ssh` Tebchaws Europe mus rau Tuam Tshoj tsis ruaj khov Nws raug pom zoo kom siv [mosh](//mosh.org)

Tsim ib tsab `c0` `PATH` lub tshuab hauv zos `~/.bin`
...
Tom qab tsim, muab kev tso cai ua tau zoo thiab muag muag `c1` `c2` .
...
Yav tom ntej [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html) koj tuaj yeem nkag `mosh` `c1` `c0`
...
Kev hloov pauv ntiv tes yuav tshwm sim `ssh-keygen -R ip` ssh
...
Yog hais tias koj yog ib tug perfectionist, koj yuav tau ua ib tug tag nrho disk compression manually cov lus txib yog raws li nram no
...
Cov ntawv sau pib yuav tsim cov ntawv teev `~/ubuntu` , tsuas yog khiav cov lus txib hauv qab no:
...
Siv `snapper -c mnt list` los saib ib daim duab ntawm lub subvolume
...
Txawm li cas los xij, qhov granularity ntawm no snapshot loj dhau ib zaug rov qab los, cov ntaub ntawv tshiab ntxiv yuav ploj tag, yog li thov siv ceev faj.
...
Nco ntsoov tias cov haujlwm saum toj no tau ua nyob rau ntawm lub tshuab cawm siav, yog li cov kev hloov pauv no tsuas yog siv tau thaum lub kaw lus rov pib dua thiab tsis nyob hauv hom kev cawm.

Qhov no txhais tau tias koj yuav tsis pom tam sim ntawd cov kev hloov pauv rau cov cav uas twb muaj lawm ua tsis tu ncua, tab sis tag nrho cov cav tsim tawm tom qab qhov no yuav txuas ntxiv mus.
...
Koj tuaj yeem tsim cov npe no nrog cov lus txib hauv qab no
...
Tom qab rov tshawb tar `uid` nws yog `1001` github action

tar Unpacking yuav tsum tau ua yam tsis muaj kev txwv `--no-same-owner` nws yuav decompressed rau `uid` rau `1001` neeg siv
