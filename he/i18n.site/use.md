...
`i18n.site` יש כלי תרגום `i18` אנא [לחץ כאן כדי לעיין במסמך `i18` כדי להגדיר את אסימון הגישה](/i18/use) .
...
לאחר ההפעלה, יווצרו קבצי תרגום ומטמון אנא זכור להוסיף `git add . ` למאגר בספריית `md` .
...
ערוך את `md/.i18n/htm/ol.yml` ושנה את `i18n.site` ב `v: //unpkg.com/i18n.site` לשם חבילה `npm` משלך.

פשוט השתמש בשם החבילה הפנויה ב- [npmjs.com](//npmjs.com)

בעת פרסום המבוסס על חבילה `npm` , **הקפד להשתמש `//unpkg.com/` עבור** הקידומת של ערך `v:` `i18n.site` ביצע אופטימיזציה של זמן המטמון של `/.v` תחת נתיב הקידומת הזה כדי להשיג צפייה בזמן של מהדורות חדשות.
...
אם אתה רוצה לארח את התוכן בעצמך, תחילה ערוך את `md/.i18n/htm/ol.yml` ושנה את `v: //unpkg.com/i18n.site` לקידומת כתובת האתר שלך, כגון `v: //i18n-v.xxx.com` .
...
לאחר מכן צור מחסן `i18n-demo.github.io` תחת ארגון זה (נא להחליף `i18n-demo` בשם הארגון שיצרת):
...
לאחר מכן, השתמש באחסון האובייקטים של ספקי ענן ביבשת סין `CDN` פרוס את התוכן הבא `out/ol/htm` +
...
אם אתה משתמש ב `i18n.site` כדי ליצור אתר כאתר הראשי שלך, בדרך כלל עליך להגדיר ניתוב מחדש של תחום הפאן, כלומר להפנות גישה ל `*.xxx.com` (כולל `www.xxx.com` ) ל `xxx.com` .
...
הוסף את שם הדומיין ב- [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) וכוון את שם הדומיין `*.xxx.com` ל `CNAME` ב- Alibaba Cloud `CDN` .
...
אנא הוסף תצורה דומה להלן בפסקה `server` של nginx אנא שנה את `/root/i18n/md/out/ol/htm` לנתיב של פרוייקט `out/ol/htm` שלך:
