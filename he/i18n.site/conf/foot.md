...
ישנם שלושה קבצים `css` מתחת ל `md/.i18n/htm` בפרויקט ההדגמה.

* `foot.css` : סגנון כותרת תחתונה
* `import.css` : סגנון `i18n.site` עבור האתר כולו
* `conf.css` : וגופנים של כותרת תחתונה
...
הוספת `class="a"` לקישור היא כדי למנוע את המרת הקישור ל `MarkDown` ראה :
 [: `YAML` כיצד למנוע המרת קישור `HTML` ל `Markdown`](/i18/qa#H2) .