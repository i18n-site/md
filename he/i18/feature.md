...
תומך בתרגום של Markdown מעורב HTML התוכן בתגיות `<pre>` ו `<code>` של HTML המוטמע ב `MarkDown` אינו מתורגם
...
תומך בתצורת הכותרת [HOGO](https://github.com/gohugoio/hugo) בלוג סטטי, שמתרגם רק שדות `title` , `summary` ו `description` .
...
`i18` יכול לזהות את הכותרת ב `MarkDown` , ויעשה רישיון אוטומטי באות הראשונה בעת תרגום לשפה תלוית רישיות.
...
`Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` לצרפתית :
