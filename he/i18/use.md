...
[i18n.site/payBill](//i18n.site/payBill) , יש לחייב כרטיס אשראי לתשלום (אין צורך בטעינה מחדש, האתר ינכה אוטומטית עמלות בהתאם לשימוש, [ראה בעמוד הבית לתמחור](/#price) ).
...
ניתן להגדיר `fromTo` שונה : ספריות משנה שונות /
...
בטבלת תצורה זו, שפת המקור לתרגום ספרייה `blog` היא `zh` ושפת המקור לתרגום ספריה `blog/your_file_name.md` היא `ja` .

### התעלם מהקובץ

כברירת מחדל, כל הקבצים המתחילים ב `.md` וב `.yml` בספריית שפת המקור יתורגמו.

אם ברצונך להתעלם מקבצים מסוימים ולא לתרגם אותם (כגון טיוטות לא גמורות), תוכל להשתמש בתצורת השדה `ignore` .
