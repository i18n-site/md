...
דגם תצוגה CPU `AMD EPYC 9224 24-Core Processor` ציון ביצועים עם ליבה אחת 1285
...
לאחר מספר דקות, השרתים כולם הופעלו.

הדבר הראשון הוא לערוך את ה- `~/.ssh/config` של המחשב המקומי הגדר כינוי עבור IP ולאחר מכן השתמש ב- `ssh-copy-id root@c2` כדי להגדיר התחברות ללא סיסמה.
...
שלב זה נמשך זמן רב לאחר המתנה של חצי שעה, השדרוג וההתקנה של תוכנות נפוצות יצליחו. אתה יכול לאשר את זה עם הפקודה הבאה:
...
הרשת מאירופה לסין אינה יציבה. מומלץ להשתמש בו `ssh` להאיץ [mosh](//mosh.org)

צור סקריפט `c0` בספריה `~/.bin` המחשב המקומי (הנוסף בעיקר למשתנה הסביבה `PATH` ) באופן הבא:
...
לאחר היצירה, הענק הרשאות הפעלה ו-soft-link `c1` `c2` .
...
בעתיד, תוכל להזין ישירות `c0` `c1` ותוכל להשתמש `mosh` כדי להתחבר ישירות ולהזין [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
שינויים בטביעת אצבע יתרחשו כאשר ssh השתמש ב `ssh-keygen -R ip`
...
סקריפט האתחול יצור `~/ubuntu` הספרייה, פשוט הפעל את הפקודה הבאה:
...
השתמש ב- `snapper list-configs` כדי להציג את נפחי המשנה שעבורם מוגדרים צילומי מצב.

<img alt="" src="https://i-01.eu.org/1718302689.avif">

השתמש `snapper -c mnt list` כדי להציג תמונת מצב של תת-נפח

`/etc/snapper/configs/mnt` אתה יכול להציג את הגדרות הגיבוי הספציפיות של תת-נפח זה
...
בדרך זו, systemd-journald יתחיל עם התצורה החדשה ויתחיל לאחסן יומנים באופן קבוע בדיסק הקשיח.

זכור כי הפעולות הנ"ל מבוצעות על מערכת הצלה, כך שהשינויים הללו ייכנסו לתוקף רק לאחר אתחול המערכת ואינה במצב הצלה.

משמעות הדבר היא שלא תראה מיד ששינויים ביומנים קיימים מתמשכים, אך כל היומנים שנוצרו לאחר מכן יישמרו.
...
לאחר חקירות חוזרות ונשנות, זה בגלל github action שהעלה `1001` tar `uid`

יש לבצע פריקה ללא פרמטרים tar `--no-same-owner` זה יפורק ל- `uid` עבור משתמש `1001`
