...
Kwerekana Icyitegererezo `AMD EPYC 9224 24-Core Processor` , amanota yimikorere imwe-amanota 1285 CPU
...
Ikintu cya mbere IP imashini yaho `~/.ssh/config` kugirango ushireho alias, hanyuma ukoreshe `ssh-copy-id root@c2` kugirango ugene ijambo ryibanga ridafite ijambo.
...
Umuyoboro uva i Burayi ujya mu Bushinwa ntabwo uhagaze neza `ssh` gukoresha [mosh](//mosh.org)

Kora inyandiko `c0` mububiko bwaho `~/.bin` (cyane cyane wongeyeho ibidukikije bihinduka `PATH` ) kuburyo bukurikira:
...
Nyuma yo kurema, tanga uruhushya rushobora gukorwa na yoroshye-ihuza `c1` na `c2` .
...
Mugihe kizaza, urashobora kwinjiza `c0` na `c1` kugirango ukoreshe `mosh` yihuta yihuta hanyuma winjire [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)

## Hindura Imiterere Y'ibice Kuva ext4 Kuri btrfs

[btrfs](https://wikipedia.org/wiki/Btrfs) gufata disiki ya snapshots kugirango igabanuke byoroshye.
...
ssh Hazabaho impinduka zintoki, koresha `ssh-keygen -R ip` kugirango usukure
...
Inyandiko yo gutangiza izakora `~/ubuntu` ububiko, koresha itegeko rikurikira:

```
~/ubuntu/snapper/init.sh
```

Reba nawe mubyanditswe [github.com/i18n-ops/ubuntu/blob/main/snapper/init.sh](https://github.com/i18n-ops/ubuntu/blob/main/snapper/init.sh)

Koresha `snapper list-configs` kugirango urebe subvolume ya snapshots yashizweho.

![](https://i-01.eu.org/1718302689.avif)

Koresha `snapper -c mnt list` kugirango urebe ifoto ya subvolume

`/etc/snapper/configs/mnt` irashobora kureba igenamigambi ryihariye ryimiterere yiyi subvolume
...
Gushoboza kwihagararaho munsi ya sisitemu yo gutabara biragusaba gukora intoki gukora ububiko bujyanye no gukora izindi ntambwe zinyongera, kuko ushobora kuba udafite serivise ya sisitemu ikora. Dore intambwe ushobora kugerageza:
...
Wibuke ko ibikorwa byavuzwe haruguru bikorerwa kuri sisitemu yo gutabara, bityo izo mpinduka ntizizatangira gukurikizwa kugeza igihe sisitemu izaba yongeye gukora kandi itakiri muburyo bwo gutabara.

Ibi bivuze ko utazahita ubona impinduka zose mubiti bihari bikomeza, ariko ibiti byose byakozwe nyuma yibyo bizakomeza.
...
Nyuma yiperereza ryakozwe, ni ukubera github action Umukoresha `uid` wa tar ari `1001`

tar bigomba kuba niba nta parameter `--no-same-owner` , bizacibwa kuri `uid` kubakoresha `1001`
