...
### `use: Toc` , Dosiye Imwe Yerekana Icyitegererezo
...
### `use: Md` , Dosiye Imwe Yicyitegererezo Idafite Urucacagu

Inyandikorugero `Md` hamwe nicyitegererezo `Toc` nimwe kandi byombi bikoreshwa mugutanga `Markdown` imwe. Ariko inyandikorugero `Md` ntabwo yerekana urucacagu kuruhande.
...
### `use: Doc` , Inyandiko-Yinyandiko Nyinshi
...
Uburyo bwo kwandika hejuru buhwanye na `url: xxx` , kandi dosiye ihuye ni `en/xxx/TOC` .

#### Imbonerahamwe Ya TOC Indangagaciro

`i18n.site` izakora `js` plug-in `.i18n/hook/after.tran/TOC.js` mububiko bwa demo kugirango usome dosiye `TOC` yububiko bwa dosiye ihuye nibishusho `doc` byerekana ibyerekezo `json` byububiko.
...
Kurugero, niba `/test` igerwaho, na `nav:` igashyirwaho nta prefix yiyi nzira, kandi ururimi ruriho ubu ni icyongereza (code `en` ), `/en/test.md` bizaterwa nubusa kandi bitangwe ukoresheje icyitegererezo cya `Md` .
