# ఉత్పత్తి లక్షణాలు

అనువదించడానికి Markdown & Yaml కమాండ్ లైన్ సాధనం `i18`

## మార్క్డౌన్ ఆకృతిని సంపూర్ణంగా నిర్వహించగలదు

గణిత సూత్రాలు లేదా లింక్లలోని పదాలను అనువదించదు.

Markdown `<pre>` HTML HTML `MarkDown` `<code>`

## గణిత సూత్రాలను గుర్తించవచ్చు మరియు అనువాదాన్ని దాటవేయవచ్చు

గణిత సూత్రాలు గుర్తించబడ్డాయి మరియు అనువాదం దాటవేయబడింది.

గణిత సూత్రాలను ఎలా వ్రాయాలో, దయచేసి [" Github గణిత వ్యక్తీకరణలను వ్రాయడం గురించి"](https://docs.github.com/get-started/writing-on-github/working-with-advanced-formatting/writing-mathematical-expressions#about-writing-mathematical-expressions) చూడండి.

## కోడ్ స్నిప్పెట్లలో వ్యాఖ్యలను అనువదించగల సామర్థ్యం

ఇన్లైన్ కోడ్ మరియు కోడ్ స్నిప్పెట్లు అనువదించబడవు, కానీ కోడ్లోని వ్యాఖ్యలను అనువదించవచ్చు.

అనువాద వ్యాఖ్యలు ` ```rust ` : సూచించాలి ` ``` `

ప్రస్తుతం , `yaml` , `json5` , `go` , `rust` , `c` , `cpp` , `java` `js` , `coffee` `python` , `bash` , `php` [మరియు ఇతర భాషలలో](https://github.com/i18n-site/rust/blob/main/getc/src/style.rs#L14) `toml` అనువాదానికి మద్దతు ఇస్తుంది.

మీకు అవసరమైన ప్రోగ్రామింగ్ భాష అందుబాటులో లేకుంటే, దయచేసి [మమ్మల్ని సంప్రదించండి](https://groups.google.com/g/i18n-site) .

## కమాండ్ లైన్ స్నేహపూర్వక

అనువాద పత్రాలను నిర్వహించడానికి అనేక [హెవీవెయిట్ సాధనాలు](https://www.capterra.com/translation-management-software) అందుబాటులో ఉన్నాయి.

, `vim` , `vscode` `git` తెలిసిన ప్రోగ్రామర్ల కోసం, పత్రాలను సవరించడానికి మరియు సంస్కరణలను నిర్వహించడానికి ఈ సాధనాలను ఉపయోగించడం నిస్సందేహంగా అభ్యాస ఖర్చును పెంచుతుంది.

`KISS` ( `Keep It Simple, Stupid` ) సూత్రాన్ని విశ్వసించేవారిలో, సంస్థ-స్థాయి పరిష్కారాలు గజిబిజిగా, అసమర్థంగా మరియు ఉపయోగించడం కష్టంగా ఉంటాయి.

కమాండ్లను ఇన్పుట్ చేయడం ద్వారా మరియు ఒక క్లిక్తో పూర్తి చేయడం ద్వారా అనువాదం జరగాలి, సంక్లిష్టమైన పర్యావరణ డిపెండెన్సీలు ఉండకూడదు.

అవసరమైతే తప్ప ఎంటిటీలను జోడించవద్దు.

## సవరణ లేదు, అనువాదం లేదు

[translate-shell](https://github.com/soimort/translate-shell) కొన్ని కమాండ్ లైన్ అనువాద సాధనాలు కూడా ఉన్నాయి.

అయినప్పటికీ, అవి ఫైల్ సవరణలను గుర్తించడానికి మద్దతు ఇవ్వవు మరియు ఖర్చులను తగ్గించడానికి మరియు సామర్థ్యాన్ని పెంచడానికి సవరించిన ఫైల్లను మాత్రమే అనువదిస్తాయి.

## అనువాదాన్ని సవరించవచ్చు మరియు మెషీన్ అనువాదం ఇప్పటికే ఉన్న సవరణలను ఓవర్రైట్ చేయదు.

అనువాదం సవరించదగినది.

అసలు వచనాన్ని సవరించండి మరియు దాన్ని మళ్లీ మెషీన్-అనువదించండి, అనువాదానికి మాన్యువల్ సవరణలు భర్తీ చేయబడవు (అసలు టెక్స్ట్ యొక్క ఈ పేరా సవరించబడకపోతే).

## Xindaya యొక్క అధిక-నాణ్యత అనువాదం

అనువాదాలను ఖచ్చితమైన, మృదువైన మరియు సొగసైనదిగా చేయడానికి సాంప్రదాయ యంత్ర అనువాద నమూనాలు మరియు పెద్ద భాషా నమూనాల సాంకేతిక ప్రయోజనాలను మిళితం చేసే కొత్త తరం అనువాద సాంకేతికతను మేము అభివృద్ధి చేసాము.

సారూప్య సేవలతో పోలిస్తే మా అనువాద నాణ్యత మెరుగ్గా ఉందని బ్లైండ్ పరీక్షలు చూపిస్తున్నాయి.

అదే నాణ్యతను సాధించడానికి, Google అనువాదానికి అవసరమైన మాన్యువల్ ఎడిటింగ్ మొత్తం `ChatGPT` `2.67` సార్లు మరియు `3.15` రెట్లు.

## <a rel=id href="#price" id="price"></a> అత్యంత పోటీ ధర

|                                                                                   | USD/మిలియన్ పదాలు |
| --------------------------------------------------------------------------------- | ------------- |
| [i18n.site](https://i18n.site)                                                    | 9             |
| [మైక్రోసాఫ్ట్](https://azure.microsoft.com/pricing/details/cognitive-services/translator) | 10            |
| [అమెజాన్](https://aws.amazon.com/translate/pricing)                                | 15            |
| [Google](https://cloud.google.com/translate/pricing)                                | 20            |
| [DeepL](https://www.deepl.com/zh/pro#developer)                                  | 25            |

గమనిక: బిల్ చేయదగిన అక్షరాల సంఖ్య = సోర్స్ ఫైల్లోని [unicode సంఖ్య](https://en.wikipedia.org/wiki/Unicode) × అనువాదంలోని భాషల సంఖ్య

## అనువాదానికి మద్దతు ఇవ్వండి `YAML`

సాధనం నిఘంటువు విలువలను మాత్రమే అనువదిస్తుంది `YAML` నిఘంటువు కీలు కాదు.

`YAML` ఆధారంగా, మీరు వివిధ ప్రోగ్రామింగ్ భాషల కోసం సులభంగా అంతర్జాతీయ పరిష్కారాలను రూపొందించవచ్చు.

## హెడర్ కాన్ఫిగరేషన్కు మద్దతు `HOGO`

[HOGO](https://github.com/gohugoio/hugo) యొక్క హెడర్ కాన్ఫిగరేషన్కు మద్దతు ఇస్తుంది , `summary` మరియు `description` ఫీల్డ్లు మాత్రమే `title`

## వేరియబుల్ పేర్లను అనువదించవద్దు

`Markdown` ఇమెయిల్ టెంప్లేట్గా ఉపయోగించబడుతుంది, `YAML` భాషా ఫైల్ కాన్ఫిగరేషన్గా, వేరియబుల్ పారామీటర్లు ఉంటాయి (ఉదాహరణకు: `${amount}` రీఛార్జ్ చేయబడింది).

ఈ రకమైన వేరియబుల్ పేరు ఆంగ్ల అనువాదంగా పరిగణించబడదు `${varname}`

## చైనా, జపాన్ మరియు కొరియా కోసం అనువాద ఆప్టిమైజేషన్

### ఆంగ్లంలోకి అనువదించబడినప్పుడు, శీర్షికలోని మొదటి అక్షరం స్వయంచాలకంగా పెద్ద అక్షరం అవుతుంది.

చైనా, జపాన్ మరియు కొరియాలో పెద్ద అక్షరాలు లేదా చిన్న అక్షరాలు లేవు, కానీ ఆంగ్ల శీర్షికలు సాధారణంగా మొదటి అక్షరాన్ని పెద్ద అక్షరం కలిగి ఉంటాయి.

`i18` లో `MarkDown` గుర్తించవచ్చు మరియు కేస్-సెన్సిటివ్ భాషలోకి అనువదించేటప్పుడు స్వయంచాలకంగా మొదటి అక్షరాన్ని క్యాపిటలైజ్ చేస్తుంది.

ఉదాహరణకు `Optimized for Reading Experience` గా అనువదించబడుతుంది `为阅读体验而优化`

### చైనీస్, జపనీస్, కొరియన్ మరియు చైనీస్ భాషలలో ఆంగ్ల పదాలు అనువదించబడలేదు

చైనా, జపాన్ మరియు కొరియా నుండి వచ్చిన పత్రాలు తరచుగా అనేక ఆంగ్ల పదాలను కలిగి ఉంటాయి.

చైనీస్, జపనీస్ మరియు కొరియన్ భాషల యంత్ర అనువాదం ఆంగ్లేతర భాషగా మారింది మరియు ఈ క్రింది చైనీస్ వాక్యం వంటి పదాలు తరచుగా కలిసి అనువదించబడతాయి:

`Falcon 得分超 Llama ？Hugging Face 排名引发争议`

మీరు Google లేదా DeepLని ఉపయోగిస్తే, అవి రెండూ అసలైన ఆంగ్ల పదాలను తప్పుగా అనువదిస్తాయి (జపనీస్ మరియు ఫ్రెంచ్లను ఉదాహరణలుగా తీసుకోండి).

### Google అనువాదం

`ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` లోకి అనువదించబడింది :

<img alt="" src="//p.3ti.site/1720199391.avif">

`Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` లోకి అనువదించబడింది :

`Falcon` `faucon` `Llama` `lama`

<img alt="" src="//p.3ti.site/1720199451.avif">

### లోతైన అనువాదం

`ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` లోకి అనువదించబడింది :

<img alt="" src="//p.3ti.site/1720199550.avif">

పైన పేర్కొన్న వాటిని ఫ్రెంచ్లోకి DeepL (సరైన పేర్లను తిరిగి వ్రాయడం మరియు బేసిని జోడించడం కూడా `... ` ):

`Un faucon marque un point sur un lama... Le classement des visages étreints suscite la controverse. `

<img alt="" src="//p.3ti.site/1720199603.avif">

### i18n.సైట్ అనువాదం

`i18` యొక్క అనువాదం చైనీస్, జపనీస్ మరియు కొరియన్ పత్రాలలో ఆంగ్ల పదాలను గుర్తిస్తుంది మరియు నిబంధనలను అలాగే ఉంచుతుంది.

ఉదాహరణకు, పైన ఉన్న జపనీస్ అనువాద ఫలితం:

`Falcon のスコアが Llama よりも高かったですか ? Hugging Face ランキングが論争を引き起こす`

ఫ్రెంచ్ అనువాదం:

`Falcon a obtenu un score supérieur à Llama ? Hugging Face Le classement suscite la controverse`

ఆంగ్ల పదం మరియు చైనీస్, జపనీస్ మరియు కొరియన్ టెక్స్ట్ మధ్య ఖాళీ లేదా ఆంగ్ల పొడవు 1 కంటే ఎక్కువగా ఉన్నప్పుడు మాత్రమే, పదం పదంగా పరిగణించబడుతుంది.

ఉదాహరణకు: `C罗` `Cristiano Ronaldo` అనువదించబడుతుంది.

## బహుళ భాషా వెబ్సైట్ నిర్మాణాన్ని `i18n.site`

`i18` బహుళ భాషా వెబ్సైట్ బిల్డింగ్ [`i18n.site`](/i18n.site) కోసం కమాండ్ లైన్ టూల్లో విలీనం చేయబడింది.

[`i18n.site`](/i18n.site) కోసం డాక్యుమెంటేషన్ చూడండి.

## కోడ్ ఓపెన్ సోర్స్

క్లయింట్ పూర్తిగా ఓపెన్ సోర్స్, మరియు సర్వర్ సైడ్ 90 [సోర్స్ కోడ్ని వీక్షించడానికి ఇక్కడ క్లిక్ చేయండి](/i18n.site/src) .

మేము ఓపెన్ సోర్స్ కోడ్ అభివృద్ధి మరియు అనువదించబడిన గ్రంథాల ప్రూఫ్ రీడింగ్లో పాల్గొనడానికి స్వచ్ఛంద సేవకుల కోసం చూస్తున్నాము.

మీకు ఆసక్తి ఉంటే, దయచేసి [→ మీ ప్రొఫైల్ను పూరించడానికి ఇక్కడ క్లిక్ చేయండి](https://ggl.link/i18n) మరియు కమ్యూనికేషన్ కోసం [మెయిలింగ్ జాబితాలో](https://groups.google.com/u/2/g/i18n-site) చేరండి.

## సన్నిహితంగా ఉండండి

ప్రోడక్ట్ అప్డేట్లకు సబ్స్క్రయిబ్ చేయడానికి మరియు <button onclick="webpush()">బ్రౌజర్ పుష్ని ఎనేబుల్ చేయడానికి</button> దయచేసి <button onclick="mailsub()">ఈ ఇమెయిల్ని క్లిక్ చేయండి</button> .

మా సామాజిక ఖాతాలను అనుసరించడానికి [i18n-site.bsky.social](https://bsky.app/profile/i18n-site.bsky.social) / [X.COM: @i18nSite](https://x.com/i18nSite)
