...
అమలు చేసిన తర్వాత, అనువాదం మరియు కాష్ ఫైల్లు రూపొందించబడతాయి, దయచేసి `git add . ` `md` డైరెక్టరీలోని రిపోజిటరీకి జోడించాలని గుర్తుంచుకోండి.
...
#### కాన్ఫిగరేషన్ ఫైల్ను పేర్కొనడానికి -c ఉపయోగించండి
...
`dev.yml` మరియు `ol.yml` డిఫాల్ట్ కాన్ఫిగరేషన్లు.
...
[npmjs.com](//npmjs.com) కంటెంట్ని ప్రచురించడం అనేది సిఫార్సు చేయబడిన డిఫాల్ట్ పరిష్కారం ( [ముందు-ముగింపు అధిక లభ్యత](/i18n.site/feature#ha) చూడండి).

##### npm login &

`nodejs` ఇన్స్టాల్ చేయండి, `npm login` తో లాగిన్ చేయండి.

`md/.i18n/htm/ol.yml` సవరించండి మరియు `i18n.site` `v: //unpkg.com/i18n.site` మీ స్వంత `npm` ప్యాకేజీ పేరుకు మార్చండి.

కేవలం ఆక్రమించని ప్యాకేజీ పేరుని ఉపయోగించండి [npmjs.com](//npmjs.com)

ప్యాకేజీ `npm` ఆధారంగా ప్రచురించేటప్పుడు, `v:` విలువ యొక్క ఉపసర్గ **కోసం `//unpkg.com/` ఉపయోగించాలని నిర్ధారించుకోండి** `i18n.site` కొత్త విడుదలలను సకాలంలో చూడటానికి ఈ ఉపసర్గ మార్గంలో `/.v` యొక్క కాష్ సమయాన్ని ప్రత్యేకంగా ఆప్టిమైజ్ చేసింది.
...
మీరు `ol.yml` లో `v:` ప్యాకేజీ పేరును సవరించినట్లయితే, దయచేసి **ముందుగా `.i18n/v/ol` తొలగించి, ఆపై ప్రచురించాలని నిర్ధారించుకోండి** .

##### npm ద్వారా ప్రాక్సీ సర్వర్ ప్రచురించబడింది

చైనా ప్రధాన భూభాగంలోని వినియోగదారులు నెట్వర్క్ సమస్యలను ఎదుర్కొంటే మరియు `npm` ప్యాకేజీలను ప్రచురించలేకపోతే, వారు ప్రాక్సీ సర్వర్ను కాన్ఫిగర్ చేయడానికి పర్యావరణ వేరియబుల్ `https_proxy` ని సెట్ చేయవచ్చు.
...
మీరు కంటెంట్ను స్వీయ-హోస్ట్ చేయాలనుకుంటే, ముందుగా `md/.i18n/htm/ol.yml` సవరించండి మరియు `v: //i18n-v.xxx.com` వంటి మీ URL ఉపసర్గకు `v: //unpkg.com/i18n.site` మార్చండి.
...
తర్వాత, `md/out/ol/v` డైరెక్టరీలోని కంటెంట్ను `v:` లో సెట్ చేసిన URL ప్రిఫిక్స్ పాత్కు కాన్ఫిగర్ చేయండి.
...
`CDN` -హోస్ట్ కంటెంట్ కోసం, మీ స్వంత సర్వర్ని ఉపయోగించడంతో పాటు, మరొక సాధారణ ఎంపిక `S3` +

మీరు `S3` సర్వర్కు లాగిన్ చేయడానికి [rclone](https://rclone.org) ని ఉపయోగించవచ్చు, ఆపై క్రింది స్క్రిప్ట్ను చూడండి మరియు సవరించండి మరియు ప్రతి విడుదలకు `S3` కి పెరుగుతున్న మార్పులను మాత్రమే కాపీ చేయండి.
...
ఆపై ఈ సంస్థ క్రింద వేర్హౌస్ `i18n-demo.github.io` సృష్టించండి (దయచేసి మీరు సృష్టించిన సంస్థ పేరుతో `i18n-demo` భర్తీ చేయండి):

![](https://p.3ti.site/1721098657.avif)

మునుపటి కథనంలోని కంటెంట్ను ప్రచురించేటప్పుడు `out/ol/htm` దయచేసి ఈ డైరెక్టరీని నమోదు చేసి రన్ చేయండి :
...
తరువాత, వేర్హౌస్ `i18n-demo.github.io` ని ఎంచుకుని, ఆపై `Begin setup` క్లిక్ చేసి, తదుపరి దశల కోసం డిఫాల్ట్ విలువలను ఉపయోగించండి.
...
తర్వాత, + ప్రధాన భూభాగంలో క్లౌడ్ విక్రేతల ఆబ్జెక్ట్ స్టోరేజ్ని ఉపయోగించండి `CDN` కింది కంటెంట్ను అమలు చేయండి `out/ol/htm` .

సింగిల్-పేజీ అప్లికేషన్లకు అనుగుణంగా మార్గాన్ని తిరిగి వ్రాయడానికి మీరు ఎడ్జ్ కంప్యూటింగ్ను ఉపయోగించవచ్చు ఉదాహరణకు, [Baidu Smart Cloud `CDN`](//cloud.baidu.com/product/cdn.html) ఇలా కాన్ఫిగర్ చేయవచ్చు:
...
రికార్డ్ `MX` మరియు రికార్డ్ `CNAME` సహజీవనం చేయలేవు కాబట్టి, మీరు అదే సమయంలో డొమైన్ పేరు ఇమెయిల్లను స్వీకరించాలనుకుంటే, మీరు `CNAME` `A` రికార్డ్ చేయడానికి [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten) సహకరించాలి.
...
[అలీబాబా CDN](https://cdn.console.aliyun.com/domain/list) డొమైన్ పేరును జోడించండి మరియు అలీబాబా క్లౌడ్ `CDN` లో డొమైన్ పేరు `*.xxx.com` నుండి `CNAME` వరకు సూచించండి.
...
దయచేసి `server` పేరాలో కిందివాటికి సారూప్యమైన కాన్ఫిగరేషన్ను జోడించండి nginx దయచేసి `/root/i18n/md/out/ol/htm` మీ స్వంత ప్రాజెక్ట్ `out/ol/htm` కి మార్చండి
