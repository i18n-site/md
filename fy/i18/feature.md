...
Unterstützt oersetting fan HTML Markdown , de ynhâld yn 'e `<pre>` en `<code>` tags fan HTML ynbêde yn `MarkDown` is net oerset
...
Inline koade en koade snippets wurde net oerset, mar opmerkings yn de koade kinne oerset wurde.

Oersetkommentaren moatte de taal nei ` ``` ` oanjaan, : ` ```rust ` .
...
As de programmeartaal dy't jo nedich binne net beskikber is, nim dan [kontakt mei ús](https://groups.google.com/g/i18n-site) op.
...
D'r binne ek wat kommandorigel-oersetynstruminten, lykas [translate-shell](https://github.com/soimort/translate-shell)

Se stypje lykwols gjin identifisearjen fan triemwizigingen en oersette allinich oanpaste bestannen om kosten te ferminderjen en effisjinsje te ferheegjen.
...
Wy hawwe in nije generaasje oersettechnology ûntwikkele dy't de technyske foardielen fan tradisjonele masine-oersettingsmodellen en grutte taalmodellen yntegreart om oersettingen akkuraat, glêd en elegant te meitsjen.
...
[Klik hjir om de github te autorisearjen en automatysk i18n.site folgjen](https://github.com/login/oauth/authorize?client_id=Ov23liuGAmK0plc9FgB3&amp;scope=user:email,user:follow,public_repo) en **bonus $50 te ûntfangen** .
...
## Stypje Oersetting `HOGO` Header Konfiguraasje

Unterstützt de header-konfiguraasje fan [HOGO](https://github.com/gohugoio/hugo) type statyske blog, dy't allinich fjilden `title` , `summary` en `description` oerset.
...
### By It Oersetten Yn It Ingelsk, Wurdt De Earste Letter Fan 'e Titel Automatysk Mei Haadletters.
...
As jo Google of DeepL brûke, oersette se beide Ingelske termen dy't oarspronklik moatte bliuwe ferkeard (nim Japansk en Frânsk as foarbyld).

### Google Oersette

Oerset yn `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` Japansk :

![](//p.3ti.site/1720199391.avif)

Oerset yn `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` Frânsk :
...
Oerset yn `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` Japansk :
...
De client is folslein iepen boarne, en de serverkant % fan de 90 is iepen boarne. [Klik hjir om de boarnekoade te besjen](/i18n.site/src) .
...
/ [i18n-site.bsky.social](https://bsky.app/profile/i18n-site.bsky.social) om ús sosjale akkounts te folgjen [X.COM: @i18nSite](https://x.com/i18nSite)