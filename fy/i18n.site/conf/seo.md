...
`i18n.site` nimt in net-ferfarske arsjitektuer `sitemap.xml` ien side oan.

As `User-Agent` fan it tagongsfersyk wurdt brûkt troch de sykmasjine-crawler, sil it fersyk trochferwiisd wurde nei de statyske side fia `302` .
...
Yn 'e konfiguraasje feroarje asjebleaft `i18n.site` nei de wearde fan `host:` yn `.i18n/htm/ol.yml` , meardere objektwinkels kinne wurde konfigureare ûnder `s3` , en it fjild `region` is opsjoneel (in protte objektwinkels hoege dit fjild net yn te stellen).
...
Foegje cache-regels ta as folget:
...
Meitsje dan it skript yn [edge EdgeJS tsjinst](//console.bce.baidu.com/cdn/#/cdn/ejs/list) as folget
...
Om't `R2` `cloudflare` tagonklik wurde kin troch in oanpaste domeinnamme, moat in objektopslach fan tredden brûkt wurde om statyske bestannen te pleatsen.

Hjir [backblaze.com](https://www.backblaze.com) wy as foarbyld om te demonstrearjen hoe objekten fan tredden te binen om op `cloudflare` te bewarjen.
