...
Nei it útfieren sille oersettings- en cache `md` `git add . ` oanmakke wurde.
...
`ol` is de ôfkoarting fan `online` , `-n` de online omjouwing oanjout, `npm` brûkt wurdt foar offisjele frijlitting.
...
Om ynhâld sels te hostjen, neist it brûken fan jo eigen server, is in oare mienskiplike opsje om `S3` + te brûken `CDN`
...
[Klikje github earst om in organisaasje te meitsjen](https://github.com/account/organizations/new?plan=free) De folgjende organisaasjenamme is `i18n-demo` as foarbyld.
...
Yn ferliking mei `github page` , it jout paad herskriuwen en is mear freonlik foar it fêstelân fan Sina en is mear tagonklik It is [cloudflare page](//pages.cloudflare.com) te rieden om te brûken.
...
Brûk dan de objektopslach fan wolkferkeapers op it fêstelân `CDN` + de folgjende ynhâld `out/ol/htm` .
...
Foegje de domeinnamme ta yn [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) en wiis de domeinnamme `*.xxx.com` nei `CNAME` yn Alibaba Cloud `CDN` .
...
Foegje asjebleaft in konfiguraasje ta lykas de folgjende yn `out/ol/htm` `/root/i18n/md/out/ol/htm` `server` paragraaf fan nginx
