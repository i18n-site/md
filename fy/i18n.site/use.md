...
As jo brûke in trochgeande yntegraasje omjouwing te publisearjen, der is gjin needsaak om te ynstallearjen `nodejs` , gewoan kopiearje de oanmeld en publisearjen tastimming `~/.npmrc` nei de omjouwing.
...
Yn [cloudflare page](//pages.cloudflare.com) mei `github page` , it jout paad herskriuwen en is mear freonlik foar fêstelân Sina en is mear tagonklik.

De ynset fan `cloudflare page` is normaal basearre op de ynset fan `github page` hjirboppe.

Meitsje in projekt en bine it `i18n-demo.github.io` pakhús hjirboppe.
