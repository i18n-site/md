...
Display CPU Model `AMD EPYC 9224 24-Core Processor` single-core prestaasjes score 1285
...
It earste ding is om de lokale masine's `~/.ssh/config` bewurkjen Stel in alias foar IP en brûk dan `ssh-copy-id root@c2` om wachtwurdfrije oanmelding te konfigurearjen.
...
Dizze stap duorret lang. Jo kinne it befestigje mei it folgjende kommando:
...
It netwurk `ssh` Europa nei Sina is net stabyl It is oan te rieden om te brûken [mosh](//mosh.org)

Meitsje in `c0` -skript yn 'e `~/.bin` fan' e lokale masine (benammen tafoege oan de omjouwingsfariabele `PATH` ) as folget:
...
Nei oanmeitsjen, jou útfierbere tagongsrjochten en soft-link `c1` `c2` .
...
Yn 'e takomst kinne jo direkt ynfiere `c0` `c1` en jo kinne `mosh` hege snelheid direkte ferbining brûke en ynfiere [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)

## Konvertearje Partitionformaat Fan ext4 Nei btrfs
...
`ssh-keygen -R ip` sille foarkomme as ssh
...
It skript sil it systeem automatysk opnij starte neidat de konverzje suksesfol is.
...
It initialisaasjeskript sil de `~/ubuntu` oanmeitsje, gewoan it folgjende kommando útfiere:
...
Brûk `snapper list-configs` om de subvolumes te besjen wêrfoar snapshots binne ynsteld.

<img alt="" src="https://i-01.eu.org/1718302689.avif">

Brûk `snapper -c mnt list` om in momintopname fan in subvolume te besjen

`/etc/snapper/configs/mnt` Jo kinne de spesifike reservekopyynstellingen fan dit subvolume besjen
...
Hâld der rekken mei dat de boppesteande operaasjes wurde útfierd op in rêdingssysteem, dus dy wizigingen sille pas effekt wurde as it systeem opnij is opstart en net mear yn 'e rêdingsmodus.

Dit betsjut dat jo gjin feroarings oan besteande logboeken daliks sille sjen bliuwend wurde, mar alle logs dy't dêrnei oanmakke wurde sille bliuwend bliuwe.
...
Ik brûkte github-aksje foar it meitsjen fan in automatysk kompilaasje- en ynsetskript foar, mar fûn dat de ynsettsjinst net ssh koe.
...
Nei werhelle ûndersiken is it github action de brûker dy't tar uploadde `1001` is `uid`

tar It útpakke moat dien wurde sûnder parameters `--no-same-owner` it sil wurde dekomprimearre yn `uid` foar `1001` brûker
