...
Efter körning kommer översättnings- och cachefiler att genereras. Kom ihåg att lägga till `git add . ` i arkivet i `md` katalogen.
...
Om du använder en kontinuerlig integrationsmiljö för att publicera, behöver du inte installera `nodejs` Kopiera bara inloggade och publiceringsbehörigheter `~/.npmrc` till miljön.
...
För att själv hosta innehåll, förutom att använda din egen server `CDN` är ett annat vanligt alternativ att använda `S3` +
...
Skapa ett projekt, bind `i18n-demo.github.io` lagret ovan.
...
Använd sedan objektlagringen för molnleverantörer i Kina + Distribuera följande innehåll `out/ol/htm` `CDN`
...
Lägg till domännamnet i [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) och peka på domännamnet `*.xxx.com` till `CNAME` i Alibaba Cloud `CDN` .
...
Vänligen lägg till en konfiguration som liknar följande i `server` stycket i nginx Ändra `/root/i18n/md/out/ol/htm` till sökvägen för ditt eget projekt `out/ol/htm` :
