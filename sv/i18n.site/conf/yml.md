...
### `use: Md` , Enstaka Filmall Utan Kontur
...
### `use: Doc` , Dokumentmall För Flera Filer
...
För parametrarna ovan är motsvarande katalogindexfil:
...
Ovanstående skrivmetod motsvarar `url: xxx` , och dess motsvarande fil är `en/xxx/TOC` .

#### TOC Innehållsförteckning Index

`i18n.site` kommer att exekvera `js` plug-in `.i18n/hook/after.tran/TOC.js` i demolagret för att läsa indexfilen med `TOC` kataloger som motsvarar `doc` mallkonfigurationen för att generera `json` katalogkonturen.

Om du använder `doc` mall måste du ha denna plug-in.

Om du initierar projekt `i18n.site` från en tom mapp, kom ihåg att kopiera demoprojekt `.i18n` till din katalog.

Mallen `Doc` kommer att återge innehållsförteckningen baserat på den genererade `json` .
