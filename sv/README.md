...
## <a rel=id href="#project" id="project"></a> projekt beskrivning
...
Översätt `Markdown` och `YAML` till ett kommandoradsverktyg ( [källkod](https://github.com/i18n-site/rust/tree/main/i18) ) på flera språk.

Kan perfekt behålla formatet på `Markdown` Kan identifiera filändringar och endast översätta ändrade filer.
...
Vi delar " `UNIX` Everything is a File " och kan hantera översättningar på hundratals språk utan att introducera komplexa och krångliga lösningar på företagsnivå.

Du kan använda de mest välbekanta verktygen för `Markdown` översättningen (men du kan inte lägga till eller ta bort stycken), och använda det mest välbekanta sättet att göra versionskontroll.

En kodbas kan skapas som en öppen källkod för språkfiler, och med hjälp av `Pull Request` s process kan globala användare delta i den kontinuerliga optimeringen av översättningar. Sömlös anslutning och andra gemenskaper [github](//github.com) öppen källkod.
...
Rent statiskt, optimerat för läsupplevelse och integrerat med översättningen av [i18](#i18) är det det bästa valet för att bygga en projektdokumentsajt.
...
Denna webbplats är utvecklad utifrån detta ramverk och inkluderar användar-, betalnings- och andra funktioner ( [källkod](/i18n.site/src) ) En detaljerad handledning kommer att skrivas senare.

[→ För användarguide, läs projektdokumentationen](/i18n.site) .
