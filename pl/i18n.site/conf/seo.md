...
`i18n.site` wykorzystuje architekturę jednej strony bez odświeżania. Aby ułatwić indeksowanie przez wyszukiwarki, generuje osobne strony statyczne oraz `sitemap.xml`, które mogą być przeszukiwane przez roboty.

Gdy `User-Agent` żądania to robot wyszukiwarki, żądanie jest przekierowywane na stronę statyczną za pomocą `302`.
...
Jako przykład, plik konfiguracyjny `.i18n/htm/ol.yml` z projektu demonstracyjnego
...
Dodaj poniższe zasady konwersji:
...
Dodaj poniższe reguły buforowania:
...
Ustaw poniższe reguły przekierowań, zmodyfikuj kod "i18n.site" na swoją nazwę domeny:
...
Jeśli chcesz świadczyć usługi w Chinach kontynentalnych, możesz skorzystać z [Baidu Smart Cloud](//cloud.baidu.com).
...
Jeśli chcesz świadczyć usługi w Chinach kontynentalnych oraz korzystać z bezpłatnego ruchu międzynarodowego z `cloudflare`, możesz użyć `DNS` z analizą regionalną.

Na przykład [Huawei Cloud DNS](https://www.huaweicloud.com) oferuje bezpłatną analizę regionalną, co pozwala na dystrybucję ruchu z Chin kontynentalnych przez Baidu Smart Cloud, a ruch międzynarodowy przez `cloudflare`.

Konfiguracja `cloudflare` wiąże się z wieloma pułapkami. Oto kilka rzeczy, na które warto zwrócić uwagę:
...
Ponieważ `R2` `cloudflare` nie umożliwia dostępu za pomocą niestandardowej nazwy domeny, do przechowywania plików statycznych należy użyć magazynu obiektów innej firmy.
...
Dodaj poniższą regułę konwersji, umieść ją na pierwszym miejscu (pierwsza ma najniższy priorytet):

![](//p.3ti.site/1725443232.avif)

`Rewrite to` wybierz dynamiczny i zmodyfikuj `your_bucketname` w `concat("/file/your_bucketname",http.request.uri.path)` na nazwę swojego wiadra.

Ponadto, w powyższej regule konwersji `cloudflare`, zmień `index.html` na `file/your_bucketname/index.html`, a inne konfiguracje pozostaw jak są.
