...
Funkcja przeglądania historycznych wersji dokumentów jest w trakcie opracowywania.

Zaleca się zmieniać numer wersji dokumentu tylko przy dużych aktualizacjach (np. `v1`, `v2`), aby uniknąć bałaganu na stronach indeksowanych przez wyszukiwarki.

## Użyj pustych plików `v`, aby podzielić indeksy plików różnych projektów

W projekcie demonstracyjnym, oprócz katalogu `en/demo2/v`, można zauważyć również, że w katalogach `en/blog` oraz `en/demo1` znajdują się puste pliki o nazwie `v`.
...
Dzieląc indeksy różnych projektów, można uniknąć opóźnień w dostępie spowodowanych jednoczesnym ładowaniem wszystkich plików na stronie.
