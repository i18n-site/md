# Zainstaluj i wdrożenie

## Konfiguracja tokena

`i18n.site` zawiera wbudowane narzędzie do tłumaczenia `i18`, proszę najpierw zapoznać się z dokumentacją `i18` i skonfigurować token dostępu [tutaj](/i18/use).
...
Nazwa katalogu klonu kodu `demo.i18n.site` musi wynosić `md`, aby umożliwić lokalny podgląd za pomocą `docker`.
...
Po uruchomieniu zostaną wygenerowane pliki tłumaczeń i pamięci podręcznej. Pamiętaj o dodaniu ich do repozytorium w `git add .` w katalogu `md`.
...
`i18n.site` stosuje architekturę [aplikacji jednostronicowej](https://developer.mozilla.org/docs/Glossary/SPA), a strona wejściowa i zawartość witryny są wdrażane niezależnie.
...
W przypadku wersji oficjalnej zmienione pliki zostaną skopiowane do nowego katalogu z numerem wersji.
...
`dev.yml` i `ol.yml` są domyślnymi konfiguracjami.
...
`ol` to skrót od `online`, wskazujący środowisko online, używane do oficjalnej wersji. Jest to także domyślny plik konfiguracyjny, gdy do wydania używane są parametry wiersza poleceń `-n` do `npm`.
...
Podczas publikowania na podstawie pakietu `npm`, **pamiętaj o użyciu `//unpkg.com/`** jako prefiksu wartości `v:`. `i18n.site` specjalnie zoptymalizował czas pamięci podręcznej dla katalogu `/.v` pod tą ścieżką prefiksu, aby umożliwić natychmiastowe wyświetlanie nowych wydań.

Uruchom `i18n.site --npm` lub `i18n.site -n` w katalogu `md`, aby przetłumaczyć i opublikować.

Jeśli używasz środowiska ciągłej integracji do publikowania, nie ma potrzeby instalowania `nodejs`. Wystarczy skopiować uprawnienia zalogowania i publikowania `~/.npmrc` do tego środowiska.
...
Jeśli użytkownicy w Chinach kontynentalnych napotkają problemy z siecią i nie mogą opublikować pakietów `npm`, mogą ustawić zmienną środowiskową `https_proxy`, aby skonfigurować serwer proxy.
...
Jeśli chcesz samodzielnie hostować treści, najpierw edytuj `md/.i18n/htm/ol.yml` i zmień `v: //unpkg.com/i18n.site` na prefiks adresu URL, na przykład `v: //i18n-v.xxx.com`.
...
Aby samodzielnie hostować zawartość, oprócz korzystania z własnego serwera, inną popularną opcją jest użycie `S3` + `CDN`.
...
Witrynę można wdrożyć w dowolnym miejscu, [github page](https://pages.github.com) i [cloudflare page](https://pages.cloudflare.com) są dobrymi opcjami.
...
Strona wejściowa witryny musi być wdrożona tylko raz i nie ma potrzeby ponownego wdrażania strony wejściowej w celu aktualizacji zawartości.
...
Podczas publikowania treści z poprzedniego artykułu wygenerowano `out/ol/htm`. Proszę wejść do tego katalogu i uruchomić:
...
[cloudflare page](//pages.cloudflare.com) w porównaniu do `github page` zapewnia przepisywanie ścieżek i jest bardziej przyjazny dla Chin kontynentalnych i jest bardziej dostępny.
...
Jeśli powiązałeś magazyn innej organizacji, konieczne może być dwukrotne kliknięcie `Add Account`, aby dwukrotnie autoryzować, zanim zostanie wyświetlona nowa organizacja.
...
Jeśli używasz `i18n.site` do stworzenia swojej głównej strony internetowej, zazwyczaj musisz skonfigurować przekierowanie dla domen ogólnych, tzn. przekierować ruch z `*.xxx.com` (w tym `www.xxx.com`) na `xxx.com`.

Tego rodzaju wymaganie można zrealizować za pomocą `EdgeScript` w Alibaba Cloud `CDN` ([dokumentacja w języku angielskim](https://www.alibabacloud.com/help/en/cdn/developer-reference/how-edgescript-works) / [dokumentacja w języku chińskim](https://help.aliyun.com/zh/cdn/developer-reference/edgescript))

W [Alibaba Cloud CDN](https://cdn.console.aliyun.com/domain/list) dodaj nazwę domeny, aby `*.xxx.com` wskazywał na `CNAME` w Alibaba Cloud `CDN`.
...
W sekcji `server` w konfiguracji nginx dodaj podobne ustawienia, gdzie `/root/i18n/md/out/ol/htm` zmień na ścieżkę do swojego projektu `out/ol/htm`:
...
W katalogu `.i18n` znajdują się pliki konfiguracyjne `i18n.site`, pamięć podręczna tłumaczeń itd., szczegóły znajdziesz w następnym rozdziale [„Konfiguracja”](/i18n.site/conf).
...
Dla konfiguracji tłumaczeń proszę się zapoznać z [i18](/i18/use).