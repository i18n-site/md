...
brief: |
  ...
  W tym artykule przedstawiamy implementację technologii wyszukiwania pełnotekstowego na czystym froncie, w tym odwrócony indeks zbudowany za pomocą IndexedDB, wyszukiwanie prefiksowe, optymalizację podziału na słowa oraz wsparcie dla wielu języków.

  W porównaniu z obecnymi rozwiązaniami, wyszukiwanie pełnotekstowe na i18n.site jest mniejsze, szybsze i odpowiednie dla małych i średnich stron internetowych, takich jak dokumenty i blogi, a także działa w trybie offline.
...
W przypadku małych i średnich, czysto statycznych witryn internetowych, takich jak dokumenty/blogi osobiste, zbudowanie samodzielnie zbudowanego zaplecza wyszukiwania pełnotekstowego jest zbyt skomplikowane i częściej wybieranym wyborem jest wyszukiwanie pełnotekstowe bez usług.
...
1. Gotowe pliki indeksowe
   ...
   Ponieważ indeks zawiera słowa ze wszystkich dokumentów, ma on duży rozmiar.
...
1. Obsługuje wyszukiwanie wielojęzyczne, ma mały rozmiar i jest wydajne. Rozmiar jądra wyszukiwania po spakowaniu `gzip` wynosi `6.9KB` (porównaj z `lunrjs`, który ma `25KB`)
1. Zbuduj odwrócony indeks w oparciu o `indexedb`, co minimalizuje zużycie pamięci i zapewnia szybką pracę
1. Kiedy dokumenty są dodawane/modyfikowane, ponownie indeksowane są tylko te dokumenty, co znacznie zmniejsza obciążenie obliczeniowe
1. Podróżuje wsparcie dla wyszukiwania prefiksowego, co pozwala na wyświetlanie wyników w czasie rzeczywistym podczas wprowadzania tekstu
...
* `word` : id - słowo
* `doc` : id - URL dokumentu - wersja dokumentu
* `docWord` : id dokumentu - tablica id słów
* `prefix` : przedrostek - tablica id słów
* `rindex` : id słowa - id dokumentu : tablica numerów wierszy

Przekaż tablicę dokumentu `url` i numer wersji `ver`, sprawdź, czy dokument istnieje w tabeli `doc`. Jeśli nie istnieje, utwórz odwrócony indeks. Jednocześnie usuń odwrócony indeks dla tych dokumentów, które nie zostały przekazane.

W ten sposób można osiągnąć indeksowanie przyrostowe i zmniejszyć ilość obliczeń.

W interakcji z interfejsem użytkownika można wyświetlić pasek postępu ładowania indeksu, aby uniknąć opóźnień podczas ładowania po raz pierwszy. Zobacz „Pasek postępu z animacją, oparty na pojedynczej progress + Czystej css implementacji” [Angielski](//dev.to/i18n-site/a-single-progress-uses-pure-css-to-achieve-animation-effects-2oo) / [Chiński](//juejin.cn/post/7413586285954154522) .
...
Załóżmy, że po słowie segmentacja jest `N` słów. Podczas zwracania wyników najpierw zostaną zwrócone wyniki zawierające wszystkie słowa kluczowe, a następnie zostaną zwrócone wyniki zawierające `N-1` , `N-2` ,..., `1` słów kluczowych.
...
Podczas wyświetlania wyników wyszukiwania zostanie wyświetlona nazwa rozdziału, a po kliknięciu będzie można nawigować po rozdziale.
