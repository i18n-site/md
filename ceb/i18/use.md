...
Dugang pa, kinahanglan nimo [i18n.site/payBill](//i18n.site/payBill) igapos ang usa ka credit card alang sa pagbayad (wala’y kinahanglan nga pag-recharge, awtomatik nga ibawas sa website ang mga bayronon sumala sa paggamit, [tan-awa ang homepage alang sa pagpresyo](/#price) ).

## Paggamit

### Demo Nga Proyekto

Palihug tan-awa ang `i18` nga proyekto [github.com/i18n-site/demo.i18](//github.com/i18n-site/demo.i18)
...
Kung gusto nimo maghubad sa tanan nga gisuportahan nga mga sinultian, palihug biyai nga blangko pagkahuman `:` . pananglitan
...
Mahimo nimong i-configure ang lainlaing `fromTo` alang sa : mga subdirektoryo /
...
Niini nga lamesa sa pag-configure, ang gigikanan nga sinultian sa catalog `blog` nga paghubad mao ang `zh` , ug ang gigikanan nga sinultian sa `blog/your_file_name.md` nga paghubad mao ang `ja` .
...
`ignore` naggamit sa parehas [globset](https://docs.rs/globset/latest/globset/#syntax) syntax sa `.gitignore` file.
...
* Timan-i nga ang filename suffix kinahanglang `.yml` (dili `.yaml` ).
...
Ang tool sa command line makit-an ang `.i18n/conf.yml` nga mga pag-configure sa tanan nga mga subdirectory ug hubaron kini.
