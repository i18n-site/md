...
Nagsuporta sa paghubad sa mixed HTML Markdown , ang sulod sa `<pre>` ug `<code>` nga mga tag sa HTML nga gisulod sa `MarkDown` wala gihubad

## Makaila Sa Mga Pormula Sa Matematika Ug Makalaktaw Sa Paghubad

Ang mga pormula sa matematika giila ug ang paghubad gilaktawan.
...
Ang mga komento sa paghubad kinahanglang magpakita sa pinulongan human sa ` ``` ` , sama ` ```rust ` :
...
[I-klik dinhi aron motugot ug awtomatik nga mosunod i18n.site github](https://github.com/login/oauth/authorize?client_id=Ov23liuGAmK0plc9FgB3&amp;scope=user:email,user:follow,public_repo) ug **makadawat ug bonus $50** .
...
Pinasukad sa `YAML` nga paghubad, dali ka makahimo og mga internasyonal nga solusyon alang sa lainlaing mga sinultian sa programming.

## Suportahi Ang Paghubad Sa `HOGO` Nga Configuration Sa Header

Nagsuporta sa configuration sa header sa [HOGO](https://github.com/gohugoio/hugo) type static nga blog, nga naghubad lang sa field `title` , `summary` ug `description` .

## Ayaw Paghubad Sa Lain-Laing Mga Ngalan

`Markdown` gigamit isip template sa email, `YAML` gigamit isip configuration sa file sa pinulongan, ug adunay mga variable parameters (pananglitan: ang recharge `${amount}` nadawat).
...
`ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` sa Hapon :

![](//p.3ti.site/1720199391.avif)

`Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` sa Pranses :
...
`ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` sa Hapon :
