...
Human sa pagdagan, ang mga hubad ug cache nga mga file mabuhat Palihug hinumdumi nga idugang `git add . ` sa repository sa `md` nga direktoryo.

### Lokal Nga Preview

I-install ug sugdi `docker` ( `MAC` user ang nagrekomendar sa paggamit [orbstack](https://orbstack.dev) isip runtime para sa `docker` ).

Dayon, pagsulod sa `docker` nga direktoryo ug pagdagan `./up.sh` , ug dayon bisitaha ang [https://127.0.0.1](https://127.0.0.1)
...
Mahimo ka usab maghimo ug uban pang mga file sa pag-configure gamit ang `--htm_conf` sa linya sa mando aron mahibal-an ang ngalan sa file sa pag-configure nga gamiton:
...
I-edit `md/.i18n/htm/ol.yml` ug usba `i18n.site` sa `v: //unpkg.com/i18n.site` sa imong kaugalingon `npm` nga ngalan sa pakete.

Gamita lang ang ngalan sa package nga walay tawo sa [npmjs.com](//npmjs.com)

Kung ang pagmantala base sa package `npm` , **siguroha nga gamiton `//unpkg.com/` para** sa prefix nga `v:` nga kantidad `i18n.site` espesyal nga na-optimize ang oras sa cache nga `/.v` sa ilawom sa prefix nga agianan aron makab-ot ang tukma sa panahon nga pagtan-aw sa mga bag-ong gipagawas.
...
Kung gusto nimo nga i-host sa kaugalingon ang sulud, una nga i-edit `md/.i18n/htm/ol.yml` ug usba ang `v: //unpkg.com/i18n.site` sa imong prefix sa URL, sama sa `v: //i18n-v.xxx.com` .
...
Sa self-host nga sulod, `CDN` sa paggamit sa imong kaugalingong server, laing komon nga kapilian mao ang paggamit sa `S3` +

Mahimo nimong gamiton ang [rclone](https://rclone.org) sa pag-log in sa `S3` server, dayon i-refer ug usbon ang mosunod nga script, ug kopyaha lang ang incremental nga mga kausaban ngadto sa `S3` alang sa matag pagpagawas.
...
Unang pag- `i18n-demo` [github aron makahimo og organisasyon](https://github.com/account/organizations/new?plan=free)

Dayon paghimo og bodega `i18n-demo.github.io` ubos niini nga organisasyon (palihug pulihan `i18n-demo` sa ngalan sa organisasyon nga imong gibuhat):
...
Dayon padagana ang mosunod nga sugo sa `htm` nga direktoryo (hinumdomi nga ilisan `i18n-demo/i18n-demo.github.io.git` sa imong kaugalingong address sa bodega) :
...
Palihug i-klik ang `Add Account` aron mahatagan ug access ang organisasyon `i18n-demo` .
...
Dayon, gamita ang butang storage sa cloud vendors sa mainland + `CDN` I-deploy ang mosunod nga sulod `out/ol/htm` .
...
Tungod kay ang rekord `MX` ug rekord `CNAME` dili mag-uban, kung gusto nimo makadawat sa mga email sa ngalan sa domain sa parehas nga oras, kinahanglan nimo nga mokooperar [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten) script sa lebel `CNAME` hangtod sa record `A` .
...
Kini nga mga deployment optimization nga mga solusyon mas komplikado ug ipaila sa lain nga mga kapitulo sa umaabot.
...
Idugang ang domain name sa [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) ug itudlo ang domain name `*.xxx.com` ngadto sa `CNAME` sa Alibaba Cloud `CDN` .
...
Palihug idugang ang usa ka configuration nga susama sa mosunod sa `server` paragraph sa nginx Palihog usba `/root/i18n/md/out/ol/htm` sa dalan sa imong kaugalingong proyekto `out/ol/htm` :
