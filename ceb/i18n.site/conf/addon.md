...
Kung gusto nimo ayohon ang bersyon, mahimo nimong isulat `i18n.addon/toc@0.1.3` .
...
Diin ang `__CONF__` ilisan sa ngalan sa kasamtangang configuration (sama sa `dev` o `ol` ).
...
Ang built-in nga `js` runtime gibase sa secondary development : [boa](https://github.com/boa-dev/boa)

* `wPath(path, txt)` : Pagsulat sa file
* `rTxt(path)` : Basaha ang text file
* `rBin(path)` : Basaha ang binary file
* `rDir(dirpath)` : Basaha ang direktoryo, ang kantidad sa pagbalik usa ka lista : , lista sa file
