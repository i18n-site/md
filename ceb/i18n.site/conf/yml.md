...
### `use: Toc` , Usa Ka Template Sa File Nga Adunay Outline
...
### `use: Md` , Single File Template Nga Walay Outline
...
### `use: Doc` , Multi-File Nga Template Sa Dokumento
...
Ang paagi sa pagsulat sa ibabaw katumbas sa pagbaton og `url: xxx` , ug ang katugbang nga file niini mao ang `en/xxx/TOC` .

#### TOC Talaan Sa Mga Sulod Index

`i18n.site` ipatuman `js` plug-in `.i18n/hook/after.tran/TOC.js` sa demo bodega aron mabasa ang `TOC` nga directory index file nga katumbas sa `doc` template configuration aron makamugna ang `json` directory outline.
