...
Dayon, usba ang `~/.config/i18n.site.yml` ug idugang ang mosunod nga configuration :
...
Sa pag-configure, palihog usba `i18n.site` ngadto sa bili sa `host:` sa `.i18n/htm/ol.yml` , daghang mga butang nga tindahan mahimong ma-configure ubos sa `s3` , ug ang `region` nga field kay opsyonal (daghang butang nga tindahan dili kinahanglan nga itakda kini nga field).

Dayon pagdagan `i18n.site -n` aron imantala pag-usab ang proyekto.

Kung imong giusab ang `~/.config/i18n.site.yml` ug gusto nimo nga i-upload pag-usab, palihug gamita ang mosunod nga sugo sa direktoryo sa gamut sa proyekto aron malimpyohan ang cache sa pag-upload :
...
Ang code sa lagda mao ang mosunod, palihug usba ang code nga "i18n.site" sa imong domain name:
...
Kung gusto nimo maghatag serbisyo sa mainland China ug gusto usab ang `cloudflare` nga libre nga internasyonal nga trapiko, mahimo nimong gamiton `DNS` nga adunay resolusyon sa rehiyon.
...
Tungod kay `R2` `cloudflare` -access sa usa ka customized domain name, usa ka third-party nga butang storage kinahanglan nga gamiton sa pagbutang sa static files.

Dinhi among gikuha ang [backblaze.com](https://www.backblaze.com) isip usa ka pananglitan aron ipakita kung giunsa ang pagbugkos sa mga butang sa ikatulo nga partido nga tipigan sa `cloudflare` .

Paghimo og balde sa `backblaze.com` , pag-upload og bisan unsang file, i-klik aron ma-browse ang file, ug kuhaa ang domain name nga `Friendly URL` , nga mao ang `f003.backblazeb2.com` dinhi.
...
Usba ang `cloudflare` sa `SSL` → encryption mode, ibutang sa `Full`
...
Dugang pa, sa `cloudflare` nga lagda sa pagkakabig sa ibabaw, ang `index.html` giusab ngadto sa `file/your_bucketname/index.html` , ug ang ubang mga pag-configure nagpabilin nga pareho.
