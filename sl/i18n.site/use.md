...
Če za objavo uporabljate okolje za neprekinjeno integracijo, ni treba namestiti `nodejs` , samo kopirajte dovoljenje za prijavo in objavo `~/.npmrc` v okolje.
...
Ker zapis `MX` in zapis `CNAME` ne moreta obstajati sočasno, morate sodelovati s skriptom [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten) na ravni `CNAME` v zapisu `A` , če želite prejemati e-poštna sporočila z imenom domene.
