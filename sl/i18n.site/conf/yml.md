...
### `use: Doc` , Predloga Dokumenta Z Več Datotekami
...
Zgornja metoda zapisovanja je enakovredna `url: xxx` , ustrezna datoteka pa je `en/xxx/TOC` .

#### TOC Kazalo Vsebine

`i18n.site` bo izvedel `js` vtičnik `.i18n/hook/after.tran/TOC.js` v predstavitvenem skladišču, da prebere indeksno datoteko `TOC` imenikov, ki ustreza konfiguraciji predloge `doc` , da ustvari oris `json` imenikov.
