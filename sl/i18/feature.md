...
`i18` orodje ukazne vrstice za prevajanje Markdown & Yaml
...
Podpira prevajanje Markdown v `MarkDown` HTML `<pre>` vsebina v HTML `<code>` ni prevedena
...
Komentarji prevoda morajo navesti : ` ```rust ` ` ``` `

`coffee` podpira prevajanje komentarjev `toml` , `yaml` , `json5` , `go` , `rust` , `c` , `cpp` `java` `js` `python` , `bash` , `php` [in druge jezike](https://github.com/i18n-site/rust/blob/main/getc/src/style.rs#L14) .
...
Za programerje, ki poznajo `git` , `vim` , `vscode` , bo uporaba teh orodij za urejanje dokumentov in upravljanje različic nedvomno povečala stroške učenja.

`KISS` ( `Keep It Simple, Stupid` ) Med privrženci tega načela so rešitve na ravni podjetja sinonim za okorne, neučinkovite in težke za uporabo.
...
## Xindayin Visokokakovosten Prevod
...
Da bi dosegli enako kakovost, je količina ročnega urejanja, ki jo zahtevata Google Translate in `ChatGPT` `2.67` -krat oziroma `3.15` -krat večja.
...
## Podprite Prevod `YAML`

Orodje prevaja samo slovarske vrednosti v `YAML` , ne slovarskih ključev.

Na podlagi `YAML` Translation lahko preprosto zgradite mednarodne rešitve za različne programske jezike.

## Podpora Prevodu `HOGO` Konfiguracija Glave

Podpira konfiguracijo statičnega bloga tipa [HOGO](https://github.com/gohugoio/hugo) , prevedena so samo polja `title` , `summary` in `description` .

## Ne Prevajajte Imen Spremenljivk

`Markdown` Uporablja se kot e-poštna predloga, `YAML` kot konfiguracija jezikovne datoteke, tam bodo spremenljivi parametri (na primer: ponovno polnjenje `${amount}` je bilo prejeto).

`${varname}` Ta vrsta imena spremenljivke ne bo obravnavana kot angleški prevod.

## Optimizacija Prevajanja Za Kitajsko, Japonsko in Korejo

### Pri Prevodu v Angleščino Je Prva Črka Naslova Samodejno Velika.

Kitajska, Japonska in Koreja nimajo velikih ali malih črk, vendar je v angleških naslovih prva črka običajno velika.

`i18` prepozna naslove v `MarkDown` in bo pri prevajanju v jezik, ki razlikuje med velikimi in malimi črkami, napisal prvo črko.

Na primer `为阅读体验而优化` bo prevedeno kot `Optimized for Reading Experience` .
...
`Falcon` se prevede kot `faucon` in `Llama` se prevede kot `lama` . Kot lastnih imen jih ne bi smeli prevajati.
...
DeepL Prevod zgornjega v francoščino (tudi prepisovanje lastnih imen in dodajanje lihega `... ` ):
...
## Lahko Se Kombinira `i18n.site` Večjezična Izdelava Spletnih Strani

`i18` Je integrirano v orodje ukazne vrstice za izdelavo večjezičnega spletnega mesta [`i18n.site`](/i18n.site) .

Za podrobnosti glejte dokumentacijo [`i18n.site`](/i18n.site)

## Odprta Koda

Odjemalec je popolnoma odprtokoden, strežniška stran pa je odprtokodna 90 [Kliknite tukaj za ogled izvorne kode](/i18n.site/src) .
