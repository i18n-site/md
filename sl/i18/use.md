...
windows Sistem, [najprej kliknite tukaj za prenos in namestitev `git bash`](https://git-scm.com/download/win)

Izvedite naslednje operacije v `git bash`
...
Ustvari `~/.config/i18n.site.yml` vanj prilepi kopirano vsebino, vsebina je naslednja:
...
Prosimo, glejte predstavitveni projekt `i18` da se naučite konfiguracije prevoda [github.com/i18n-site/demo.i18](//github.com/i18n-site/demo.i18)

Uporabniki na Kitajskem lahko klonirajo [atomgit.com/i18n/demo.i18](//atomgit.com/i18n/demo.i18)

Po kloniranju vnesite imenik in zaženite prevod `i18`
...
Imenik `en` vsebuje prevedene demo datoteke, ki so samo primer in jih je mogoče izbrisati.

### Zaženi Prevod

Vnesite imenik in zaženite prevod `i18`
...
Med njimi : naslednja vsebina `.i18n/cache/.gitignore`
...
To pomeni, da prezrete vse datoteke v `.i18n/cache/` (razen `.i18n/cache/.gitignore` ).

Če vaša programska oprema za nadzor različic ni `git` , jo prezrite glede na to konfiguracijo.

## Konfiguracijska Datoteka

`.i18n/conf.yml` To je konfiguracijska datoteka orodja za prevajanje `i18` line
...
V konfiguracijski datoteki so podrejeni `fromTo`
...
Če želite prevesti v vse podprte jezike, pustite prazno za `:` na primer
...
Če želite prezreti določene datoteke in jih ne prevesti (na primer nedokončane osnutke), lahko uporabite konfiguracijo polja `ignore`

Uporablja `ignore` sintakso kot `.gitignore` [globset](https://docs.rs/globset/latest/globset/#syntax)

Na primer, v zgornji konfiguracijski datoteki `_* ` pomeni, da datoteke, ki se začnejo z `_` ne bodo prevedene.
...
bo prevedeno kot `i18n/zh/i18n.yml`
...
Dokler ustvarjate `.i18n/conf.yml` (ni vam treba vsakič začeti s predlogo demo projekta), bo `i18` delovalo dobro.

Orodje ukazne vrstice bo našlo konfiguracijo `.i18n/conf.yml` v vseh podimenikih in jo prevedlo.
...
Nastavite spremenljivke okolja `TO` Določite lahko na primer namestitveni imenik :
