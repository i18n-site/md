<h1 style="display:flex;justify-content:space-between">i18n.site Mednarodne rešitve<img src="//p.3ti.site/logo.svg" style="user-select:none;margin-top:-1px;width:42px"></h1>
...
V informacijski eksploziji in svetovnem trgu **je podpora več jezikov osnovna veščina** , da bi tekmovali za redko pozornost.
...
## <a rel=id href="#project" id="project"></a> Opis projekta
...
Prevedite `Markdown` in `YAML` v orodje ukazne vrstice ( [izvorna koda](https://github.com/i18n-site/rust/tree/main/i18) ) v več jezikih.
...
Delimo " `UNIX` je datoteka " in lahko upravljamo prevode v stotine jezikov brez uvajanja zapletenih in okornih rešitev na ravni podjetja.

Za `Markdown` prevoda lahko uporabite najbolj znana orodja (vendar ne morete dodajati ali brisati odstavkov) in uporabite najbolj znan način za nadzor različic.

Osnovo kode je mogoče ustvariti kot odprto kodo za jezikovne datoteke, s pomočjo procesa `Pull Request` pa lahko globalni uporabniki sodelujejo pri nenehni optimizaciji prevodov. Brezhibna povezava [github](//github.com) In druge odprtokodne skupnosti.
...
Čisto statično, optimizirano za bralno izkušnjo, integrirano s prevodom [i18](#i18) je najboljša izbira za gradnjo spletnega mesta s projektnimi dokumenti.
...
To spletno mesto je razvito na podlagi tega ogrodja in vključuje uporabniške, plačilne in druge funkcije ( [izvorna koda](/i18n.site/src) ) bo napisana kasneje.
