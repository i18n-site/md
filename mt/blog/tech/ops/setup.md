...
Li juri Mudell `AMD EPYC 9224 24-Core Processor` , punteġġ ta' prestazzjoni b'qalba waħda 1285 CPU
...
L-ewwel ħaġa IP li teditja `~/.ssh/config` tal-magna lokali biex tissettja alias, u mbagħad tuża `ssh-copy-id root@c2` biex tikkonfigura login mingħajr password.
...
Dan il-pass jieħu żmien twil Wara li tistenna għal nofs siegħa, l-aġġornament u l-installazzjoni ta 'softwer użat b'mod komuni se jkunu ta' suċċess. Tista' tikkonferma dan bil-kmand li ġej:
...
In-netwerk mill-Ewropa lejn iċ-Ċina mhux stabbli Huwa rakkomandat li tuża `ssh` [mosh](//mosh.org)

Oħloq `c0` skripts fid-direttorju lokali `~/.bin` (prinċipalment miżjud mal-varjabbli ambjentali `PATH` ) kif ġej:
...
Wara l-ħolqien, agħti permessi eżekutibbli u soft-link `c1` u `c2` .
...
Fil-futur, tista 'tidħol direttament `c0` u `c1` biex tuża `mosh` konnessjoni diretta b'veloċità għolja u tidħol [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Se jkun hemm bidliet fil-marki tas-swaba ssh , uża `ssh-keygen -R ip` biex tnaddafha
...
L-iskrittura tal-inizjalizzazzjoni se toħloq id-direttorju `~/ubuntu` , mexxi biss il-kmand li ġej:
...
`/etc/snapper/configs/mnt` jista 'jara s-settings speċifiċi tal-backup ta' dan is-subvolum
...
Żomm f'moħħok li l-operazzjonijiet ta 'hawn fuq isiru fuq sistema ta' salvataġġ, u għalhekk dawk il-bidliet ma jidħlux fis-seħħ sakemm is-sistema terġa 'tibda u ma tibqax fil-modalità ta' salvataġġ.

Dan ifisser li m'intix se tara immedjatament xi tibdiliet fir-zkuk eżistenti li jsiru persistenti, iżda z-zkuk kollha ġġenerati wara se jibqgħu jippersistu.
...
Użajt azzjoni github biex nagħmel script ta' kumpilazzjoni u skjerament awtomatiku qabel, iżda sibt li s-servizz ta' skjerament ma setax ikun ssh.
...
Wara investigazzjonijiet ripetuti, huwa github action l-Utent `uid` ta tar huwa `1001`

L-ispakkjar għandu jkun jekk ma jkunx hemm parametru `--no-same-owner` , ikun dekompressat għal `uid` għal `1001` utenti tar
