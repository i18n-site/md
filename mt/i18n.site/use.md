...
Jekk tuża ambjent ta 'integrazzjoni kontinwa biex tippubblika, m'hemmx bżonn li tinstalla `nodejs` , tikkopja biss il-permess li illoggja u tippubblika `~/.npmrc` għall-ambjent.
...
Minħabba li l-websajt tuża arkitettura [ta 'applikazzjoni ta' paġna waħda](https://developer.mozilla.org/docs/Glossary/SPA) , ftakar li tikteb mill-ġdid il-mogħdijiet tal-URL li ma fihomx `. ` sa `index.html` .
...
Imbagħad, uża l-ħażna tal-oġġetti tal `out/ol/htm` bejjiegħa tas-sħab fiċ-Ċina kontinentali + Użu l-kontenut li ġej `CDN` .
