...
[i18n.site/token](//i18n.site/token) እና በዋናው ጽሑፍ መካከል ያለው ካርታ `.i18h/hash` ጋር የተያያዘ ነው።

## `YAML` አገናኝ `HTML` ወደ `Markdown` እንዳይቀየር እንዴት ማስወገድ :

የ `YAML` እሴት ለትርጉም እንደ `MarkDown` ይቆጠራል።

አንዳንድ ጊዜ ከ `HTML` → `MarkDown` መለወጥ የምንፈልገውን አይደለም, ለምሳሌ `<a href="/">Home</a>` ወደ `[Home](/)` ይቀየራል.

ከ `href` ሌላ ማንኛውንም ባህሪ ወደ `a` መለያ እንደ `<a class="A" href="/">Home</a>` ማከል ይህንን መለወጥ ያስወግዳል።

## `./i18n/hash` ፋይል ግጭቶች ከዚህ በታች
