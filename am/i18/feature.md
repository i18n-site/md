...
ቅርጸቱን ሳይጎዳ የማርክዳውን ሰንጠረዦች መተርጎምን ይደግፋል፤ በሂሳብ ቀመሮች ወይም አገናኞች ውስጥ ቃላትን አይተረጎምም።

የተቀላቀለ ትርጉምን ይደግፋል በ HTML ውስጥ በ `<pre>` Markdown `<code>` HTML ውስጥ ያለው ይዘት አልተተረጎመም `MarkDown`
...
አንዳንድ የትዕዛዝ መስመር የትርጉም መሳሪያዎችም አሉ፣ ለምሳሌ [translate-shell](https://github.com/soimort/translate-shell)
...
`i18` በ `MarkDown` ውስጥ ያለውን ርዕስ ማወቅ ይችላል, እና ወደ ኬዝ-ስሱ ቋንቋ ሲተረጎም የመጀመሪያውን ፊደል በራስ-ሰር ያዘጋጃል.
...
`Falcon` እንደ `faucon` ተተርጉሟል እና `Llama` እንደ `lama` ተተርጉሟል. እንደ ትክክለኛ ስሞች, መተርጎም የለባቸውም.
...
ከላይ ያለውን ወደ ፈረንሳይኛ DeepL (እንዲሁም ትክክለኛዎቹን ስሞች እንደገና በመጻፍ እና ያልተለመደ `... ` ማከል)
...
ደንበኛው ሙሉ በሙሉ ክፍት ነው, እና የአገልጋዩ 90 % ክፍት ነው [ምንጭ ኮድ ለማየት](/i18n.site/src) .
