...
የተቀላቀለ HTML Markdown በ `MarkDown` ውስጥ በ `<pre>` እና `<code>` መለያዎች ውስጥ ያለው ይዘት አልተተረጎመም HTML
...
`git` ፣ `vim` እና `vscode` ለሚያውቁ ፕሮግራመሮች እነዚህን መሳሪያዎች ሰነዶችን ለማረም እና ስሪቶችን ለማስተዳደር መጠቀማቸው የመማሪያ ወጪን እንደሚጨምር ጥርጥር የለውም።
...
መስኮችን `title` ፣ `summary` እና `description` ብቻ የሚተረጉመውን የ [HOGO](https://github.com/gohugoio/hugo)
...
### የእንግሊዝኛ ቃላት በቻይንኛ፣ጃፓንኛ፣ ኮሪያኛ እና ቻይንኛ አልተተረጎሙም።
...
ጎግልን ወይም DeepLን የምትጠቀም ከሆነ ሁለቱም ኦሪጅናል ሆነው መቆየት ያለባቸውን የእንግሊዝኛ ቃላት በትክክል ተርጉመዋል (ጃፓን እና ፈረንሳይኛን እንደ ምሳሌ ውሰድ)።
...
ከላይ ወደ DeepL መተርጎም (እንዲሁም ትክክለኛዎቹን ስሞች እንደገና መጻፍ እና ያልተለመደ `... ` ማከል)
