# አገልጋዩን ያስጀምሩ (የBTRFS ክፍልፍል፣ የ snapper መርሐግብር የተያዘለት ቅጽበታዊ ገጽ እይታን ይጠቀሙ)
...
ማሳያ CPU ሞዴል `AMD EPYC 9224 24-Core Processor` ነጠላ-ኮር የአፈጻጸም ነጥብ 1285
...
አገልጋዮቹ አልፎ አልፎ ይዘጋሉ፣ ይህም ከፍተኛ ተደራሽነት ባለው የድረ-ገጽ ተደራሽነት ያረጋግጣል።
...
የመጀመሪያው ነገር የሃገር ውስጥ ማሽንን ማስተካከል ነው `~/.ssh/config` ለ IP ተለዋጭ ስም ማዘጋጀት እና ከዚያ የይለፍ ቃል-ነጻ መግቢያን ለማዋቀር `ssh-copy-id root@c2` ይጠቀሙ።
...
CN=1 ቋንቋ ወደ ቻይንኛ እና የሰዓት ዞን ወደ ቤጂንግ መቀየር ማለት ነው።

በነባሪነት የተጫነው [i18n-ops/ubuntu](https://github.com/i18n-ops/ubuntu) [direnv](https://direnv.net/) [mise](https://github.com/jdx/mise)

ይህ እርምጃ ለግማሽ ሰዓት ያህል ከቆየ በኋላ ብዙ ጊዜ ጥቅም ላይ የሚውለውን ሶፍትዌር ማሻሻል እና መጫን ስኬታማ ይሆናል። በሚከተለው ትዕዛዝ ማረጋገጥ ይችላሉ፡-
...
ከአውሮፓ እስከ ቻይና ያለው አውታረመረብ የተረጋጋ አይደለም `ssh` ለመጠቀም ይመከራሉ [mosh](//mosh.org)

`~/.bin` ማሽን ውስጥ የ `c0` ስክሪፕት ይፍጠሩ (በዋነኛነት ወደ የአካባቢ ተለዋዋጭ `PATH` ) ማውጫ ውስጥ እንደሚከተለው ይፍጠሩ።
...
ከተፈጠረ በኋላ፣ ተፈፃሚ የሆኑ ፈቃዶችን እና ለስላሳ- `c2` ይስጡ `c1`
...
ለወደፊት፡ በቀጥታ `c0` ማስገባት ትችላላችሁ እና `mosh` ከፍተኛ ፍጥነት `c1` ቀጥተኛ ግንኙነትን መጠቀም እና [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)

## የክፋይ ቅርፀትን ከ ext4 ወደ btrfs ቀይር
...
`ssh-keygen -R ip` የጣት አሻራ ለውጦች ይከሰታሉ ssh
...
የጨመቁትን ጥምርታ ለመፈተሽ ትእዛዝ በነባሪነት በራስ-ሰር ይጨመቃል።
...
የማስጀመሪያው ስክሪፕት `~/ubuntu` ይፈጥራል፣ የሚከተለውን ትዕዛዝ ብቻ ያሂዱ፡-
...
ቅጽበተ-ፎቶዎች የተቀናበሩበትን ንዑስ ጥራዞች ለማየት `snapper list-configs` ።

<img alt="" src="https://i-01.eu.org/1718302689.avif">

የአንድ ንዑስ ጥራዝ ቅጽበታዊ ገጽ እይታን ለማየት `snapper -c mnt list`

የዚህን ንዑስ ጥራዝ ልዩ የመጠባበቂያ ቅንጅቶችን ማየት `/etc/snapper/configs/mnt`
...
ብዙውን ጊዜ btrfs ቅጽበተ-ፎቶዎችን ለመጠቀም የበለጠ አመቺ ነው።
...
ከላይ ያሉት ስራዎች የሚከናወኑት በማዳኛ ስርዓት ላይ መሆኑን አስታውስ፣ ስለዚህ እነዚያ ለውጦች የሚተገበሩት ስርዓቱ እንደገና ከተጀመረ እና ከአሁን በኋላ በማዳን ሁነታ ላይ ካልሆነ ብቻ ነው።

ይህ ማለት በነባር ምዝግብ ማስታወሻዎች ላይ ምንም አይነት ለውጦች ቋሚ ሲሆኑ ወዲያውኑ አታዩም ነገር ግን ከዚህ በኋላ የሚፈጠሩ ሁሉም ምዝግብ ማስታወሻዎች ይቀጥላሉ ማለት ነው።
...
ከዚህ በፊት አውቶማቲክ ማጠናቀር እና ማሰማራት ስክሪፕት ለመስራት github እርምጃን ተጠቀምኩ፣ ነገር ግን የማሰማራቱ አገልግሎት ssh አልቻለም።
...
ከተደጋጋሚ ምርመራዎች በኋላ `1001` tar የሰቀለው ተጠቃሚ `uid` ነው github action

tar ያለ መለኪያ መከናወን አለበት `--no-same-owner` ለ `1001` ተጠቃሚ `uid` ይከፋፈላል።
