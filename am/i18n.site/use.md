...
ለማተም ያልተቋረጠ የውህደት አካባቢን ከተጠቀሙ፣ `nodejs` መጫን አያስፈልግም፣ የገባውን ብቻ ይቅዱ እና ፍቃድ `~/.npmrc` ለአካባቢው ያትሙ።
...
ይዘቱን እራስዎ ማስተናገድ ከፈለጉ መጀመሪያ `md/.i18n/htm/ol.yml` ያርትዑ እና `v: //unpkg.com/i18n.site` ወደ የእርስዎ URL ቅድመ ቅጥያ ይለውጡ፣ ለምሳሌ `v: //i18n-v.xxx.com` ።
...
በቀደመው መጣጥፍ ውስጥ ያለውን ይዘት ሲያትሙ `out/ol/htm` ተፈጥሯል እባኮትን ይህንን ማውጫ ያስገቡ እና ያሂዱ :
...
ከዚያ በዋና + ውስጥ የደመና አቅራቢዎችን ዕቃ ማከማቻ ይጠቀሙ `CDN` የሚከተለውን ይዘት `out/ol/htm` ያሰማሩ።
...
`out/ol/htm` በ `server` አንቀጽ ውስጥ `/root/i18n/md/out/ol/htm` ጋር ተመሳሳይ የሆነ ውቅር ያክሉ nginx
