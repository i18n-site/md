...
### `use: Md` ፣ ነጠላ የፋይል አብነት (ምንም ዝርዝር የለም)
...
ለምሳሌ, `/test` ከደረሰ, እና `nav:` ያለዚህ መንገድ ከተዋቀረ, እና የገጹ ቋንቋ እንግሊዝኛ (ኮድ `en` ) ከሆነ, `/en/test.md` በነባሪነት ተጭኖ አብነት `Md` በመጠቀም ይቀርባል.
...
`NB` , የ `Name Breif` አህጽሮተ ቃል, ተቆልቋይ ምናሌው የፕሮጀክቱን ስም እና መፈክር ማሳየት ይችላል.
...
`i18n.site` የ `json` ዳይሬክተሩን ዝርዝር ለማመንጨት ከ `TOC` አብነት ውቅር ጋር የሚዛመደውን `doc` ማውጫ ማውጫ ፋይል ለማንበብ `js` plug-in `.i18n/hook/after.tran/TOC.js` በማሳያው መጋዘን ውስጥ ያስፈጽማል።
...
`TOC` ፋይሎች የዝርዝሩን ተዋረዳዊ ግንኙነት ለማመልከት ገብተዋል እና ባለብዙ ደረጃ መግባቱን ይደግፋሉ።
