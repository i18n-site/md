...
`TOC` የ `Table of Contents` ምህጻረ ቃል ነው. ይህ አብነት ሲሰራ, የዚህ `Markdown` ፋይል ዝርዝር በጎን አሞሌ ላይ ይታያል.

`url:` የ `Markdown` ፋይልን መንገድ ይወክላል ( `/` ከስር ማውጫ `/README.md` ጋር ይዛመዳል ፣ ይህ የፋይል ስም አቢይ ሆሄ እና ትንሽ ቅጥያ ይፈልጋል)።

### `use: Md` ፣ ነጠላ ፋይል አብነት ያለ ዝርዝር
...
ከላይ ያለው የአጻጻፍ ዘዴ ከ `url: xxx` ጋር እኩል ነው, እና ተዛማጅ ፋይል `en/xxx/TOC` ነው.

#### የTOC ማውጫ ማውጫ

`i18n.site` የ `json` ዳይሬክተሩን ዝርዝር ለማመንጨት ከ `doc` አብነት ውቅር ጋር የሚዛመደውን `TOC` ማውጫ ማውጫ ፋይል ለማንበብ `js` plug-in `.i18n/hook/after.tran/TOC.js` በማሳያው መጋዘን ውስጥ ያስፈጽማል።

`doc` አብነት የሚጠቀሙ ከሆነ፣ ይህ ተሰኪ ሊኖርዎት ይገባል።

ፕሮጄክት `i18n.site` ከባዶ አቃፊ ካስጀመርክ፣ የማሳያ ፕሮጄክት `.i18n` ወደ ማውጫህ መቅዳት እንዳለብህ አስታውስ።
...
`Deme Two` ከተቆልቋይ ምናሌው በታች የራሱ `Your Project slogan` መለያ መስመር እና የካታሎግ ዝርዝር የፕሮጀክት ስም እንዳለው ማየት ይችላሉ :
...
ለምሳሌ, `/test` ከተደረሰ እና `nav:` ያለ የዚህ መንገድ ቅድመ ቅጥያ ከተዋቀረ እና አሁን ያለው የአሰሳ ቋንቋ እንግሊዝኛ (ኮድ `en` ) ከሆነ `/en/test.md` በነባሪ ይጫናል እና አብነት `Md` በመጠቀም ይቀርባል.
