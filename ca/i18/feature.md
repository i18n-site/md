...
`i18` és una eina de línia d'ordres per traduir Markdown & Yaml
...
Admet la traducció de Markdown en HTML mixtes `<pre>` HTML i el contingut en `<code>` no es tradueix `MarkDown`
...
Els comentaris de traducció han d'indicar l'idioma ` ``` ` , com ` ```rust ` :

`rust` `js` `yaml` `coffee` `cpp` `go` `json5` `c` `toml` `java` `python` `bash` `php`[](https://github.com/i18n-site/rust/blob/main/getc/src/style.rs#L14)
...
Per als programadors familiaritzats amb `git` `vim` , `vscode` , l'ús d'aquestes eines per editar documents i gestionar versions, sens dubte, augmentarà el cost de l'aprenentatge.

( `Keep It Simple, Stupid` ) Entre els que creen en el principi, les solucions a nivell empresarial són `KISS` de ser feixugues, ineficients i difícils d'utilitzar.
...
Modifiqueu el text original i torneu a traduir-lo automàticament, les modificacions manuals a la traducció no se sobreescriuran (si aquest paràgraf del text original no s'ha modificat).

## La Traducció D'alta Qualitat De Xindaya

Hem desenvolupat una nova generació de tecnologia de traducció que integra els avantatges tècnics dels models tradicionals de traducció automàtica i dels grans models d'idiomes per fer que les traduccions siguin precises, suaus i elegants.
...
Per aconseguir la mateixa qualitat, la quantitat d'edició manual requerida per Google Translate i `ChatGPT` és `2.67` vegades i `3.15` vegades la nostra, respectivament.
...
## `HOGO` La Configuració De La Capçalera

Admet la configuració de capçalera de bloc estàtic de tipus [HOGO](https://github.com/gohugoio/hugo) , `summary` es tradueixen els `description` `title`

## No Traduïu Noms De Variables

`Markdown` com a plantilla de correu electrònic, `YAML` com a configuració de fitxer d'idioma, hi haurà paràmetres variables (per exemple: s'ha rebut la recàrrega `${amount}` ).

Aquest tipus de nom variable `${varname}` es considerarà una traducció a l'anglès.
...
La Xina, el Japó i Corea no tenen lletres majúscules ni minúscules, però els títols en anglès generalment posen en majúscula la primera lletra.

`i18` reconèixer els títols en `MarkDown` , i automàticament posarà en majúscula la primera lletra quan es tradueixi a un idioma que distingeix entre majúscules i minúscules.

Per exemple `为阅读体验而优化` es traduirà com `Optimized for Reading Experience` .
...
La traducció automàtica d'idiomes xinès, japonès i coreà s'ha convertit en una llengua no anglesa, i els termes sovint es tradueixen junts, com ara la següent frase xinesa:
...
Traduït al `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :
...
DeepL de l'anterior al francès (també reescrivint els noms propis i afegint els estranys `... ` ):
...
`i18` traducció reconeixerà els termes anglesos en documents xinès, japonès i coreà i deixarà els termes intactes.
...
Aquesta paraula només es considerarà un terme si hi ha espais entre la paraula anglesa i el text xinès, japonès i coreà o la longitud de la paraula anglesa és superior a 1.

Per exemple: `C罗` es traduirà com `Cristiano Ronaldo` .

## Es Pot Combinar `i18n.site` Creació De Llocs Web en Diversos Idiomes

`i18` Està integrat a l'eina de línia d'ordres per a la creació de llocs web en diversos idiomes [`i18n.site`](/i18n.site) .

Consulteu la documentació de [`i18n.site`](/i18n.site)

## Codi De Codi Obert

El client és de codi obert 90 el % del codi és de codi obert. [Feu clic aquí per veure el codi font](/i18n.site/src) .
...
Si us plau, <button onclick="mailsub()">feu clic a aquest correu electrònic per subscriure's a les actualitzacions del producte</button> i <button onclick="webpush()">activar el push del navegador</button> . Us avisarem quan es facin actualitzacions del producte.

Benvingut també a seguir els nostres comptes socials / [X.COM: @i18nSite](https://x.com/i18nSite) [i18n-site.bsky.social](https://bsky.app/profile/i18n-site.bsky.social)
