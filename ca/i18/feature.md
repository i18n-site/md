...
Admet la traducció de HTML Markdown , el contingut de les etiquetes `<pre>` i `<code>` de HTML incrustat a `MarkDown` no es tradueix
...
Admet la configuració de capçalera de bloc estàtic de tipus [HOGO](https://github.com/gohugoio/hugo) , que només tradueix els camps `title` , `summary` i `description` .
...
Traducció de l'anterior al francès (també reescrivint els noms propis i afegint el `... ` senar) DeepL
