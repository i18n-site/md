...
Línia d'ordres Markdown l'eina Yaml , t'ajuda a crear un lloc de documents internacionals que admet [centenars d'idiomes](/i18/LANG_CODE) ...
...
Tot i que el navegador té una traducció integrada, els motors de cerca encara no poden fer consultes entre idiomes.
...
Ens subscrivim a la filosofia de `UNIX` Tot és un fitxer " i podem gestionar traduccions a centenars d'idiomes sense introduir solucions complexes i feixugues " nivell empresarial.

Podeu utilitzar les eines més familiars per editar la `Markdown` (però no podeu afegir ni suprimir paràgrafs) i fer servir la manera més familiar de fer el control de versions.

Es pot crear una base de codi com a codi obert per a fitxers d'idioma i, amb l'ajuda de processos `Pull Request` , els usuaris globals poden participar en l'optimització contínua de les traduccions. Connexió [github](//github.com) i altres comunitats de codi obert.
...
[Feu clic aquí per autoritzar i seguir automàticament i18n.site github](https://github.com/login/oauth/authorize?client_id=Ov23liuGAmK0plc9FgB3&amp;scope=user:email,user:follow,public_repo) i **rebre una bonificació $50** .
