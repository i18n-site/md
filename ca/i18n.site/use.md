...
Si utilitzeu un entorn d'integració contínua per publicar, no cal que instal·leu `nodejs` , només cal que copieu el permís `~/.npmrc` d'inici de sessió i de publicació a l'entorn.

Si modifiqueu el nom del paquet d' `v:` en `ol.yml` , **assegureu-vos d'eliminar primer `.i18n/v/ol`** i després publicar-lo.
