...
Setelah dijalankan, file terjemahan dan cache akan dibuat. Harap ingat untuk `git add . ` ke repositori di direktori `md` .
...
Edit `md/.i18n/htm/ol.yml` dan ubah `i18n.site` in `v: //unpkg.com/i18n.site` menjadi `npm` nama paket Anda sendiri.
...
Saat menerbitkan berdasarkan paket `npm` , **pastikan untuk menggunakan `//unpkg.com/` untuk** awalan nilai `v:` `i18n.site` telah secara khusus mengoptimalkan waktu cache `/.v` di bawah jalur awalan ini untuk mencapai penayangan rilis baru secara tepat waktu.
...
Kemudian, konfigurasikan konten di direktori `md/out/ol/v` ke jalur awalan URL yang diatur dalam `v:` .
...
Untuk menghosting sendiri konten, `CDN` menggunakan server Anda sendiri, opsi umum lainnya adalah menggunakan `S3` +
...
Kemudian, gunakan penyimpanan objek vendor cloud di daratan Tiongkok + Terapkan konten berikut `CDN` `out/ol/htm`
...
Tambahkan nama domain di [Alibaba Cloud CDN](https://cdn.console.aliyun.com/domain/list) dan arahkan nama domain `*.xxx.com` ke `CNAME` di Alibaba Cloud `CDN` .

![](https://p.3ti.site/1721122000.avif)

Misalnya konfigurasi pengalihan nama pan-domain `*.i18n.site` pada gambar di atas adalah sebagai berikut:
