...
### `use: Doc` , Templat Dokumen Multi-File
...
Cara penulisan di atas setara dengan memiliki `url: xxx` , dan file terkaitnya adalah `en/xxx/TOC` .

#### Indeks Daftar Isi TOC

`i18n.site` akan menjalankan `js` plug-in `.i18n/hook/after.tran/TOC.js` di gudang demo untuk membaca file indeks `TOC` direktori yang sesuai dengan konfigurasi templat `doc` untuk menghasilkan garis besar `json` direktori.
