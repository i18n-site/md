...
Dalam konfigurasi, harap ubah `i18n.site` ke nilai `host:` di `.i18n/htm/ol.yml` , beberapa penyimpanan objek dapat dikonfigurasi di bawah `s3` , dan bidang `region` bersifat opsional (banyak penyimpanan objek tidak perlu menyetel bidang ini).
...
Karena `R2` `cloudflare` diakses dengan nama domain yang disesuaikan, penyimpanan objek pihak ketiga perlu digunakan untuk menempatkan file statis.
