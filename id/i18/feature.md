...
Mendukung terjemahan campuran HTML Markdown , konten dalam tag `<pre>` dan `<code>` HTML yang tertanam dalam `MarkDown` tidak diterjemahkan

## Dapat Mengenali Rumus Matematika Dan Melewati Terjemahan

Rumus matematika dikenali dan terjemahan dilewati.

Untuk cara menulis rumus matematika, silakan merujuk pada [" Github Tentang Menulis Ekspresi Matematika"](https://docs.github.com/get-started/writing-on-github/working-with-advanced-formatting/writing-mathematical-expressions#about-writing-mathematical-expressions) .
...
Kami telah mengembangkan teknologi terjemahan generasi baru yang mengintegrasikan keunggulan teknis model terjemahan mesin tradisional dan model bahasa besar untuk menjadikan terjemahan akurat, lancar, dan elegan.
...
[Klik di sini untuk mengotorisasi dan secara otomatis i18n.site github](https://github.com/login/oauth/authorize?client_id=Ov23liuGAmK0plc9FgB3&amp;scope=user:email,user:follow,public_repo) dan **menerima bonus $50** .
...
Mendukung konfigurasi header blog statis tipe [HOGO](https://github.com/gohugoio/hugo) , yang hanya menerjemahkan kolom `title` , `summary` dan `description` .
...
Diterjemahkan ke dalam bahasa `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` :

![](//p.3ti.site/1720199391.avif)

Diterjemahkan ke dalam bahasa `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :
...
Diterjemahkan ke dalam bahasa `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` :
...
Klien sepenuhnya open source, dan sisi server 90 % kodenya adalah open [source](/i18n.site/src) .
