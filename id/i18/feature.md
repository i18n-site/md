...
`i18` adalah alat baris perintah untuk menerjemahkan Markdown & Yaml
...
Mendukung terjemahan Markdown dalam campuran HTML `<pre>` HTML dalam `MarkDown` dan konten dalam tag `<code>` tidak diterjemahkan
...
Komentar terjemahan harus menunjukkan bahasa setelahnya ` ``` ` , ` ```rust ` :

Saat ini mendukung terjemahan komentar `toml` , `yaml` , `json5` , `go` , `rust` , `c` , `cpp` , `java` `js` `coffee` , `python` , `bash` , `php` [dan bahasa lainnya](https://github.com/i18n-site/rust/blob/main/getc/src/style.rs#L14) .
...
Bagi programmer yang familiar dengan `git` , `vim` , `vscode` , menggunakan alat ini untuk mengedit dokumen dan mengelola versi pasti akan meningkatkan biaya pembelajaran.

`KISS` ( `Keep It Simple, Stupid` ) Bagi para penganut prinsip ini, solusi tingkat perusahaan identik dengan kerumitan, tidak efisien, dan sulit digunakan.
...
## Terjemahan Xindaya Yang Berkualitas Tinggi

Kami telah mengembangkan teknologi terjemahan generasi baru yang mengintegrasikan keunggulan teknis model terjemahan mesin tradisional dan model bahasa besar untuk membuat terjemahan akurat, lancar, dan elegan.

Tes buta menunjukkan bahwa kualitas terjemahan kami jauh lebih baik dibandingkan dengan layanan serupa.

Untuk mencapai kualitas yang sama, jumlah pengeditan manual yang diperlukan oleh Google Terjemahan dan `ChatGPT` masing-masing adalah `2.67` kali dan `3.15` kali milik kami.
...
## Mendukung Terjemahan `HOGO` Konfigurasi Tajuk

Mendukung konfigurasi header blog statis tipe [HOGO](https://github.com/gohugoio/hugo) , hanya kolom `title` , `summary` dan `description` yang diterjemahkan.

## Jangan Terjemahkan Nama Variabel

`Markdown` Digunakan sebagai template email, `YAML` sebagai konfigurasi file bahasa, akan ada parameter variabel (misalnya: isi ulang `${amount}` telah diterima).

`${varname}` Nama variabel semacam ini tidak akan dianggap sebagai terjemahan bahasa Inggris.
...
Tiongkok, Jepang, dan Korea tidak menggunakan huruf besar atau kecil, namun judul bahasa Inggris umumnya menggunakan huruf kapital pada huruf pertama.
...
Misalnya `为阅读体验而优化` akan diterjemahkan sebagai `Optimized for Reading Experience` .
...
Diterjemahkan ke dalam `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` :

<img alt="" src="//p.3ti.site/1720199391.avif">

Diterjemahkan ke dalam `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :
...
Diterjemahkan ke dalam `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` :

<img alt="" src="//p.3ti.site/1720199550.avif">

DeepL Terjemahan di atas ke dalam bahasa Prancis (juga menulis ulang nama diri dan menambahkan yang ganjil `... ` ):
...
Terjemahan `i18` akan mengenali istilah bahasa Inggris dalam dokumen berbahasa Mandarin, Jepang, dan Korea dan membiarkan istilah tersebut tetap utuh.
...
## Dapat Digabungkan `i18n.site` Pembuatan Situs Web Multi-Bahasa

`i18` Terintegrasi ke dalam alat baris perintah untuk pembuatan situs web multi-bahasa [`i18n.site`](/i18n.site) .

Lihat dokumentasi [`i18n.site`](/i18n.site) untuk detailnya.

## Kode Sumber Terbuka

Klien sepenuhnya open source, dan sisi server 90 % kodenya open [source](/i18n.site/src) .
...
/ selamat datang untuk mengikuti [i18n-site.bsky.social](https://bsky.app/profile/i18n-site.bsky.social) sosial kami [X.COM: @i18nSite](https://x.com/i18nSite)
