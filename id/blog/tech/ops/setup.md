...
Tampilan CPU Model `AMD EPYC 9224 24-Core Processor` skor kinerja inti tunggal 1285
...
Hal pertama adalah mengedit `~/.ssh/config` Tetapkan alias untuk IP lalu gunakan `ssh-copy-id root@c2` untuk mengonfigurasi login tanpa kata sandi.
...
Jaringan dari Eropa ke China tidak stabil. Disarankan `ssh` menggunakan [mosh](//mosh.org)

Buat skrip `c0` di direktori `~/.bin` (terutama ditambahkan ke variabel lingkungan `PATH` ) mesin lokal sebagai berikut:
...
Setelah pembuatan, berikan izin yang dapat dieksekusi dan tautan lunak `c1` , `c2` .
...
Di masa depan, Anda dapat langsung memasukkan `c0` , `c1` dan Anda dapat menggunakan koneksi langsung `mosh` berkecepatan tinggi dan memasukkan [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Perubahan sidik jari akan terjadi ketika ssh , gunakan `ssh-keygen -R ip`
...
`/etc/snapper/configs/mnt` Anda dapat melihat pengaturan cadangan khusus subvolume ini
...
Ingatlah bahwa operasi di atas dilakukan pada sistem penyelamatan, sehingga perubahan tersebut hanya akan berlaku setelah sistem di-boot ulang dan tidak lagi dalam mode penyelamatan.

Artinya, Anda tidak akan langsung melihat perubahan apa pun pada log yang ada menjadi tetap, namun semua log yang dihasilkan setelah ini akan tetap ada.
...
Saya menggunakan tindakan github untuk membuat skrip kompilasi dan penerapan otomatis sebelumnya, tetapi menemukan bahwa layanan penerapan tidak dapat ssh.
...
Setelah diselidiki berulang kali, itu karena github action Pengguna yang mengunggah tar adalah `1001` `uid`

tar Pembongkaran harus dilakukan tanpa parameter `--no-same-owner` maka akan didekompresi menjadi `uid` untuk `1001` pengguna
