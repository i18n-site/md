...
പ്രവർത്തിപ്പിച്ചതിന് ശേഷം, വിവർത്തനവും കാഷെ ഫയലുകളും സൃഷ്ടിക്കപ്പെടും, `git add . ` `md` ഡയറക്ടറിയിൽ ചേർക്കുന്നത് ഓർക്കുക.
...
തുടർന്ന്, `docker` ഡയറക്ടറി നൽകി `./up.sh` പ്രവർത്തിപ്പിക്കുക, തുടർന്ന് പ്രാദേശികമായി പ്രിവ്യൂ ചെയ്യാൻ [https://127.0.0.1](https://127.0.0.1) .
...
ഉദാഹരണത്തിന്, `.i18n/htm/ol.yml` `out/ol` ഡയറക്ടറി സൃഷ്ടിക്കും.

`dev.yml` , `ol.yml` എന്നിവയാണ് ഡിഫോൾട്ട് കോൺഫിഗറേഷനുകൾ.
...
`md/.i18n/htm/ol.yml` എഡിറ്റ് ചെയ്ത് നിങ്ങളുടെ സ്വന്തം `npm` പാക്കേജ് പേരിലേക്ക് `i18n.site` `v: //unpkg.com/i18n.site` .

എന്നതിലെ ഉപയോഗശൂന്യമായ പാക്കേജ് നാമം ഉപയോഗിക്കുക [npmjs.com](//npmjs.com)

പാക്കേജ് `npm` അടിസ്ഥാനമാക്കി പ്രസിദ്ധീകരിക്കുമ്പോൾ, `v:` മൂല്യത്തിൻ്റെ പ്രിഫിക്സിന് **`//unpkg.com/` ഉപയോഗിക്കുന്നത് ഉറപ്പാക്കുക** , പുതിയ റിലീസുകൾ സമയബന്ധിതമായി കാണുന്നതിന് ഈ പ്രിഫിക്സ് പാതയ്ക്ക് കീഴിൽ `i18n.site` `/.v` കാഷെ സമയം പ്രത്യേകം ഒപ്റ്റിമൈസ് ചെയ്തിട്ടുണ്ട്.
...
`ol.yml` -ൽ `v:` ൻ്റെ പാക്കേജിൻ്റെ പേര് നിങ്ങൾ പരിഷ്ക്കരിക്കുകയാണെങ്കിൽ, ദയവായി **ആദ്യം `.i18n/v/ol` ഇല്ലാതാക്കി അത് പ്രസിദ്ധീകരിക്കുന്നത് ഉറപ്പാക്കുക** .
...
നിങ്ങൾക്ക് ഉള്ളടക്കം സ്വയം ഹോസ്റ്റ് ചെയ്യണമെങ്കിൽ, ആദ്യം `md/.i18n/htm/ol.yml` എഡിറ്റ് ചെയ്ത് `v: //i18n-v.xxx.com` പോലെയുള്ള നിങ്ങളുടെ URL പ്രിഫിക്സിലേക്ക് `v: //unpkg.com/i18n.site` മാറ്റുക.
...
തുടർന്ന്, `md/out/ol/v` ഡയറക്ടറിയിലെ ഉള്ളടക്കം `v:` -ൽ സജ്ജീകരിച്ചിരിക്കുന്ന URL പ്രിഫിക്സ് പാതയിലേക്ക് കോൺഫിഗർ ചെയ്യുക.
...
സ്വന്തം സെർവർ ഉപയോഗിക്കുന്നതിന് `CDN` , ഉള്ളടക്കം സ്വയം ഹോസ്റ്റുചെയ്യുന്നതിന്, മറ്റൊരു പൊതു ഓപ്ഷൻ `S3` +

`S3` സെർവറിൽ `S3` ചെയ്യാൻ നിങ്ങൾക്ക് [rclone](https://rclone.org)
...
വെബ്സൈറ്റ് എവിടെയും വിന്യസിക്കാൻ കഴിയും [github page](https://pages.github.com) കൂടാതെ [cloudflare page](https://pages.cloudflare.com) നല്ല ചോയ്സുകളാണ്.
...
[ഒരു ഓർഗനൈസേഷൻ സൃഷ്ടിക്കാൻ ആദ്യം ഇവിടെ ക്ലിക്ക് github](https://github.com/account/organizations/new?plan=free) `i18n-demo` ഒരു ഉദാഹരണം.

തുടർന്ന് ഈ ഓർഗനൈസേഷൻ്റെ കീഴിൽ വെയർഹൗസ് `i18n-demo.github.io` സൃഷ്ടിക്കുക (നിങ്ങൾ സൃഷ്ടിച്ച സ്ഥാപനത്തിൻ്റെ പേര് ഉപയോഗിച്ച് `i18n-demo` മാറ്റിസ്ഥാപിക്കുക):

![](https://p.3ti.site/1721098657.avif)

മുമ്പത്തെ ലേഖനത്തിലെ ഉള്ളടക്കം പ്രസിദ്ധീകരിക്കുമ്പോൾ `out/ol/htm` ഈ ഡയറക്ടറി നൽകി പ്രവർത്തിപ്പിക്കുക :
...
വിന്യാസത്തിന് ശേഷം, നിങ്ങൾക്ക് ഒരു ഇഷ്ടാനുസൃത ഡൊമെയ്ൻ നാമം ബൈൻഡ് ചെയ്യാം.
...
മുകളിലെ ചിത്രത്തിലെ നിയമങ്ങൾ ചുവടെയുള്ള ആദ്യ വരിയിലെ `i18n.site` പകരം നിങ്ങൾ ബന്ധിപ്പിച്ച ഡൊമെയ്ൻ നാമം നൽകുക.
...
തുടർന്ന്, + ക്ലൗഡ് വെണ്ടർമാരുടെ ഒബ്ജക്റ്റ് സ്റ്റോറേജ് ഉപയോഗിക്കുക `CDN` ഇനിപ്പറയുന്ന ഉള്ളടക്കം വിന്യസിക്കുക `out/ol/htm` .
...
[ആലിബാബ CDN](https://cdn.console.aliyun.com/domain/list) ഡൊമെയ്ൻ നാമം ചേർക്കുകയും ആലിബാബ ക്ലൗഡ് `CDN` ൽ ഡൊമെയ്ൻ നാമം `*.xxx.com` മുതൽ `CNAME` വരെ സൂചിപ്പിക്കുകയും ചെയ്യുക.
...
nginx എന്നതിൻ്റെ `server` ഖണ്ഡികയിൽ ഇനിപ്പറയുന്നതിന് സമാനമായ ഒരു കോൺഫിഗറേഷൻ ദയവായി നിങ്ങളുടെ സ്വന്തം പ്രോജക്റ്റ് `out/ol/htm` -ൻ്റെ പാതയിലേക്ക് `/root/i18n/md/out/ol/htm`
...
`.i18n` ഡയറക്ടറിക്ക് കീഴിൽ `i18n.site` ൻ്റെ കോൺഫിഗറേഷൻ ഫയലുകൾ, വിവർത്തന കാഷെ മുതലായവയുണ്ട്. വിശദാംശങ്ങൾക്ക് അടുത്ത അധ്യായം ["കോൺഫിഗറേഷൻ"](/i18n.site/conf) കാണുക.
