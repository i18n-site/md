...
මිශ්ර HTML Markdown හි පරිවර්තන සඳහා සහය දක්වයි, `MarkDown` හි කාවැද්දූ `<pre>` සහ `<code>` HTML වල අන්තර්ගතය පරිවර්තනය නොවේ
...
පරිවර්තන අදහස් ` ``` ` ට පසුව ` ```rust ` : භාෂාව දැක්විය යුතුය.
...
පරිවර්තන සිදු කළ යුත්තේ විධාන ඇතුළත් කර එය එක් ක්ලික් කිරීමකින් සම්පූර්ණ කිරීමෙනි.
...
`title` , `summary` සහ `description` ක්ෂේත්ර පමණක් පරිවර්තනය කරන [HOGO](https://github.com/gohugoio/hugo) හි ශීර්ෂ වින්යාසයට සහය දක්වයි
...
`i18` ට `MarkDown` හි මාතෘකාව හඳුනාගත හැකි අතර, සිද්ධි සංවේදී භාෂාවකට පරිවර්තනය කිරීමේදී පළමු අකුර ස්වයංක්රීයව විශාලනය කරයි.
