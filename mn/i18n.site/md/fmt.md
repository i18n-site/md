...
`HTML` кодыг `pug` кодонд оруулж болно.

`<pre>` элемент дэх контентыг орчуулахгүй.

Эдгээр хоёр цэгийг нэгтгэснээр та янз бүрийн дэлгэцийн эффектийг авахын тулд 0-д `MarkDown` `HTML` хялбархан бичиж болно.
...
Дээрх `<pre>` `<style>` мөн тодорхойлсон болохыг анхаарна уу.