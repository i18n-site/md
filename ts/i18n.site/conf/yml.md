...
Xivumbeko xa `i18n.site` i `.i18n/conf.yml` .

Handle ka swiyimiso swa [`i18`](/i18) , `ignore:` na `i18n:` , fayili ya vuhlanganisi yi hi ndlela leyi landzelaka:
...
Exikarhi ka tona, `i18n: home` yi fambelana na `home: Home` eka `en/i18n.yml` .

`en/i18n.yml` yi ta hundzuluxeriwa eka tindzimi to tala, to tanihi `zh/i18n.yml` .
...
`use: Toc` swi vula ku hundzuluxela hi ku tirhisa xifaniso `Toc` , lexi nga ku hundzuluxela xifaniso xin’we xa `Markdown` .
...
### `use: Md` , Xifaniso Xa Fayili Yin’we (Ku Hava Xivumbeko) .

Xifaniso xa `Md` na xifaniso xa `Toc` swa fana naswona havumbirhi bya swona swi tirhisiwa ku humesa fayili yin’we `Markdown` . Kambe xifaniso xa `Md` a xi kombisi xivumbeko xa le tlhelo eka barhi ya le tlhelo.
...
Loko ndlela yo karhi yi nghenisiwile naswona xirhangi xa yona xa ndlela xi nga hleriwanga eka `nav:` , fayili `MarkDown` leyi fambelanaka na ndlela yi ta layicha hi ku tiyimisela naswona yi ta endliwa hi ku tirhisa xifaniso xa `Md` .

Xikombiso, loko `/test` yi nghenisiwile, naswona `nav:` yi lulamisiwile handle ka ndlela leyi, naswona ririmi ra pheji ri ri Xinghezi (khodi `en` ), `/en/test.md` yi ta layicha hi ku tiyimisela naswona yi ta endliwa hi ku tirhisa xifaniso xa `Md` .

Loko `/en/test.md` fayili leyi yi nga ri kona, tluka `404` ra ntolovelo ri ta kombisiwa.
...
`blog` nga laha henhla i movha wa nchumu wun’we wa `Doc` .
...
Ndlela yo tsala leyi nga laha henhla yi ringana na `url: blog` , naswona fayili ya yona leyi fambelanaka na yona i `en/blog/TOC` .

#### Tiphurojeke to Tala

Ku lulamisiwa ka `i18n:doc` eka `.i18n/conf.yml` i movha wa tiphurojeke to tala.
...
Laha, `menu: NB demo1,demo2` , swi vula ku tirhisa xifaniso xa `NB` ku humesa menyu leyi nga ehansi.
...
`i18n.site` yi ta tirhisa `js` plug-in `.i18n/hook/after.tran/TOC.js` eka vuhlayiselo bya demo ku hlaya fayili ya `doc` directory index leyi fambelanaka na `TOC` template configuration ku tumbuluxa `json` directory outline.
...
U nga vona leswaku `Deme Two` yi na tagline ya yona ya phurojeke ehansi ka menyu leyi rhetaka na vito ra phurojeke ya xivumbeko : khathalogi `Your Project slogan` .

![](https://p.3ti.site/1721276842.avif)

Leswi swi fambelana na layini yo sungula `en/demo2/README.md` :
...
Vatirhisi vo huma eChina, Japan na Korea, hi kombela mi xiya leswaku mi fanele ku tirhisa kholoni ya hafu ya ku `:` ematshan’wini ya kholoni ya ku anama loku heleleke.
