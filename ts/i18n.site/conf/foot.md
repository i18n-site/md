...
Kuna tifayela tinharhu ta `css` ehansi ka `md/.i18n/htm` eka phurojeke ya demo.

* `foot.css` : Xitayili xa le hansi
* `import.css` : `i18n.site` xa ntolovelo xa sayiti hinkwayo
...
Ku engetela `class="a"` eka link i ku sivela link ku hundzuriwa eka `MarkDown` Vona :
 [➔ `YAML` Ndlela yo sivela link `HTML` ku hundzuriwa yi `Markdown` :](/i18/qa#H2) .