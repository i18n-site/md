...
`i18n.site` yi na xitirhisiwa xa vuhundzuluxi `i18` lexi akiweke endzeni Hi kombela [u tikhoma laha ku languta eka tsalwa `i18` ku lulamisa xikombiso xa mfikelelo](/i18/use) .
...
Xo sungula, nghenisa eka `md` directory ivi u fambisa `i18n.site` , leyi nga ta hundzuluxela `en` ku ya eka `zh` .

<img src="https://p.3ti.site/1721114619.avif" style="width:350px">

Endzhaku ko tirha, tifayela ta vuhundzuluxi na ta cache ti ta endliwa Hi kombela u tsundzuka ku `git add . ` engetela eka vuhlayiselo eka `md` directory.
...
Laha, `dev` swi vula leswaku yi akiwile hi ku ya hi fayili ya `.i18n/htm/dev.yml` ya vuhlanganisi.
...
Loko vatirhisi va le tikweni-nkulu ra China va hlangana na swiphiqo swa netiweke naswona va nga swi koti ku kandziyisa tiphasela `npm` , va nga veka xihlawulekisi xa mbango `https_proxy` ku lulamisa sevha ya vuyimeri.
...
Kutani, lulamisa leswi nga eka xikombo xa `md/out/ol/v` eka ndlela ya xirhangi xa URL leyi vekiweke eka `v:` .
...
Ku tikhoma swilo, ku engetela eka ku tirhisa sevha ya wena, ndlela yin’wana `CDN` tolovelekeke i ku tirhisa `S3` +

U nga tirhisa [rclone](https://rclone.org) ku nghena eka sevha `S3` , kutani u languta no cinca tsalwa leri landzelaka, naswona u kopa ntsena ku cinca loku engetelekeke eka `S3` eka nkandziyiso wun’wana na wun’wana.
...
Xo sungula `i18n-demo` [github ku tumbuluxa nhlangano](https://github.com/account/organizations/new?plan=free) .
...
Hikuva rhekhodo `MX` na rhekhodo `CNAME` a swi nge hanyi swin’we, loko u lava ku amukela ti-imeyili ta mavito ya domain hi nkarhi `CNAME` , u fanele ku tirhisana na tsalwa `A` [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten)
...
Engetelani vito ra domain eka [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) naswona u kombetela vito ra domain `*.xxx.com` ku ya eka `CNAME` eka Alibaba Cloud `CDN` .
...
Hi kombela u engetela xivumbeko `/root/i18n/md/out/ol/htm` fanaka na lexi `out/ol/htm` eka ndzimana ya `server` ya nginx
