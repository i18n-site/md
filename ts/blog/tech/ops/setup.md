...
Ku kombisa Model `AMD EPYC 9224 24-Core Processor` , xikoro xa matirhelo ya xisekelo xin’we 1285 CPU
...
Nchumu wo sungula IP ku hlela `~/.ssh/config` ya muchini wa laha kaya ku veka alias, ivi u tirhisa `ssh-copy-id root@c2` ku lulamisa ku nghena loku nga riki na phaswedi.
...
Goza leri ri teka nkarhi wo leha Endzhaku ko rindza hafu ya awara, ku ndlandlamuxiwa ni ku nghenisiwa ka software leyi tirhisiwaka ngopfu swi ta humelela. U nga tiyisisa leswi hi xileriso lexi landzelaka:
...
Netiweke ku suka eYuropa ku ya eChayina a yi tshamisekanga Ku bumabumeriwa ku [mosh](//mosh.org) tirhisa ku hatlisisa `ssh` .

Endla `c0` wa swikripti eka xikombo xa laha kaya `~/.bin` (ngopfu-ngopfu swi engeteriwile eka xihlawulekisi xa mbango `PATH` ) hi ndlela leyi landzelaka:
...
Endzhaku ko tumbuluxa, nyika mpfumelelo lowu tirhisiwaka na soft-link `c1` na `c2` .
...
Enkarhini lowu taka, u nga nghenisa hi ku kongoma `c0` na `c1` ku tirhisa `mosh` wa vuhlanganisi byo kongoma bya rivilo lerikulu ivi u nghenisa [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
ssh Ku ta va na ku cinca ka swikombiso swa tintiho, tirhisa `ssh-keygen -R ip` ku yi basisa
...
Yi ta tshikilela hi ku tisungulela hi ku tiyimisela Xileriso xo kambela mpimo wa ku tshikileriwa hi ndlela leyi landzelaka:
...
Hilaha swi kombisiweke hakona eka xifaniso lexi nga laha hansi, u nga vona leswaku endzhaku ka ku tshikileriwa hi voko eka sisiteme leyintshwa, kwalomu ka 100MB yin’wana yi tshikileriwile.
...
Xikripti xo sungula xi ta tumbuluxa xikombo xa `~/ubuntu` , ntsena tirhisa xileriso lexi landzelaka:
...
`/etc/snapper/configs/mnt` yi nga languta swiletelo swo karhi swa vuhlayiselo bya vholumo leyitsongo leyi
...
Hi ntolovelo swi olova ngopfu ku tirhisa swifaniso swa btrfs.
...
Tsundzuka leswaku mintirho leyi nga laha henhla yi endliwa eka sisiteme yo ponisa, hikwalaho ku cinca koloko a ku nge tirhi ku kondza sisiteme yi pfuriwa nakambe naswona yi nga ha ri eka xiyimo xo ponisa.

Leswi swi vula leswaku a wu nge hatli u vona ku cinca kwihi na kwihi eka tilog leti nga kona leti hundzukaka leti phikelelaka, kambe tilog hinkwato leti endliweke endzhaku ka sweswo ti ta phikelela.
...
Ndzi tirhise github action ku endla script ya automatic compilation na deployment before, kambe ndzi kume leswaku vukorhokeri bya deployment a byi nga ta va ssh.
...
Endzhaku ka tar byo phindha-phindha, swi `uid` `1001` github action

tar Ku pfula swi fanele ku va loko ku nga ri na parameter `--no-same-owner` , swi ta decompressed ku ya eka `uid` eka `1001` wa vatirhisi
