...
` ```rust ` moet die taal na ` ``` ` aandui, soos :
...
Ondersteun die kopkonfigurasie van [HOGO](https://github.com/gohugoio/hugo) statiese blog, wat slegs velde `title` , `summary` en `description` vertaal.
...
Vertaal in : `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす`

![](//p.3ti.site/1720199391.avif)

Vertaal in `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :
...
Vertaal in : `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す`

![](//p.3ti.site/1720199550.avif)

Vertaling van bogenoemde in Frans (herskryf ook die eiename en voeg die vreemde `... ` DeepL ):
