...
Vertoon CPU Model `AMD EPYC 9224 24-Core Processor` enkelkern prestasie telling 1285
...
Die eerste ding is om die plaaslike masjien se `~/.ssh/config` Stel 'n alias vir IP en gebruik dan `ssh-copy-id root@c2` om wagwoordvrye aanmelding op te stel.
...
CN=1 beteken om taal oor te skakel na Chinees en tydsone na Beijing.
...
Hierdie stap neem 'n lang tyd Nadat u 'n halfuur gewag het, sal die opgradering en installering van algemeen gebruikte sagteware suksesvol wees. U kan dit bevestig met die volgende opdrag:
...
Die netwerk van Europa na China is nie stabiel `ssh` . Dit word aanbeveel om te gebruik [mosh](//mosh.org)

Skep 'n `c0` script in die plaaslike masjien `~/.bin` (hoofsaaklik by die omgewingsveranderlike `PATH` ) gids soos volg:
...
Na die skepping, gee uitvoerbare toestemmings en sagte skakel `c1` `c2` .
...
In die toekoms kan jy direk `c0` `c1` invoer en jy kan `mosh` hoëspoed direkte verbinding gebruik en [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)

## Skakel Partisie Formaat Van ext4 Na btrfs

[btrfs](https://wikipedia.org/wiki/Btrfs) Kan skyffoto's neem vir maklike rugsteun.

Gaan eers die reddingstelsel in
...
Vingerafdrukveranderinge `ssh-keygen -R ip` plaasvind wanneer ssh
...
Soos in die figuur hieronder getoon, kan jy sien dat na handmatige kompressie in die nuwe stelsel, byna 100MB meer saamgepers is.
...
Gebruik `snapper list-configs` om die subvolumes waarvoor foto's gestel is, te sien.

<img alt="" src="https://i-01.eu.org/1718302689.avif">

Gebruik `snapper -c mnt list` om 'n momentopname van 'n subvolume te sien

`/etc/snapper/configs/mnt` Jy kan die spesifieke rugsteuninstellings van hierdie subvolume sien
...
### Hoe Om Toegang Tot Die Hardeskyf Te Kry Nadat Jy Redding Betree &
...
Herlaai in jou Ubuntu-stelsel: Nadat jy bogenoemde veranderinge aangebring het, moet jy jou masjien normaalweg en in jou Ubuntu-stelsel herlaai in plaas van die reddingstelsel.

Op hierdie manier sal systemd-journald met die nuwe konfigurasie begin en logs aanhoudend op die hardeskyf begin stoor.

Hou in gedagte dat die bogenoemde bewerkings op 'n reddingstelsel uitgevoer word, so daardie veranderinge sal eers in werking tree sodra die stelsel herlaai is en nie meer in reddingsmodus is nie.

Dit beteken dat jy nie dadelik sal sien dat enige veranderinge aan bestaande logs aanhoudend word nie, maar alle logs wat daarna gegenereer word, sal volgehou word.
...
Ek het voorheen github-aksie gebruik om 'n outomatiese samestelling en ontplooiingskrip te maak, maar het gevind dat die ontplooiingsdiens nie kon ssh nie.
...
Na herhaalde ondersoeke is dit omdat die gebruiker wat tar opgelaai het `uid` `1001` is github action

tar Uitpak moet sonder parameters gedoen word `--no-same-owner` dit sal gedekomprimeer word in `uid` vir `1001` gebruiker
