...
As jy 'n deurlopende integrasie-omgewing gebruik om te publiseer, is dit nie nodig om `nodejs` te installeer nie, kopieer net die aangemelde en publiseertoestemming `~/.npmrc` na die omgewing.
...
Omdat die webwerf 'n [enkelbladsy-toepassingsargitektuur](https://developer.mozilla.org/docs/Glossary/SPA) gebruik, onthou om URL-paaie te herskryf wat nie `. ` tot `index.html` bevat nie.
...
Gebruik dan die objekberging van wolkverkopers op die vasteland van China + Ontplooi die volgende inhoud `CDN` `out/ol/htm`
