...
### `use: Md` , Enkele Lêer Sjabloon Sonder Buitelyn
...
### `use: Doc` , Multi-Lêer Dokument Sjabloon
...
Die bogenoemde skryfmetode is gelykstaande aan `url: xxx` , en die ooreenstemmende lêer is `en/xxx/TOC` .

#### TOC Inhoudsopgawe Indeks

`i18n.site` sal `js` inprop `.i18n/hook/after.tran/TOC.js` in die demo-pakhuis uitvoer om die `TOC` gidsindekslêer te lees wat ooreenstem met die `doc` sjabloonkonfigurasie om die `json` gidsomtrek te genereer.
...
Jy kan sien dat `Deme Two` sy projekbyskrif onder die aftrekkieslys `Your Project slogan` :
