...
Verwys asseblief na die volgende, die styl is in die ooreenstemmende `.css` geskryf, en die interaksie word verkry deur webkomponente te skep.
...
* `import.css` : `i18n.site` vir die hele webwerf
* `conf.css` : Voettekst ikone en lettertipes

### Ikoon Lettertipe

Die voetskrif-ikoon word gegenereer deur `F` iconfont.cn skep ( [Engelse weergawe](https://www.iconfont.cn/?lang=en-us) /[中文版](https://www.iconfont.cn/?lang=zh)).
...
Om `class="a"` by die skakel te voeg, is om te verhoed dat die skakel na `MarkDown` omgeskakel word. Sien :
 [➔ `YAML` Hoe om te verhoed : skakel `HTML` na `Markdown` omgeskakel word](/i18/qa#H2) .