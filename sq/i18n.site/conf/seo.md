...
Në konfigurim, ju lutemi ndryshoni `i18n.site` në vlerën `host:` në `.i18n/htm/ol.yml` , dyqane të shumta objektesh mund të konfigurohen nën `s3` dhe fusha `region` është opsionale (shumë dyqane objektesh nuk kanë nevojë të vendosin këtë fushë).
...
Kodi i rregullit është si më poshtë, ju lutemi modifikoni kodin "i18n.site" në emrin tuaj të domenit:
...
Të dhënat ngarkohen në Baidu Object Storage dhe lidhen me Baidu Content Distribution Network.

Pastaj krijoni skriptin në [EdgeJS e skajit](//console.bce.baidu.com/cdn/#/cdn/ejs/list) si më poshtë
...
Për shkak se `R2` `cloudflare` të aksesohet nga një emër domaini i personalizuar, duhet të përdoret një ruajtje e objekteve të palëve të treta për të vendosur skedarë statikë.

Këtu [backblaze.com](https://www.backblaze.com) si shembull për të demonstruar se si të lidhim objektet e palëve të treta që do të ruhen në `cloudflare` .
