...
## Navigacion Në Krye Të Navigimit
...
### `use: Doc` , Shabllon Dokumenti Me Shumë Skedarë
...
Metoda e mësipërme e shkrimit është ekuivalente me `url: xxx` , dhe skedari i saj përkatës është `en/xxx/TOC` .

#### Indeksi I Tabelës Së Përmbajtjes TOC

`i18n.site` do të ekzekutojë `js` plug-in `.i18n/hook/after.tran/TOC.js` në magazinë demo për të lexuar skedarin e indeksit të `TOC` direktorive që korrespondon me konfigurimin e shabllonit `doc` për të gjeneruar skicën e drejtorive `json` .
...
Për shembull, nëse aksesohet `/test` dhe `nav:` është konfiguruar pa prefiksin e kësaj rruge dhe gjuha aktuale e shfletimit është anglishtja (kodi `en` ), `/en/test.md` do të ngarkohet si parazgjedhje dhe do të jepet duke përdorur shabllonin `Md` .
