...
Për të vetë-strehuar përmbajtjen, përveç përdorimit të serverit tuaj, një opsion tjetër i zakonshëm është përdorimi `CDN` `S3` +
...
Kur publikoni përmbajtjen në artikullin e mëparshëm, `out/ol/htm` është krijuar :
...
Më pas, përdorni ruajtjen e objekteve të shitësve të cloud në + kontinentale `CDN` Vendosni përmbajtjen e mëposhtme `out/ol/htm` .
...
Shtoni emrin e domenit në [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) dhe vendosni emrin e domenit `*.xxx.com` në `CNAME` në Alibaba Cloud `CDN` .
