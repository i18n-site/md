...
Mbështet përkthimin e të HTML Markdown , përmbajtja në etiketat `<pre>` dhe `<code>` të HTML të ngulitura në `MarkDown` nuk përkthehet
...
Mbështet konfigurimin e kokës [HOGO](https://github.com/gohugoio/hugo) blogut të tipit statik, i cili përkthen vetëm fushat `title` , `summary` dhe `description` .
