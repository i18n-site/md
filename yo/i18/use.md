...
windows , jọwọ [tẹ ibi lati ṣe igbasilẹ ati fi sori ẹrọ `git bash`](https://git-scm.com/download/win) ni akọkọ.
...
O le tunto oriṣiriṣi `fromTo` fun oriṣiriṣi awọn iwe-iṣakoso : faili /
...
Ninu tabili iṣeto ni yii, ede orisun ti itumọ katalogi `blog` jẹ `zh` , ati ede orisun ti itumọ `blog/your_file_name.md` jẹ `ja` .
...
`ignore` nlo sintasi [globset](https://docs.rs/globset/latest/globset/#syntax) bi faili `.gitignore` .
