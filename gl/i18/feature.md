...
Admite a tradución de HTML Markdown , o contido das etiquetas `<pre>` e `<code>` de HTML incrustadas en `MarkDown` non se traduce
...
Os comentarios de tradución deben indicar o idioma despois ` ```rust ` ` ``` ` , como : .
...
Admite a configuración de cabeceira de [HOGO](https://github.com/gohugoio/hugo) blog estático, que só traduce os campos `title` , `summary` e `description` .
...
Tradución do anterior ao DeepL (reescribindo tamén os nomes propios e engadindo o impar `... ` ):
