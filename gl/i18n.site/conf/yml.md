...
### `use: Doc` , Modelo De Documento De Varios Ficheiros
...
O método de escritura anterior equivale a `url: xxx` e o ficheiro correspondente é `en/xxx/TOC` .

#### Índice De Contidos TOC

`i18n.site` executará `js` complemento `.i18n/hook/after.tran/TOC.js` no almacén de demostración para ler o ficheiro de índice de `TOC` directorios correspondente á configuración de `doc` modelos para xerar o esquema de directorio `json` .
...
Por exemplo, se se accede `/test` e se configura `nav:` sen o prefixo desta ruta e o idioma de navegación actual é o inglés (código `en` ), `/en/test.md` cargarase de forma predeterminada e representarase mediante o modelo `Md` .
