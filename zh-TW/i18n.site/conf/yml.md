...
除去[`i18`](/i18)的`ignore:`和`i18n:`的設置外，配置文件如下：
...
其中，`i18n: home`對應着`en/i18n.yml`中`home: Home`。

`en/i18n.yml`會被翻譯為多語言，比如`zh/i18n.yml`。
...
如果訪問的某個路徑，其路徑前綴沒有在`nav:`中配置，則默認加載路徑對應的`MarkDown`文件，并用`Md`模板渲染。

比如，訪問`/test`， 同時`nav:`配置未此路徑，頁面語言是英文（代碼`en`），則默認會加載`/en/test.md`，并使用`Md`模板渲染。
...
##### url 為空時，默認等于 i18n 的值

若不寫`url`，`url`默認為`i18n`的值，此規則對于其他模板同樣生效。

上面寫法等價于有`url: blog`，其對應的文件是`en/blog/TOC`。

#### 多項目

`.i18n/conf.yml`中`i18n:doc`的配置是多項目模式。
...
`i18n.site`會執行演示倉庫中`js`插件`.i18n/hook/after.tran/TOC.js`來讀取`doc`模板配置對應`TOC`目錄索引文件，來生成目錄大綱的`json`。

如果用`doc`模板，必须要有此插件。

如果你是從空文件夾初始化`i18n.site`項目，記得复制演示項目中的`.i18n`到您的目錄下。

`Doc`模板會基于生成的`json`來渲染目錄大綱。
