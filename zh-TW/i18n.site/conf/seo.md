...
請首先修改上面`host:`的值為您的域名，比如`i18n.site`。
...
配置中,`i18n.site`請修改為`.i18n/htm/ol.yml`中`host:`的值,`s3`下面可以配置多個對象存儲,`region`字段是可選的(很多對象存儲不需要設置此字段)。
