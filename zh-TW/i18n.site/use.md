...
比如，`.i18n/htm/ol.yml`會創建`out/ol`目錄。

`dev.yml`和`ol.yml`是默認自帶的配置。
...
如果修改了`ol.yml`中`v:`的包名，請**務必先刪除`.i18n/v/ol`**，然后再发布。
...
若想自托管內容，首先，編輯`md/.i18n/htm/ol.yml`，修改`v: //unpkg.com/i18n.site`為您的網址前綴，比如`v: //i18n-v.xxx.com`。
...
然后，配置`md/out/ol/v`目錄下的內容到`v:`中設置的網址前綴路徑。
...
在前文发布內容時，已經生成`out/ol/htm`，這里請進入此目錄，然后運行:
...
然后，利用中國大陸云廠商的對象存儲+`CDN`部署`out/ol/htm`下面的內容即可。
...
請在nginx的`server`段落中添加類似如下的配置，其中`/root/i18n/md/out/ol/htm`請改為你自己項目`out/ol/htm`的路徑：
