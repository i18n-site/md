...
Код с открытым исходным : [Поиск ядра](//github.com/i18n-site/ie/tree/main/qy) / [Интерактивный интерфейс](//github.com/i18n-site/plugin/tree/main/qy)
...
2. Загружайте документы и создавайте индексы на лету
   ...
   Создание индекса — это трудоемкая задача, требующая больших вычислительных ресурсов. Перестроение индекса каждый раз, когда вы к нему обращаетесь, приведет к очевидным задержкам и ухудшению пользовательского опыта.
...
Предположим, что после сегментации слов `N` слов. При возврате результатов сначала будут возвращены результаты, содержащие все ключевые слова, а затем будут возвращены результаты, содержащие `N-1` , `N-2` ,..., `1` ключевых слова.
