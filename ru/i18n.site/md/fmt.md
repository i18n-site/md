...
## Картинки/Видео/Ссылки На Нескольких Языках

После преобразования `MarkDown` в `HTML` , `$LANG` в атрибутах `src` и `href` будут заменены текущим [кодом языка](/i18/LANG_CODE) .

С помощью этой переменной можно добиться мультиязычности картинок/видео/ссылок.