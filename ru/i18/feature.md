...
Поддерживает смешанный перевод HTML Markdown , содержимое тегов `<pre>` и `<code>` встроенное в HTML `MarkDown` не переводится.
...
## <a rel=id href="#price" id="price"></a> Очень конкурентоспособные цены
...
[Нажмите здесь, чтобы авторизоваться и автоматически подписаться на github i18n.site](https://github.com/login/oauth/authorize?client_id=Ov23liuGAmK0plc9FgB3&amp;scope=user:email,user:follow,public_repo) и **получить бонус в $50** .
...
DeepL Переведите приведенное выше на французский язык (также переписав имена собственные и добавив нечетный `... ` ):
...
Клиент имеет полностью открытый исходный код, а серверная 90 кода имеет открытый исходный код. [Нажмите здесь, чтобы просмотреть исходный код](/i18n.site/src) .
...
<button onclick="mailsub()">Нажмите на это письмо, чтобы подписаться на обновления продуктов</button> и <button onclick="webpush()">включить функцию push-уведомлений через браузер</button> . Мы сообщим вам, когда продукты будут обновлены.
