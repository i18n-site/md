...
Поддерживает перевод Markdown в смешанных тегах HTML `<pre>` и содержимое в `<code>` `MarkDown` переводятся HTML
...
В комментариях к переводу после ` ``` ` необходимо указывать язык, ` ```rust ` :

В настоящее время поддерживается перевод комментариев `toml` , `yaml` , `json5` , `go` , `rust` , `c` , `cpp` , `java` `js` , `coffee` `python` , `bash` , `php` [и других языках](https://github.com/i18n-site/rust/blob/main/getc/src/style.rs#L14) .
...
Для программистов, знакомых с `git` , `vim` , `vscode` , использование этих инструментов для редактирования документов и управления версиями, несомненно, увеличит стоимость обучения.

`KISS` ( `Keep It Simple, Stupid` ) Среди сторонников этого принципа решения корпоративного уровня являются синонимом громоздкости, неэффективности и сложности в использовании.
...
## Высококачественный Перевод Xindaya
...
Чтобы добиться такого же качества, объем ручного редактирования, необходимый Google Translate и `ChatGPT` в `2.67` раз и `3.15` раз превышает наш соответственно.
...
## Поддержите Перевод `YAML`

Инструмент переводит только значения словаря в `YAML` , а не ключи словаря.

На основе `YAML` Translation вы можете легко создавать международные решения для различных языков программирования.

## Поддержка Перевода `HOGO`

Поддерживает конфигурацию заголовка статического блога типа [HOGO](https://github.com/gohugoio/hugo) , переводятся только поля `title` , `summary` и `description` .

## Не Переводить Имена Переменных

`Markdown` Используется в качестве шаблона электронной почты, `YAML` в качестве конфигурации языкового файла, будут переменные параметры (например: получено пополнение `${amount}` ).

`${varname}` Такое имя переменной не будет рассматриваться как перевод на английский язык.
...
В Китае, Японии и Корее нет прописных или строчных букв, но в английских названиях первая буква обычно пишется с заглавной буквы.

`i18` может распознавать заголовки в `MarkDown` и автоматически делать первую букву заглавной при переводе на язык с учетом регистра.
...
Переведено на `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` :

<img alt="" src="//p.3ti.site/1720199391.avif">

Переведено на `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :
...
Переведено на `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` :

<img alt="" src="//p.3ti.site/1720199550.avif">

DeepL Перевод вышеизложенного на французский язык (также переписывая имена собственные и добавляя лишние `... ` ):
...
## Можно Комбинировать Создание Многоязычного Сайта `i18n.site`
...
Клиент имеет полностью открытый исходный код, а серверная часть кода имеет открытый исходный код 90 [Нажмите здесь, чтобы просмотреть исходный код](/i18n.site/src) .
