...
[i18n.site/payBill](//i18n.site/payBill) , вам необходимо привязать кредитную карту для оплаты (пополнение счета не требуется, веб-сайт автоматически списывает комиссию в зависимости от использования, [цены см. на главной странице](/#price) ).
...
Для разных подкаталогов можно настроить разные `fromTo` / Демонстрация записывается следующим образом: :
...
В этой таблице конфигурации исходный язык для перевода каталога `blog` — `zh` , а исходный язык для перевода каталога `blog/your_file_name.md` — `ja` .
