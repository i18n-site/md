...
Galite sukonfigūruoti skirtingus `fromTo` skirtingiems : /
...
Šioje konfigūracijos lentelėje `blog` katalogo vertimo šaltinio kalba yra `zh` , o `blog/your_file_name.md` vertimo šaltinio kalba yra `ja` .
...
`ignore` naudoja tą pačią [globset](https://docs.rs/globset/latest/globset/#syntax) kaip ir `.gitignore` failas.
