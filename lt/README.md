<h1 style="display:flex;justify-content:space-between">i18n.site Tarptautiniai sprendimai<img src="//p.3ti.site/logo.svg" style="user-select:none;margin-top:-1px;width:42px"></h1>
...
## <a rel=id href="#project" id="project"></a> Projekto aprašymas
...
Išverskite `Markdown` ir `YAML` į komandinės eilutės įrankį ( [šaltinio kodą](https://github.com/i18n-site/rust/tree/main/i18) ) keliomis kalbomis.

Gali puikiai išlaikyti formatą `Markdown` Gali nustatyti failų modifikacijas ir išversti tik pakeistus failus.
...
Mes dalijamės " Viskas yra failas " `UNIX` ir galime tvarkyti vertimus į šimtus kalbų nepristatydami sudėtingų ir sudėtingų įmonės lygio sprendimų.

Vertimui `Markdown` galite naudoti žinomiausius įrankius (bet negalite pridėti ar ištrinti pastraipų) ir naudoti žinomiausią versijos valdymo būdą.

Kodų bazė gali būti sukurta kaip atviras kalbos failų šaltinis, o `Pull Request` 's proceso pagalba pasauliniai vartotojai gali dalyvauti nuolatiniame vertimų optimizavime. Sklandus ryšys [github](//github.com) kitos atvirojo kodo bendruomenės.
...
Grynai statinis, optimizuotas skaitymo patirčiai ir integruotas su [i18](#i18) tai geriausias pasirinkimas kuriant projekto dokumentų svetainę.
...
Ši svetainė sukurta remiantis šia sistema ir apima vartotojo, mokėjimo ir kitas funkcijas ( [šaltinio kodas](/i18n.site/src) ).

[→ Norėdami gauti vartotojo vadovą, skaitykite projekto dokumentaciją](/i18n.site) .
