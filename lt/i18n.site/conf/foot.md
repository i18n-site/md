...
* `foot.css` : stilius
* `import.css` : visos svetainės `i18n.site`
* `conf.css` : piktogramos ir šriftai

### Piktogramos Šriftas

Poraštės piktograma sugeneruojama sukūrus šriftą `F` iconfont.cn [angliška versija](https://www.iconfont.cn/?lang=en-us) /[中文版](https://www.iconfont.cn/?lang=zh)).
...
Prie nuorodos pridėjus `class="a"` neleisite nuorodos konvertuoti į `MarkDown` Žiūrėkite :
 [➔ `YAML` Kaip neleisti : `HTML` konvertuoti į `Markdown`](/i18/qa#H2) .