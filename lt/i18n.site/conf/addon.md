...
Integruotas `js` vykdymo laikas yra pagrįstas : [boa](https://github.com/boa-dev/boa)

* `wPath(path, txt)` : į failą
* `rTxt(path)` : tekstinį failą
* `rBin(path)` : dvejetainį failą
* `rDir(dirpath)` : katalogą, grąžinama reikšmė yra masyvas : list, failų sąrašas
