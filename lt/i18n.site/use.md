...
Paleidus bus sugeneruoti vertimo ir talpyklos failai. Nepamirškite `git add . ` pridėti prie saugyklos `md` kataloge.
...
`ol` yra `online` santrumpa, nurodanti internetinę aplinką, kuri naudojama oficialiam leidimui. Tai taip pat yra numatytasis konfigūracijos failas, kai išleidžiant naudojami komandų eilutės parametrai nuo `-n` iki `npm` .

Taip pat galite sukurti kitus konfigūracijos failus. Komandinėje `--htm_conf` nurodykite konfigūracijos failo pavadinimą:
...
Jei norite savarankiškai priglobti turinį, ne tik naudojate savo serverį, bet `CDN` dar viena įprasta parinktis yra naudoti `S3` +

Norėdami prisijungti prie `S3` serverio, `S3` naudoti [rclone](https://rclone.org)
...
Palyginti [cloudflare page](//pages.cloudflare.com) `github page` , jis suteikia galimybę perrašyti kelią ir yra palankesnis žemyninei Kinijai ir yra labiau prieinamas.
...
Pridėkite domeno pavadinimą [„Alibaba Cloud CDN](https://cdn.console.aliyun.com/domain/list) ir nurodykite domeno pavadinimą nuo `*.xxx.com` iki `CNAME` „Alibaba Cloud `CDN` .
...
Pridėkite konfigūraciją, panašią į toliau pateiktą ! `server` pastraipoje nginx Pakeiskite `/root/i18n/md/out/ol/htm` į savo projekto kelią `out/ol/htm` :
