...
Subtenas tradukon de HTML Markdown , la enhavo en la `<pre>` kaj `<code>` etikedoj de HTML enigita en `MarkDown` ne estas tradukita
...
Subtenas la kapan agordon de [HOGO](https://github.com/gohugoio/hugo) statika blogo, kiu tradukas nur kampojn `title` , `summary` kaj `description` .
...
`ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` en la japanan :
...
`ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` en la japanan :

![](//p.3ti.site/1720199550.avif)

Traduko de ĉi-supra en la francan (ankaŭ reverkante la proprajn nomojn DeepL aldonante la neparan `... ` ):
