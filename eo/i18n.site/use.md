...
Post funkciado, tradukado kaj kaŝmemoro dosieroj estos generitaj. Bonvolu memori aldoni `git add . ` al la deponejo en la dosierujo `md` .
...
Por memgastigi enhavon, krom uzi vian propran servilon `CDN` alia komuna opcio estas uzi `S3` + .
...
Unue [alklaku ĉi github por krei organizon](https://github.com/account/organizations/new?plan=free) La sekva nomo de organizo estas `i18n-demo` .
...
Tiam, uzu la objektan stokadon de nubaj vendistoj en kontinenta Ĉinio + Deploji la sekvan enhavon `CDN` `out/ol/htm`
...
Ĉar rekordo `MX` kaj rekordo `CNAME` ne povas kunekzisti, se vi volas ricevi retpoŝtojn de domajna nomo samtempe, vi devas kunlabori kun [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten) skripto al nivelo `CNAME` en rekordon `A` .
...
Aldonu la domajnan nomon en [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) kaj indiku la domajnan nomon `*.xxx.com` al `CNAME` en Alibaba Cloud `CDN` .
