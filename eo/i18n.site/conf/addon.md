...
* `i18n.addon/toc` : Indekso de dosierujo
  Generu dosierujon de `json` bazita sur `TOC` , ebligita defaŭlte
...
Ĝi estos vokita post kiam la traduko estas kompletigita, kaj la parametroj transdonitaj estas kiel sekvas.
...
* `wPath(path, txt)` : Skribu al dosiero
* `rTxt(path)` : Legu tekstodosieron
* `rBin(path)` : Legu binaran dosieron
* `rDir(dirpath)` Legu la : , la revena valoro estas tabelo : , dosierlisto
