...
Krom la senpaga areokotizo en Eŭropo, aliaj regionoj havas malsamajn nivelojn de prezaltiĝo.
...
Montru CPU Modelo `AMD EPYC 9224 24-Core Processor` unukerna rendimento-poentaro 1285
...
La unua afero estas redakti la lokan `~/.ssh/config` Agordu kaŝnomon por IP kaj poste uzu `ssh-copy-id root@c2` por agordi senpasvortan ensaluton.
...
La programaro instalita defaŭlte estas agordita laŭ miaj personaj uzkutimoj, kiel ekzemple [mise](https://github.com/jdx/mise) , [direnv](https://direnv.net/) , ktp. Se necese, vi povas kopii la [i18n-ops/ubuntu](https://github.com/i18n-ops/ubuntu) stokejon kaj modifi ĝin laŭbezone.

Ĉi tiu paŝo prenas longan tempon Post atendado de duonhoro, la ĝisdatigo kaj instalado de kutime uzata programaro estos sukcesa. Vi povas konfirmi ĝin per la sekva komando:
...
La reto de Eŭropo al Ĉinio ne estas stabila `ssh` Oni rekomendas uzi [mosh](//mosh.org)

Kreu `c0` -skripton en la `~/.bin` de la loka maŝino (ĉefe aldonita al la mediovariablo `PATH` ) jene:
...
Post kreado, donu ruleblajn permesojn kaj soft- `c1` , `c2` .
...
Estonte, vi povas rekte eniri `c0` `c1` kaj vi povas uzi `mosh` por konekti rekte kaj eniri [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Ŝanĝoj `ssh-keygen -R ip` fingrospuroj okazos kiam ssh
...
La komenca skripto kreos la `~/ubuntu` , nur rulu la sekvan komandon:
...
Uzu `snapper -c mnt list` por vidi foton de subvolumo

`/etc/snapper/configs/mnt` Vi povas vidi la specifajn rezervajn agordojn de ĉi tiu subvolumo
...
Enigu la 22 Post kiam la haveno estas [tcping](https://github.com/paradiseduo/tcping) alirebla, vi ne povos aliri ĝin per la fiksita pasvorto dum 2-3 minutoj.
...
Memoru, ke ĉi-supraj operacioj estas faritaj sur savsistemo, do tiuj ŝanĝoj efektiviĝos nur post kiam la sistemo estos rekomencita kaj ne plu en savreĝimo.

Ĉi tio signifas, ke vi ne tuj vidos ajnajn ŝanĝojn al ekzistantaj protokoloj iĝi konstantaj, sed ĉiuj protokoloj generitaj post tio estos konservitaj.
...
Mi uzis github-ago por fari aŭtomatan kompilon kaj deplojan skripton antaŭe, sed trovis, ke la deploja servo ne povis ssh.
...
Post ripetaj esploroj, estas github action La uzanto kiu alŝutis tar estas `1001` `uid`

Malpakaĵo devas esti farita sen parametroj tar `--no-same-owner` ĝi estos malkunpremita en `uid` por `1001` uzanto
