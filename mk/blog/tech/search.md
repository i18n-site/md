...
Оваа статија ќе ја сподели имплементацијата на `i18n.site` чиста технологија за пребарување со целосен текст Посетете го [i18n.site](//i18n.site)

[Јадрото за пребарување на](//github.com/i18n-site/ie/tree/main/qy) / со отворен код [Интерактивен интерфејс](//github.com/i18n-site/plugin/tree/main/qy)

## Преглед На Решенија За Пребарување На Целосен Текст Без Сервер

За мали веб-локации, како што се документи/лични блогови кои се чисто статични, несомнено е претешко да се создаде сам позадинско пребарување за целосен текст, а пребарувањето во цел текст без услуги несомнено е подобра тежина.

Постоечките решенија за пребарување на целосен текст без сервер спаѓаат во две широки категории.

Еден е давател на услуги за пребарување од трета страна, сличен на [algolia.com](//algolia.com) кој обезбедува компоненти за пребарување на целосен текст од предниот дел.

Таквите услуги бараат плаќање и не се достапни за корисниците во Кина поради проблеми со усогласеноста на веб-страниците.
...
Попознатите чисто предни пребарувања со целосен текст [lunrjs](https://lunrjs.com) и [ ElasticLunr.js ] [https://github.com/weixsong/elasticlunr.js](%E5%9F%BA%E4%BA%8E%60lunrjs%60%E4%BA%8C%E6%AC%A1%E5%BC%80%E5%8F%91) .

`lunrjs` Постојат два начини за градење индекси, но и двата имаат свои проблеми.
...
[fusejs](https://www.fusejs.io) пресметајте ја сличноста помеѓу низите за пребарување.

Перформансите на ова решение се екстремно слаби и не можат да се користат за пребарување на целосен текст (видете [Fuse.js Долгото барање трае повеќе од 10 секунди, како да се оптимизира?](https://stackoverflow.com/questions/70984437/fuse-js-takes-10-seconds-with-semi-long-queries) ).

[TinySearch](https://github.com/tinysearch/tinysearch) користете го филтерот Блум за пребарување, не може да се користи за пребарување на префиксот (на пример, внесете `goo` , пребарувајте `good` , `google` ) и не може да постигнете сличен ефект на автоматско завршување.

Поради незадоволство од недостатоците на постоечките решенија, `i18n.site` разви ново чисто решение за пребарување на целосен текст од предниот дел, кое ги има следните карактеристики :
...
1. Поддржува пребарување со префикс, што може да ги прикажува резултатите од пребарувањето во реално време додека корисникот пишува.
1. Достапно офлајн
...
Во интеракцијата од предниот дел, лентата css [напредок](https://dev.to/i18n-site/a-single-progress-uses-pure-css-to-achieve-animation-effects-2oo) на вчитувањето на индексот може / се прикаже [за](https://juejin.cn/post/7413586285954154522) да се избегне задоцнување при progress + за прв пат.

### IndexedDB Високо Истовремено Пишување

Проектот е [idb](https://www.npmjs.com/package/idb) врз основа на асинхроната инкапсулација на IndexedDB
...
Забележете дека секој пат кога повторно ќе пребарувате по `yield` , треба повторно да отворите трансакција со барање од `IndexedDB` .
...
Превртено пребарување на целосен текст имплементирано чисто на предниот дел, со брз одговор и без потреба од сервер.

Многу погоден за мали и средни веб-страници како што се документи и лични блогови.