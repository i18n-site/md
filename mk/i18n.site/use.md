...
По извршувањето, ќе се генерираат датотеките за превод и кеш, Ве молиме не заборавајте да `git add . ` додадете во складиштето во директориумот `md` .
...
`ol` е кратенката од `online` , што `-n` означува онлајн околината, која `npm` користи за официјално објавување.
...
Кога објавувате врз основа на пакетот `npm` , `i18n.site` **заборавајте да користите `//unpkg.com/` за** `/.v` од `v:` вредност.
...
За само-домаќин на содржина, покрај користењето `CDN` вашиот сопствен сервер, друга вообичаена опција е да користите `S3` +

Можете да `S3` `S3` [rclone](https://rclone.org)
...
Запомнете да го конфигурирате `CDN` така што времето за кеширање на патеката што завршува на `/.v` е `1s` , инаку до новообјавената содржина не може веднаш да се пристапи.
...
Во споредба [cloudflare page](//pages.cloudflare.com) `github page` , тој обезбедува препишување на патеката и е попријателски настроен кон копното на Кина и се препорачува да се користи.
...
`i18n.site` на горната слика се како што следува.
...
Потоа, користете го складирањето `out/ol/htm` објекти на продавачите на облак во Кина + Распоредете ја следнава содржина `CDN` .
...
Додадете го името на доменот во [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) и посочете го името на доменот `*.xxx.com` до `CNAME` во Alibaba Cloud `CDN` .
...
Ве `/root/i18n/md/out/ol/htm` додадете конфигурација слична на следната во пасус `server` `out/ol/htm` nginx
