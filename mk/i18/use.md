...
windows , ве молиме [кликнете овде за да преземете и инсталирате `git bash`](https://git-scm.com/download/win) прво.
...
Меѓу нив, содржината на `.i18n/cache/.gitignore` е следнава :
...
Ова значи дека сите датотеки во директориумот `.i18n/cache/` (освен `.i18n/cache/.gitignore` ) се игнорирани.
...
`.i18n/conf.yml` е конфигурациската датотека на алатката за превод од `i18` командна линија
...
: да конфигурирате различни `fromTo` за различни поддиректориуми /
...
Во оваа конфигурациска табела, изворниот јазик на преводот на каталогот `blog` е `zh` , а изворниот јазик на преводот на `blog/your_file_name.md` е `ja` .
...
`ignore` ја користи истата [globset](https://docs.rs/globset/latest/globset/#syntax) како и датотеката `.gitignore` .
