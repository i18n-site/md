...
Поддржува превод на HTML Markdown , содржината во ознаките `<pre>` и `<code>` од HTML вградени во `MarkDown` не е преведена
...
Коментарите на преводот треба да го наведат јазикот по ` ``` ` , како на : ` ```rust ` .
...
Поддржува конфигурација на заглавието [HOGO](https://github.com/gohugoio/hugo) типот статичен блог, кој ги преведува само полињата `title` , `summary` и `description` .
...
`ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` на јапонски :
...
`ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` на јапонски :
