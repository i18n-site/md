...
Kui kasutate avaldamiseks pidevat integratsioonikeskkonda, pole `nodejs` installimist vaja, piisab, kui kopeerida sisselogitud ja avaldamisluba `~/.npmrc` keskkonda.
...
Seejärel käivitage kataloogis `htm` järgmine käsk (ärge unustage asendada `i18n-demo/i18n-demo.github.io.git` oma lao aadressiga) :
...
Vaadake tõlke konfiguratsiooni [i18](/i18/use)