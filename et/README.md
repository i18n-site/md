<h1 style="display:flex;justify-content:space-between">i18n.site Rahvusvahelised lahendused<img src="//p.3ti.site/logo.svg" style="user-select:none;margin-top:-1px;width:42px"></h1>
...
## <a rel=id href="#project" id="project"></a> Projekti kirjeldus
...
Tõlgi `Markdown` ja `YAML` käsurea tööriistaks ( [lähtekoodiks](https://github.com/i18n-site/rust/tree/main/i18) ) mitmes keeles.
...
Jagame `UNIX` on fail " " ja saame hallata tõlkeid sadadesse keeltesse ilma keerulisi ja tülikaid ettevõtte tasemel lahendusi kasutusele võtmata.

Tõlke `Markdown` saate kasutada kõige tuttavamaid tööriistu (kuid te ei saa lõike lisada ega kustutada) ja kasutada versioonihalduseks kõige tuttavamat viisi.

Keelefailide jaoks saab luua avatud lähtekoodina koodibaasi ja `Pull Request` 'i protsessi abil saavad globaalsed kasutajad osaleda tõlgete pidevas optimeerimises. Sujuv ühendus [github](//github.com) muud avatud lähtekoodiga kogukonnad.
...
Puhtalt staatiline, lugemiskogemuse jaoks optimeeritud ja [i18](#i18) -i tõlkega integreeritud see on parim valik projektidokumendi saidi koostamiseks.
...
See veebisait on välja töötatud selle raamistiku alusel ja sisaldab kasutaja-, makse- ja muid funktsioone ( [lähtekood](/i18n.site/src) ) Üksikasjalik õpetus kirjutatakse hiljem.

[→ Kasutusjuhendi saamiseks lugege palun projekti dokumentatsiooni](/i18n.site) .
