...
Tõlke kommentaarides tuleb märkida keel pärast ` ``` ` , : ` ```rust ` .
...
Tööriist tõlgib ainult sõnastiku väärtused `YAML` , mitte sõnastiku võtmed.
...
Toetab [HOGO](https://github.com/gohugoio/hugo) tüüpi staatilise ajaveebi päise konfiguratsiooni, mis tõlgib ainult välju `title` , `summary` ja `description` .
...
`i18` tunneb pealkirja `MarkDown` ära ja kasutab tõstutundlikku keelde tõlkides esimest tähte automaatselt.
...
Jaapani keelde `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` :

![](//p.3ti.site/1720199391.avif)

`Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` keelde tõlgitud :
...
Jaapani keelde `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` :

![](//p.3ti.site/1720199550.avif)

Ülaltoodud tõlge DeepL keelde (ka pärisnimede ümberkirjutamine ja paaritu `... ` lisamine):
