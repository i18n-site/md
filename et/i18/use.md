...
Selles konfiguratsioonitabelis on kataloogi `blog` tõlke lähtekeel `zh` ja `blog/your_file_name.md` tõlke lähtekeel `ja` .
...
`ignore` kasutab sama [globset](https://docs.rs/globset/latest/globset/#syntax) mis `.gitignore` fail.
...
Tööriist tõlgib ainult sõnastiku väärtused `.yml` , mitte sõnastiku võtmed.
...
Kui `/usr/local/bin` pole kirjutamisõigust, installitakse see `~/.bin` le.
