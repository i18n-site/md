...
Sellised teenused nõuavad otsingumahu alusel tasumist ja sageli ei ole need Mandri-Hiina kasutajatele saadaval selliste probleemide tõttu nagu veebisaitide järgimine.
...
`lunrjs` Indekside koostamiseks on kaks võimalust ja mõlemal on oma probleemid.
...
Olemasolevate lahenduste puuduste tõttu on `i18n.site` välja töötanud uue puhta esiotsa täistekstiotsingu lahenduse, millel on järgmised omadused :
