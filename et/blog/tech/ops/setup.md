...
Kuvatakse mudel `AMD EPYC 9224 24-Core Processor` , ühetuumaline jõudlusskoor 1285 CPU
...
Uued kasutajad peavad ostmisel esitama oma ID-kaardi pärisnime autentimiseks.

## Seadistage Paroolivaba Sisselogimine

Mõne minuti pärast aktiveeriti kõik serverid.

Kõigepealt IP redigeerida aliase määramiseks kohaliku masina `~/.ssh/config` ja seejärel kasutada paroolivaba sisselogimise konfigureerimiseks `ssh-copy-id root@c2` .
...
See samm võtab kaua aega Pärast pooletunnist ootamist õnnestub sageli kasutatava tarkvara uuendamine ja installimine. Seda saate kinnitada järgmise käsuga:
...
[mosh](//mosh.org) Euroopast `ssh` ei ole stabiilne.

Looge kohalikus kataloogis `~/.bin` (peamiselt keskkonnamuutujale `PATH` lisatud) `c0` skripti järgmiselt:
...
Pärast loomist andke käivitamisõigused ja pehme link `c1` ja `c2` .
...
Tulevikus saate otse sisestada `c0` ja `c1` et kasutada `mosh` kiiret otseühendust ja sisestada [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Sõrmejäljed muutuvad, kasutage `ssh-keygen -R ip` puhastamiseks ssh
...
Nagu on näidatud alloleval joonisel, näete, et pärast käsitsi pakkimist uues süsteemis tihendati veel ligi 100 MB.
...
Initsialiseerimisskript loob `~/ubuntu` kataloogi, lihtsalt käivitage järgmine käsk:
...
Kasutage `snapper list-configs` et vaadata alammahtusid, mille jaoks hetktõmmised on määratud.

![](https://i-01.eu.org/1718302689.avif)

Kasutage `snapper -c mnt list` et vaadata alamköite hetktõmmist

`/etc/snapper/configs/mnt` saab vaadata selle alamköite konkreetseid varundusseadeid
...
Pidage meeles, et ülaltoodud toimingud tehakse päästesüsteemis, mistõttu need muudatused jõustuvad alles siis, kui süsteem on taaskäivitatud ega ole enam päästerežiimis.

See tähendab, et te ei näe kohe muudatusi olemasolevates logides, mis muutuvad püsivaks, kuid kõik pärast seda loodud logid jäävad alles.
...
Pärast korduvaid uuringuid on see sellepärast github action et tar kasutaja `uid` on `1001`

Lahtipakkimine peab olema tar kui parameetrit `--no-same-owner` pole, siis `1001` kasutaja puhul tihendatakse see väärtuseks `uid`
