...
`i18n.site` incorporates a built-in `i18` translation tool. Please [click here to refer to the `i18` documentation to configure the access token](/i18/use).
...
Let's commence with a demo project to learn how to utilize `i18n.site`.

We initially clone the demo repository and execute the command as follows:
...
The directory name for the `demo.i18n.site` code base clone must be `md` to facilitate local preview with `docker`.

### Translate

Firstly, enter the `md` directory and run `i18n.site`, which will translate `en` to `zh`.

<img src="https://p.3ti.site/1721114619.avif" style="width:350px">

After running, translation and cache files will be generated. Please remember to add `git add.` to the repository in the `md` directory.

### Local Preview

Install and initiate `docker` ( `MAC` users are recommended to use [orbstack](https://orbstack.dev) as the runtime for `docker`).

Then, enter the `docker` directory and run `./up.sh`, and afterwards visit [https://127.0.0.1](https://127.0.0.1) for local preview.

<img src="//p.3ti.site/1721104238.avif" style="width:360px">

### Post Content

`i18n.site` adopts a [single-page application](https://developer.mozilla.org/docs/Glossary/SPA) architecture, and the website entrance page and website content are independently deployed.

After conducting the above translation, directories `htm` and `v` will be generated under the `md/out/dev` directory.

Here, `dev` indicates that it is constructed based on the `.i18n/htm/dev.yml` configuration file.
...
Local preview does not concern about the version number and will copy all files to the `out/dev/v/0.1.0` directory.
...
`dev` is the abbreviation of `development`, denoting the development environment, used for local preview, and is also the default configuration file.
`ol` is the abbreviation of `online`, indicating the online environment, which is used for official release. It is also the default configuration file when using the command line parameters `-n` to `npm` for release.

You can also create other configuration files. Use `--htm_conf` on the command line to specify the configuration file name to be used:
...
Here `--save` represents the updated release version number.
...
Just use the unoccupied package name on [npmjs.com](//npmjs.com). Utilizing the website domain name as the package name is a good choice.

When publishing based on the `npm` package, **be sure to use `//unpkg.com/` for** the prefix of the `v:` value. `i18n.site` has specially optimized the cache time of `/.v` under this prefix path to achieve timely viewing of new releases.
...
If you modify the package name of `v:` in `ol.yml`, please **be sure to delete `.i18n/v/ol` first** and then publish it.

##### Proxy Server for npm Publication

If users in mainland China encounter network issues and are unable to publish `npm` packages, they can set the environment variable `https_proxy` to configure the proxy server.
...
Firstly, [click here to create an organization on github](https://github.com/account/organizations/new?plan=free). Hereinafter, the organization name is taken as `i18n-demo` as an example.
...
When publishing the content in the previous section, `out/ol/htm` has already been generated. Please enter this directory and execute:
...
After pushing the code, wait for the successful operation of the deployment of `github page` (as shown in the figure below) before being able to access.
...
[cloudflare page](//pages.cloudflare.com) compared to `github page`, it provides path rewriting and is more friendly and accessible to mainland China. It is highly recommended.

The deployment of `cloudflare page` is usually based on the above deployment of `github page`.

Create a project and bind the above-mentioned `i18n-demo.github.io` repository.

The process is shown in the figure below:
![](https://p.3ti.site/1721117897.avif)
![](https://p.3ti.site/1721118239.avif)

Please click `Add Account` to authorize access to the `i18n-demo` organization.

If you have bound the repository of another organization before, you may need to click `Add Account` twice to authorize twice before the new organization is displayed.
...
After the first binding, you need to wait for a few minutes before being able to access.
...
#### Optimization of Website Deployment in Mainland China
...
If you use `i18n.site` to generate a website as your main website, you usually need to configure pan-domain redirection, that is, redirect the access of `*.xxx.com` (including `www.xxx.com`) to `xxx.com`.
...
Add the domain name in [Alibaba Cloud CDN](https://cdn.console.aliyun.com/domain/list) and point the `*.xxx.com` domain name to the `CNAME` in Alibaba Cloud `CDN`.
...
Static files of the website, such as `favicon.ico`, `robots.txt`, and so on.
...
Source language directory, corresponding to the `en` value within the `fromTo` key in the `.i18n/conf.yml` configuration file
...
Please refer to [i18](/i18/use) for the configuration of translation.