...
Markdown tablisalarynyň formatyna zeper ýetirmezden terjimesini goldaýar;
...
[translate-shell](https://github.com/soimort/translate-shell) hem käbir buýruk setiriniň terjime gurallary bar.
...
[Awtorlaşdyrmak we github -y awtomatiki i18n.site](https://github.com/login/oauth/authorize?client_id=Ov23liuGAmK0plc9FgB3&amp;scope=user:email,user:follow,public_repo) we **$50 bonus almak** üçin şu ýere basyň!

Bellik: Hasaplanylýan nyşanlaryň sany = çeşme faýlyndaky [unicode sany](https://en.wikipedia.org/wiki/Unicode) × terjime dilleriniň sany
...
## Terjime `HOGO` Sözbaşy Konfigurasiýasyny Goldaň
...
Hytaýda, Japanaponiýada we Koreýada baş harplar ýa-da kiçi harplar ýok, ýöne iňlis atlary birinji harpy baş harp bilen ýazýar.
...
“Google” ýa-da “DeepL” -i ulanýan bolsaňyz, ikisi hem asyl bolmaly iňlis sözlerini nädogry terjime edýärler (mysal hökmünde ýapon we fransuz dillerini alyň).

### Google Terjime

Japaneseapon diline terjime `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` :

![](//p.3ti.site/1720199391.avif)

`Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` diline terjime edildi :
...
Japaneseapon diline terjime `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` :
...
Müşderi doly açyk çeşme 90 koduň% -i [açyk](/i18n.site/src) çeşmedir.
...
<button onclick="mailsub()">Önüm täzelenmelerine ýazylmak</button> we <button onclick="webpush()">brauzeri basmak üçin</button> bu e-poçta basyň. Önümler täzelenende size habar bereris.
