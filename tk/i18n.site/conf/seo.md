...
Sazlamada `i18n.site` `.i18n/htm/ol.yml` -den `host:` e üýtgediň, köp obýekt dükany `s3` aşakda düzülip bilner we `region` meýdan islege bagly däl (köp obýekt dükanlary bu meýdany bellemeli däl).
...
Domen [cloudflare](//www.cloudflare.com)
...
`R2` ýöriteleşdirilen domen ady bilen girip `cloudflare` sebäpli, statiki faýllary ýerleşdirmek üçin üçünji tarap obýekt ammary ulanylmaly.
...
`backblaze.com` da çelek dörediň, islendik faýly ýükläň, faýla göz aýlamak üçin basyň we şu ýerde `Friendly URL` `f003.backblazeb2.com` domen adyny alyň.
...
`Full` → şifrlemek re modeiminiň `cloudflare` `SSL`
...
`Rewrite to` dinamiki saýlaň we çelek adyňyza `concat("/file/your_bucketname",http.request.uri.path)` `your_bucketname` den üýtgediň.

Mundan başga-da, ýokardaky `cloudflare` öwrüliş düzgüninde `index.html` `file/your_bucketname/index.html` üýtgedildi we beýleki konfigurasiýalar öňküligine galýar.
