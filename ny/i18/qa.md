...
Mtengo wa `YAML` umatengedwa ngati `MarkDown` womasulira.

Nthawi zina kutembenuka kuchokera ku `HTML` → `MarkDown` sizomwe tikufuna, monga `<a href="/">Home</a>` kusinthidwa kukhala `[Home](/)` .

Kuyika china chilichonse kupatula `href` ku `a` tag, monga `<a class="A" href="/">Home</a>` , kungapewe kutembenukaku.
