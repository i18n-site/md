# Yambitsani Seva (Gwiritsani Ntchito Magawo a BTRFS, Chithunzi Chojambulidwa Cha Snapper)
...
Ndinagula ma seva atatu nthawi imodzi ndikuyambitsanso nthawi imodzi.
...
Chinthu choyamba IP makina am'deralo `~/.ssh/config` kuti muyike dzina, ndiyeno gwiritsani ntchito `ssh-copy-id root@c2` kukonza malowedwe opanda mawu achinsinsi.
...
Yoyamba ndikutsegula pamanja ipv6 ndikuyesa
...
[mosh](//mosh.org) ochokera ku Europe kupita ku China `ssh` .

Pangani zolemba `c0` mu chikwatu chapafupi `~/.bin` (makamaka zowonjezeredwa ku variable variable `PATH` ) motere:
...
Pambuyo pakulenga, perekani zilolezo zomwe zingagwiritsidwe ntchito ndi ulalo wofewa `c1` ndi `c2` .
...
M'tsogolomu, mutha kulowa mwachindunji `c0` ndi `c1` kuti mugwiritse ntchito `mosh` yolumikizana mwachangu ndikulowa [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)

## Sinthani Mawonekedwe Ogawa Kuchokera Ku ext4 Kupita Ku btrfs
...
ssh kusintha kwa zala, gwiritsani ntchito `ssh-keygen -R ip` kuti muyeretse
...
Idzangodzipanikiza mwachisawawa Lamulo loyang'ana kuchuluka kwa compression ndi motere:
...
Monga tawonera pachithunzichi, mutha kuwona kuti pambuyo pa kukanikizana pamanja mudongosolo latsopano, pafupifupi 100MB ina idatsindikizidwa.
...
Zolemba zoyambira zidzapanga chikwatu cha `~/ubuntu` , ingoyendetsani lamulo ili:
...
Gwiritsani ntchito `snapper list-configs` kuti muwone ma subvolumes omwe zithunzithunzi zimayikidwa.

![](https://i-01.eu.org/1718302689.avif)

Gwiritsani ntchito `snapper -c mnt list` kuti muwone chithunzithunzi cha subvolume

`/etc/snapper/configs/mnt` akhoza kuwona zosunga zobwezeretsera za gawoli
...
Kuti mubwezeretse chithunzithunzi cha seva, choyamba dinani Crontol mu menyu oyang'anira
...
Komabe, granularity ya chithunzithunzichi ndi chachikulu kwambiri Chikabwezeretsedwa, deta yomwe yangowonjezedwa posachedwa idzatayika, choncho chonde igwiritseni ntchito mosamala.
...
Kuthandizira kulimbikira kwa chipika pansi pa njira yopulumutsira kumafuna kuti mupange pamanja chikwatu chofananira ndikuchita zina zowonjezera, chifukwa mwina mulibe ntchito yoyendetsa. Nazi njira zomwe mungayesere:
...
Pambuyo pofufuza mobwerezabwereza, ndichifukwa github action Wogwiritsa `uid` wa tar ndi `1001`
