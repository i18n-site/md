...
Isu tinotanga tabatanidza iyo demo repository uye tomhanyisa murairo seizvi:
...
Mushure `md` , shanduro uye cache `git add . ` anozogadzirwa.
...
`dev.yml` uye `ol.yml` ndiyo inogadzika zvigadziriso.
...
Kuti uzvigadzirise zvemukati, kuwedzera pakushandisa yako wega sevha, imwe yakajairika sarudzo `CDN` kushandisa `S3` + .
...
Chekutanga `i18n-demo` [github kuti ugadzire sangano](https://github.com/account/organizations/new?plan=free) .
...
Kuenzaniswa [cloudflare page](//pages.cloudflare.com) `github page` inopa nzira yekunyorazve uye inoshamwaridzana nenyika yeChina uye inosvikika zvakanyanya.
...
Mushure mekusunga zita renzvimbo, ndapota enda kuzita rezita kuti ugadzirise nzira yekunyorazve yepeji-peji application, sezvaratidzwa pazasi:

![](https://p.3ti.site/1721119320.avif)

`i18n.site` iri pamufananidzo uri pamusoro ndeiyi inotevera.
...
Nekuti rekodhi `MX` uye rekodha `CNAME` haigone kugarisana, kana iwe uchida kugamuchira zita rezita maemail panguva imwe chete, iwe unofanirwa kushandira pamwe [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten) script kusvika padanho `CNAME` mune rekodhi `A` .
...
Ichi chinodiwa chinogona kuwanikwa nerubatsiro rweAlibaba Cloud `CDN` `EdgeScript` ( [gwaro](https://www.alibabacloud.com/help/en/cdn/developer-reference/how-edgescript-works) [reChirungu](https://help.aliyun.com/zh/cdn/developer-reference/edgescript) /

Wedzera zita renzvimbo muAlibaba [CDN](https://cdn.console.aliyun.com/domain/list) uye nongedzera zita renzvimbo `*.xxx.com` kusvika `CNAME` muAlibaba Cloud `CDN` .
...
Ndokumbira uwedzere gadziriso `/root/i18n/md/out/ol/htm` neinotevera mundima `server` `out/ol/htm` nginx
