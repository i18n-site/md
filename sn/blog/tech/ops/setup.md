...
Ratidza CPU `AMD EPYC 9224 24-Core Processor` single-core performance score 1285
...
Chinhu chekutanga kugadzirisa emuno muchina `~/.ssh/config` IP uye shandisa `ssh-copy-id root@c2` kugadzirisa password-yemahara login.
...
Iyi nhanho inotora nguva yakareba mushure mekumirira kwehafu yeawa, kukwidziridzwa uye kuiswa kweanowanzo shandiswa software kuchabudirira. Unogona kuzvisimbisa nemurairo unotevera:
...
Iyo network kubva kuEurope kuenda kuChina haina kugadzikana Inokurudzirwa `ssh` [mosh](//mosh.org)

Gadzira `c0` script mumushini `~/.bin` (kunyanya yakawedzerwa kune nharaunda inosiyana `PATH` ) dhairekitori sezvinotevera:
...
Mushure mekugadzira, ipa mvumo inogoneka uye yakapfava-link `c1` `c2` .
...
`mosh` ramangwana [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html) unogona `c0` `c1`
...
Kuchinja kweminwe kuchaitika kana ssh shandisa kuchenesa `ssh-keygen -R ip`
...
Iyo inozozvimanikidza neyakagadzika Murairo wekutarisa iyo compression reshiyo ndouyu:
...
Iyo yekutanga script ichagadzira `~/ubuntu` dhairekitori, ingo mhanyisa unotevera kuraira:
...
Shandisa `snapper list-configs` kuona mavhoriyamu akaiswa mapikicha
...
Kugonesa kushingirira kwegi pasi pehurongwa hwekununura kunoda kuti iwe ugadzire dhairekitori rinoenderana uye uite mamwe matanho ekuwedzera, nekuti unogona kunge usina inomhanya systemd sevhisi. Heano matanho aungaedza:
...
Nenzira iyi, systemd-journald inotanga nekugadzirisa kutsva uye kutanga kuchengetedza matanda zvakaramba pane hard disk.

Ramba uchifunga kuti maoparesheni ari pamusoro anoitwa pane yekununura system, saka shanduko idzodzo dzinongoitika kana system yacho yatangwazve uye isisiri mumodhi yekununura.

Izvi zvinoreva kuti iwe hauzongoona chero shanduko kumatanda aripo achienderera mberi, asi matanda ese anogadzirwa mushure meizvi anozoenderera mberi.

Nenzira iyi iwe unogona kuona matanda eiyo ssh sevhisi
...
Mushure mekuongorora kwakadzokororwa, imhaka github action Mushandisi `1001` tar `uid`
