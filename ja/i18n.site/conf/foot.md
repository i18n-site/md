...
以下を参照してください。スタイルは対応する`.css`ファイルに書き込み、インタラクションはウェブコンポーネントの作成で実現します。

## フッターのスタイル

デモプロジェクトには`md/.i18n/htm`の下に3つの`css`ファイルがあります。
...
ここで`${I18N.C}`は`en/i18n.yml`の対応部分です:

```
C: Power By <a class="a" href="https://i18n.site">i18n.site</a>
```

`${I18N.xxx}`のような書き方を使用し、`i18n.yml`と組み合わせることで、フッターの多言語対応を国际化できます。

リンクに`class="a"`を追加することで、リンクが`Markdown`形式に変換されなくなります。参照:
