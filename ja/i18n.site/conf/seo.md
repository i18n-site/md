...
`i18n.site`は無刷新の単一ページアーキテクチャを採用しており、検索インデックスの作成を容易にするために、個別の静的ページと`sitemap.xml`を生成してクローラーが取得できるようにします。

アクセスリクエストの`User-Agent`が検索エンジンのクローラーである場合、リクエストは`302`リダイレクトを通じて静的ページに転送されます。

静的ページ上では、`link`タグを使用してこのページの異なる言語バージョンのリンクを示します。例えば:
...
デモプロジェクトの`.i18n/htm/ol.yml`設定ファイルを例にとって説明します
...
設定では、`i18n.site`を`.i18n/htm/ol.yml`の`host:`の値に変更し、`s3`の下に複数のオブジェクトストレージを設定できます。`region`フィールドは任意です(多くのオブジェクトストレージではこのフィールドは不要です)。
...
`URL redirect`を動的リダイレクトに選択し、`concat("/en",http.request.uri.path,".htm")`の`/en`を検索エンジンがインデックス化するデフォルトの言語に変更してください。
...
その後、[EdgeJSエッジサービス](//console.bce.baidu.com/cdn/#/cdn/ejs/list)で以下のスクリプトを作成します
...
### ドメインが他の`DNS`に托管されている場合、`cloudflare`をどのように使用するか

まず任意のドメインを`cloudflare`にバインドし、次に`SSL/TLS` → カスタムドメインを使用してメインドメインをこのドメインに関連付けます。

![](https://p.3ti.site/1725438658.avif)

### `cloudflare R2`はカスタムドメインを介してアクセス不可

`cloudflare`自带のオブジェクトストレージ`R2`はカスタムドメインを介してアクセス不可ため、静的ファイルを配置するにはサードパーティのオブジェクトストレージを使用する必要があります。

ここでは、[backblaze.com](https://www.backblaze.com)を例にとって、サードパーティのオブジェクトストレージを`cloudflare`にバインドする方法を説明します。

`backblaze.com`でバケットを作成し、任意のファイルをアップロードし、クリックしてファイルを参照し、ドメイン名`Friendly URL` (ここでは`f003.backblazeb2.com` ) を取得します。

![](//p.3ti.site/1725440783.avif)

`cloudflare`でドメイン名を`CNAME`で`f003.backblazeb2.com`に変更し、プロキシを有効にします。
