...
`md/.i18n/htm/ol.yml`を編集し、`v: //unpkg.com/i18n.site`の`i18n.site`を独自の`npm`パッケージ名に変更してください。

[npmjs.com](//npmjs.com)で利用可能なパッケージ名を使用してください。ウェブサイトのドメイン名をパッケージ名として使用することをお勧めします。

`npm`パッケージに基づいて公開する場合、`v:`のプレフィックスには**`//unpkg.com/`を使用してください**。新しいリリースを迅速に反映できるように、`i18n.site`はこのプレフィックスパスの`/.v`のキャッシュ時間を特別に最適化しています。
