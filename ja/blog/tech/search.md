...
この記事では、`i18n.site`の純粋なフロントエンド全文検索技術の実装方法について共有します。検索効果を体験するには、[i18n.site](//i18n.site)をご訪問ください。

コードはオープンソースです:[検索内核](//github.com/i18n-site/ie/tree/main/qy) / [インタラクティブなインターフェース](//github.com/i18n-site/plugin/tree/main/qy)

## サーバーレス全文検索ソリューションの概観

ドキュメントや個人のブログなど、中小規模の純粋に静的な Web サイトの場合、独自に構築した全文検索バックエンドを構築するのは重すぎるため、サービス不要の全文検索がより一般的な選択肢となります。

無サービス型全文検索ソリューションは主に以下の二つのカテゴリーに分けられます

まず、[algolia.com](//algolia.com)のようなサードパーティの検索サービスプロバイダーが提供する、全文検索用のフロントエンドコンポーネントがあります。

このようなサービスは検索回数に応じて料金が発生し、Webサイトの規制遵守などの問題で、中国本土のユーザーが利用できないことが多いです。
...
純粋なフロントエンドでの逆引き全文検索を実装しており、サーバー不要で、特にドキュメントや個人ブログなどの中小規模サイトに適しています。

`i18n.site`が开源した自社開発の純粋なフロントエンド検索は、体积が小さく、反応が速く、現在の純粋なフロントエンド全文検索の問題点を解決し、より良いユーザーエクスペリエンスを提供します。