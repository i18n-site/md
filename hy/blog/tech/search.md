...
* `docWord` : Փաստաթղթի զանգված id - id
* `prefix` : id նախածանցի -
...
IndexedDB-ի ընթերցումները և գրությունները ասինխրոն են: Ինդեքս ստեղծելիս փաստաթղթերը միաժամանակ կբեռնվեն՝ ինդեքսը ստեղծելու համար:
...
Միևնույն ժամանակ, քանի որ `service worker` պահում է բոլոր հոդվածները, երբ օգտատերը որոնում է կատարում, ամբողջ կայքը, ներառյալ որոնումը, հասանելի է անցանց:
