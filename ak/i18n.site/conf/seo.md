...
### Fa Mmara Ahorow No Dan Kɔ Baabi Foforo
...
Sɛ nhwɛso `cloudflare` , [Huawei Cloud DNS](https://www.huaweicloud.com)
...
Esiane sɛ `cloudflare` mfa domain din a wɔahyɛ da ayɛ so nkɔ `R2` so nti, ɛsɛ sɛ wɔde adekorabea a ɛto so abiɛsa di dwuma de fa fael ahorow a ɛnyɛ hwee sisi hɔ.
...
`Rewrite to` paw dynamic na sesa `your_bucketname` wɔ `concat("/file/your_bucketname",http.request.uri.path)` kɔ wo bokiti din mu.
