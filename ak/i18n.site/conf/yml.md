...
### `use: Toc` , Fael Biako Nsusuwso a Ɛwɔ Nkyerɛkyerɛmu
...
### `use: Md` , Fael Biako Nsusuwso a Enni Nkyerɛkyerɛmu
...
### `use: Doc` , Fael Ahorow Pii Krataa Nsusuwso
...
##### Sɛ url Yɛ Hunu a, Ɛyɛ Default Sɛ i18n Boɔ

Sɛ wɔankyerɛw `url` a, `url` default to the value of `i18n` Saa mmara yi nso di dwuma ma template afoforo.

Ɔkwan a wɔfa so kyerɛw wɔ atifi hɔ no ne `url: xxx` yɛ pɛ , na ne fael a ɛne no hyia no yɛ `en/xxx/TOC` .

#### TOC Nsɛm a Ɛwɔ Pon So Index

`i18n.site` bɛyɛ `js` plug-in `.i18n/hook/after.tran/TOC.js` wɔ demo warehouse no mu de akenkan `TOC` directory index fael a ɛne `doc` template nhyehyeɛ no hyia de ayɛ `json` directory outline no.
...
Hyɛ no nsow sɛ fael yi mu nsɛm nkyerɛ emu nsɛm no ho nhyehyɛe, enti wɔkamfo kyerɛ sɛ wobɛto ne tenten ano hye na woakyerɛw nnianim asɛm tiawa.
...
Sɛ nhwɛsoɔ no, sɛ wɔkɔ `/test` mu, na wɔhyehyɛ `nav:` a wɔmfa saa kwan yi anim nni dwuma, na mprempren browsing kasa no yɛ Borɔfo (code `en` ), `/en/test.md` bɛhyɛ default so na wɔde template `Md` akyerɛ ase.
