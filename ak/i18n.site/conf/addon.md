...
`js` runtime a wɔasisi no gyina nkɔso a ɛto so abien : ɛwɔ [boa](https://github.com/boa-dev/boa)

* `wPath(path, txt)` : Twerɛ kɔ fael mu
* `rTxt(path)` : Kenkan nkyerɛwee fael
* `rBin(path)` : Kenkan fael a ɛwɔ afã abien
* `rDir(dirpath)` Kenkan kyerɛwtohɔ no, sanba botae : yɛ array : list, fael list
