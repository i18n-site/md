...
Sɛ wode continuous integration environment di dwuma de tintim a, ɛho nhia sɛ wobɛ instɔl `nodejs` , kɔpi logged in ne publishing permission `~/.npmrc` no kɔ environment no mu kɛkɛ.
...
Afei, fa nneɛma a wɔkora so a cloud vendors wɔ + asasepɔn so no di dwuma `CDN` Deploy the following content `out/ol/htm` .
...
Yɛsrɛ wo fa nhyehyeɛ a ɛte sɛ deɛ ɛdidi soɔ `/root/i18n/md/out/ol/htm` wɔ `server` `out/ol/htm` a ɛwɔ nginx
