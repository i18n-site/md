...
Hiikkaa mixed HTML Markdown ni deeggara, qabiyyeen mallattoo `<pre>` fi `<code>` HTML `MarkDown` keessatti ukkaamfame keessa jiru hin hiikamu
...
Yaadonni hiikkaa afaan ` ``` ` booda jiru agarsiisuu qabu , kan ` ```rust ` :
...
Afaan sagantaa isin barbaachisu yoo hin argamne [nu](https://groups.google.com/g/i18n-site) qunnamaa .
...
Meeshaaleen hiikkaa sarara ajajaa tokko tokkos jiru, kan akka [translate-shell](https://github.com/soimort/translate-shell)
...
Dhaloota haaraa teeknooloojii hiikkaa faayidaa teeknikaa moodeelota hiikkaa maashinii aadaa fi moodeelota afaan gurguddoo walitti makuun hiikkaa sirrii, sirriifi bareedaa taasisu hojjenneerra.
...
[Hayyama kennuu fi ofumaan github i18n.site](https://github.com/login/oauth/authorize?client_id=Ov23liuGAmK0plc9FgB3&amp;scope=user:email,user:follow,public_repo) fi **bonasii $50 argachuuf** as tuqi !
...
## Qindaa'ina Mataduree `HOGO` Hiikkaa Deeggaru

`summary` `title` [HOGO](https://github.com/gohugoio/hugo) `description`
...
Afaan Jaappaanitti `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` hiikame :

![](//p.3ti.site/1720199391.avif)

Gara Faransaayitti `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` hiikame :
...
Afaan Jaappaanitti `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` hiikame :
...
`i18` meeshaa sarara ajajaa ijaarsa weebsaayitii afaanota hedduu qabu [`i18n.site`](/i18n.site) keessatti walitti makama.

Bal'ina isaaf galmee [`i18n.site`](/i18n.site) ilaali.

## Madda Banaa Koodii

Maamiltoonni guutummaatti madda banaa dha, fi 90 sarvarii % koodii madda [banaa](/i18n.site/src) dha.
