...
VPS [contabo.com](//contabo.com) gatii Yuuroo filachuu hin dagatin, kunis gatii salphaadha.

<img alt="" src="https://i-01.eu.org/1718285782.avif">

Contabo'f kaffaltiin yeroo tokkoo dhaabuu ni jira.
Haa ta'u malee ji'a ji'aan guyyoonni muraasni kaffaltiin fe'iinsaa bilisa ta'e jiru Galmaa'uudhaan iimeelii beeksisaaf xiyyeeffannaa kennuu qofa.
...
Agarsiisa Moodeela CPU `AMD EPYC 9224 24-Core Processor` qabxii raawwii hojii wiirtuu tokkoo 1285
...
Wanti jalqabaa `ssh-copy-id root@c2` maashinii IP gulaaluudha `~/.ssh/config`
...
Tarkaanfiin kun yeroo dheeraa kan fudhatu Walakkaa sa’aatii erga eegnee booda, fooyya’iinsi fi fe’iinsi sooftiweerii yeroo baay’ee itti fayyadamnu ni milkaa’a. Ajaja armaan gadii kanaan mirkaneessuu dandeessu:
...
Neetworkiin Awurooppaa irraa gara Chaayinaa jiru tasgabbaa'aa `ssh` Fayyadamaa kottaa [mosh](//mosh.org)

`~/.bin` maashinii naannoo (baay'inaan jijjiiramaa naannoo `PATH` irratti dabalame) keessatti iskiriiptii `c0` akka armaan gadiitti uumi:

```bash
#!/usr/bin/env bash jedhu
...
Erga uumamee booda, hayyama raawwatamuu danda'uu fi soft-link `c1` `c2` kenni.
...
`c1` `mosh` [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html) `c0`
...
`ssh-keygen -R ip` ashaaraa qubaa yeroo ssh
...
Yoo nama mudaa hin qabne taate, akkasumas dhiibbaa diskiin guutuu harkaan raawwachuu dandeessa Ajajni akka armaan gadiitti

```bash
btrfs filesystem defragment -r -czstd /
```

Akkuma fakkii armaan gadii irratti mul'atutti sirna haaraa keessatti harkaan erga dhiibamee booda gara 100MB dabalataan akka dhiibame arguu dandeessu.
...
Iskiriiptiin jalqabsiisaa galmee `~/ubuntu`
...
Jildii xiqqaa suuraawwan yeroof saagaman ilaaluuf `snapper list-configs`

<img alt="" src="https://i-01.eu.org/1718302689.avif">

Fakkii jildii xiqqaa ilaaluuf `snapper -c mnt list` fayyadami

`/etc/snapper/configs/mnt` Sajoo duwwaa addaa jildii xiqqaa kanaa ilaaluu dandeessa
...
Hojiiwwan armaan olii sirna baraarsaa irratti kan raawwataman ta’uu isaanii yaada keessa galchaa, kanaaf jijjiiramni sun hojiirra kan oolu erga sirnichi deebi’ee jalqabee fi kana booda haala baraarsaa keessa hin jirre qofa.

Kana jechuun jijjiiramni kamiyyuu galmeewwan jiran irratti akka itti fufiinsa ta'u battalumatti hin argitu, garuu galmeewwan kana booda uumaman hundi ni itti fufu.
...
Irra deddeebiin erga qoratamee booda fayyadamaan tar `1001` `uid` ta'eef github action
