...
Ruwasqamanta, tikray chaymanta waqaychasqa willañiqikuna paqarichisqa kanqa Ama hina kaspa, `md` willañiqipi waqaychasqaman `git add . ` yuyariy.
...
`ol` nisqaqa `online` nisqap pisichasqa kayninmi, internetpi pachata rikuchispa, chaytaq oficial kacharinapaq llamk'achisqa kachkan, kamachiy chiru parámetros `-n` kaqmanta `npm` kacharinapaq llamk'achkaspa.
...
Mana runayuq paquete sutita llamk'achiylla [npmjs.com](//npmjs.com) Web kitip kamachiy sutinta llamk'achiyta paquete suti hina allin akllana.
...
`CDN` qunakuypaq, kikin sirwiqniyki llamk'achisqaykimanta aswan, huk común akllana `S3` +
...
Imaraykuchus web kiti [huk p'anqayuq](https://developer.mozilla.org/docs/Glossary/SPA) ruwana arquitectura llamk'achin, yuyariy URL ñankunata wakmanta qillqayta mayqinkunachus mana `. ` kaqmanta `index.html` kaqkamachu.
...
Ñawpaqta kaypi `i18n-demo` [github tantanakuyta kamanaykipaq](https://github.com/account/organizations/new?plan=free) .
...
Chaymanta, China hatun suyupi `out/ol/htm` ranqhaqkunap objeto waqaychayninta llamk'achiy + Kay qatiq willayta mast'ariy `CDN` .
...
`A` registro `MX` chaymanta `CNAME` `CNAME` mana kuska kawsayta atinkuchu, sichus huk pachallapi dominio suti correo electrónicos chaskiyta munanki, [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten)
...
[Alibaba CDN](https://cdn.console.aliyun.com/domain/list) kaqpi kamachiy sutita yapay chaymanta Alibaba Cloud `CDN` kaqpi `*.xxx.com` kaqmanta `CNAME` kaqkama kamachiy sutita rikuchiy.
...
Ama `server` `out/ol/htm` `/root/i18n/md/out/ol/htm` kayman rikch'akuq ruwayta yapay nginx
...
`.i18n` willañiqip urayninpiqa `i18n.site` wakichiy willañiqikuna, tikray waqaychasqa, hukkunapas kachkan.Qhaway qatiq rakita ["Wakichiy"](/i18n.site/conf) sut'inchaypaq.
