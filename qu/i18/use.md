...
windows Sistema, ama hina [kaspa kaypi ñit'iy `git bash` ñawpaqta uraykachinaykipaq hinaspa churanaykipaq](https://git-scm.com/download/win) .
...
Kay wakichiy tablapiqa, catálogo `blog` tikraypa pukyu siminqa `zh` , `blog/your_file_name.md` tikraypa pukyuta simitaq `ja` .
...
`ignore` kikin sintaxis `.gitignore` willañiqiwan [globset](https://docs.rs/globset/latest/globset/#syntax) .
