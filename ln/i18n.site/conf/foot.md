...
Icône ya footer esalemi na kosala fonte na kombo ya `F` iconfont.cn ( [version ya Lingelesi](https://www.iconfont.cn/?lang=en-us) /[中文版](https://www.iconfont.cn/?lang=zh)).
...
Okoki kokoma `js` na `foot.pug` Soki interaction esengeli, svp personnaliser composant web.
...
Kobakisa `class="a"` na lien ezali kopekisa lien ebongwana na `MarkDown` Tala :
 [➔ `YAML` Ndenge nini kopekisa lien `HTML` ebongwana `Markdown` :](/i18/qa#H2) .