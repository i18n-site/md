...
### `use: Md` , Modèle Ya Fichier Moko Sans Contour
...
### `use: Doc` , Modèle Ya Mokanda Ya Ba Fichiers Ebele
...
Lolenge ya kokoma oyo ezali likolo ekokani na `url: xxx` , mpe fichier na yango oyo ekokani na yango ezali `en/xxx/TOC` .

#### Index Ya Tableau Ya Contenus Ya TOC

`i18n.site` ekosala `js` plug-in `.i18n/hook/after.tran/TOC.js` na entrepôt ya démonstration pona kotanga fichier index `TOC` répertoire oyo ekokani na configuration ya modèle `doc` pona ko générer contour ya `json` répertoire.
...
Ndakisa, soki `/test` ezwami, mpe `nav:` ebongisi kozanga liboso ya nzela oyo, mpe monoko ya botali ya lelo ezali Lingelesi (code `en` ), `/en/test.md` ekozwama na ndenge ya libela mpe ekopesama na kosalelaka modèle `Md` .
