...
Soki osaleli environnement ya intégration continue pona kobimisa, ezali na besoin te ya ko installer `nodejs` , kaka ko copier ndingisa ya kokota pe ya kobimisa `~/.npmrc` na environnement.
...
Na sima, salela stockage ya objet ya ba vendeurs ya cloud na Chine + `CDN` Déployer contenus oyo elandi `out/ol/htm` .
...
Lokola enregistrement `MX` na enregistrement `CNAME` `A` kozala esika moko te `CNAME` soki olingi ozua ba emails ya kombo ya domaine en même temps, esengeli o coopérer na script [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten)
