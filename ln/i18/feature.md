...
Ezali ko soutenir traduction ya mixed HTML Markdown , contenus na ba étiquettes `<pre>` na `<code>` ya HTML intégré na `MarkDown` ebongolami te
...
` ```rust ` commentaires ya traduction esengeli kolakisa monoko sima ya ` ``` ` , lokola :
...
Ezali mpe na mwa bisaleli ya kobongola molongo ya komande, lokola [translate-shell](https://github.com/soimort/translate-shell)

Kasi, basungaka te koyeba mbongwana ya ba fichiers mpe babongoli kaka ba fichiers oyo ebongisami mpo na kokitisa ba coûts mpe komatisaka efficacité.
...
## Lisungi Ya Libongoli `HOGO` Configuration Ya Motó Ya Likambo

Ezali ko soutenir configuration ya tête [HOGO](https://github.com/gohugoio/hugo) type blog statique, oyo ebongoli kaka ba champs `title` , `summary` na `description` .
...
Ebongolami na `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` ya Japonais :

![](//p.3ti.site/1720199391.avif)

Ebongolami na `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :
...
Ebongolami na `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` ya Japonais :
...
Client ezali mobimba source ouverte, mpe 90 serveur % ya code ezali ouverte [Cliquez ici mpo na komona code source](/i18n.site/src) .
