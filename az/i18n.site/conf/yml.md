...
### `use: Toc` , Konturlu Tək Fayl Şablonu
...
### `use: Md` , Kontursuz Tək Fayl Şablonu
...
### `use: Doc` , Çox Fayllı Sənəd Şablonu
...
Yuxarıdakı yazı metodu `url: xxx` a bərabərdir və ona uyğun fayl `en/xxx/TOC` dir.

#### TOC Məzmun Indeksi

`i18n.site` `json` kataloq konturunu yaratmaq üçün `doc` şablon konfiqurasiyasına uyğun gələn `TOC` kataloq indeks faylını oxumaq üçün demo anbarında `js` plug-in `.i18n/hook/after.tran/TOC.js` icra edəcək.
...
`Doc` şablonu yaradılan `json` əsaslanaraq məzmunun konturunu göstərəcək.
...
Yuxarıdakı `en/blog/TOC` -ın birinci cərgəsindəki `README.md` , layihənin adı olan aşağıdakı şəkildəki `i18n.site` yə uyğundur.
...
Məsələn, `/test` a daxil olunarsa və `nav:` bu yolun prefiksi olmadan konfiqurasiya edilirsə və cari baxış dili ingilis dilidirsə (kod `en` ), `/en/test.md` defolt olaraq yüklənəcək və şablon `Md` dən istifadə edilərək göstəriləcək.
