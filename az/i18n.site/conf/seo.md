...
`i18n.site` yenilənməyən `sitemap.xml` səhifə arxitekturasını qəbul edir.
...
Konfiqurasiyada, lütfən, `i18n.site` `.i18n/htm/ol.yml` -də `host:` dəyərinə dəyişin, çoxlu obyekt anbarları `s3` altında konfiqurasiya edilə bilər və `region` sahəsi isteğe bağlıdır (bir çox obyekt mağazalarında bu sahəni təyin etmək lazım deyil).
...
Sonra [kənar EdgeJS](//console.bce.baidu.com/cdn/#/cdn/ejs/list) skripti aşağıdakı kimi yaradın
...
`R2` fərdiləşdirilmiş domen adı ilə daxil olmaq `cloudflare` , statik faylları yerləşdirmək üçün üçüncü tərəf obyekt yaddaşından istifadə edilməlidir.

Burada `cloudflare` da saxlanacaq üçüncü tərəf obyektlərinin necə bağlanacağını nümayiş etdirmək üçün nümunə [backblaze.com](https://www.backblaze.com) .
