...
`i18n.site` [`i18`](/i18/use) `i18` tərcümə aləti var.
...
İşlədikdən sonra tərcümə və keş faylları yaradılacaq, `git add . ` `md` qovluğuna əlavə etməyi unutmayın.
...
Sonra, `docker` kataloqunu daxil edin və `./up.sh` işə salın və sonra yerli olaraq önizləmə üçün ziyarət [https://127.0.0.1](https://127.0.0.1) .
...
`ol` rəsmi buraxılış üçün istifadə `npm` onlayn mühiti ifadə edən `online` `-n` in qısaltmasıdır.

Siz həmçinin digər konfiqurasiya faylları `--htm_conf` bilərsiniz.
...
`npm` paketi əsasında dərc edərkən, yeni buraxılışların vaxtında baxılmasına nail olmaq üçün `v:` dəyərinin prefiksi üçün `/.v` - `i18n.site` **`//unpkg.com/` etdiyinizə əmin olun** .
...
Öz serverinizdən istifadə etməklə yanaşı, məzmunu özünə yerləşdirmək üçün başqa bir ümumi seçim `S3` + istifadə etməkdir `CDN`
...
[Təşkilat yaratmaq üçün əvvəlcə bura github](https://github.com/account/organizations/new?plan=free) . Aşağıdakı təşkilat adı nümunə olaraq `i18n-demo` dır.
...
`github page` ilə [cloudflare page](//pages.cloudflare.com) , yolun yenidən yazılmasını təmin edir və materik Çin üçün daha əlverişlidir və istifadə etmək tövsiyə olunur.
...
Yuxarıdakı şəkildəki qaydalar aşağıdakı kimidir. Zəhmət olmasa, aşağıdakı birinci sətirdəki `i18n.site` bağladığınız domen adı ilə əvəz edin.
...
Sonra, materik Çində `out/ol/htm` satıcılarının obyekt yaddaşından istifadə edin + Aşağıdakı məzmunu yerləşdirin `CDN` .
...
[Alibaba CDN](https://cdn.console.aliyun.com/domain/list) -da domen adını əlavə edin və Alibaba Cloud `CDN` -də domen adını `*.xxx.com` dan `CNAME` yə yönəldin.
...
Lütfən ! `out/ol/htm` `server` paraqrafında `/root/i18n/md/out/ol/htm` kimi konfiqurasiya əlavə edin nginx
