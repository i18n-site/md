...
1. Əvvəlcədən qurulmuş indeks faylları
   ...
   İndeksdə bütün sənədlərdən sözlər olduğu üçün ölçüsü böyükdür.
...
Bu həllin performansı son dərəcə zəifdir və tam mətn axtarışı üçün istifadə edilə bilməz (bax [Fuse.js Uzun sorğu 10 saniyədən çox çəkir, onu necə optimallaşdırmaq olar?](https://stackoverflow.com/questions/70984437/fuse-js-takes-10-seconds-with-semi-long-queries) ).
...
* `/\p{P}/` durğu işarələrinə uyğun gələn müntəzəm ifadədir. Xüsusi uyğun simvollara aşağıdakılar daxildir: `! " # $ % & ' ( ) * + , - . / : ; < = > ? @ [ \ ] ^ _ ` { | } ~. `.</p><ul><li> `split('.')` , çünki `Firefox` brauzer sözünün seqmentasiyası `.` seqment etmir.</li>
...
* `prefix` - id prefiks :
* `rindex` : Word id - Sənəd id : Sətir nömrələri massivi
...
İlk dəfə css progress + / almaq [üçün](https://dev.to/i18n-site/a-single-progress-uses-pure-css-to-achieve-animation-effects-2oo) ön tərəfdə qarşılıqlı əlaqədə indeksin [yüklənmə](https://juejin.cn/post/7413586285954154522) tərəqqi çubuğu göstərilə bilər.

### IndexedDB Yüksək Paralel Yazı

Layihə asinxron inkapsulyasiya əsasında [idb](https://www.npmjs.com/package/idb) IndexedDB
