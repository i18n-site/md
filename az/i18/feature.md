...
Markdown HTML dəstəkləyir, `MarkDown` a daxil edilmiş HTML -ın `<pre>` və `<code>` teqlərindəki məzmun tərcümə edilmir
...
Tərcümə şərhlərində ` ``` ` ` ```rust ` sonra dil göstərilməlidir, məsələn :
...
`git` , `vim` və `vscode` ilə tanış olan proqramçılar üçün sənədləri redaktə etmək və versiyaları idarə etmək üçün bu vasitələrdən istifadə, şübhəsiz ki, öyrənmə xərclərini artıracaq.
...
## Heç Bir Dəyişiklik, Tərcümə Yoxdur
...
[github -ə icazə vermək və avtomatik i18n.site](https://github.com/login/oauth/authorize?client_id=Ov23liuGAmK0plc9FgB3&amp;scope=user:email,user:follow,public_repo) və **$50 almaq üçün** bura klikləyin!
...
Yalnız `title` , `summary` və `description` sahələrini tərcümə edən statik blogun [HOGO](https://github.com/gohugoio/hugo) konfiqurasiyasını dəstəkləyir.
...
`i18` başlığı `MarkDown` də tanıya bilər və böyük hərflərə həssas dilə tərcümə edərkən ilk hərfi avtomatik olaraq böyük hərflə yazacaq.
...
`ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` dilinə tərcümə :
...
`ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` dilinə tərcümə :
