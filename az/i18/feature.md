...
Tərcümə şərhlərində ` ``` ` sonra dil ` ```rust ` , məsələn :
...
Yuxarıdakıların fransız dilinə DeepL (həmçinin xüsusi adların yenidən yazılması və tək `... ` əlavə edilməsi):
...
Müştəri tamamilə açıq mənbədir və kodun % server 90 [mənbə koduna baxmaq üçün bura klikləyin](/i18n.site/src) .
