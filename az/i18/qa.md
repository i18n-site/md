...
## `YAML` : `HTML` in `Markdown` Yə Çevrilməsinin Qarşısını Almaq Olar

`YAML` dəyəri tərcümə üçün `MarkDown` kimi qəbul edilir.

Bəzən `HTML` → `MarkDown` -dən çevrilmə bizim istədiyimiz kimi olmur, məsələn, `<a href="/">Home</a>` `[Home](/)` -ə çevrilməsi.

`<a class="A" href="/">Home</a>` kimi `a` teqinə `href` dən başqa hər hansı atribut əlavə etməklə bu çevrilmənin qarşısını ala bilər.

## `./i18n/hash` Fayl Aşağıda Ziddiyyət Təşkil Edir

Ziddiyyətli faylları silin və `i18` tərcüməni təkrarlayın.