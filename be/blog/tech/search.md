...
У гэтым артыкуле будзе расказана пра рэалізацыю тэхналогіі поўнатэкставага пошуку [i18n.site](//i18n.site) `i18n.site` . Вы можаце адчуць эфект пошуку.

[Ядро пошуку](//github.com/i18n-site/ie/tree/main/qy) з адкрытым кодам / [Інтэрактыўны інтэрфейс](//github.com/i18n-site/plugin/tree/main/qy)

## Агляд Бессерверных Рашэнняў Паўнатэкставага Пошуку

Для невялікіх вэб-сайтаў, такіх як дакументы/асабістыя блогі, якія з'яўляюцца чыста статычнымі, несумненна, занадта цяжка ствараць бэкэнд паўнатэкставага пошуку самастойна, і паўнатэкставы пошук без паслуг, несумненна, з'яўляецца лепшай вагай.

Існуючыя бессерверныя рашэнні для паўнатэкставага пошуку дзеляцца на дзве шырокія катэгорыі.

Адным з іх з'яўляецца старонні пастаўшчык паслуг пошуку, падобны на [algolia.com](//algolia.com) які забяспечвае ўваходныя кампаненты поўнатэкставага пошуку.

Такія паслугі патрабуюць аплаты і недаступныя для карыстальнікаў у мацерыковым Кітаі з-за праблем з адпаведнасцю вэб-сайта.
...
Вядомы чысты інтэрфейсны паўнатэкставы пошук уключае [lunrjs](https://lunrjs.com) і [ ElasticLunr.js ] [https://github.com/weixsong/elasticlunr.js](%E5%9F%BA%E4%BA%8E%60lunrjs%60%E4%BA%8C%E6%AC%A1%E5%BC%80%E5%8F%91) .

`lunrjs` Ёсць два спосабы стварэння індэксаў, але абодва маюць свае праблемы.
...
2. Загружайце дакументы і стварайце індэксы на лета
   ...
   Стварэнне індэкса з'яўляецца інтэнсіўнай вылічальнай задачай. Перабудова індэкса пры кожным доступе да яго прывядзе да відавочных затрымак і дрэннага карыстальніцкага досведу.

У дадатак да `lunrjs` ёсць некаторыя іншыя рашэнні для паўнатэкставага пошуку, такія як :

[fusejs](https://www.fusejs.io) , вылічыць падабенства паміж радкамі для пошуку.

Прадукцыйнасць гэтага рашэння надзвычай нізкая, і яго нельга выкарыстоўваць для паўнатэкставага пошуку (гл. [Fuse.js Доўгі запыт займае больш за 10 секунд, як яго аптымізаваць?](https://stackoverflow.com/questions/70984437/fuse-js-takes-10-seconds-with-semi-long-queries) ).

[TinySearch](https://github.com/tinysearch/tinysearch) , выкарыстоўвайце для пошуку фільтр Блума, нельга выкарыстоўваць для пошуку па прэфіксе (напрыклад, увядзіце `goo` , шукайце `good` , `google` ), і нельга дасягнуць падобнага эфекту аўтаматычнага завяршэння.

З-за незадаволенасці недахопамі існуючых рашэнняў `i18n.site` распрацаваў новае чыстае інтэрфейснае рашэнне для паўнатэкставага пошуку, якое мае наступныя функцыі :
...
1. Падтрымлівае пошук па прэфіксах, які можа адлюстроўваць вынікі пошуку ў рэжыме рэальнага часу, пакуль карыстальнік набірае тэкст.
...
* `/\p{P}/` - гэта рэгулярны выраз, які адпавядае знакам прыпынку. Спецыфічныя адпаведныя сімвалы ўключаюць: `! " # $ % & ' ( ) * + , - . / : ; < = > ? @ [ \ ] ^ _ ` { | } ~. `.</p><ul><li> `split('.')` таму, што `Firefox` сегментацыя слова браўзера не сегментуе `.` .</li>
...
Такім чынам можна дасягнуць паступовай індэксацыі і паменшыць аб'ём разліку.

У інтэрфейсным [узаемадзеянні](https://dev.to/i18n-site/a-single-progress-uses-pure-css-to-achieve-animation-effects-2oo) можа / адлюстравана панэль ходу загрузкі індэкса, css пазбегнуць затрымкі пры загрузцы ў першы progress + . [Гл](https://juejin.cn/post/7413586285954154522) .

### IndexedDB З Высокай Адначасовай Запісам

Праект [idb](https://www.npmjs.com/package/idb) на аснове асінхроннай інкапсуляцыі IndexedDB
...
Вынікі пошуку, якія адлюстроўваюцца першымі, забяспечваюць дакладнасць запыту, а вынікі, якія загружаюцца пасля (націсніце кнопку загрузіць больш), забяспечваюць узровень запамінання.
...
Перавернуты паўнатэкставы пошук рэалізаваны выключна на інтэрфейсе, з хуткім адказам і без неабходнасці выкарыстання сервера.

Ён вельмі падыходзіць для вэб-сайтаў малога і сярэдняга памеру, такіх як дакументы і асабістыя блогі.