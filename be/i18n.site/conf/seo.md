...
Калі сканер пошукавай сістэмы выкарыстоўвае `User-Agent` запыту доступу, запыт будзе перанакіраваны на статычную старонку праз `302` .
...
У канфігурацыі змяніце `i18n.site` на значэнне `host:` у `.i18n/htm/ol.yml` , некалькі сховішчаў аб'ектаў можна наладзіць у `s3` , а поле `region` неабавязковае (у многіх сховішчах аб'ектаў усталёўваць гэта поле не трэба).
...
Калі вы хочаце прадастаўляць паслугі ў мацерыковым Кітаі, а таксама хочаце `cloudflare` бясплатных міжнародных трафікаў, вы можаце выкарыстоўваць `DNS` з рэгіянальным дазволам.
...
Спачатку прывяжыце адвольнае даменнае імя да `cloudflare` , а затым выкарыстоўвайце `SSL/TLS` → карыстацкае даменнае імя, каб звязаць асноўнае даменнае імя з гэтым даменным імем.

![](https://p.3ti.site/1725438658.avif)

### `cloudflare R2` Нельга Атрымаць Праз Карыстацкае Даменнае Імя

Паколькі `R2` `cloudflare` атрымаць доступ праз наладжанае даменнае імя, для размяшчэння статычных файлаў трэба выкарыстоўваць сховішча аб'ектаў трэцяга боку.

Вось [backblaze.com](https://www.backblaze.com) , каб прадэманстраваць, як прывязаць староннія аб'екты да `cloudflare` .
