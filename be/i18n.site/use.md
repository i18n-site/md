...
Пасля запуску будуць створаныя `git add . ` перакладу `md` кэша.
...
Пры публікацыі на аснове пакета `npm` **абавязкова выкарыстоўвайце `//unpkg.com/` для** прэфікса `v:` Значэнне `i18n.site` спецыяльна аптымізавала час кэшавання `/.v` пад гэтым шляхам прэфікса для своечасовага прагляду новых выпускаў.
...
Калі карыстальнікі ў мацерыковым Кітаі сутыкаюцца з праблемамі сеткі і не могуць апублікаваць `npm` пакетаў, яны могуць усталяваць зменную асяроддзя `https_proxy` для канфігурацыі проксі-сервера.
...
#### Кантэнт, Які Размяшчаецца Самастойна
...
Для самастойнага размяшчэння кантэнту, у `CDN` да выкарыстання ўласнага сервера, яшчэ адзін распаўсюджаны варыянт - выкарыстоўваць `S3` +
...
Спачатку [націсніце тут github](https://github.com/account/organizations/new?plan=free) напрыклад, `i18n-demo` арганізацыі.
...
Затым выкарыстоўвайце сховішча аб'ектаў пастаўшчыкоў воблака ў мацерыковым Кітаі + `out/ol/htm` `CDN`
...
Дадайце даменнае імя ў [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) і ўкажыце даменнае імя `*.xxx.com` да `CNAME` у Alibaba Cloud `CDN` !
