...
У каментарыях да перакладу трэба пазначаць мову пасля ` ``` ` , ` ```rust ` :
...
Падтрымлівае канфігурацыю загалоўка тыпу [HOGO](https://github.com/gohugoio/hugo) static blog, які перакладае толькі палі `title` , `summary` і `description` .
...
### Англійскія Тэрміны На Кітайскай, Японскай, Карэйскай І Кітайскай Мовах Не Перакладаюцца
...
Пераклад на японскую `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` :

![](//p.3ti.site/1720199391.avif)

Пераклад на французскую `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :
...
Пераклад на японскую `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` :
