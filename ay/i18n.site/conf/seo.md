...
`i18n.site` `sitemap.xml` mä arquitectura de una sola página jan refrescante ukaruw apnaqi.
...
Configuración ukanxa, `i18n.site` ukarux `host:` in `.i18n/htm/ol.yml` ukar mayjt’ayañamawa, walja yänak imañ utanakax `s3` ukarjam wakicht’atäspawa, ukatx `region` chiqax munañjamawa (walja yänak imañ utanakax janiw uka chimpunak utt’ayañax wakiskiti).
...
China continental uksar yanapt’añax wakisispa ukhax [Baidu Smart Cloud ukampiw](//cloud.baidu.com) apnaqasispa.
...
### `cloudflare R2` Janiw Mä Costumbre Dominio Sutimpix Mantañjamäkiti

Kunatix `R2` `cloudflare` mä dominio sutimp uñt’ayatjam mantañjamäkiti, ukat kimsïr yänak imañ utax archivos estáticos ukanakar uñstayañatakix apnaqañaw wakisi.
...
Dominio suti `CNAME` ukhat `f003.backblazeb2.com` ukar `cloudflare` mayjt’ayaña ukat proxy ukar ch’amanchaña.
...
Ukhamaraki, `cloudflare` jaqukipañ kamachinx `index.html` ukax `file/your_bucketname/index.html` ukar mayjt’ayatawa, ukatx yaqha configuración ukanakax pachpakiw qhipararaki.
