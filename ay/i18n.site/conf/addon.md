...
Jaqukipaña kamachi `i18n.site` ukaxa convención archivo ukawa plug-in package ukaru uskuta ukatxa phuqhañapawa.

Amtawi arsuwi sutinakaxa akhamawa

### htmIndex.js

`htmIndex.js` `.i18n/htm/index.js` tukuykamaw inyectatäni.

Kawkhantix `__CONF__` jichha configuración ukan sutipampiw turkata (kunjamatix `dev` jan ukax `ol` ).
...
* `rBin(path)` : Uñakipt’añataki binario archivo
* `rDir(dirpath)` : :
