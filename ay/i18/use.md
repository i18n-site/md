...
windows Sistema, [aka tuqir ch’iqt’am nayraqat `git bash` apsuñataki ukat instalañataki](https://git-scm.com/download/win) .
...
Kunayman subdirectorios ukanakatakix kunayman `fromTo` ukanakaw wakicht’asispa : /
...
Aka tabla de configuración ukanxa, catálogo `blog` jaqukipäwin phuqhat arupax `zh` ukhamawa, ukatx `blog/your_file_name.md` jaqukipäwin phunchhawi arupax `ja` ukhamawa.
...
`ignore` ukax `.gitignore` [globset](https://docs.rs/globset/latest/globset/#syntax)

Amuyt’añataki, `_* ` aka pata configuración qillqatanx `_` ukamp qalltañ qillqatanakax janiw jaqukipatäkaniti sañ muni.
