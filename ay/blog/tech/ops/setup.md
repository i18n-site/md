# Servidor Qalltaña (Btrfs Partición Apnaqaña, Snapper Programado Instantánea)
...
Uñacht’ayaña Modelo CPU `AMD EPYC 9224 24-Core Processor` mä sapa lurawi puntuación 1285
...
IP kunatix local `ssh-copy-id root@c2` `~/.ssh/config`
...
Europa markat China markar red ukax janiw estable ukhamäkiti [mosh](//mosh.org) Jutapxam jank'aki `ssh`

Mä `c0` script luraña local maquinan `~/.bin` (jilpachax variable de medio ambiente `PATH` ukar yapxatatawa) directorio ukanx akhamawa:

```bash
#!/usr/bin/env ukax mä juk’a pachanakanwa
...
Lurañ tukuyatatxa, ejecutables permisos ukat `c2` -link `c1`
...
[tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html) `mosh` `c1` `c0`
...
Huella digital mayjt’awinakax kunapachatix ssh `ssh-keygen -R ip` apnaqañaw q’umachañataki
...
Ukhamaraki, mä perfeccionista ukhamawa, ukhamaraki mä compresión de disco completo manualmente lurasirakispawa Kamachixa akhamawa
...
Ukhamatwa, systemd-journald ukax machaq configuración ukamp qalltasini ukatx registros ukanakax disco duro ukan jan jaytjasaw imañ qalltani.

Amtañani, aka pata lurawinakax mä sistema de rescate ukan luratawa, ukhamax uka mayjt’awinakax mä kutikiw sistema ukax wasitat qalltatäni ukatx janiw modo de rescate ukanx utjxani ukhakiw phuqhasini.

Ukax sañ muniw janiw jank’ak uñjkätati kuna mayjt’äwinakas utjki uka qillqatanakarux jan jaytjañapataki, ukampis taqi qillqatanakax ukat qhipat lurat ukanakax utjaskakiniwa.
...
Walja kuti yatxatatanakatxa, kunatix tar `1001` apnaqirix `uid` github action

tar Despaquete ukax jan parámetros ukanakampiw lurasiñapa `--no-same-owner` ukax `uid` for `1001` user ukar descomprimido ukhamawa

Ukhamatwa, rsync ukax saphi directorio ukan apnaqir tamap mayjt’ayaspa, ukatx janiw ssh ukar mantañax utjkaniti wasitat qalltañat qhipatxa.
