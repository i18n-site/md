...
بلٽ ان `js` رن ٽائم [boa](https://github.com/boa-dev/boa) ثانوي ترقي تي ٻڌل آهي، ۽ بلٽ ان فنڪشن هن ريت آهن :

* `wPath(path, txt)` : ۾ لکو
* `rTxt(path)` : فائل پڙهو
* `rBin(path)` : بائنري فائل
* `rDir(dirpath)` : پڙهو، واپسي جي قيمت هڪ صف آهي : لسٽ، فائل لسٽ
