...
Sýna CPU Model `AMD EPYC 9224 24-Core Processor` einskjarna árangursstig 1285
...
Það fyrsta er að breyta staðbundnu vélinni `~/.ssh/config` Stilltu alias fyrir IP og notaðu síðan `ssh-copy-id root@c2` til að stilla innskráningu án lykilorðs.
...
Þetta skref tekur langan tíma Eftir að hafa beðið í hálftíma mun uppfærsla og uppsetning á algengum hugbúnaði ganga vel. Þú getur staðfest það með eftirfarandi skipun:
...
Netið frá Evrópu til Kína er ekki stöðugt Það er mælt `ssh` því að nota [mosh](//mosh.org)

Búðu til `c0` skriftu í `~/.bin` á staðnum (aðallega bætt við umhverfisbreytuna `PATH` ) á eftirfarandi hátt:
...
Eftir stofnun, veittu keyranlegar heimildir og mjúkan hlekk `c1` `c2` .
...
Í framtíðinni geturðu beint inn `c0` `c1` og þú getur notað `mosh` háhraða beintengingu og slegið inn [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Bíddu þar til þjónninn endurræsist

Fingrafarbreytingar munu eiga sér stað `ssh-keygen -R ip` ssh
...
Handritið mun sjálfkrafa endurræsa kerfið eftir að umbreytingin hefur tekist.
...
Frumstillingarforskriftin mun búa til `~/ubuntu` , keyrðu bara eftirfarandi skipun:
...
Notaðu `snapper -c mnt list` til að skoða mynd af undirbindi

`/etc/snapper/configs/mnt` Þú getur skoðað sérstakar öryggisafritunarstillingar þessa undirbindi
...
Til að endurheimta skyndimynd netþjóns skaltu fyrst smella á Crontol í stjórnunarvalmyndinni
...
Hins vegar er nákvæmni þessarar skyndimyndar of stór Þegar nýlega bætt gögnum hefur verið endurheimt glatast það að fullu, svo vinsamlegast notaðu þau með varúð.
...
### Hvernig Á Að Fá Aðgang Að Harða Disknum Eftir Að Hafa Farið Í Björgun &
...
Hafðu í huga að ofangreindar aðgerðir eru framkvæmdar á björgunarkerfi, þannig að þessar breytingar munu aðeins taka gildi þegar kerfið er endurræst og ekki lengur í björgunarham.

Þetta þýðir að þú munt ekki strax sjá neinar breytingar á núverandi annálum verða viðvarandi, en allar skrár sem myndast eftir þetta verða viðvarandi.
...
Ég notaði github aðgerð til að búa til sjálfvirka samantekt og dreifingarforskrift áður, en komst að því að dreifingarþjónustan gat ekki ssh.
...
Eftir endurteknar rannsóknir er það vegna þess github action notandinn sem hlóð upp tar er `1001` `uid`

tar Upptaka verður að fara fram án breytu `--no-same-owner` það verður þjappað niður í `uid` fyrir `1001` notanda

Fyrir vikið mun rsync breyta notendahópi rótarskrárinnar og þá muntu ekki geta skráð þig inn á ssh eftir endurræsingu.

Breyttu afþjöppunarforskriftinni sem hér segir:
