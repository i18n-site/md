...
### `use: Md` , Eitt Skráarsniðmát Án Útlínu
...
### `use: Doc` , Skjalasniðmát Fyrir Margar Skrár
...
Ritunaraðferðin hér að ofan jafngildir því að hafa `url: xxx` og samsvarandi skrá hennar er `en/xxx/TOC` .

#### TOC Efnisyfirlit

`i18n.site` mun keyra `js` viðbót `.i18n/hook/after.tran/TOC.js` í kynningarvörugeymslunni til að lesa `TOC` möppuskrána sem samsvarar `doc` sniðmátsstillingunum til að búa til `json` möppuútlitið.
...
Athugið að innihald þessarar skráar sýnir ekki efnisyfirlit og því er mælt með því að takmarka lengdina og skrifa stuttan inngang.
