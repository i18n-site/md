...
Eftir keyrslu verða þýðingar- og skyndiminni skrár búnar til. Mundu að bæta `git add . ` við geymsluna í `md` möppunni.
...
`i18n.site` tekur upp [einnar síðu forritaarkitektúr](https://developer.mozilla.org/docs/Glossary/SPA) og inngangssíða vefsíðunnar og innihald vefsíðunnar eru sett upp sjálfstætt.
...
`ol` er skammstöfunin á `online` , sem gefur til kynna `-n` , sem `npm` notað fyrir opinbera útgáfu.
...
Breyttu `md/.i18n/htm/ol.yml` og breyttu `i18n.site` í `v: //unpkg.com/i18n.site` í þitt eigið `npm` pakkaheiti.

Notaðu bara óupptekna pakkanafnið á [npmjs.com](//npmjs.com)

Þegar þú birtir út frá pakka `npm` , **vertu viss um að nota `//unpkg.com/` fyrir** forskeytið `v:` gildi `i18n.site` hefur sérstaklega fínstillt skyndiminni `/.v` undir þessari forskeytisleið til að fá tímanlega skoðun á nýjum útgáfum.
...
Til að hýsa efni sjálf, auk `CDN` að nota þinn eigin netþjón, er annar algengur valkostur að nota `S3` +
...
Notaðu síðan hlutgeymslu skýjaframleiðenda á meginlandi Kína `CDN` Settu upp eftirfarandi efni `out/ol/htm` +
...
Bættu við léninu í [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) og bentu léninu `*.xxx.com` á `CNAME` í Alibaba Cloud `CDN` .
