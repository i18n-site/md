...
`i18n.site` มีเครื่องมือแปลในตัว `i18` โปรด [คลิกที่นี่เพื่อดูเอกสาร `i18` เพื่อกำหนดค่าโทเค็นการเข้าถึง](/i18/use)
...
หลังจากรันแล้ว ไฟล์การแปลและแคชจะถูกสร้างขึ้น โปรดอย่าลืมเพิ่ม `git add . ` ไปยังที่เก็บในไดเร็กทอรี `md`

### การแสดงตัวอย่างในท้องถิ่น

ติดตั้งและเริ่ม `docker` (ผู้ใช้ `MAC` รายแนะนำให้ใช้ [orbstack](https://orbstack.dev) เป็นรันไทม์สำหรับ `docker` )
...
หลังจากรันการแปลข้างต้นแล้ว ไดเร็กทอรี `htm` และ `v` จะถูกสร้างขึ้นภายใต้ไดเร็กทอรี `md/out/dev`
...
เมื่อเผยแพร่ตามแพ็คเกจ `npm` **ต้องแน่ใจว่าใช้ `//unpkg.com/` สำหรับ** คำนำหน้าของค่า `v:` `i18n.site` ได้ปรับเวลาแคชให้เหมาะสมเป็นพิเศษที่ `/.v` ภายใต้พาธคำนำหน้านี้เพื่อให้สามารถดูรีลีสใหม่ได้ทันท่วงที
...
จากนั้นใช้ที่เก็บข้อมูลออบเจ็กต์ของผู้จำหน่ายระบบคลาว `out/ol/htm` ในจีนแผ่นดินใหญ่ + ปรับใช้เนื้อหาต่อไปนี้ `CDN`
...
เพิ่มชื่อโดเมนใน [Alibaba Cloud CDN](https://cdn.console.aliyun.com/domain/list) และชี้ชื่อโดเมน `*.xxx.com` ถึง `CNAME` ใน Alibaba Cloud `CDN`
