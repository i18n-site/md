...
รันไทม์ `js` ในตัวนั้นขึ้นอยู่กับการพัฒนารองของ [boa](https://github.com/boa-dev/boa) และฟังก์ชันในตัวมีดังนี้ :

* `wPath(path, txt)` : เขียนลงไฟล์
* `rTxt(path)` : อ่านไฟล์ข้อความ
* `rBin(path)` : อ่านไฟล์ไบนารี
* `rDir(dirpath)` : อ่านไดเร็กทอรี ค่าที่ส่งคืนคืออาร์เรย์ #รายการ : รายการไฟล์
