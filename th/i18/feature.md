...
รองรับการแปล Markdown แบบผสม HTML `<pre>` HTML ใน `MarkDown` และเนื้อหาในแท็ก `<code>` จะไม่ถูกแปล
...
ความคิดเห็นการแปลต้องระบุ : ` ```rust ` ` ``` `

`php` `bash` รองรับการแปลความคิดเห็น `toml` , `yaml` , `json5` , `go` `python` `rust` , `c` , `cpp` , `java` `js` [,](https://github.com/i18n-site/rust/blob/main/getc/src/style.rs#L14) `coffee`

หากไม่มีภาษาการเขียนโปรแกรมที่คุณต้องการ โปรด [ติดต่อเรา](https://groups.google.com/g/i18n-site)

## บรรทัดคำสั่งที่เป็นมิตร

มี [เครื่องมือ](https://www.capterra.com/translation-management-software) จำนวนมากสำหรับจัดการเอกสารการแปล

สำหรับโปรแกรมเมอร์ที่คุ้นเคยกับ `git` , `vim` , `vscode` การใช้เครื่องมือเหล่านี้เพื่อแก้ไขเอกสารและจัดการเวอร์ชันจะทำให้ต้นทุนการเรียนรู้เพิ่มขึ้นอย่างไม่ต้องสงสัย
...
การแปลควรทำโดยการป้อนคำสั่งและดำเนินการให้เสร็จสิ้นในคลิกเดียว ไม่ควรมีการอ้างอิงถึงสภาพแวดล้อมที่ซับซ้อน
...
เพื่อให้บรรลุถึงคุณภาพเดียวกัน จำนวนการแก้ไขด้วยตนเองที่ Google Translate ต้องการและ `ChatGPT` คือ `2.67` เท่าและ `3.15` เท่าของเราตามลำดับ
...
เครื่องมือจะแปลเฉพาะค่าพจนานุกรมเป็น `YAML` ไม่ใช่คีย์พจนานุกรม

จากการแปล `YAML` คุณสามารถสร้างโซลูชันระดับสากลสำหรับภาษาการเขียนโปรแกรมต่างๆ ได้อย่างง่ายดาย

## รองรับการแปล `HOGO`

รองรับการกำหนดค่าส่วนหัวของบล็อกแบบคงที่ประเภท [HOGO](https://github.com/gohugoio/hugo) โดยจะแปลเฉพาะฟิลด์ `title` , `summary` และ `description`

## ห้ามแปลชื่อตัวแปร

`Markdown` ใช้เป็นเทมเพลตอีเมล `YAML` เป็นการกำหนดค่าไฟล์ภาษา โดยจะมีพารามิเตอร์ตัวแปร (เช่น ได้รับ `${amount}` เติมเงินแล้ว)

`${varname}` ชื่อตัวแปรประเภทนี้จะไม่ถือเป็นการแปลภาษาอังกฤษ
...
จีน ญี่ปุ่น และเกาหลีไม่มีอักษรตัวพิมพ์ใหญ่หรือตัวพิมพ์เล็ก แต่ชื่อภาษาอังกฤษโดยทั่วไปจะใช้อักษรตัวแรกเป็นตัวพิมพ์ใหญ่

`i18` สามารถจดจำชื่อเรื่องใน `MarkDown` และจะใช้อักษรตัวพิมพ์ใหญ่โดยอัตโนมัติเมื่อแปลเป็นภาษาที่คำนึงถึงตัวพิมพ์เล็กและตัวพิมพ์ใหญ่

เช่น `为阅读体验而优化` จะแปลว่า `Optimized for Reading Experience`
...
แปลเป็น `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` :

<img alt="" src="//p.3ti.site/1720199391.avif">

แปลเป็น `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :

`Falcon` แปลว่า `faucon` และ `Llama` แปลเป็น `lama` . เนื่องจากเป็นคำนามที่เหมาะสม จึงไม่ควรแปล

<img alt="" src="//p.3ti.site/1720199451.avif">

### การแปล DeepL

แปลเป็น `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` :

<img alt="" src="//p.3ti.site/1720199550.avif">

DeepL แปลข้อความข้างต้นเป็นภาษาฝรั่งเศส (เขียนชื่อใหม่และเพิ่มคำแปลกด้วย `... ` ):
...
การแปลของ `i18` จะจดจำคำศัพท์ภาษาอังกฤษในเอกสารภาษาจีน ญี่ปุ่น และเกาหลี และคงข้อกำหนดไว้ครบถ้วน
...
ตัวอย่างเช่น: `C罗` จะแปลว่า `Cristiano Ronaldo`

## สามารถรวมกันได้ `i18n.site`

`i18` ถูกรวมเข้ากับเครื่องมือบรรทัดคำสั่งสำหรับการสร้างเว็บไซต์หลายภาษา [`i18n.site`](/i18n.site)

ดูเอกสารประกอบของ [`i18n.site`](/i18n.site)

## รหัสโอเพ่นซอร์ส

ไคลเอนต์เป็นโอเพ่นซอร์สโดยสมบูรณ์ 90 % ของโค้ด [ฝั่ง](/i18n.site/src) เซิร์ฟเวอร์เป็นโอเพ่นซอร์ส
