...
ʻO [`pug`](https://pugjs.org) kahi ʻōlelo maʻamau e hoʻopuka ana i nā `HTML` 's.

[➔ Kaomi ma aneʻi e aʻo i ka grammar o pug](https://pugjs.org)

**Mai kākau i `css` a me `js` i `foot.pug`** , i ʻole e hewa.

E ʻoluʻolu e nānā i kēia, ua kākau ʻia ke ʻano i ka `.css` pili, a loaʻa ka pilina ma ka hana ʻana i nā ʻāpana pūnaewele.
...
* `foot.css` :
* `import.css` : `i18n.site` no ka pūnaewele holoʻokoʻa
* `conf.css` :
