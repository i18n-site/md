...
## <a rel=id href="#project" id="project"></a> 프로젝트 설명
...
`Markdown` 및 `YAML` 여러 언어의 명령줄 도구( [소스 코드](https://github.com/i18n-site/rust/tree/main/i18) )로 번역합니다.

의 형식을 완벽하게 유지할 수 있습니다 `Markdown` 파일 수정 사항을 식별하고 변경된 파일만 번역할 수 있습니다.

번역은 편집 가능합니다.

원본 텍스트를 수정하고 다시 기계 번역하면 번역에 대한 수동 수정 사항이 덮어쓰여지지 않습니다(원문의 이 단락이 수정되지 않은 경우).

우리는 Everything is a File " `UNIX` " 을 공유하며 복잡하고 번거로운 기업 수준의 솔루션을 도입하지 않고도 수백 가지 언어로 번역을 관리할 수 있습니다.

가장 친숙한 도구를 사용하여 번역을 `Markdown` 할 수 있으며(단락을 추가하거나 삭제할 수는 없음) 버전 제어를 수행하는 데 가장 친숙한 방법을 사용할 수 있습니다.

코드 베이스는 언어 파일의 오픈 소스로 생성될 수 있으며, `Pull Request` 의 프로세스를 통해 전 세계 사용자가 지속적인 번역 최적화에 참여할 수 있습니다. 원활한 연결 [github](//github.com) 그리고 기타 오픈 소스 커뮤니티.

[→ 사용자 가이드는 프로젝트 문서를 읽어보세요](/i18) .
...
이 웹사이트는 이 프레임워크를 기반으로 개발되었으며 사용자, 결제 및 기타 기능( [소스 코드](/i18n.site/src) )을 포함합니다. 자세한 튜토리얼은 나중에 작성됩니다.

[→ 사용자 가이드는 프로젝트 문서를 읽어보세요](/i18n.site) .
...
광활한 인파 속에서 같은 생각을 가진 사람들의 만남을 기대합니다.
