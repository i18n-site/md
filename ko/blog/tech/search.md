...
[TinySearch](//github.com/tinysearch/tinysearch) , 블룸 필터를 사용하여 검색하고 접두어 검색(예: `goo` 입력, `good` , `google` 검색)에 사용할 수 없으며 유사한 자동 완성 효과를 얻을 수 없습니다.
...
1. 사용자가 입력하는 동안 실시간으로 검색 결과를 표시할 수 있는 접두사 검색을 지원합니다.
1. 오프라인으로 이용 가능
...
손떨림 방지 기능 `debounce` 프런트 엔드 상호 작용(다음과 같이 구현됨)에도 사용되어 검색을 유발하는 사용자 입력 빈도를 줄이고 계산량을 줄입니다.
