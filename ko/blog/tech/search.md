...
본 글에서는 `i18n.site` 순수 프론트엔드 전문 검색 기술 구현에 대해 공유하겠습니다. 방문 [i18n.site](//i18n.site) 검색 효과를 경험하실 수 있습니다.

코드 오픈 소스 [검색 커널 대화](//github.com/i18n-site/ie/tree/main/qy) [형 인터페이스](//github.com/i18n-site/plugin/tree/main/qy) /

## 서버리스 전체 텍스트 검색 솔루션 개요

순전히 정적인 문서/개인 블로그와 같은 소규모 웹사이트의 경우, 전체 텍스트 검색 백엔드를 직접 구축하는 것은 의심할 여지 없이 너무 무겁고, 서버리스 전체 텍스트 검색은 더 나은 선택입니다.

기존 서버리스 전체 텍스트 검색 솔루션은 크게 두 가지 범주로 나뉩니다.

하나는 프런트 엔드 전체 텍스트 검색 구성 요소를 제공하는 [algolia.com](//algolia.com) 과 유사한 타사 검색 서비스 제공자입니다.

이러한 서비스는 결제가 필요하며 웹사이트 규정 준수 문제로 인해 중국 본토 사용자는 사용할 수 없습니다.

오프라인 사용이 불가능하고 인터넷 내망에서도 사용할 수 없으며 큰 제한이 있습니다. 이 기사에서는 이에 대해 많은 내용을 다루지 않습니다.

두 번째는 순수한 프런트엔드 전체 텍스트 검색입니다.

더 잘 알려진 순수 프런트엔드 전체 텍스트 검색에는 [lunrjs](/0) 및 [ ElasticLunr.js ] [https://github.com/weixsong/elasticlunr.js](%E5%9F%BA%E4%BA%8E%60lunrjs%60%E4%BA%8C%E6%AC%A1%E5%BC%80%E5%8F%91) 포함됩니다.

`lunrjs` 인덱스를 구축하는 방법에는 두 가지가 있지만 둘 다 고유한 문제가 있습니다.
...
기존 솔루션의 단점에 대한 불만으로 `i18n.site`은 다음과 같은 기능을 갖춘 새로운 순수 프런트엔드 전체 텍스트 검색 솔루션을 개발했습니다 :
...
순수 프런트엔드로 구현된 역전체 텍스트 검색은 응답 속도가 빠르며 서버가 필요하지 않습니다.

문서, 개인 블로그 등 중소 규모의 웹사이트에 매우 적합합니다.