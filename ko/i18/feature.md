...
`i18` 는 Markdown & Yaml 번역하기 위한 명령줄 도구입니다.

## Markdown 형식을 완벽하게 유지할 수 있습니다.

형식을 손상시키지 않고 Markdown 테이블 번역을 지원합니다. 수학 공식이나 링크의 단어는 번역하지 않습니다.

Markdown 의 번역을 지원합니다 HTML `MarkDown` 에 HTML `<code>` 태그의 내용 `<pre>` 번역되지 않습니다.
...
번역 주석은 : ` ```rust ` 과 같이 뒤에 오는 언어를 표시해야 합니다 ` ``` `
...
`KISS` ( `Keep It Simple, Stupid` ) 원칙을 믿는 사람들 사이에서 엔터프라이즈급 솔루션은 번거롭고 비효율적이며 사용하기 어렵다는 동의어입니다.
...
우리는 전통적인 기계 번역 모델과 대규모 언어 모델의 기술적 장점을 통합하여 번역을 정확하고 매끄럽고 우아하게 만드는 차세대 번역 기술을 개발했습니다.

블라인드 테스트를 통해 당사의 번역 품질이 유사한 서비스에 비해 훨씬 더 나은 것으로 나타났습니다.

동일한 품질을 달성하기 위해 Google 번역과 `ChatGPT` 에 필요한 수동 편집의 양은 각각 우리의 `2.67` 배와 `3.15` 배입니다.
...
이 도구는 사전 키가 아닌 `YAML` 의 사전 값만 번역합니다.

`YAML` Translation을 기반으로 다양한 프로그래밍 언어에 대한 국제 솔루션을 쉽게 구축할 수 있습니다.

## 헤더 구성 지원 `HOGO`

[HOGO](https://github.com/gohugoio/hugo) 유형 정적 블로그의 헤더 구성을 지원하며 `title` , `summary` 및 `description` 필드만 번역됩니다.

## 변수 이름을 번역하지 마세요

`Markdown` 이메일 템플릿으로 사용되며 `YAML` 언어 파일 구성으로 변수 매개변수가 있습니다(예: 충전 `${amount}` 가 수신되었습니다).

`${varname}` 이런 변수 이름은 영어 번역으로 간주되지 않습니다.

## 중국, 일본, 한국에 대한 번역 최적화

### 영어로 번역하면 제목의 첫 글자가 자동으로 대문자로 표시됩니다.

중국, 일본, 한국은 대문자나 소문자를 표기하지 않지만, 영어 제목은 일반적으로 첫 글자를 대문자로 표기합니다.

`i18` `MarkDown` 의 제목을 인식할 수 있으며 대소문자를 구분하는 언어로 번역할 때 자동으로 첫 글자를 대문자로 표시합니다.

예를 들어 `为阅读体验而优化` 는 `Optimized for Reading Experience` 으로 번역됩니다.

### 중국어, 일본어, 한국어, 중국어의 영어 용어는 번역되지 않습니다.

중국, 일본, 한국의 문서에는 영어 용어가 많이 포함되어 있는 경우가 많습니다.

중국어, 일본어, 한국어의 기계번역은 이제 비영어권이 되었으며, 다음 중국어 문장과 같이 용어를 함께 번역하는 경우가 많습니다.
...
`ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` 로 번역 :

<img alt="" src="//p.3ti.site/1720199391.avif">

`Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` 로 번역되었습니다 :

`Falcon` `faucon` 으로 번역되고 `Llama` `lama` 로 번역됩니다. 고유명사이므로 번역하면 안 됩니다.

<img alt="" src="//p.3ti.site/1720199451.avif">

### DeepL 번역

`ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` 로 번역 :

<img alt="" src="//p.3ti.site/1720199550.avif">

DeepL 위 내용을 프랑스어로 번역합니다(또한 고유명사를 다시 작성하고 홀수를 추가합니다 `... ` ):
...
`i18` 의 번역은 중국어, 일본어, 한국어 문서의 영어 용어를 인식하고 용어를 그대로 유지합니다.
...
예를 들어 `C罗` 는 `Cristiano Ronaldo` 으로 번역됩니다.

## 다국어 홈페이지 구축 가능 `i18n.site`

`i18` 다국어 웹사이트 구축을 위한 명령줄 도구에 통합되어 있습니다 [`i18n.site`](/i18n.site)

자세한 내용은 [`i18n.site`](/i18n.site) 의 문서를 참조하세요.
