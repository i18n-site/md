...
`git` , `vim` , `vscode` 에 익숙한 프로그래머라면 이러한 도구를 사용하여 문서를 편집하고 버전을 관리하는 것은 의심할 여지없이 학습 비용을 증가시킬 것입니다.
...
[HOGO](https://github.com/gohugoio/hugo) 유형 정적 블로그의 헤더 구성을 지원하며 `title` , `summary` , `brief` 및 `description` 필드만 번역합니다.
