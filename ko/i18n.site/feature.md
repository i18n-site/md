...
이 프로그램에는 번역이 내장되어 있습니다 `i18` 구체적인 사용법은 [➔ `i18`](/i18) 참조하세요.
...
`i18n.site` 으로 사이트 콘텐츠는 `npmjs.com` [jsdelivr.com](//jsdelivr.com) [unpkg.com](//unpkg.com) 및 여러 `CDN` 콘텐츠 `npm` .

이를 바탕으로 중국 사용자가 안정적인 액세스를 확보하고 **높은 프런트엔드 가용성을** 달성할 수 있도록 중국 본토의 미러 소스가 추가되었습니다.

원칙은 [`service worker`](https://developer.mozilla.org/docs/Web/API/Service_Worker_API) 의 도움으로 요청을 가로채고, 요청이 실패하면 다른 `CDN` 에서 다시 시도하고, 가장 빠르게 응답하는 원본 스테이션을 기본 로딩 소스로 적응적으로 활성화하는 것입니다.
...
기본적으로 [Alimama 이중 축 가변 직사각형 글꼴](https://www.iconfont.cn/fonts/detail?cnid=pOvFIr086ADR) , [MiSans](https://hyperos.mi.com/font/zh/download/) 및 기타 온라인 글꼴이 웹페이지에서 활성화되어 다양한 플랫폼에서 사용자의 읽기 경험을 통합합니다.
...
페이지에 떠다니는 귀여운 유령이 `404` , 그 눈은 마우스와 함께 움직입니다. [➔ 보려면 여기를 클릭하세요](/404) .

## 코드 오픈 소스

[코드는 오픈 소스입니다](/i18n.site/src) . 개발에 참여하고 싶다면 [메일링 리스트](//groups.google.com/u/2/g/i18n-site) 에 자신을 소개하세요.

중요하지만 급하지 않은 소소한 요구사항이 많이 있습니다. 개발팀에서는 귀하가 잘하는 기술에 따라 실습 작업을 할당하고, 요구사항을 할당하면서 개발 문서를 개선합니다.
