...
위의 쓰기 방법은 `url: xxx` 과 동일하며 해당 파일은 `en/xxx/TOC` 입니다.

#### TOC 목차 색인

`i18n.site` 데모 웨어하우스에서 `js` 플러그인 `.i18n/hook/after.tran/TOC.js` 를 실행하여 `doc` 템플릿 구성에 해당하는 `TOC` 디렉터리 인덱스 파일을 읽고 `json` 디렉터리 개요를 생성합니다.

`doc` 템플릿을 사용하는 경우 이 플러그인이 있어야 합니다.

빈 폴더에서 프로젝트 `i18n.site` 초기화하는 경우 데모 프로젝트 `.i18n` 디렉터리에 복사해야 합니다.
...
위 `en/blog/TOC` 의 첫 번째 행에 있는 `README.md` 아래 그림의 `i18n.site` 에 해당하며, 이는 프로젝트 이름입니다.
...
`TOC` 파일은 개요의 계층적 관계를 나타내기 위해 들여쓰기가 되어 있으며, 다단계 들여쓰기 및 `#` 로 시작하는 줄 주석을 지원합니다.
...
참고로 이 파일의 내용은 목차 개요를 보여주지 않으므로 길이를 제한하고 짧은 소개를 작성하는 것이 좋습니다.
...
예를 들어 `/test` 에 액세스하고 `nav:` 이 경로의 접두사 없이 구성되었으며 현재 검색 언어가 영어(코드 `en` )인 경우 기본적으로 `/en/test.md` 로드되고 템플릿 `Md` 사용하여 렌더링됩니다.
