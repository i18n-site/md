...
구성에서 `i18n.site` `host:` in `.i18n/htm/ol.yml` 값으로 변경하세요. `s3` 에서 여러 객체 저장소를 구성할 수 있으며 `region` 필드는 선택 사항입니다(많은 객체 저장소에서는 이 필드를 설정할 필요가 없습니다).
...
여기서는 [backblaze.com](https://www.backblaze.com) 예로 들어 타사 개체를 `cloudflare` 에 저장하는 방법을 보여줍니다.
