...
예를 들어 `.i18n/htm/ol.yml` `out/ol` 디렉터리를 생성합니다.

`dev.yml` 과 `ol.yml` 이 기본 구성입니다.

`dev` 은 `development` 의 약자로 개발 환경을 나타내며 로컬 미리보기에 사용되며 기본 구성 파일이기도 합니다.
`ol` 은 `online` 의 약자로 공식 릴리스에 사용되는 온라인 환경을 의미하며 명령줄 매개변수 `-n` ~ `npm` 사용하여 릴리스할 때 기본 구성 파일이기도 합니다.
...
패키지 `npm` 기반으로 게시하는 경우 `v:` 값의 접두사에 **`//unpkg.com/` 사용해야 합니다** . `i18n.site` 새 릴리스를 적시에 볼 수 있도록 이 접두사 경로 아래의 캐시 시간 `/.v` 를 특별히 최적화했습니다.
...
이전 글의 내용을 게시할 때 `out/ol/htm` 생성되었습니다. 이 디렉터리에 들어가서 : 실행하세요.
...
다른 조직의 창고를 바인딩한 경우 새 조직이 표시되기 전에 `Add Account` 두 번 클릭하여 두 번 승인해야 할 수도 있습니다.
...
레코드 `MX` 과 레코드 `CNAME` 공존할 수 없기 때문에 도메인 이름 이메일을 동시에 수신하려면 레벨 `CNAME` 에서 레코드 `A` 으로 [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten) 스크립트와 협력해야 합니다.
...
`i18n.site` 사용하여 웹사이트를 기본 웹사이트로 생성하는 경우 일반적으로 팬 도메인 리디렉션, 즉 액세스를 `*.xxx.com` ( `www.xxx.com` 포함)에서 `xxx.com` 으로 리디렉션하도록 구성해야 합니다.
...
!의 `server` 단락에 다음과 유사한 구성을 추가하십시오 nginx `/root/i18n/md/out/ol/htm` 자신의 프로젝트 `out/ol/htm` 의 경로로 변경하십시오.
...
`.i18n/conf.yml` 구성 파일의 `fromTo` 에 해당하는 소스 `en` 디렉터리
