...
이 두 가지 점을 결합하면 `HTML` `MarkDown` 으로 쉽게 써서 다양한 디스플레이 효과를 얻을 수 있습니다.
...
`MarkDown` `HTML` 로 변환된 후 속성 `src` 와 `href` 의 `$LANG` 현재 [언어 코드](/i18/LANG_CODE) 로 대체됩니다.

이 변수를 사용하면 사진/비디오/링크의 다국어화를 달성할 수 있습니다.