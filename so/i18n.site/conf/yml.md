...
`TOC` waa soo gaabinta `Table of Contents` Marka qaab-dhismeedkan la sameeyo, dulucda `Markdown` da faylka waxa lagu soo bandhigi doonaa dhinaca dhinaceeda.
...
### `use: Md` , Hal Qaab Oo Fayl Ah Oo Aan Dulmar Lahayn
...
### `use: Doc` , Template Dukumeenti Fayl Badan
...
Habka qoraalka ee kore wuxuu u dhigmaa `url: xxx` , galkiisa u dhigmana waa `en/xxx/TOC` .

#### Tusmada Tusmada Ka Kooban TOC

`i18n.site` wuxuu fulin doonaa `js` plug-in `.i18n/hook/after.tran/TOC.js` gudaha bakhaarka demo si uu u akhriyo `TOC` ta tusaha tusaha ee u dhigma qaabeynta `doc` template si loo soo saaro `json` hagaha.
...
Waxaad arki kartaa in `Deme Two` uu leeyahay sumaddiisa mashruuca oo ka `Your Project slogan` :
...
Tusaale ahaan, haddii `/test` la galo, oo `nav:` la habeeyo iyada oo aan la isticmaalin horgalaha waddadan, iyo luqadda wax lagu baadho ee hadda waa Ingiriisi (code `en` ), `/en/test.md` waxaa lagu shubi doonaa si default ah oo loo sameeyay iyadoo la isticmaalayo template `Md` .
