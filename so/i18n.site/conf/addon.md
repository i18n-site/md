...
Magacyada faylka lagu heshiiyey waa sidan soo socota
...
* `lang_li` : luqadda, luqadda koowaad waa luqadda isha
* `changed` :
* `root` :
...
* `wPath(path, txt)` : qor si aad u fayl garaysato
* `rTxt(path)` Akhri : qoraalka
* `rBin(path)` :
