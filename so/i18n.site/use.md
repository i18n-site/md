...
`i18n.site` waxa uu leeyahay `i18` qalab turjumaad [`i18`](/i18/use) ku dhex jira
...
Kadib orodka, tarjumaada iyo `git add . ` kaydinta ayaa la soo saari `md`
...
Si `CDN` iskiis u martigeliso, marka lagu daro isticmaalka serverkaaga, ikhtiyaar kale oo caadi ah waa isticmaalka `S3` + .
...
Sababtoo ah `github page` ma taageerayo dib u qorista dariiqa URL, `404.html` waa la isticmaalaa.
...
`i18n.site` sawirka kore waa sida soo socota.
...
Kadibna, adeegso kaydinta shayga daruuraha iibiya ee dhul weynaha Shiinaha + Geli waxa soo socda `CDN` `out/ol/htm`
...
Sababtoo ah diiwaanka `MX` iyo diiwaanka `CNAME` ma wada noolaan karaan, haddii aad rabto inaad hesho emails magac domain isku mar, waxaad u baahan tahay [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten) la shaqeyso qoraalka `CNAME` ilaa heerka `A` .
...
Ku dar magaca domain gudaha [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) oo ku tilmaam magaca domain `*.xxx.com` ilaa `CNAME` gudaha Alibaba Cloud `CDN` .
