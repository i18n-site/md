...
Lå oss starte med et demoprosjekt for å lære hvordan vi bruker `i18n.site`.

Vi kloner først demonstrasjonslagret og kjører kommandoen som følger:
...
Katalognavnet til `demo.i18n.site`-kodebasen må være `md` for å lette lokal forhåndsvisning med `docker`.
...
Etter kjøring vil oversettelses- og cache-filer bli generert. Husk å legge dem til i `md` med `git add .`.
...
`i18n.site` bruker en [enkeltsides applikasjonsarkitektur](https://developer.mozilla.org/docs/Glossary/SPA), og inngangssiden til nettstedet og innholdet distribueres uavhengig.

Etter å ha kjørt oversettelsen ovenfor, vil katalogene `htm` og `v` bli generert under `md/out/dev`.

Her betyr `dev` at det er bygget basert på `.i18n/htm/dev.yml` konfigurasjonsfilen.
...
`ol` er forkortelsen for `online`, som representerer et online-miljø. Dette brukes for offisiell utgivelse og er standardinnstilling når kommandolinjeparameteret `-n` brukes til å publisere til `npm`.
...
#### <a rel=id href="#npm" id="npm"></a> Publisering av innhold til npmjs.com
...
Hvis du bruker et kontinuerlig integrasjonsmiljø for å publisere, er det ikke nødvendig å installere `nodejs`, bare kopier den påloggede og publiseringstillatelsen `~/.npmrc` til miljøet.

Hvis du endrer pakkenavnet `v:` i `ol.yml`, må du **sørge for å slette `.i18n/v/ol` først** og deretter publisere det.

##### npm Publiseringsproxyserver

Hvis brukere i Kina støter på nettverksproblemer og ikke klarer å publisere `npm` pakker, kan de sette miljøvariabel `https_proxy` for å konfigurere proxy-serveren.
...
For å vertse innhold, i tillegg til å bruke egen server, er en vanlig alternativ løsning å bruke `S3` kombinert med en `CDN`.
...
Først [klikk her for å opprette en organisasjon på github](https://github.com/account/organizations/new?plan=free). Organisasjonsnavnet nedenfor er `i18n-demo` som et eksempel.
...
Dersom `github page` ikke støtter omskriving av URL-bane, brukes `404.html` i stedet.
...
[cloudflare page](//pages.cloudflare.com) gir baneomskriving sammenlignet med `github page` og er mer tilgjengelig for Kina.
...
I tillegg, vennligst konfigurer hurtigbufferreglene, som vist nedenfor, og sett hurtigbuffervarigheten til en måned.

![](https://p.3ti.site/1721125111.avif)

Vennligst endre domenenavnet som samsvarer i det andre trinnet i bildet ovenfor til domenenavnet du har bundet.

#### Optimalisering av nettstedutplassering i Kina
...
Fordi post `MX` og post `CNAME` ikke kan eksistere side om side, hvis du ønsker å motta e-poster med domenenavn samtidig, må du samarbeide med [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten) for å flattre `CNAME` til `A`-rekord.

I tillegg, fordi de utenlandske trafikkkostnadene til kinesiske skyleverandører er relativt høye, hvis du ønsker å optimalisere kostnadene, kan du bruke [Huawei Cloud DNS gratis geografisk oppløsning](https://support.huaweicloud.com/usermanual-dns/dns_usermanual_0041.html) og [Cloudflare for SaaS](https://developers.cloudflare.com/cloudflare-for-platforms/cloudflare-for-saas) tilpassede domenenavn (som vist nedenfor) for å realisere trafikkomdirigering ── trafikk i Kina går gjennom Baidu Cloud `CDN`, internasjonal trafikk går gjennom cloudflare.

![](https://p.3ti.site/1721119788.avif)

Dette er mer komplekse løsninger for optimalisering av distribusjon og vil bli introdusert i separate kapitler i fremtiden.

#### Generisk domenenavnsomdirigering
