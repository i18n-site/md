...
`i18n.site`-avtalen for ankerpunkter innebærer å sette inn tekst som `<a rel=id href="#xxx" id="xxx"></a>` i `MarkDown` for å manuelt opprette posisjoneringsanker.

`<a rel=id href="#xxx" id="xxx"></a>`, hvor `rel=id` definerer ankerets sidestil. Erstatt `xxx` med din faktiske engelske ankerforklaring.

Ankerpunkter plasseres vanligvis i titler, for eksempel:

```md
### <a rel=id href="#i18" id="i18"></a>i18 : MarkDown Command Line Translation Tool
```

Visningseffekten vil være som følger:
...
Innholdet i `<pre>`-elementer blir ikke oversatt.

Ved å kombinere disse to punktene kan du enkelt skrive `HTML` i `MarkDown` for å oppnå forskjellige visningseffekter.
...
Merksomhet, i det foregående `<pre>` er også `<style>` definert.