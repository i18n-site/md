...
`i18n.site` bruker en en-sides applikasjonsarkitektur uten oppdateringer. For å gjøre det lettere for søkemotorer å indeksere, genereres separate statiske sider og `sitemap.xml`.

Når `User-Agent` for en forespørsel er en søkemotorbot, vil forespørselen bli omdirigert til en statisk side via `302`.
...
Statiske filer kan genereres lokalt, men det er vanligere å laste dem opp til objektlagring.

Se `.i18n/htm/ol.yml` konfigurasjonsfilen i demoprosjektet som et eksempel
...
I konfigurasjonen, vennligst endre `i18n.site` til verdien av `host:` i `.i18n/htm/ol.yml`, flere objektlagre kan konfigureres under `s3`, og `region`-feltet er valgfritt (mange objektlagre trenger ikke å sette dette feltet).
...
Domenenavn er vert på [cloudflare](//www.cloudflare.com).
...
### Legg til følgende bufringsregler:
...
### Sett opp følgende omdirigeringsregler:

Endre koden "i18n.site" til ditt domenenavn i følgende regel:
...
Hvis du trenger å tilby tjenester til fastlands-Kina, kan du bruke [Baidu Intelligent Cloud](//cloud.baidu.com).

Dataene lastes opp til Baidu Object Storage og bindes til Baidu Content Distribution Network.
...
Because `cloudflare`'s innebygde objektlager `R2` kan ikke nås via et tilpasset domenenavn, må en tredjeparts objektlagring brukes til å plassere statiske filer.

La oss ta [backblaze.com](https://www.backblaze.com) som et eksempel for å vise hvordan man binder tredjeparts objektlagring til `cloudflare`.
...
I tillegg, i `cloudflare`'s konverteringsregler ovenfor, endres `index.html` til `file/your_bucketname/index.html`, og andre konfigurasjoner forblir de samme.
