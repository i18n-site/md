...
Denne artikkelen vil dele den tekniske implementeringen av `i18n.site` rent front-end fulltekstsøk. Besøk [i18n.site](//i18n.site) for å oppleve søkeeffekten.
...
`lunrjs` har to metoder for å bygge indekser, og begge har sine egne utfordringer.

1. Forhåndsbygde indeksfiler

   Fordi indeksen inneholder alle ord fra dokumentene, er den stor i størrelse.
...
Utover `lunrjs` finnes det også andre fulltekstsøkeløsninger, som for eksempel:

[fusejs](//www.fusejs.io), beregner likheten mellom strenger for å søke.
...
[TinySearch](//github.com/tinysearch/tinysearch), bruker Bloom-filteret for å søke, kan ikke brukes til prefikssøk (for eksempel å skrive `goo` og søke etter `good` og `google`), og kan ikke oppnå lignende automatisk fullføringseffekt.

På grunn av ulempene ved eksisterende løsninger har `i18n.site` utviklet en helt ny ren front-end fulltekstsøkeløsning med følgende egenskaper:
...
1. Basert på `IndexedDB` for å bygge en invertert indeks, noe som krever mindre minne og gir raskere hastighet
1. Når dokumenter legges til/endres, blir bare de endrede dokumentene re-indeksert, noe som reduserer beregningsmengden
1. Støtter prefikssøk, som kan vise søkeresultater i sanntid mens brukeren skriver
1. Tilgjengelig Offline

Nedenfor vil vi detaljert introdusere tekniske implementeringsdetaljer for `i18n.site`.

## Flerspråklig ordsegmentering

Ordsegmentering bruker nettleserens innebygde `Intl.Segmenter`, som støttes av de fleste moderne nettlesere.
...
I `IndexedDB` er det opprettet fem objektlagringstabeller:

* `word`: id - ord
* `word`: id - ord
* `docWord`: dokumentid - array av ordid
* `prefix`: prefiks - array av ordid
* `rindex`: ordid - array av dokumentid og linjenummer
...
Prosjektet er basert på asynkron innkapsling av IndexedDB ved hjelp av [idb](//www.npmjs.com/package/idb).
...
Anta at det er `N` ord etter segmentering. Ved retur av resultater vil vi først vise resultater som inneholder alle søkeord, deretter resultater med `N-1`, `N-2`, ..., `1` søkeord.
...
Merke at etter hver `yield` må en ny `IndexedDB`-spørringstransaksjon åpnes før neste søk.

## Prefiks Sanntidssøk

For å vise søkeresultater dynamisk mens brukeren skriver, som for eksempel å vise ord som starter med `wor` som `words` og `work` når brukeren skriver `wor`.
...
Antidemping-funksjonen `debounce` brukes også i front-end interaksjon (som følger), for å redusere frekvensen av brukerinput som utløser søk og redusere beregningsmengden.
...
`i18n.site`'s ren front-end søkeløsning er optimalisert for `Markdown`-dokumenter.
...
`i18n.site`'s åpen kildekodebaserte, egenutviklede ren front-end søk er kompakt og responsivt, løser mange av problemene med dagens ren front-end fulltekstsøk og gir en forbedret brukeropplevelse.