...
Začnime s ukážkovým projektom, aby sme sa naučili používať `i18n.site` .
...
Po spustení sa vygenerujú súbory prekladu a vyrovnávacej pamäte, nezabudnite `git add . ` pridať do úložiska v adresári `md` .
...
Upravte `md/.i18n/htm/ol.yml` a zmeňte `i18n.site` v `v: //unpkg.com/i18n.site` na svoj vlastný názov balíka `npm` .

Stačí použiť názov neobsadeného balíka na [npmjs.com](//npmjs.com)

Pri publikovaní na základe balíka `npm` **nezabudnite použiť `//unpkg.com/` pre** hodnotu predpony `v:` `i18n.site` špeciálne optimalizoval čas vyrovnávacej pamäte `/.v` pod touto cestou predpony, aby ste dosiahli včasné zobrazenie nových vydaní.
...
Na vlastné hosťovanie obsahu je `CDN` použitia vlastného servera ďalšou bežnou možnosťou použiť `S3` +
...
Pridajte názov domény v [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) a uveďte názov domény `*.xxx.com` do `CNAME` v Alibaba Cloud `CDN` .
