...
* `wPath(path, txt)` :
* `rTxt(path)` Prečítajte : textový súbor
* `rBin(path)` : binárny súbor
* `rDir(dirpath)` Prečítajte : adresár, návratová hodnota je pole : adresárov, zoznam súborov
