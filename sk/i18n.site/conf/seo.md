...
`i18n.site` používa architektúru jednej stránky bez obnovovania Aby sa uľahčilo indexovanie vyhľadávania, pre prehľadávače sa vygeneruje samostatná statická stránka a stránka `sitemap.xml` .
...
Statické súbory je možné generovať lokálne, ale bežnejším prístupom je nahrať ich do úložiska objektov.
...
V konfigurácii zmeňte hodnotu `i18n.site` na hodnotu `host:` v `.i18n/htm/ol.yml` , viaceré úložiská objektov je možné nakonfigurovať pod `s3` a pole `region` je voliteľné (veľa skladov objektov toto pole nemusí nastavovať).
...
Kód pravidla je nasledujúci, upravte kód „i18n.site“ na názov svojej domény:
