...
`i18n.site` používa architektúru jednej stránky bez obnovenia, aby sa uľahčilo indexovanie vyhľadávania, pre prehľadávače sa vygeneruje samostatná statická stránka a `sitemap.xml` .
...
Ako príklad si vezmite konfiguračný súbor `.i18n/htm/ol.yml` v demo projekte
...
Najprv upravte hodnotu `host:` vyššie na názov svojej domény, napríklad `i18n.site` .
...
V konfigurácii zmeňte `i18n.site` na hodnotu `host:` v `.i18n/htm/ol.yml` , viaceré úložiská objektov je možné nakonfigurovať pod `s3` a pole `region` je voliteľné (veľa skladov objektov toto pole nemusí nastavovať).
...
Potom vytvorte skript v [EdgeJS edge](//console.bce.baidu.com/cdn/#/cdn/ejs/list) nasledovne
...
Pretože `R2` `cloudflare` pristupovať pomocou prispôsobeného názvu domény, na umiestnenie statických súborov je potrebné použiť ukladací priestor objektov tretej strany.
