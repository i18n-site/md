...
So sociálnymi sieťami a e-mailom sú ľudia zvyknutí zamerať sa na informačné zdroje vo svojom materinskom jazyku.
...
## <a rel=id href="#project" id="project"></a> Popis projektu

Táto stránka v súčasnosti poskytuje dva nástroje príkazového riadka s otvoreným zdrojom:

* [i18 : MarkDown na preklad príkazového riadku](/i18/feature)
...
Preložte `Markdown` a `YAML` do nástroja príkazového riadka ( [zdrojový kód](https://github.com/i18n-site/rust/tree/main/i18) ) vo viacerých jazykoch.

Dokáže dokonale zachovať formát `Markdown` Dokáže identifikovať úpravy súborov a prekladať iba zmenené súbory.
...
Zdieľame " `UNIX` je súbor " a dokážeme spravovať preklady do stoviek jazykov bez zavádzania zložitých a ťažkopádnych riešení na podnikovej úrovni.

Na `Markdown` prekladu môžete použiť najznámejšie nástroje (ale nemôžete pridávať ani odstraňovať odseky) a používať najznámejší spôsob kontroly verzií.

Kódová základňa môže byť vytvorená ako otvorený zdroj pre jazykové súbory a pomocou procesu `Pull Request` sa môžu globálni používatelia podieľať na nepretržitej optimalizácii prekladov. Bezproblémové [github](//github.com) a ďalšie komunity s otvoreným zdrojom.
...
Čisto statické, optimalizované na čítanie a integrované s prekladom [i18](#i18) je tou najlepšou voľbou na vytvorenie stránky projektových dokumentov.

Základný front-end framework využíva plnú zásuvnú architektúru, ktorá je jednoduchá pre sekundárny vývoj V prípade potreby je možné integrovať back-end funkcie.

Táto webová stránka je vyvinutá na základe tohto rámca a obsahuje používateľské, platobné a ďalšie funkcie ( [zdrojový kód](/i18n.site/src) ).

[→ Návod na použitie nájdete v projektovej dokumentácii](/i18n.site) .
...
>
> PÁN to videl a povedal: Všetky ľudské bytosti majú rovnaký jazyk a rasu.
>
...
Chceme vybudovať internet bez jazykovej izolácie.
Dúfame, že sa celé ľudstvo spojí so spoločným snom.

Stránka prekladov a dokumentácie Markdown je len začiatok.
Synchronizujte uverejňovanie obsahu na sociálnych médiách;
Podporuje dvojjazyčné komentáre a chatovacie miestnosti;
...
Tešíme sa na stretnutie s rovnako zmýšľajúcimi ľuďmi v šírom mori ľudí.
