...
`i18` parancssori eszköz a fordításhoz Markdown & Yaml
...
Támogatja Markdown nyelvű fordítást HTML `<pre>` HTML `MarkDown` és `<code>` címkéket nem fordítja le
...
A matematikai képletek írásával [kapcsolatban olvassa el a " Github A matematikai kifejezések írásáról" című](https://docs.github.com/get-started/writing-on-github/working-with-advanced-formatting/writing-mathematical-expressions#about-writing-mathematical-expressions) részt.
...
A fordítási megjegyzésekben meg kell adni a nyelvet ` ``` ` például ` ```rust ` :
...
Azon programozók számára `git` akik ismerik a , `vim` , `vscode` , ezeknek az eszközöknek a használata a dokumentumok szerkesztéséhez és a verziók kezeléséhez kétségtelenül növeli a tanulási költségeket.

( `Keep It Simple, Stupid` ) Az elv hívei körében `KISS` vállalati szintű megoldások egyet jelentenek azzal, hogy nehézkesek, nem hatékonyak és nehezen használhatók.
...
Vannak olyan parancssori fordítóeszközök is, mint a [translate-shell](https://github.com/soimort/translate-shell)
...
## Xindaya Kiváló Minőségű Fordítása
...
Ugyanezen minőség elérése érdekében a Google Fordító által megkövetelt kézi szerkesztés mennyisége `2.67` `3.15` , a `ChatGPT`
...
## Támogassa a Fordítást `YAML`

Az eszköz csak a szótár értékeit fordítja le `YAML` a szótár kulcsait nem.

`YAML` Translation alapján könnyedén készíthet nemzetközi megoldásokat különféle programozási nyelvekhez.

## `HOGO` a Fejléc Konfigurációját

Támogatja a statikus blog [HOGO](https://github.com/gohugoio/hugo) konfigurációját, csak `title` , `summary` és `description` mezőket fordítja le.

## Ne Fordítsa Le a Változóneveket

`Markdown` -mail sablonként, `YAML` nyelvi fájl konfigurációként használva változó paraméterek lesznek (például: feltöltés `${amount}` megérkezett).

Ez `${varname}` fajta változónév nem tekinthető angol fordításnak.

## Fordításoptimalizálás Kínára, Japánra És Koreára

### Ha Angolra Fordítják, a Cím Első Betűje Automatikusan Nagybetűs Lesz.

Kínában, Japánban és Koreában nincs kis- vagy nagybetű, de az angol címek általában nagybetűvel írják az első betűt.

`i18` képes felismerni a `MarkDown` nyelvű címeket, és a kis- és nagybetűket megkülönböztető nyelvekre történő fordításkor automatikusan nagybetűt ír.

Például `Optimized for Reading Experience` `为阅读体验而优化`
...
`ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` lefordítva :

<img alt="" src="//p.3ti.site/1720199391.avif">

`Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` lefordítva :

`faucon` `Falcon` `Llama` `lama`

<img alt="" src="//p.3ti.site/1720199451.avif">

### DeepL Fordítás

`ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` lefordítva :

<img alt="" src="//p.3ti.site/1720199550.avif">

A fentiek fordítása DeepL (a tulajdonnevek átírásával és a páratlan hozzáadásával is `... ` :
...
Például: `C罗` lesz lefordítva `Cristiano Ronaldo` .

## `i18n.site` Többnyelvű Weboldal Készítés

A többnyelvű webhelykészítés parancssori eszközébe integrálva [`i18n.site`](/i18n.site) `i18`

A részletekért lásd a dokumentációt [`i18n.site`](/i18n.site)

## Nyílt Forráskódú Kód

A kliens teljesen nyílt forráskódú, a kód %-a 90 nyílt forráskódú. [Kattintson ide a forráskód megtekintéséhez](/i18n.site/src) .
