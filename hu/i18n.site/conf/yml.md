...
### `use: Toc` , Egy Fájl Sablon Körvonallal
...
### `use: Md` , Egyetlen Fájl Sablon Vázlat Nélkül
...
### `use: Doc` , Többfájlos Dokumentumsablon
...
A fenti írásmód `url: xxx` val egyenértékű, a megfelelő fájl pedig `en/xxx/TOC` .
...
Például, ha `/test` elérése, és `nav:` az elérési út előtagja nélkül van konfigurálva, és az aktuális böngészési nyelv az angol (kód `en` ), akkor `/en/test.md` alapértelmezés szerint betöltődik, és `Md` sablon használatával jeleníthető meg.
