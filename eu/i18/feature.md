...
HTML mistoaren itzulpena onartzen Markdown , `MarkDown` n txertatutako HTML -ren `<pre>` eta `<code>` etiketetako edukia ez da itzuli
...
`Markdown` posta elektronikoko txantiloi gisa erabiltzen da, `YAML` hizkuntza-fitxategiaren konfigurazio gisa erabiltzen da eta parametro aldakorrak egongo dira (adibidez: karga `${amount}` jaso da).
...
DeepL Itzuli aurrekoa frantsesera (izen propioak ere berridatzi eta `... ` bakoitia gehituz):
...
Bezeroa guztiz irekia da, eta zerbitzariaren 90 kode irekia da. [Egin klik hemen iturburu-kodea ikusteko](/i18n.site/src) .
