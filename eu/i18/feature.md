...
Markdown mistoaren itzulpena onartzen du HTML `MarkDown` n txertatutako HTML -ren `<pre>` eta `<code>` etiketetako edukia ez da itzuli
...
Gaur egun, oharpenen itzulpena onartzen du `toml` , `yaml` , `json5` , `go` , `rust` , `c` , `cpp` , `java` , `js` , `coffee` , `python` , `bash` , `php` [eta beste hizkuntza batzuetan](https://github.com/i18n-site/rust/blob/main/getc/src/style.rs#L14) .
...
[Egin klik hemen baimendu eta automatikoki github i18n.site](https://github.com/login/oauth/authorize?client_id=Ov23liuGAmK0plc9FgB3&amp;scope=user:email,user:follow,public_repo) eta **jaso $50 bonus** !
...
Blog estatikoen goiburuko konfigurazioa onartzen du [HOGO](https://github.com/gohugoio/hugo) `title` , `summary` eta `description` eremuak soilik itzultzen dituena.
...
Txinak, Japoniak eta Koreak ez dituzte letra larriak eta minuskulak, baina ingelesezko tituluetarako konbentzioa lehen letra larriz idaztea da.

`i18` izenburua `MarkDown` antzeman dezake, eta automatikoki letra larriz idatziko du lehen letra maiuskulak bereizten dituen hizkuntza batera itzultzean.
...
DeepL Aurrekoaren itzulpena frantsesera (izen propioak ere berridatzi eta `... ` bakoitia gehituz):
