...
Exekutatu ondoren, itzulpen eta cache fitxategiak sortuko dira. Gogoratu `md` direktorioko biltegian `git add . ` .
...
`i18n.site` [orrialde bakarreko aplikazio-](https://developer.mozilla.org/docs/Glossary/SPA) arkitektura hartzen du, eta webgunearen sarrera orria eta webgunearen edukia modu independentean zabaltzen dira.
...
`v:` `ol.yml` paketearen izena aldatzen baduzu, **ziurtatu lehenik `.i18n/v/ol` ezabatzen duzula** eta gero argitaratu.
...
Edukia norberak ostatatzeko, zure zerbitzaria erabiltzeaz gain, ohiko beste aukera `CDN` `S3` + erabiltzea da.
...
Gogoratu `CDN` konfiguratzen duzula `/.v` amaitzen den bidearen cache-denbora `1s` izan dadin, bestela kaleratu berri den edukia ezin izango da berehala sartu.
...
`github page` rekin [cloudflare page](//pages.cloudflare.com) , bideen berridazketa eskaintzen du eta errespetatzen da Txina kontinentalarekin eta erabilerrazagoa da.
...
Gehitu domeinu-izena [Alibaba Cloud CDN en](https://cdn.console.aliyun.com/domain/list) eta apuntatu domeinu-izena `*.xxx.com` `CNAME` Alibaba Cloud `CDN` en.
