...
Konfigurazioan, mesedez, aldatu `i18n.site` `host:` in `.i18n/htm/ol.yml` baliora, hainbat objektu denda `s3` azpian konfigura daitezke eta `region` eremua aukerakoa da (objektu denda askok ez dute eremu hau ezarri behar).
...
Ondoren, sortu script-a [EdgeJS zerbitzuan](//console.bce.baidu.com/cdn/#/cdn/ejs/list)
...
Adibidez, [Huawei Cloud DNS](https://www.huaweicloud.com) doako eskualdeko analisia eskaintzen du, eta horrekin Txinako kontinenteko trafikoa Baidu Smart Cloud-etik pasa daiteke eta nazioarteko trafikoa `cloudflare` pasa daiteke.
...
Lehenik eta behin, lotu arbitrariozko domeinu-izen bat `cloudflare` , eta gero erabili `SSL/TLS` → domeinu-izen pertsonalizatua domeinu-izen nagusia domeinu-izen honi lotzeko.

![](https://p.3ti.site/1725438658.avif)

### `cloudflare R2` Ezin Da Domeinu-Izen Pertsonalizatu Baten Bidez Sartu

`R2` `cloudflare` domeinu-izen pertsonalizatu batek atzitu, hirugarrenen objektuen biltegiratze bat erabili behar da fitxategi estatikoak jartzeko.

Hemen adibide gisa hartzen [backblaze.com](https://www.backblaze.com) hirugarrenen objektuak `cloudflare` n gordetzeko nola lotu erakusteko.
