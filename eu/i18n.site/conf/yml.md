...
### `use: Doc` , Fitxategi Anitzeko Dokumentu Txantiloia
...
Goiko idazketa-metodoa `url: xxx` ren baliokidea da eta dagokion fitxategia `en/xxx/TOC` da.

#### TOC Aurkibidea

`i18n.site` `js` plug-in `.i18n/hook/after.tran/TOC.js` exekutatuko du demo biltegian `doc` txantiloiaren konfigurazioari dagokion `TOC` direktorio-indize fitxategia irakurtzeko `json` direktorio-eskema sortzeko.
...
Koska-maila anitz daudenean, guraso-mailak izenburua bakarrik idazten du eta ez edukia. Bestela, tipografia nahastu egingo da.
...
### Lehenetsitako Kargatzea Konfigurazio Biderik Gabe
...
Adibidez, `/test` sartzen bada, eta `nav:` bide honen aurrizkirik gabe konfiguratuta badago eta uneko arakatze-hizkuntza ingelesa bada ( `en` kodea), `/en/test.md` lehenespenez kargatuko da eta `Md` txantiloiaren bidez errendatuko da.
