...
Erakutsi CPU `AMD EPYC 9224 24-Core Processor` nukleo bakarreko errendimenduaren puntuazioa 1285
...
Lehenik eta behin, tokiko makinaren `~/.ssh/config` ezizena editatzea da IP eta gero erabili `ssh-copy-id root@c2` pasahitz gabeko saioa konfiguratzeko.
...
Urrats honek ordu erdi itxaron ondoren, normalean erabiltzen den softwarea berritzea eta instalatzea arrakastatsua izango da. Komando honekin berretsi dezakezu:
...
Europatik Txinara ez da egonkorra `ssh` gomendatzen da [mosh](//mosh.org)

Sortu `c0` script bat `~/.bin` lokalean (nagusiki ingurune-aldagaian `PATH` ) direktorioan:
...
Sortu ondoren, eman exekutagarriak diren baimenak eta soft-link `c1` `c2` .
...
Etorkizunean, zuzenean sartu dezakezu `c0` `c1` eta `mosh` abiadura handiko konexio zuzena erabil dezakezu eta sartu [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Hatz-marken aldaketak gertatuko dira ssh erabili `ssh-keygen -R ip`
...
Hasierako script-ak `~/ubuntu` direktorioa sortuko du, besterik gabe exekutatu komando hau:
...
Erabili `snapper list-configs` argazkiak ezarrita dauden azpibolumenak ikusteko.

<img alt="" src="https://i-01.eu.org/1718302689.avif">

Erabili `snapper -c mnt list` azpibolumen baten argazkia ikusteko

`/etc/snapper/configs/mnt` Azpibolumen honen babeskopia-ezarpen zehatzak ikus ditzakezu
...
Hala ere, argazki honen xehetasuna handiegia da berreskuratu ondoren, gehitu berri diren datuak erabat galduko dira, beraz, erabili kontuz.
...
Ikusi desberdinen testu zehatza
...
Kontuan izan goiko eragiketak erreskate-sistema batean egiten direla, beraz, aldaketa horiek sistema berrabiarazi eta erreskate moduan ez dagoenean bakarrik izango dute eragina.

Horrek esan nahi du ez duzula berehala ikusiko lehendik dauden erregistroetan aldaketarik iraunkor bihurtzen, baina horren ondoren sortutako erregistro guztiak mantendu egingo dira.
...
Behin eta berriro ikertu ondoren, tar `1001` duen erabiltzailea `uid` github action

tar Despaketatzea parametrorik gabe egin behar da `--no-same-owner` `1001` erabiltzailearentzat `uid` da
