...
`i18n.site` използва架构 на една страница без презареждане, за да улесни индексирането от търсачките, и ще генерира отделна статична страница и `sitemap.xml` за обхождане от роботите.

Когато `User-Agent` на заявката за достъп е на робот на търсачка, заявката ще бъде пренасочена към статичната страница чрез `302`.

На статичните страници използвайте `link`, за да посочите връзки към различни езикови версии на тази страница, например:
...
Статичните файлове могат да се генерират локално, но по-често се качат в обектно хранилище.
...
Моля, първо променете стойността на `host:` по-горе към името на вашия домейн, например `i18n.site`.
...
В конфигурацията, моля, променете `i18n.site` на стойността `host:` в `.i18n/htm/ol.yml`, под `s3` могат да бъдат конфигурирани няколко обектни хранилища, полето `region` еoptional (много обектни хранилища не изискват това поле).

След това изпълнете `i18n.site -n`, за да republikuvate проекта.

Ако сте направили промени в `~/.config/i18n.site.yml` и искате да качите отново, моля, използвайте следната команда в коренната директория на проекта, за да изчистите кеша за качване:
...
Добавете правилата за преобразуване, както е показано на下图:
...
Добавете правилата за кеширане, както следва:
...
`URL redirect` изберете динамично пренасочване, моля, променете `/en` в пътя за пренасочване `concat("/en", http.request.uri.path, ".htm")` към езика по подразбиране, който искате търсачките да индексират.

## Конфигурация на интелигентната облачна услуга на Baidu

Ако трябва да предоставяте услуги за континентален Китай, можете да използвате [Baidu Smart Cloud](//cloud.baidu.com).

Данните се качват в Baidu Object Storage и се свързват към Baidu Content Distribution Network.

След това създайте скрипт в [EdgeJS边缘服务](//console.bce.baidu.com/cdn/#/cdn/ejs/list) както следва:
...
## Разширено използване: Разпределение на трафика въз основа на регионална резолюция
...
Тъй като `R2` `cloudflare` да бъде достъпно чрез персонализирано име на домейн, трябва да се използва хранилище на обекти на трета страна за поставяне на статични файлове.

Тук [backblaze.com](https://www.backblaze.com) пример, за да демонстрираме как да обвържем обекти на трети страни, които да се съхраняват на `cloudflare` .
...
Добавете правилото за преобразуване, както е показано на下图, поставете го на първо място (първото има най-нисък приоритет):
