...
### `use: Toc` , Шаблон За Един Файл С Контур
...
### `use: Doc` , Многофайлов Шаблон На Документ
...
Горният метод на писане е еквивалентен на наличието на `url: xxx` , а съответният му файл е `en/xxx/TOC` .

#### TOC Индекс На Съдържанието

`i18n.site` ще изпълни `js` плъгин `.i18n/hook/after.tran/TOC.js` в демонстрационния склад, за да прочете индексния файл на `TOC` директории, съответстващ на конфигурацията на `doc` шаблона, за да генерира структурата на `json` директории.
