...
Програмата има вграден `i18` превод, моля вижте [➔ `i18`](/i18) за конкретна употреба.
...
След като потребителят превключи ръчно езика, изборът на потребителя ще бъде запомнен.
...
`i18n.site` По подразбиране съдържанието на сайта ще бъде публикувано на `npmjs.com` , с помощта на [jsdelivr.com](//jsdelivr.com) и множество `CDN` съдържание [unpkg.com](//unpkg.com) заредено на `npm` .
...
Принципът е: прехванете заявката с помощта на [`service worker`](https://developer.mozilla.org/docs/Web/API/Service_Worker_API) , ако заявката е неуспешна, опитайте отново на друга `CDN` и активирайте адаптивно най-бързо отговарящата станция като източник за зареждане по подразбиране.
...
> Изоставя ненужната украса и представя съдържанието в най-чистата му форма.
...
По подразбиране [двуосните променливи правоъгълни шрифтове на Alimama](https://www.iconfont.cn/fonts/detail?cnid=pOvFIr086ADR) , [MiSans](https://hyperos.mi.com/font/zh/download/) и други онлайн шрифтове са активирани на уеб страницата, за да унифицират изживяването при четене на потребителите на различни платформи.
...
Когато превъртате съдържанието отдясно, очертанията отляво едновременно ще маркират текущо четената глава.
...
`404` На страницата има малък сладък плаващ призрак, очите му ще се движат с мишката, [➔ Щракнете тук, за да видите](/404) ,

## Код С Отворен Код

[Кодът е с отворен код](/i18n.site/src) . Ако се интересувате от участие в разработката, моля, представете се в [пощенския списък](//groups.google.com/u/2/g/i18n-site) .

Има много малки изисквания, които са важни, но не спешни, ще възложи практически задачи според технологията, в която сте добри, и ще подобри документите за разработка, докато възлага изискванията.
