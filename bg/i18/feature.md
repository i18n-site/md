...
е инструмент за превод Markdown & Yaml команден ред `i18`
...
Поддържа превод на Markdown в смесен HTML `<pre>` HTML в `MarkDown` и съдържание в `<code>` етикети не се превеждат
...
Коментарите за превод трябва ` ```rust ` посочват езика : ` ``` `

`php` момента поддържа превод на коментари `toml` , `yaml` , `json5` , `go` , `rust` , `c` , `cpp` `java` `js` `coffee` , `python` , `bash` [и други езици](https://github.com/i18n-site/rust/blob/main/getc/src/style.rs#L14) .
...
За програмисти, които са запознати с `git` , `vim` , `vscode` , използването на тези инструменти за редактиране на документи и управление на версии несъмнено ще увеличи разходите за обучение.

`KISS` ( `Keep It Simple, Stupid` ) Сред вярващите в принципа решенията на корпоративно ниво са синоним на това, че са тромави, неефективни и трудни за използване.
...
## Висококачественият Превод На Xindaya
...
За постигане на същото качество количеството ръчно редактиране, изисквано от Google Translate и `ChatGPT` е съответно `2.67` пъти и `3.15` пъти нашето.
...
## Подкрепете Превода `YAML`

Инструментът превежда само стойностите в `YAML` , а не ключовете на речника.

Въз основа на `YAML` Translation можете лесно да създавате международни решения за различни езици за програмиране.

## Поддръжка На Превод `HOGO` Конфигурация На Заглавката

Поддържа конфигурацията на заглавката [HOGO](https://github.com/gohugoio/hugo) статичен блог, превеждат се само полетата `title` , `summary` и `description` .

## Не Превеждайте Имена На Променливи

`Markdown` Използва се като шаблон за имейл, `YAML` като конфигурация на езиков файл, ще има променливи параметри (например: recharge `${amount}` has been Получено).

`${varname}` Този вид име на променлива няма да се счита за превод на английски.
...
Китай, Япония и Корея нямат главни или малки букви, но в заглавията на английски обикновено първата буква е главна.

`i18` разпознава заглавия в `MarkDown` и автоматично ще пише главна буква при превод на език, чувствителен към малки и големи букви.

Например `为阅读体验而优化` ще се преведе като `Optimized for Reading Experience` .
...
`Falcon` се превежда като `faucon` и `Llama` се превежда като `lama` . Като собствени имена, те не трябва да се превеждат.
...
DeepL Превод на горното на френски (също пренаписване на правилните имена и добавяне на нечетните `... ` ):
...
## Може Да Се Комбинира `i18n.site` Изграждане На Многоезичен Уебсайт

`i18` Интегриран е в инструмента за команден ред за изграждане на многоезичен уебсайт [`i18n.site`](/i18n.site) .

Вижте документацията на [`i18n.site`](/i18n.site)

## Код С Отворен Код

Клиентът е с напълно отворен код, а сървърната страна е с отворен код 90 [Щракнете тук, за да видите изходния код](/i18n.site/src) .
