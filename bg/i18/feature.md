...
Поддържа конфигурацията на заглавката на статичен блог тип [HOGO](https://github.com/gohugoio/hugo) , която превежда само полета `title` , `summary` и `description` .

## Не Превеждайте Имена На Променливи

`Markdown` се използва като шаблон за имейл, `YAML` като конфигурация на езиков файл и ще има променливи параметри (например: презареждане `${amount}` е получено).
...
Ако използвате Google или DeepL, и двамата неправилно превеждат английските термини, които трябва да останат оригинални (вземете японски и френски като примери).
...
Преводът на `i18` ще разпознае английските термини в китайски, японски и корейски документи и ще остави термините непокътнати.
