...
Тази статия ще сподели техническото изпълнение на `i18n.site` търсене в [i18n.site](//i18n.site) край.
...
Предайте масива от документ `url` и номер на версия `ver` и потърсете дали документът съществува в таблица `doc` Ако не съществува, създайте обърнат индекс. В същото време премахнете обърнатия индекс за тези документи, които не са предадени.

По този начин може да се постигне постепенно индексиране и да се намали количеството на изчислението.

При взаимодействие в предния край лентата за [напредъка](//juejin.cn/post/7413586285954154522) на зареждането на индекса може / се покаже [,](//dev.to/i18n-site/a-single-progress-uses-pure-css-to-achieve-animation-effects-2oo) css да се избегне забавянето при зареждане за първи progress + .
