...
* `i18n.addon/toc` :
  Mamorona fanondroana lahatahiry `json` mifototra amin'ny `TOC` , alefa amin'ny alàlan'ny default
...
Ny baikon'ny dikanteny `i18n.site` dia hametraka ny rakitra fivoriamben'ny fonosana plug-in ary avy eo dia manatanteraka izany.
...
Ny fampandehanana `js` naorina dia mifototra amin'ny fampandrosoana faharoa : [boa](https://github.com/boa-dev/boa)

* `wPath(path, txt)` : Soraty amin'ny rakitra
* `rTxt(path)` :
* `rBin(path)` : Vakio ny rakitra binary
* `rDir(dirpath)` : Vakio ny lahatahiry, ny sanda miverina dia lisitry : , lisitry ny rakitra
