...
Mampiseho CPU Modely `AMD EPYC 9224 24-Core Processor` , isa tokana 1285
...
Ny zavatra voalohany IP ny manitsy ny `~/.ssh/config` an'ny milina eo an-toerana mba hametrahana alias, ary avy eo dia ampiasao `ssh-copy-id root@c2` hanamboarana ny fidirana tsy misy tenimiafina.
...
Ny tambajotra avy any Eoropa mankany Shina dia tsy miorina `ssh` [mosh](//mosh.org)

Mamorona script `c0` ao amin'ny lahatahiry eo an-toerana `~/.bin` (ampiana indrindra amin'ny fari-piainana `PATH` ) toy izao manaraka izao:
...
Aorian'ny famoronana dia omeo alalana azo tanterahina sy rohy malefaka `c1` sy `c2` .
...
Amin'ny ho avy, azonao atao ny miditra mivantana `c0` sy `c1` hampiasa fifandraisana mivantana `mosh` haingam-pandeha ary miditra [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
ssh fiovana amin'ny fanondro, ampiasao `ssh-keygen -R ip` hanadiovana azy
...
Raha mpanao fahalavorariana ianao dia azonao atao ihany koa ny manao famatrarana kapila feno amin'ny tanana Ny baiko dia toy izao:
...
[snapper](https://manual.siduction.org/sys-admin-btrfs-snapper_en.html) no mpiara-miasa tsara indrindra amin'ny backup Btrfs .

Ny script fanombohana dia hamorona ny lahatahiry `~/ubuntu` , araho fotsiny ity baiko manaraka ity:
...
Ampiasao `snapper list-configs` raha hijerena ny subvolume izay ametrahana sary.

![](https://i-01.eu.org/1718302689.avif)

Mampiasà `snapper -c mnt list` raha hijerena saripika amin'ny subvolume iray

`/etc/snapper/configs/mnt` dia afaka mijery ny firafitry ny backup manokana amin'ity subvolume ity
...
Ataovy ao an-tsaina fa ireo hetsika etsy ambony dia atao amin'ny rafi-pamonjena, noho izany dia tsy hisy fiantraikany ireo fanovana ireo raha tsy averina averina ilay rafitra ary tsy ao anatin'ny fomba famonjena intsony.

Midika izany fa tsy ho hitanao avy hatrany izay fiovana amin'ny diary efa misy izay lasa maharitra, fa ny diary rehetra avoaka aorian'izany dia hitohy.
...
Nampiasa hetsika github aho mba hanaovana famoriam-bola mandeha ho azy sy script deployment taloha, saingy hitako fa tsy mety ho ssh ny serivisy fametrahana.

Ny fidirana ao amin'ny rafitra mpamonjy voina sy ny fijerena ny diary dia olana fahazoan-dàlana:
...
Taorian'ny fanadihadiana miverimberina dia github action `1001` ny mpampiasa `uid` amin'ny tar

tar famoahana dia tsy maintsy atao raha tsy misy paramètre `--no-same-owner` , dia ho `uid` ho an'ny mpampiasa `1001` izany

Vokatr'izany dia hanova ny vondrona mpampiasa ny lahatahiry fototra i rsync, ary avy eo dia tsy ho afaka miditra amin'ny ssh ianao rehefa manomboka.
