...
ستشارك هذه المقالة في تقديم تنفيذ تقني للبحث الكامل في الواجهة الأمامية لـ `i18n.site`، يمكنك تجربة تأثير البحث بزيارة [i18n.site](//i18n.site).

الcode مفتوح المصدر: [نواة البحث](//github.com/i18n-site/ie/tree/main/qy) / [واجهة التفاعل](//github.com/i18n-site/plugin/tree/main/qy)

## مراجعة لحلول البحث عن النص الكامل بدون خادم

بالنسبة لمواقع الويب الثابتة البحتة الصغيرة والمتوسطة الحجم، مثل المستندات/المدونات الشخصية، يعد إنشاء واجهة خلفية للبحث عن النص الكامل أمرًا ثقيلًا جدًا، ويعتبر البحث عن النص الكامل دون خدمة هو الخيار الأكثر شيوعًا.

تتكون حلول البحث الكامل بدون خدمة من فئتين رئيسيتين:

الأولى، مثل [algolia.com](//algolia.com)، حيث يقدم مزودو خدمات البحث من الجهات الخارجية مكونات البحث الكامل في الواجهة الأمامية.

تتطلب هذه الخدمات دفعًا بناءً على كمية البحث، وغالبًا ما تكون غير متاحة في الصين القارية بسبب قضايا الامتثال لموقع الويب.

لا يمكن استخدامه دون الاتصال بالإنترنت، ولا يمكن استخدامه على الإنترانت، وله قيود كبيرة. هذه المقالة لا تناقش الكثير.

الثانية هي البحث الكامل في الواجهة الأمامية.

في الوقت الحالي، هناك بعض أدوات البحث الكامل في واجهة المستخدم الأمامية مثل [lunrjs](//lunrjs.com) و [ElasticLunr.js](//github.com/weixsong/elasticlunr.js) (مبني على `lunrjs` كتطوير ثانوي).

`lunrjs` يوفر طريقتين لإنشاء الفهارس، ولكل منهما مشاكله الخاصة.

1. ملفات الفهرس المعدة مسبقًا

   ولأن الفهرس يحتوي على كلمات من كافة المستندات، فهو كبير الحجم.
...
[fusejs](//www.fusejs.io)، يستخدم لحساب تشابه النصوص بهدف البحث.

هذا الحل يعاني من أداء ضعيف جدًا وغير مناسب للبحث الكامل في النصوص (انظر [Fuse.js: استعلامات طويلة تستغرق أكثر من 10 ثوانٍ، كيف يمكن تحسينها؟](//stackoverflow.com/questions/70984437/fuse-js-takes-10-seconds-with-semi-long-queries)).

[TinySearch](//github.com/tinysearch/tinysearch)، يستخدم مرشح Bloom للبحث،但它 لا يدعم البحث بناءً على البادئة (مثل إدخال `goo` للبحث عن `good` أو `google`)، و لا يمكنه تحقيق تأثير الإكمال التلقائي.

نظرًا للعيوب الموجودة في الحلول الحالية، قام `i18n.site` بتطوير حل جديد للبحث الكامل في الواجهة الأمامية ويتميز بالخصائص التالية:
...
في التفاعل الأمامي، يمكن عرض شريط التقدم في تحميل الفهرس لتجنب الشعور بالتأخير عند التحميل لأول مرة. انظر إلى "شريط التقدم المزخرف، مبنى على تطبيق progress الواحد + Pure CSS" [بالإنجليزية](//dev.to/i18n-site/a-single-progress-uses-pure-css-to-achieve-animation-effects-2oo) / [بالصينية](//juejin.cn/post/7413586285954154522).

### IndexedDB الكتابة المتزامنة عالية

تم تطوير هذا المشروع بناءً على حزمة [idb](//www.npmjs.com/package/idb) التي توفر تغليفًا غير متزامن لـ IndexedDB.
...
البحث الكامل المقلوب في الواجهة الأمامية فقط، دون الحاجة إلى خادم، مما يجعله مناسبًا جدًا للمواقع الصغيرة والمتوسطة الحجم مثل الوثائق والمدونات الشخصية.

`i18n.site` يقدم بحثًا أماميًا مفتوح المصدر تم تطويره ذاتيًا، صغير الحجم وسريع الاستجابة، ويحل مشاكل البحث الكامل الحالي في الواجهة الأمامية ويقدم تجربة مستخدم محسنة.