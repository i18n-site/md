...
على سبيل المثال، توفر [华为云DNS](https://www.huaweicloud.com) تحليلًا إقليميًا مجانيًا، حيث يمكن لحركة المرور في البر الرئيسي الصيني أن تمر عبر百度智能云، ويمكن أن تمر حركة المرور الدولية عبر `cloudflare`.

`cloudflare` يحتوي على العديد من الأكوام، وهنا بعض النقاط التي يجب ملاحظتها:

### اسم المجال المستضاف في `DNS` أخرى، وكيفية استخدام `cloudflare`
...
نظرًا لأنه لا يمكن الوصول إلى مخزن الكائنات `R2` التابع لـ `cloudflare` من خلال اسم نطاق مخصص، لذا يلزم استخدام مخزن كائنات خارجي لتحميل الملفات الثابتة.

نحن هنا نأخذ [backblaze.com](https://www.backblaze.com) لتوضيح كيفية ربط كائنات الطرف الثالث ليتم تخزينها عند `cloudflare`.

في `backblaze.com`، أنشئ مجموعة وقم بتحميل أي ملف، وانقر لتصفح الملف، واحصل على اسم المجال `Friendly URL`، وهو `f003.backblazeb2.com` هنا.
