...
Nakon pokretanja, datoteke prijevoda i predmemorije bit će generirane. Ne zaboravite `git add . ` dodati u repozitorij u direktoriju `md` .
...
`dev` je skraćenica od `development` , koja označava razvojno okruženje, koje se koristi za lokalni pregled, a također je i zadana konfiguracijska datoteka.
...
Kada objavljujete na temelju paketa `npm` , **budite sigurni da koristite `//unpkg.com/` za** prefiks `v:` Vrijednost `i18n.site` je posebno optimizirala vrijeme predmemorije `/.v` pod ovom stazom prefiksa kako bi se postiglo pravovremeno pregledavanje novih izdanja.
...
Možete [rclone](https://rclone.org) prijaviti na `S3` poslužitelj, zatim pogledati i modificirati sljedeću skriptu i kopirati samo inkrementalne promjene u `S3` za svako izdanje.
...
Zatim upotrijebite pohranu objekata dobavljača u oblaku u kontinentalnoj Kini + Postavite sljedeći sadržaj `CDN` `out/ol/htm`
...
Dodajte naziv domene u [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) i usmjerite naziv domene `*.xxx.com` do `CNAME` u Alibaba Cloud `CDN` !
