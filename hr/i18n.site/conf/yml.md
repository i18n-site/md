...
Osim postavki [`i18`](/i18) , `ignore:` i `i18n:` , konfiguracijska datoteka je sljedeća:
...
Ako se pristupi određenoj stazi, a njezin prefiks staze nije konfiguriran u `nav:` , datoteka `MarkDown` koja odgovara stazi bit će učitana prema zadanim postavkama i prikazana pomoću predloška `Md` .

Na primjer, ako se pristupi `/test` , a `nav:` je konfiguriran bez ove staze, a jezik stranice je engleski (kod `en` ), `/en/test.md` će se učitati prema zadanim postavkama i prikazati pomoću predloška `Md` .
...
`blog` iznad je način rada s jednom stavkom od `Doc` .
...
`i18n.site` će izvršiti `js` dodatak `.i18n/hook/after.tran/TOC.js` u demo skladištu za čitanje indeksne datoteke `doc` direktorija koja odgovara konfiguraciji `TOC` predloška za generiranje obrisa `json` direktorija.

Ako koristite predložak `doc` , morate imati ovaj dodatak.

Ako inicijalizirate projekt `i18n.site` iz prazne mape, ne zaboravite kopirati demo projekt `.i18n` u svoj direktorij.

Predložak `Doc` prikazat će obris tablice sadržaja na temelju generiranog `json` .
...
Ovo odgovara prvom retku `en/demo2/README.md` :
