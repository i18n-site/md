...
Komentari prijevoda moraju navesti jezik nakon ` ``` ` , kao ` ```rust ` je :
...
Podržava konfiguraciju zaglavlja statičnog bloga tipa [HOGO](https://github.com/gohugoio/hugo) , koji prevodi samo polja `title` , `summary` i `description` .
...
`ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` na japanski :

![](//p.3ti.site/1720199391.avif)

`Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` na francuski :
...
`ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` na japanski :
