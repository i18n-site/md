...
`i18` alat naredbenog retka za prevođenje Markdown & Yaml
...
Podržava prijevod Markdown u HTML HTML `MarkDown` sadržaj u `<code>` oznakama `<pre>` ne prevode
...
Trenutno podržava prijevod komentara `toml` , `yaml` , `json5` , `go` , `rust` , `c` , `cpp` , `java` `js` `coffee` `python` , `bash` , `php` [i druge jezike](https://github.com/i18n-site/rust/blob/main/getc/src/style.rs#L14) .
...
Za programere koji su upoznati s `git` , `vim` , `vscode` , korištenje ovih alata za uređivanje dokumenata i upravljanje verzijama nedvojbeno će povećati troškove učenja.

`KISS` ( `Keep It Simple, Stupid` ) Među onima koji vjeruju u načelo, rješenja na razini poduzeća sinonim su za glomazna, neučinkovita i teška za korištenje.
...
Razvili smo novu generaciju tehnologije prevođenja koja integrira tehničke prednosti tradicionalnih modela strojnog prevođenja i velikih jezičnih modela kako bi prijevodi bili točni, glatki i elegantni.

Slijepi testovi pokazuju da je naša kvaliteta prijevoda znatno bolja u usporedbi sa sličnim uslugama.

Da bismo postigli istu kvalitetu, količina ručnog uređivanja koju zahtijevaju Google Prevoditelj i `ChatGPT` je `2.67` puta i `3.15` puta veća od naše.
...
## Podržite Prijevod `YAML`

Alat prevodi samo vrijednosti iz rječnika `YAML` a ne ključeve iz rječnika.

Na temelju `YAML` Prijevoda možete jednostavno izgraditi međunarodna rješenja za različite programske jezike.

## Podrška Za Prijevod `HOGO` Konfiguracija Zaglavlja

Podržava konfiguraciju [HOGO](https://github.com/gohugoio/hugo) statičnog bloga, samo su polja `title` , `summary` i `description` prevedena.

## Nemojte Prevoditi Nazive Varijabli

`Markdown` Koristi se kao predložak e-pošte, `YAML` kao konfiguracija jezične datoteke, postojat će varijabilni parametri (na primjer: nadoplata `${amount}` je primljena).

`${varname}` naziv varijable neće se smatrati engleskim prijevodom.
...
Kina, Japan i Koreja nemaju velika ili mala slova, ali naslovi na engleskom jeziku uglavnom imaju veliko prvo slovo.

`i18` prepoznaje naslove u `MarkDown` i automatski će napisati veliko slovo prilikom prevođenja na jezik koji razlikuje velika i mala slova.

Na primjer `为阅读体验而优化` će se prevesti kao `Optimized for Reading Experience` .
...
Ako koristite Google ili DeepL, oba netočno prevode engleske izraze koji bi trebali ostati izvorni (uzmite japanski i francuski kao primjer).
...
`Falcon` se prevodi kao `faucon` , a `Llama` se prevodi kao `lama` . Kao vlastite imenice, ne bi se trebale prevoditi.
...
DeepL Prijevod navedenog na francuski (također prepisivanje vlastitih imena i dodavanje neparnih `... ` ):
...
`i18` će prepoznati engleske izraze u kineskim, japanskim i korejskim dokumentima i ostaviti ih netaknutima.
...
## Može Se Kombinirati `i18n.site` Izrada Višejezične Web Stranice

Integriran `i18` u alat naredbenog retka za izradu višejezičnih web stranica [`i18n.site`](/i18n.site) .

Pogledajte dokumentaciju za [`i18n.site`](/i18n.site)

## Kod Otvorenog Koda

Klijent je potpuno otvorenog koda, a poslužiteljska strana je otvorenog koda 90 [Kliknite ovdje da vidite izvorni kod](/i18n.site/src) .
...
Također [i18n-site.bsky.social](https://bsky.app/profile/i18n-site.bsky.social) da pratite naše društvene / [X.COM: @i18nSite](https://x.com/i18nSite)
