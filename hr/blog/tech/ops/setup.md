...
Zaslon CPU Model `AMD EPYC 9224 24-Core Processor` ocjena izvedbe s jednom jezgrom 1285
...
Prva stvar je urediti lokalni stroj `~/.ssh/config` Postavite alias za IP a zatim upotrijebite `ssh-copy-id root@c2` za konfiguraciju prijave bez lozinke.
...
Mreža od Europe do Kine nije stabilna `ssh` Dođite da ubrzate [mosh](//mosh.org)

Stvorite skriptu `c0` u direktoriju `~/.bin` (uglavnom dodan u varijablu okruženja `PATH` ) na sljedeći način:
...
Nakon stvaranja, dodijelite izvršne dozvole i meku `c2` `c1`
...
Ubuduće možete izravno unijeti `c0` , `c1` i možete koristiti izravnu `mosh` velike brzine i unijeti [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Promjene otisaka prstiju dogodit `ssh-keygen -R ip` se kada ssh
...
Ako ste perfekcionist, možete izvršiti punu kompresiju diska kako slijedi
...
Skripta za inicijalizaciju će stvoriti direktorij `~/ubuntu` , samo pokrenite sljedeću naredbu:
...
Upotrijebite `snapper -c mnt list` za prikaz snimke podvoluma

`/etc/snapper/configs/mnt` Možete vidjeti specifične postavke sigurnosne kopije ovog podskupina
...
Međutim, granularnost ove snimke je prevelika. Nakon vraćanja novododani podaci bit će potpuno izgubljeni, stoga ih koristite s oprezom.
...
Omogućavanje postojanosti dnevnika pod sustavom za spašavanje zahtijeva da ručno kreirate odgovarajući direktorij i izvršite neke dodatne korake, jer možda nemate pokrenutu uslugu systemd. Evo koraka koje možete isprobati:
...
Ponovo pokrenite svoj Ubuntu sustav: Nakon što izvršite gore navedene promjene, morate ponovno pokrenuti svoje računalo normalno i na svoj Ubuntu sustav umjesto na sustav za spašavanje.

Na taj način, systemd-journald će započeti s novom konfiguracijom i početi trajno pohranjivati ​​zapise na tvrdi disk.

Imajte na umu da se gore navedene operacije izvode na sustavu za spašavanje, tako da će te promjene stupiti na snagu tek kada se sustav ponovno pokrene, a ne više u načinu za spašavanje.

To znači da nećete odmah vidjeti da promjene postojećih zapisa postaju postojane, ali će svi zapisi generirani nakon toga biti postojani.
...
tar Raspakiranje se mora obaviti bez parametara `--no-same-owner` bit će dekomprimirano u `uid` za `1001` korisnika
