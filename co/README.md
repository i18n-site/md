<h1 style="display:flex;justify-content:space-between">i18n.site ⋅ Soluzioni internaziunali<img src="//p.3ti.site/logo.svg" style="user-select:none;margin-top:-1px;width:42px"></h1>
...
Ancu s'ellu u navigatore hà una traduzzione integrata, i mutori di ricerca ùn ponu ancu interrogà in tutte e lingue.
...
## <a rel=id href="#project" id="project"></a> Descrizzione di u prugettu

Stu situ furnisce attualmente dui strumenti di linea di cummanda open source:

* [i18 : MarkDown di traduzzione in linea di cumanda](/i18/feature)
...
Traduce `Markdown` è `YAML` in un strumentu di linea di cummanda ( [codice fonte](https://github.com/i18n-site/rust/tree/main/i18) ) in parechje lingue.
...
Spartemu " filusufìa `UNIX` Everything is a File " è ponu gestisce traduzioni in centinaie di lingue senza introduzione di soluzioni cumplesse è ingombranti à livellu di l'impresa.

Pudete utilizà l'arnesi più familiari per `Markdown` a traduzzione (ma ùn pudete micca aghjunghje o sguassate paragrafi), è utilizate a manera più familiare per fà u cuntrollu di versione.

Una basa di codice pò esse creata cum'è una fonte aperta per i schedarii di lingua, è cù l'aiutu `Pull Request` u prucessu di l'utilizatori glubale pò participà à l'ottimisazione cuntinua di e traduzioni. Cunnessione senza saldatura [github](//github.com) altre cumunità open source.
...
Puramente statica, ottimizzata per l'esperienza di lettura, è integrata cù a traduzzione di [i18](#i18) hè a megliu scelta per custruisce un situ di documentu di prughjettu.

U framework di front-end sottostanti adopta una architettura plug-in cumpleta, chì hè faciule per u sviluppu secundariu Se necessariu, e funzioni back-end ponu esse integrate.

Stu situ web hè sviluppatu basatu annantu à questu quadru è include l'utilizatori, u pagamentu è altre funzioni ( [codice fonte](/i18n.site/src) ).

[→ Per a guida d'utilizatore, leghjite a documentazione di u prugettu](/i18n.site) .
