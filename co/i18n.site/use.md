...
Se aduprate un ambiente d'integrazione cuntinuu per publicà, ùn ci hè bisognu di installà `nodejs` , basta copià u permessu logged in è pubblicà `~/.npmrc` à l'ambiente.

Se mudificà u nome di u pacchettu di `v:` in `ol.yml` , **assicuratevi di sguassà prima `.i18n/v/ol`** è poi di publicà.
...
Dopu avè vintu u nome di duminiu persunalizatu, andate à u nome di duminiu per cunfigurà a riscrittura di u percorsu di l'applicazione di una sola pagina, cum'è mostra quì sottu:
...
Allora, aduprate l'almacenamentu di l'ughjettu di `out/ol/htm` venditori di nuvola in Cina continentale + Impulsate u cuntenutu seguente `CDN` .
