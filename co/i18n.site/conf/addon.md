...
* `lang_li` Lista : lingue, a prima lingua hè a lingua fonte
...
`file` hè a lista di u fugliale di output, `path` hè u percorsu di u schedariu, è `txt` hè u cuntenutu di u schedariu.

## Funzioni Integrate

U runtime integratu `js` hè basatu annantu à u sviluppu secundariu [boa](https://github.com/boa-dev/boa) è e funzioni integrate sò i seguenti :
