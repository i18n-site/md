...
In a cunfigurazione, per piacè cambià `i18n.site` à u valore di `host:` in `.i18n/htm/ol.yml` , parechje magazzini d'uggetti ponu esse cunfigurati sottu `s3` , è u campu `region` hè opzionale (assai magazzini d'uggetti ùn anu micca bisognu di stabilisce stu campu).
...
I dati sò caricati in Baidu Object Storage è ligati à Baidu Content Distribution Network.
...
## Utilizazione Avanzata: Distribuite U Trafficu Basatu Nantu À a Risoluzione Regiunale
...
Perchè `R2` `cloudflare` accede à un nome di duminiu persunalizatu, un almacenamentu d'ughjettu di terzu deve esse usatu per mette i schedari statichi.

Quì avemu [backblaze.com](https://www.backblaze.com) cum'è un esempiu per dimustrà cumu ligà l'oggetti di terzu per esse guardatu à `cloudflare` .
...
Inoltre, in a regula di cunversione `cloudflare` sopra, `index.html` hè cambiatu à `file/your_bucketname/index.html` , è altre cunfigurazioni restanu listessi.
