...
L'icona di u piè di pagina hè generata creendu un font chjamatu iconfont.cn ( [versione inglese](https://www.iconfont.cn/?lang=en-us) /[中文版](https://www.iconfont.cn/?lang=zh)`F` ).
...
Aghjunghjendu `class="a"` à u ligame hè di impedisce chì u ligame sia cunvertitu in `MarkDown` Vede :
 [➔ `YAML` Cumu impedisce : ligame `HTML` da esse cunvertitu in `Markdown`](/i18/qa#H2) .