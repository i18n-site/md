# Caratteristica Di U Pruduttu

## Traduzzione Integrata `I18`

U prugramma hà una traduzzione `i18` , vede [➔ `i18`](/i18) per un usu specificu.
...
Per `i18n.site` , u cuntenutu di u situ serà publicatu à `npmjs.com` , cù l'aiutu [jsdelivr.com](//jsdelivr.com) u cuntenutu `CDN` [unpkg.com](//unpkg.com) `npm` .
...
U principiu hè: intercepte a dumanda cù l'aiutu di [`service worker`](https://developer.mozilla.org/docs/Web/API/Service_Worker_API) , se a dumanda falla, riprova nantu à l'altru `CDN` , è attivate in modu adattatu a stazione d'origine chì risponde più veloce cum'è a fonte di carica predeterminata.
...
> Abbanduna a decorazione innecessaria è presenta u cuntenutu in a so forma più pura.
...
### Caricate Fonti in Linea, Sustene U Cinese
...
Scroll up and the hidden navigation apparirà di novu.
...
Quandu scorri u cuntenutu à a diritta, u cuntornu à a manca mette in evidenza simultaneamente u capitulu di lettura attuale.
...
### `404` U Picculu Fantasma

Ci hè un simpaticu fantasma flottante nantu à a pagina `404` i so ochji si movenu cù u mouse, [➔ Cliccate quì per vede](/404) ,
