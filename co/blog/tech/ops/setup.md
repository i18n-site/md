...
Mostra u mudellu `AMD EPYC 9224 24-Core Processor` , puntuazione di rendimentu unicu core 1285 CPU
, score multi-core 4277 Vede più risultati [vpsbenchmarks.com](https://www.vpsbenchmarks.com/yabs/contabo-6c-16gb-3fb7d8)

Aghju compru trè servitori in un tempu è attivatu à u stessu tempu Questu assicura chì i servitori sò in a listessa sala di l'informatica, facenu più faciule per custruisce una alta dispunibilità in u futuru.
...
A prima cosa IP di edità `~/.ssh/config` di a macchina locale per stabilisce un alias, è dopu aduprà `ssh-copy-id root@c2` per cunfigurà login senza password.
...
Dopu aghjurnà u sistema à ubuntu 24 cun un clic è installate u software cumunimenti utilizatu.
...
Stu passu pigghia assai tempu Dopu à aspittà per una meza ora, l'aghjurnamentu è a stallazione di u software cumunimenti utilizatu serà successu. Pudete cunfirmà questu cù u cumandimu seguitu:
...
A reta da l'Europa à a Cina ùn hè micca `ssh` [mosh](//mosh.org)

Crea `c0` scripts in u repertoriu lucale `~/.bin` (principalmente aghjuntu à a variabile d'ambiente `PATH` ) cum'è seguente:
...
Dopu a creazione, cuncede permessi eseguibili è soft-link `c1` è `c2` .
...
In u futuru, pudete inserisce direttamente `c0` è `c1` per aduprà `mosh` cunnessione diretta à alta velocità è entre [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
ssh saranu cambiamenti di impronte digitali, utilizate `ssh-keygen -R ip` per pulisce
...
Cumpresserà automaticamente per automaticamente U cumandimu per verificà u rapportu di compressione hè u seguente:
...
Sè vo site un perfeccionista, pudete ancu eseguisce una cumpressione di discu sanu manualmente.

```bash
btrfs filesystem defragment -r -czstd /
```

Cum'è mostra in a figura sottu, pudete vede chì dopu a compressione manuale in u novu sistema, quasi 100MB più hè stata cumpressata.
...
U script d'inizializazione creà u cartulare `~/ubuntu` , basta à eseguisce u cumandimu seguente:
...
Aduprate `snapper list-configs` per vede i subvolumi per i quali snapshots sò stabiliti.

![](https://i-01.eu.org/1718302689.avif)

Aduprate `snapper -c mnt list` per vede una snapshot di un subvolume

`/etc/snapper/configs/mnt` pò vede i paràmetri di salvezza specifiche di stu subvolume
...
Vede u testu difficu specificu
...
Tenite in mente chì l'operazioni di sopra sò realizate nantu à un sistema di salvezza, perchè questi cambiamenti ùn anu micca effettu finu à chì u sistema hè riavviatu è ùn hè più in modu di salvezza.

Questu significa chì ùn vede micca immediatamente cambiamenti à i logs esistenti chì diventanu persistenti, ma tutti i logs generati dopu seranu persistuti.
...
Dopu investigazioni ripetute, hè github action l'Usuariu `uid` di tar hè `1001`

Unpacking deve esse s'ellu ùn ci hè micca paràmetru `--no-same-owner` , serà decompressu à `uid` per `1001` utilizatori tar

In u risultatu, rsync hà da cambià u gruppu d'utilizatori di u repertoriu radicali, è ùn puderete micca login in ssh dopu à riavvia.
