...
I cumenti di traduzzione anu da indicà a lingua dopu à ` ``` ` , : ` ```rust ` .
...
Supporta a cunfigurazione di [HOGO](https://github.com/gohugoio/hugo) intestazione di u blog static, chì traduce solu i campi `title` , `summary` è `description` .
...
`Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` in francese :
...
Traduzzione di u sopra in francese (riscrive ancu i nomi propiu è aghjunghjendu u disparu `... ` ) DeepL
