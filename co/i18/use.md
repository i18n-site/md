...
[i18n.site/payBill](//i18n.site/payBill) , avete bisognu di ligà una carta di creditu per u pagamentu (ùn hè micca necessariu ricaricamentu, u situ web deduce automaticamente i tariffi secondu l'usu, [vede a pagina iniziale per i prezzi](/#price) ).
...
Differenti `fromTo` ponu esse cunfigurati per i schedarii diversi Una dimostrazione hè scritta cum'è : /
...
In questa tabella di cunfigurazione, a lingua fonte per a traduzzione di u repertoriu `blog` hè `zh` , è a lingua fonte per a traduzzione di u repertoriu `blog/your_file_name.md` hè `ja` .

### Ignore U Schedariu

Per automaticamente, tutti i fugliali chì cumincianu cù `.md` è `.yml` in u cartulare di a lingua fonte seranu tradutti.

Se vulete ignurà certi schedari è ùn li traduce micca (cum'è abbozzi infiniti), pudete aduprà a cunfigurazione di u campu `ignore` .
...
L'uttellu di linea di cumanda truverà tutti i fugliali chì finiscinu cù `.yml` in u cartulare di u schedariu di a lingua fonte è li traduce.
...
L'uttellu di linea di cummanda truverà `.i18n/conf.yml` cunfigurazioni in tutti i subdirectorii è li traduce.
