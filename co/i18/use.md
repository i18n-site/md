...
windows System, [cliccate quì per scaricà è installà `git bash`](https://git-scm.com/download/win) prima.
...
Pudete cunfigurà `fromTo` differenti per i schedarii diversi Una dimostrazione hè scritta cum'è : /
...
In questa tavula di cunfigurazione, a lingua fonte di a traduzzione di u catalogu `blog` hè `zh` , è a lingua fonte di a traduzzione di `blog/your_file_name.md` hè `ja` .
...
`ignore` usa a stessa [globset](https://docs.rs/globset/latest/globset/#syntax) cum'è u schedariu `.gitignore` .
