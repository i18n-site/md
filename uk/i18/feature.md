...
Підтримує конфігурацію заголовка типу [HOGO](https://github.com/gohugoio/hugo) static blog, яка перекладає лише поля `title`, `summary` і `description`.
...
Перекладено на японську: `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす`:

![](//p.3ti.site/1720199391.avif)

Перекладено на французьку: `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique`:
...
Перекладено на японську: `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す`:
...
## Можна поєднувати з більш ніж `i18n.site` мовами для створення багатомовного веб-сайту
