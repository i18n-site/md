...
`i18` може розпізнавати заголовки в `Markdown` і автоматично资本化首字母 при перекладі на мови, що розрізняють регістр.
...
Перекладено на `ファルконがラマに勝利、ハグ顔ランキングが物議を醸す` мову :
...
`i18` розпізнає англійські терміни в китайських, японських і корейських документах і залишить терміни без змін.
