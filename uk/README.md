...
Через соціальні медіа та електронну пошту люди звикли зосереджуватися на джерелах інформації рідною мовою.
...
## <a rel=id href="#project" id="project"></a> Опис проекту
...
Перекладіть `Markdown` і `YAML` в інструмент командного рядка ( [вихідний код](https://github.com/i18n-site/rust/tree/main/i18) ) кількома мовами.
...
Ми поділяємо `UNIX` Everything is a File " і можемо керувати перекладами сотнями мов " не вводячи складних і громіздких рішень корпоративного рівня.

Ви можете використовувати найвідоміші інструменти для `Markdown` перекладу (але ви не можете додавати або видаляти абзаци) і використовувати найвідоміший спосіб для керування версіями.

Кодову базу можна створити як відкритий код для мовних файлів, і за допомогою процесу `Pull Request` користувачі можуть брати участь у безперервній оптимізації перекладів. Безперебійне підключення [github](//github.com) інші спільноти з відкритим кодом.

[→ Для отримання посібника користувача прочитайте проектну документацію](/i18) .
...
Чисто статичний, оптимізований для читання та інтегрований із перекладом [i18](#i18) це найкращий вибір для створення сайту проектних документів.
...
Цей веб-сайт розроблено на основі цього фреймворку та містить функції для користувача, платежі та інші ( [вихідний код](/i18n.site/src) буде написано пізніше).

[→ Для отримання посібника користувача прочитайте проектну документацію](/i18n.site) .
...
Ми сподіваємося, що все людство об’єднається разом із спільною мрією.

Сайт перекладу та документації Markdown – це лише початок.
Синхронізувати публікацію контенту в соціальних мережах;
Підтримує двомовні коментарі та чати;
