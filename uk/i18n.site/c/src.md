# Репозиторій коду

## Фронтенд

* Головний сайт [github](https://github.com/i18n-site/site) / [atomgit](https://atomgit.com/i18n/proto)
* Документація [github](https://github.com/i18n-site/md) / [atomgit](https://atomgit.com/i18n/md)
* Незалежний компонент [github](https://github.com/i18n-site/18x) / [atomgit](https://atomgit.com/i18n/18x)
* Компонент Svelte [github](https://github.com/i18n-site/plugin) / [atomgit](https://atomgit.com/i18n/plugin)
* Інтерфейс [github](https://github.com/i18n-site/proto) / [atomgit](https://atomgit.com/i18n/proto)
* Універсальна базова бібліотека [github](https://github.com/i18n-site/lib) / [atomgit](https://atomgit.com/i18n/lib)
* Базова бібліотека браузера [github](https://github.com/i18n-site/ie) / [atomgit](https://atomgit.com/i18n/ie)
* Фронтендова бібліотека, яку можна використовувати без打包ування, [github](https://github.com/i18n-site/x) / [atomgit](https://atomgit.com/i18n/x)
  Зручне посилання безпосередньо в ініціалізаційному скрипті Markdown і одночасно використання на зібраному веб-сайті для уникнення дублювання коду.

## Бекенд

Для побудови документного сайту потрібен лише фронтенд, без бекенду.

Цей бекенд є власним бекендом сайту i18n.site, що включає системи користувачів, платежів, відправлення повідомлень тощо.

* Сервіс [github](https://github.com/i18n-api/srv) / [atomgit](https://atomgit.com/i18n-api/srv)
* Плагін [github](https://github.com/i18n-api/pub) / [atomgit](https://atomgit.com/i18n-api/pub)
* Базова бібліотека [github](https://github.com/i18n-site/rust) / [atomgit](https://atomgit.com/i18n/rust)

## Розробка

* Локальний контейнер [github](https://github.com/i18n-api/srv.docker) / [atomgit](https://atomgit.com/i18n-api/srv.docker)

## Експлуатація та обслуговування

* Скрипт онлайн-розгортання [github](https://github.com/i18n-ops/ops) / [atomgit](https://atomgit.com/i18n-ops/ops)
* Налаштування системи Ubuntu [github](https://github.com/i18n-ops/ubuntu) / [atomgit](https://atomgit.com/i18n-ops/ubuntu)
* Заплановані завдання [github](https://github.com/i18n-cron/cron) / [atomgit](https://atomgit.com/i18n/cron)

## Технологічний стек

### Фронтенд

* [svelte](//svelte.dev)
* [coffeescript](//coffeescript.org)
* [pug](https://github.com/pugjs/pug)
* [stylus](https://stylus.com)

### Бекенд

* [rust](//rust.org)
  * [axum](//github.com/tokio-rs/axum)
  * [boa](//github.com/boa-dev/boa)
  * [swc](//swc.rs)
* [node](//nodejs.org)
* [bun](//bun.dev)

## Участь у розробці

Шукаємо волонтерів для участі в розробці відкритого коду та вичитку перекладених текстів.

Якщо ви зацікавлені, будь ласка [→ Натисніть тут, щоб заповнити свій профіль](https://ggl.link/i18n) , а потім приєднайтеся [до списку розсилки](https://groups.google.com/u/2/g/i18n-site) для спілкування.