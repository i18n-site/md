...
Pagkatapos, i-edit `~/.config/i18n.site.yml` at idagdag ang sumusunod na configuration :
...
Sa pagsasaayos, mangyaring baguhin `i18n.site` sa halaga ng `host:` sa `.i18n/htm/ol.yml` , maaaring i-configure ang maraming object store sa ilalim `s3` , at opsyonal ang `region` na field (maraming object store ang hindi kailangang itakda ang field na ito).

Pagkatapos ay patakbuhin `i18n.site -n` upang muling i-publish ang proyekto.

Kung binago mo ang `~/.config/i18n.site.yml` at nais mong muling i-upload, mangyaring gamitin ang sumusunod na command sa direktoryo ng ugat ng proyekto upang i-clear ang cache ng pag-upload :
...
I-click `Debug` , pagkatapos ay i-click ang I-publish sa buong network.
...
Halimbawa, `cloudflare` [ang Huawei Cloud DNS](https://www.huaweicloud.com)
...
Dahil `cloudflare` -access `R2` ng isang customized na domain name, kailangang gumamit ng third-party na object storage para maglagay ng mga static na file.

Dito namin kinuha ang [backblaze.com](https://www.backblaze.com) bilang isang halimbawa upang ipakita kung paano i-bind ang mga third-party na bagay na iimbak sa `cloudflare` .
...
Baguhin ang `cloudflare` ng `SSL` → encryption mode, itakda sa `Full`
