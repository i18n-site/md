...
Pumili ako ng isang European server Anyway, pagkatapos gumamit ng cloudflare, walang gaanong pagkakaiba sa pagitan ng mga server.
...
Ipakita ang Modelo CPU `AMD EPYC 9224 24-Core Processor` single-core na 1285 ng pagganap
...
Pagkalipas ng ilang minuto, ang lahat ng mga server ay na-activate.

Ang unang bagay ay i-edit ang lokal na makina `~/.ssh/config` Magtakda ng alias para sa IP at pagkatapos ay gamitin `ssh-copy-id root@c2` upang i-configure ang pag-login na walang password.
...
Ang network mula `ssh` Europa hanggang China ay hindi stable. Inirerekomenda na gamitin [mosh](//mosh.org)

Lumikha ng `c0` script sa lokal na makina ng `~/.bin` (pangunahing idinagdag sa environment variable `PATH` ) na direktoryo tulad ng sumusunod:
...
Pagkatapos ng paggawa, magbigay ng mga executable na pahintulot at soft- `c2` `c1`
...
Sa hinaharap, maaari mong direktang ipasok ang `c0` , `c1` at maaari mong gamitin `mosh` mataas na bilis ng direktang koneksyon at pumasok [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Ang mga pagbabago sa fingerprint ay magaganap kapag ssh , `ssh-keygen -R ip` upang linisin ito
...
Ito ay awtomatikong i-compress bilang default Ang utos upang suriin ang compression ratio ay ang mga sumusunod.
...
Kung ikaw ay isang perfectionist, maaari ka ring magsagawa ng isang buong disk compression nang manu-mano Ang utos ay ang mga sumusunod
...
Ang script ng pagsisimula ay lilikha ng `~/ubuntu` , patakbuhin lamang ang sumusunod na utos:
...
Gamitin ang `snapper list-configs` upang tingnan ang mga subvolume kung saan nakatakda ang mga snapshot.

<img alt="" src="https://i-01.eu.org/1718302689.avif">

Gamitin ang `snapper -c mnt list` upang tingnan ang isang snapshot ng isang subvolume

`/etc/snapper/configs/mnt` Maaari mong tingnan ang mga partikular na setting ng backup ng subvolume na ito
...
Ipasok ang rescue system 22 Pagkatapos ng port ay [tcping](https://github.com/paradiseduo/tcping) accessible, hindi mo ito maa-access gamit ang nakatakdang password sa loob ng 2-3 minuto.
...
Sa ganitong paraan, magsisimula ang systemd-journald sa bagong configuration at magsisimulang mag-imbak ng mga log nang tuluy-tuloy sa hard disk.

Tandaan na ang mga operasyon sa itaas ay ginagawa sa isang rescue system, kaya ang mga pagbabagong iyon ay magkakabisa lamang kapag ang system ay na-reboot at wala na sa rescue mode.

Nangangahulugan ito na hindi mo kaagad makikita ang anumang mga pagbabago sa mga umiiral nang log na magiging paulit-ulit, ngunit ang lahat ng mga log na nabuo pagkatapos nito ay magpapatuloy.
...
Gumamit ako ng github action para gumawa ng awtomatikong compilation at deployment script dati, ngunit nalaman kong hindi ma-ssh ng deployment service.
...
Pagkatapos ng paulit-ulit na pagsisiyasat, ito ay github action ang user na nag-upload tar ay `1001` `uid`

tar pag-unpack ay dapat gawin nang walang mga parameter `--no-same-owner` ito ay made-decompress sa `uid` para `1001` user
