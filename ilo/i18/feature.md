...
Dagiti komento ti panagipatarus ket kasapulan a mangipakita ti pagsasao kalpasan ` ``` ` , a kas ` ```rust ` :
...
Dikay mangnayon kadagiti entidad malaksid no kasapulan.
...
Ti ramit ket mangipatarus laeng kadagiti pateg ti diksionario iti `YAML` , saan a dagiti tulbek ti diksionario.
...
Suportaranna ti panagisaad `summary` `description` `title` [HOGO](https://github.com/gohugoio/hugo)

## Saan Nga Ipatarus Dagiti Nagan Ti Variable

`Markdown` ket maus-usar a kas ti plantilia ti email, `YAML` ket naus-usar a kas ti panagisaad ti papeles ti pagsasao, ken addanto dagiti variable a parametro (kas pagarigan: naawat ti panagkarga manen ti `${amount}` ).
...
`Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` iti Pranses :

`Falcon` ket maipatarus a kas `faucon` ken `Llama` ket naipatarus a kas `lama` . Kas proper noun, saan koma a maipatarus dagitoy.
...
Ti kliyente ket naan-anay a silulukat a taudan, ken ti sikigan 90 serbidor % ti kodigo ket silulukat a taudan [I-klik ditoy tapno makita ti taudan a kodigo](/i18n.site/src) .
