...
### `use: Md` , Eenzeg Datei Schabloun Ouni Kontur
...
### `use: Doc` , Multi-Datei Dokument Schabloun
...
Déi uewe genannte Schreifmethod ass gläichwäerteg mat `url: xxx` , a seng entspriechend Datei ass `en/xxx/TOC` .

#### TOC Inhaltsverzeechnes Index

`i18n.site` wäert `js` Plug-in `.i18n/hook/after.tran/TOC.js` am Demo Warehouse ausféieren fir d' `TOC` Verzeechnes Indexdatei ze liesen entspriechend der `doc` Schabloun Konfiguratioun fir de `json` Verzeechnes Kontur ze generéieren.
...
Den Inhalt nom éischte Colon `:` vum éischte-Niveau Titel vum Projet `README.md` gëtt als Projet Slogan ugesinn.

Benotzer aus China, Japan a Korea, notéiert w.e.g. datt Dir d'Halschent-Breet Colon `:` amplaz vun der Vollbreet Colon benotze sollt.
...
Fir e bestëmmte Wee deen zougänglech ass, wann de Wee Präfix net an `nav:` konfiguréiert ass, gëtt déi `MarkDown` Datei, déi dem Wee entsprécht, als Standard gelueden a mat der `Md` Schabloun ofgeleet.

Zum Beispill, wann `/test` zougänglech ass, an `nav:` ouni de Präfix vun dësem Wee konfiguréiert ass, an déi aktuell Browsersprooch Englesch ass (Code `en` ), gëtt `/en/test.md` par défaut gelueden a mat der Schabloun `Md` gemaach.
