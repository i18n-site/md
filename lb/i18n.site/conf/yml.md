...
Déi uewe genannte Schreifmethod ass gläichwäerteg mat `url: xxx` ze hunn, a seng entspriechend Datei ass `en/xxx/TOC` .
...
`TOC` Dateien ginn indentéiert fir d'hierarchesch Relatioun vun der Kontur unzeweisen, Multi-Level-Induzéierung z'ënnerstëtzen, an Zeilkommentarer mat `#` unzefänken.
...
Dir kënnt gesinn datt `Deme Two` säi `Your Project slogan` :
