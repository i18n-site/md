...
Nom Laafen ginn d'Iwwersetzungs- a Cachedateien generéiert. Denkt drun, `git add . ` an de Repository am `md` Verzeichnis ze addéieren.
...
`dev.yml` an `ol.yml` sinn d'Standardkonfiguratiounen.
...
`ol` `npm` d'Ofkierzung vun `online` , `-n` d'Online-Ëmfeld ugeet, déi fir offiziell Verëffentlechung benotzt gëtt.
...
Dann, konfiguréieren den Inhalt am `md/out/ol/v` Verzeichnis op den URL Präfix Wee an `v:` festgeluecht.
...
Fir den Inhalt selwer ze hosten, zousätzlech fir Ären eegene Server ze benotzen, ass eng aner gemeinsam Optioun `S3` + ze benotzen `CDN`
...
Als éischt `i18n-demo` [github fir eng Organisatioun ze kreéieren](https://github.com/account/organizations/new?plan=free) .
...
Am Verglach [cloudflare page](//pages.cloudflare.com) `github page` , bitt et Wee nei schreiwen an ass méi frëndlech fir Festland China an ass méi zougänglech Et ass recommandéiert ze benotzen.
...
Nodeems Dir de personaliséierten Domain Numm gebonnen hutt, gitt w.e.g. op den Domain Numm fir de Wee ze schreiwen vun der eenzeger Säit Applikatioun ze konfiguréieren, wéi hei ënnendrënner:

![](https://p.3ti.site/1721119320.avif)

D'Regelen am Bild hei uewen sinn wéi follegt Ersetzt w.e.g. `i18n.site` an der éischter Linn hei drënner mam Domain Numm deen Dir gebonnen hutt.
...
Well Rekord `MX` a Rekord `CNAME` kënnen net coexistéieren, wann Dir Domain Numm E-Maile gläichzäiteg wëllt kréien, musst Dir [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten) Skript op den Niveau `CNAME` an de Rekord `A` kooperéieren.
...
Füügt den Domain Numm an [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) a weist den Domain Numm `*.xxx.com` op `CNAME` an Alibaba Cloud `CDN` .
...
Füügt w.e.g. `/root/i18n/md/out/ol/htm` Konfiguratioun wéi déi `out/ol/htm` am `server` Paragraphe vun nginx
