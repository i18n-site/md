...
Gitt w.e.g. op den [github.com/i18n-site/demo.i18](//github.com/i18n-site/demo.i18) fir d'Konfiguratioun vun `i18` Iwwersetzung ze léieren.
...
Dir kënnt verschidde `fromTo` fir verschidden : konfiguréieren /
...
An dëser Konfiguratiounstabell ass d'Quellsprooch vum Katalog `blog` Iwwersetzung `zh` , an d'Quellsprooch vun `blog/your_file_name.md` Iwwersetzung ass `ja` .
...
`ignore` benotzt déi selwecht [globset](https://docs.rs/globset/latest/globset/#syntax) wéi d' `.gitignore` Datei.
...
D'Iwwersetzung ass editable. Ännert den Originaltext an iwwersetzt et nach eng Kéier, déi manuell Ännerunge vun der Iwwersetzung ginn net iwwerschriwwen (wann dëse Paragraf vum Originaltext net geännert gouf).
