...
Markdown ang pagsasalin ng mixed HTML , ang nilalaman sa mga tag `<pre>` at `<code>` na naka-embed sa HTML `MarkDown` ay hindi isinalin
...
Kailangang ipahiwatig ng mga komento sa pagsasalin ang wika pagkatapos ` ``` ` , tulad : ` ```rust ` .
...
Sinusuportahan ang configuration ng header ng [HOGO](https://github.com/gohugoio/hugo) type static na blog, na nagsasalin lamang ng mga field `title` , `summary` at `description` .
...
`i18` ay maaaring makilala ang pamagat sa `MarkDown` , at awtomatikong i-capitalize ang unang titik kapag nagsasalin sa isang case-sensitive na wika.
...
`ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` sa Japanese :
...
`ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` sa Japanese :
