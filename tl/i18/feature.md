...
`i18` ay isang command line tool para sa pagsasalin Markdown & Yaml
...
Sinusuportahan ang pagsasalin ng Markdown sa mixed HTML `<pre>` HTML sa `MarkDown` at hindi isinalin ang nilalaman sa `<code>` tag

## Maaaring Makilala Ang Mga Mathematical Formula at Laktawan Ang Pagsasalin

Kinikilala ang mga formula sa matematika at nilaktawan ang pagsasalin.
...
Kailangang ipahiwatig ` ```rust ` mga komento sa pagsasalin ang wika pagkatapos : ` ``` `

`yaml` `js` `json5` `rust` `go` `cpp` `c` `toml` `java` `coffee` `python` `bash` `php`[](https://github.com/i18n-site/rust/blob/main/getc/src/style.rs#L14)
...
Para sa mga programmer na pamilyar sa `git` , `vim` , `vscode` , ang paggamit ng mga tool na ito upang i-edit ang mga dokumento at pamahalaan ang mga bersyon ay walang alinlangan na tataas ang gastos sa pag-aaral.

`KISS` ( `Keep It Simple, Stupid` ) Sa mga naniniwala sa prinsipyo, ang mga solusyon sa antas ng negosyo ay kasingkahulugan ng pagiging mahirap, hindi mabisa at mahirap gamitin.
...
## Ang Mataas Na Kalidad Na Pagsasalin Ng Xindaya
...
Upang makamit ang parehong kalidad, ang dami ng `3.15` `2.67` manong pag-edit na kinakailangan ng Google Translate at `ChatGPT`
...
Ang tool ay nagsasalin lamang ng mga halaga ng diksyunaryo sa `YAML` , hindi ang mga susi ng diksyunaryo.
...
## Suportahan Ang Pagsasalin `HOGO`

Sinusuportahan ang pagsasaayos ng header [HOGO](https://github.com/gohugoio/hugo) static na blog, tanging ang `title` , `summary` at `description` na mga field ang isinasalin.

## Huwag Isalin Ang Mga Variable Na Pangalan

`Markdown` Ginamit bilang template ng email, `YAML` bilang configuration ng file ng wika, magkakaroon ng mga variable na parameter (halimbawa: natanggap ang recharge `${amount}` ).

`${varname}` Ang ganitong uri ng variable na pangalan ay hindi ituturing na pagsasalin sa Ingles.
...
Ang China, Japan, at Korea ay walang malalaking titik o maliliit na titik, ngunit karaniwang ginagamit ng mga pamagat sa Ingles ang unang titik.

`MarkDown` `i18`

Halimbawa `为阅读体验而优化` ay isasalin bilang `Optimized for Reading Experience` .
...
Isinalin sa `ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` :

<img alt="" src="//p.3ti.site/1720199391.avif">

Isinalin sa `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :

`Falcon` ay isinalin bilang `faucon` at `Llama` ay isinalin bilang `lama` . Bilang mga pangngalang pantangi, hindi dapat isalin ang mga ito.

<img alt="" src="//p.3ti.site/1720199451.avif">

### DeepL Pagsasalin

Isinalin sa `ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` :
...
Halimbawa `C罗` isasalin bilang `Cristiano Ronaldo` .

## Maaaring Pagsamahin `i18n.site` Pagbuo Ng Maraming Wika

`i18` Ay isinama sa command line tool para sa multi-language website building [`i18n.site`](/i18n.site) .

Tingnan ang dokumentasyon ng [`i18n.site`](/i18n.site)

## Code Open Source

Ang kliyente ay ganap na open source, at ang server 90 % ng code ay open source [Mag-click dito upang tingnan ang source code](/i18n.site/src) .
...
Mangyaring <button onclick="mailsub()">i-click ang email na ito upang mag-subscribe sa mga update ng produkto</button> at <button onclick="webpush()">paganahin ang browser push</button> .

/ [i18n-site.bsky.social](https://bsky.app/profile/i18n-site.bsky.social) i-follow ang aming mga social account [X.COM: @i18nSite](https://x.com/i18nSite)
