...
Ang profile para sa `i18n.site` ay `.i18n/conf.yml` .

Maliban sa mga setting ng [`i18`](/i18) , `ignore:` at `i18n:` , ang configuration file ay ang mga sumusunod:
...
Kabilang sa mga ito, `i18n: home` ay tumutugma sa `home: Home` sa `en/i18n.yml` .
...
### `use: Md` , Single File Template (Walang Outline)
...
Kung ang isang partikular na path ay na-access at ang path prefix nito ay hindi na-configure sa `nav:` , ang `MarkDown` file na naaayon sa path ay ilo-load bilang default at ire-render gamit ang `Md` template.

Halimbawa, kung `/test` ay na-access, at `nav:` ay na-configure nang wala ang path na ito, at ang wika ng pahina ay English (code `en` ), `/en/test.md` ay ilo-load bilang default at ire-render gamit ang template `Md` .
...
`blog` sa itaas ay ang single-item mode ng `Doc` .
...
Ang configuration ng `i18n:doc` sa `.i18n/conf.yml` ay multi-project mode.
...
`i18n.site` ay magpapatupad ng `js` plug-in `.i18n/hook/after.tran/TOC.js` sa demo warehouse upang basahin ang `doc` na file ng index ng direktoryo na naaayon sa `TOC` configuration ng template upang makabuo ng `json` balangkas ng direktoryo.
...
Ire-render ng template na `Doc` ang balangkas ng talaan ng mga nilalaman batay sa nabuong `json` .
...
Makikita mo na ang `Deme Two` ay may tagline `Your Project slogan` :

![](https://p.3ti.site/1721276842.avif)

Ito ay tumutugma sa unang hilera `en/demo2/README.md` :
...
Halimbawa, kung Chinese ang source language, `TOC` sa itaas ay `zh/blog/TOC` .
