...
Dažādiem apakšdirektorijiem var konfigurēt dažādus / Demonstrācija ir `fromTo` šādi :
...
Šajā konfigurācijas tabulā `blog` direktorija tulkojuma avota valoda ir `zh` , bet `blog/your_file_name.md` direktorija tulkojuma avota valoda ir `ja` .

### Ignorēt Failu

Pēc noklusējuma visi faili, kas sākas ar `.md` un `.yml` avota valodas direktorijā, tiks tulkoti.

Ja vēlaties ignorēt noteiktus failus un netulkot tos (piemēram, nepabeigtos melnrakstus), varat izmantot lauka konfigurāciju `ignore` .
...
`YAML` tulkojumu var mainīt arī manuāli (bet nepievienojiet un nedzēsiet tulkojumā taustiņus vai rindiņas).
