...
Déanfaidh líne ordaithe aistriúcháin `i18n.site` comhad coinbhinsiúin an phacáiste plug-in a shuiteáil agus ansin é a fhorghníomhú.
...
Tá an t-am rite ionsuite `js` bunaithe ar fhorbairt thánaisteach : [boa](https://github.com/boa-dev/boa)

* `wPath(path, txt)` : Scríobh chuig an gcomhad
* `rTxt(path)` : Léigh an téacschomhad
* `rBin(path)` : Léigh an comhad dénártha
* `rDir(dirpath)` : Léigh an t-eolaire, is é an luach tuairisceáin liosta : , liosta comhad
