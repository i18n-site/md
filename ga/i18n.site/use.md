...
Tar `md` iad a rith, ginfear `git add . ` aistriúcháin agus taisce.
...
Agus `i18n.site` ag foilsiú bunaithe ar phacáiste `npm` , bí cinnte go n- `/.v` **tú `//unpkg.com/` don** réimír de `v:` luach.
...
Is féidir leat úsáid `S3` bhaint `S3` [rclone](https://rclone.org)
...
[Cliceáil anseo github dtús chun eagraíocht a chruthú](https://github.com/account/organizations/new?plan=free) `i18n-demo`
...
I [cloudflare page](//pages.cloudflare.com) le `github page` , soláthraíonn sé athscríobh cosáin agus tá sé níos cairdiúla don mhórthír na Síne agus tá sé níos inrochtana.
...
Tar éis duit an t-ainm fearainn saincheaptha a cheangal, téigh chuig an ainm fearainn chun athscríobh cosán an fheidhmchláir aon-leathanaigh a chumrú, mar a thaispeántar thíos:

![](https://p.3ti.site/1721119320.avif)

Is iad seo a leanas `i18n.site` rialacha sa phictiúr thuas.
...
Ansin, bain úsáid as stóráil réad na ndíoltóirí scamall sa tSín mórthír + Imscaradh an t-ábhar seo `out/ol/htm` `CDN`
...
Toisc nach féidir taifead `MX` agus taifead `CNAME` a bheith ar `A` `CNAME` , más mian leat ríomhphoist ainm fearainn a fháil ag an am céanna, ní mór duit comhoibriú leis an script [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten)
...
Cuir an t-ainm fearainn in [Alibaba Cloud CDN](https://cdn.console.aliyun.com/domain/list) agus cuir an t-ainm fearainn `*.xxx.com` go `CNAME` in Alibaba Cloud `CDN` .
...
Cuir cumraíocht cosúil leis an méid seo a `out/ol/htm` `/root/i18n/md/out/ol/htm` san alt `server` de nginx
