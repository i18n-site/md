...
Mabitso a lifaele tseo ho lumellanoeng ka tsona ke tse latelang
...
Nako ea ho sebetsa `js` e hahelletsoeng e ipapisitse le nts'etsopele ea bobeli ea [boa](https://github.com/boa-dev/boa) mme mesebetsi e hahelletsoeng ke e latelang :

* `wPath(path, txt)` :
* `rTxt(path)` :
* `rBin(path)` :
* `rDir(dirpath)` Bala lenane, boleng ba ho khutla ke lenane : : , lenane la lifaele
