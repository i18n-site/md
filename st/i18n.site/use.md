...
`md` ho sebetsa, `git add . ` tsa phetolelo le cache li tla hlahisoa.
...
Mona, `dev` e bolela hore e hahiloe ho latela faele ea `.i18n/htm/dev.yml` ea tlhophiso.

`dev` directory :

Lenane la `htm` ke leqephe la ho kena webosaeteng.
...
Ha o hatisa ho ipapisitsoe le sephutheloana sa `npm` , **etsa bonnete ba hore o sebelisa `//unpkg.com/` bakeng sa** sehlomathiso sa boleng `/.v` `v:` `i18n.site` ena ea pele ho fihlela ho boha lintlafatso tse ncha ka nako.
...
Ho iketsetsa litaba, ntle le ho sebelisa seva sa hau `CDN` khetho e 'ngoe e tloaelehileng ke ho sebelisa `S3` + .

U ka sebelisa [rclone](https://rclone.org) ho kena ho `S3` seva, ebe u sheba le ho fetola script e latelang, 'me u kopitse feela liphetoho tse ntseng li eketseha ho `S3` bakeng sa tokollo e' ngoe le e 'ngoe.
...
Ha u phatlalatsa litaba tse hlahang sehloohong se fetileng, `out/ol/htm` e entsoe ka kopo, kenya lenane lena 'me u tsamaise :
...
Kamora ho sututsa khoutu, emela hore ho behoe `github page` ho sebetsa ka katleho (joalo ka ha ho bonts'itsoe ka tlase) pele o ka e fumana.
...
Ha e bapisoa [cloudflare page](//pages.cloudflare.com) `github page` , e fana ka mokhoa oa ho ngola hape 'me e na le botsoalle ho naha ea China mme e fumaneha habonolo.

Ho romelloa ha `cloudflare page` hangata ho ipapisitse le ho romelloa ha `github page` ka holimo.

Theha morero 'me u tlamelle ntlo ea polokelo ea `i18n-demo.github.io` ka holimo.

Mokhoa ona o bontšoa setšoantšong se ka tlase:
![](https://p.3ti.site/1721117897.avif)
![](https://p.3ti.site/1721118239.avif)

Ka kopo, tobetsa `Add Account` ho fana ka monyetla oa ho kena mokhatlong oa `i18n-demo` .
...
Kenya lebitso la domain ho [Alibaba Cloud CDN](https://cdn.console.aliyun.com/domain/list) me u supe lebitso la domain `*.xxx.com` ho ea ho `CNAME` ho Alibaba Cloud `CDN` .
...
Ka kopo `out/ol/htm` eketsa tlhophiso e ts'oanang le e latelang serapeng `/root/i18n/md/out/ol/htm` `server` sa nginx
