...
E bonts'a Mohlala `AMD EPYC 9224 24-Core Processor` , lintlha tsa ts'ebetso ea mantlha e le 'ngoe 1285 CPU
...
Ke ile ka reka li-server tse tharo ka nako e le 'ngoe' me ka li kenya tšebetsong ka nako e le 'ngoe.
...
Ntho ea pele IP ho hlophisa `~/.ssh/config` ea mochini oa lehae ho beha alias, ebe o sebelisa `ssh-copy-id root@c2` ho hlophisa ho kena ntle le password.
...
Mohato ona o nka nako e telele Ka mor'a ho ema halofo ea hora, ho ntlafatsa le ho kenya software e sebelisoang hangata ho tla atleha. O ka netefatsa sena ka taelo e latelang:
...
Marang-rang a tsoang Europe ho ea Chaena ha `ssh` tsitsa [mosh](//mosh.org)

Theha mangolo `c0` bukeng ea lehae `~/.bin` (haholo-holo e kenyellelitsoe ho feto-fetoha ha tikoloho `PATH` ) ka tsela e latelang:
...
Kamora ho bopa, fana ka litumello tse sebetsang le sehokelo se bonolo `c1` le `c2` .
...
Nakong e tlang, o ka kenya `c0` le `c1` ka kotloloho ho sebelisa likhokahano `mosh` tse lebelo le holimo ebe o kenya [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Ho tla ba ssh liphetoho tsa menoana, sebelisa `ssh-keygen -R ip` ho e hloekisa
...
Haeba u motho ea ratang phetheho, u ka etsa compression e felletseng ea disk ka letsoho.
...
Sengoloa sa ho qala se tla theha `~/ubuntu` directory, tsamaisa feela taelo e latelang:
...
Sebelisa `snapper list-configs` ho sheba li-subvolumes tseo linepe tse behiloeng bakeng sa tsona.

![](https://i-01.eu.org/1718302689.avif)

Sebelisa `snapper -c mnt list` ho sheba senepe sa molumo o monyenyane

`/etc/snapper/configs/mnt` e ka sheba litlhophiso tse khethehileng tsa "backup" tsa subvolume ena
...
Leha ho le joalo, granularity ea snapshot ena e kholo haholo Ha e se e khutlisitsoe, data e sa tsoa eketsoa e tla lahleha ka botlalo, ka kopo e sebelise ka hloko.
...
Hopola hore ts'ebetso e ka holimo e etsoa tsamaisong ea pholoso, kahoo liphetoho tseo li ke ke tsa sebetsa ho fihlela tsamaiso e tsosolosoa 'me e se e le boemong ba pholoso.

Sena se bolela hore u ke ke ua bona hang-hang liphetoho ho li-log tse seng li ntse li tsoela pele, empa lintlha tsohle tse hlahisoang ka mor'a moo li tla tsoela pele.
...
Kamora ho etsa lipatlisiso khafetsa, ke github action mosebelisi `uid` oa tar ke `1001`
