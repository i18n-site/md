...
& libeke tse markdown maloa tsa nts'etsopele, [i18n.site](//i18n.site)
...
Ea bobeli ke patlo e felletseng ea mongolo o ka pele.
...
Ka lebaka la bofokoli ba litharollo tse teng, `i18n.site` o hlahisitse tharollo e ncha e hloekileng ea ho batla mongolo o felletseng, e nang le litšobotsi tse latelang :
...
Lentsoe karolo ea `coffeescript` khoutu e tjena
...
* `word` mantsoe : id -
* `doc` : id - url - ea nomoro ea mofuta oa Tokomane
* `docWord` Lethathamo id tokomane id - lentsoe :
* `prefix` : - id
* `rindex` : id - Tokomane id : Letoto la linomoro tsa line

Feta lethathamong la tokomane `url` le ea mofuta oa `ver` , 'me u batlisise hore na tokomane e teng tafoleng `doc` Haeba e le sieo, etsa index inverted. Ka nako e ts'oanang, tlosa index ea inverted bakeng sa litokomane tse sa kang tsa fetisoa.
...
Ho totobatsa liphetho tsa lipatlisiso ho hloka ho kenya mongolo oa mantlha bocha, 'me ho tsamaisana le `service worker` ho ka qoba likopo tse pheta-phetoang tsa marang-rang.
