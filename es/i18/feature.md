...
`i18` es una herramienta de línea de comandos para traducir Markdown & Yaml
...
Admite la traducción de Markdown en HTML `<pre>` HTML en `MarkDown` y el contenido de las etiquetas `<code>` no se traducen.
...
Actualmente admite traducción de comentarios `toml` , `yaml` , `json5` , `go` , `rust` , `c` , `cpp` , `java` `js` `coffee` , `python` , `bash` , `php` [y otros idiomas](https://github.com/i18n-site/rust/blob/main/getc/src/style.rs#L14) .
...
Para los programadores que están familiarizados con `git` , `vim` , `vscode` , el uso de estas herramientas para editar documentos y administrar versiones sin duda aumentará el costo de aprendizaje.

`KISS` ( `Keep It Simple, Stupid` ) Entre los creyentes en el principio, las soluciones a nivel empresarial son sinónimo de engorrosas, ineficientes y difíciles de usar.
...
## La Traducción De Alta Calidad De Xindaya

Hemos desarrollado una nueva generación de tecnología de traducción que integra las ventajas técnicas de los modelos tradicionales de traducción automática y los grandes modelos de lenguaje para realizar traducciones precisas, fluidas y elegantes.
...
Para lograr la misma calidad, la cantidad de edición manual requerida por Google Translate y `ChatGPT` es `2.67` veces y `3.15` veces la nuestra respectivamente.
...
## ¡Soporta Traducción `HOGO`

Admite la configuración de encabezado de blog estático de tipo [HOGO](https://github.com/gohugoio/hugo) , solo se traducen los campos `title` , `summary` y `description` .

## No Traducir Nombres De Variables

`Markdown` Se utiliza como plantilla de correo electrónico, `YAML` como configuración de archivo de idioma, habrá parámetros variables (por ejemplo: se ha recibido la recarga `${amount}` ).

`${varname}` Este tipo de nombre de variable no se considerará traducción al inglés.
...
China, Japón y Corea no tienen letras mayúsculas ni minúsculas, pero los títulos en inglés generalmente escriben en mayúscula la primera letra.

`i18` puede reconocer títulos en `MarkDown` y automáticamente pondrá en mayúscula la primera letra al traducir a un idioma que distinga entre mayúsculas y minúsculas.
...
Traducido al `Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` :
...
DeepL Traducción de lo anterior al francés (reescribiendo también los nombres propios y añadiendo los impares `... ` ):
...
## ¡Se Puede Combinar `i18n.site` Creación De Sitios Web en Varios Idiomas

`i18` Está integrado en la herramienta de línea de comandos para [`i18n.site`](/i18n.site) creación de sitios web en varios idiomas.
