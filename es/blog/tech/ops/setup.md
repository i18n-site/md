...
Mostrar CPU Modelo `AMD EPYC 9224 24-Core Processor` puntuación de rendimiento de un solo núcleo 1285
...
Sus servidores ocasionalmente se cuelgan, lo que garantiza la accesibilidad del sitio web gracias a su alta disponibilidad.
...
Lo primero es editar el nombre de usuario de la máquina local `~/.ssh/config` Establezca un alias para IP y luego use `ssh-copy-id root@c2` para configurar el inicio de sesión sin contraseña.
...
Este paso lleva mucho tiempo. Después de esperar media hora, la actualización e instalación del software de uso común se realizará correctamente. Puedes confirmarlo con el siguiente comando:
...
La red de Europa a China no es estable. `ssh` Se recomienda su uso [mosh](//mosh.org)

Cree un script `c0` en el directorio `~/.bin` de la máquina local (principalmente agregado a la variable de entorno `PATH` ) de la siguiente manera:
...
Después de la creación, otorgue permisos ejecutables y enlace suave `c1` `c2` .
...
En el futuro, puedes ingresar directamente `c0` , `c1` y puedes usar `mosh` para conectarte directamente e ingresar [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Los cambios de huellas dactilares se producirán cuando ssh , utilice `ssh-keygen -R ip`
...
Si es un perfeccionista, también puede realizar una compresión completa del disco manualmente. El comando es el siguiente.
...
Luego, haga clic en Instantáneas para restaurar.
...
Tenga en cuenta que las operaciones anteriores se realizan en un sistema de rescate, por lo que esos cambios solo tendrán efecto una vez que el sistema se reinicie y ya no esté en modo de rescate.

Esto significa que no verá inmediatamente que ningún cambio en los registros existentes se vuelva persistente, pero todos los registros generados después de esto sí lo harán.
...
Puede crear este directorio con el siguiente comando
...
Después de repetidas investigaciones, es porque github action El usuario que subió tar es `1001` `uid`

tar El descomprimido debe realizarse sin parámetros `--no-same-owner` se descomprimirá en `uid` para `1001` usuario
