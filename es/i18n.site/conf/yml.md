...
El archivo de configuración para `i18n.site` es `.i18n/conf.yml`.
...
Entre ellas, `i18n: home` corresponde a `home: Home` en `en/i18n.yml`.

`en/i18n.yml` se traducirá a varios idiomas, como `zh/i18n.yml`.

<img src="//p.3ti.site/1721051689.avif" style="width:320px">

Después de completar la traducción, puede modificar los valores de los archivos `yml` de traducción, pero no debe agregar ni eliminar claves de los archivos `yml`.
...
`TOC` es la abreviatura de `Table of Contents`. Cuando se renderice esta plantilla, se mostrará el índice de este archivo `Markdown` en la barra lateral.
...
La plantilla `Md` y la plantilla `Toc` son iguales y ambas se utilizan para renderizar un archivo `Markdown` único. Pero la plantilla `Md` no muestra el índice en la barra lateral.
...
Si se accede a una ruta y su prefijo no está configurado en `nav:`, se cargará de forma predeterminada el archivo `Markdown` correspondiente a la ruta y se renderizará utilizando la plantilla `Md`.

Por ejemplo, si se accede a `/test` y la configuración `nav:` no incluye esta ruta, y el idioma de la página es inglés (código `en`), se cargará `/en/test.md` de forma predeterminada y se renderizará usando la plantilla `Md`.
...
Se indica el uso de `Doc` para la renderización de la plantilla.

La plantilla `Doc` admite la integración de varios archivos `Markdown` para generar un índice de documentos para uno o varios proyectos.
...
La escritura anterior es equivalente a `url: blog`, y su archivo correspondiente es `en/blog/TOC`.
...
El archivo de índice de directorio correspondiente a estos parámetros es:
...
`i18n.site` ejecutará el complemento `js` `.i18n/hook/after.tran/TOC.js` en el repositorio de demostración para leer el archivo de índice `TOC` correspondiente a la configuración de la plantilla `doc` y generar el índice de directorios en `json`.

Si se usa la plantilla `doc`, debe tener este complemento.
...
La plantilla `Doc` renderizará el índice de directorios basado en el `json` generado.
...
El archivo `TOC` utiliza sangrías para indicar la relación jerárquica del índice y admite sangrías de varios niveles.

##### El nivel principal debe escribir solo el título, no el contenido

En caso de sangrías de varios niveles, el nivel principal debe escribir solo el título y no el contenido, de lo contrario, se puede desordenar la maquetación.

##### Proyecto README.md

El contenido se puede escribir en el archivo `README.md` del proyecto, como `en/demo2/README.md`.

Tenga en cuenta que el contenido de este archivo no muestra un resumen del índice, por lo que se recomienda limitar su extensión y escribir una breve introducción.

###### El eslogan del proyecto

Puede ver que `Deme Two` tiene el eslogan del proyecto debajo del menú desplegable y `Your Project slogan` debajo del nombre del proyecto en el índice del catálogo.
...
##### ¿Cómo mover el TOC de manera masiva?

El archivo `TOC` debe colocarse en el directorio del idioma de origen.
