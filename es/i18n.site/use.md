...
`i18n.site` incluye una herramienta de traducción `i18` integrada. Haga [clic aquí para consultar el documento de configuración de `i18` para el token de acceso](/i18/use).
...
El nombre del directorio del clon del repositorio `demo.i18n.site` debe ser `md` para facilitar la previsualización local con `docker`.
...
Instale e inicie `docker` (el usuario de `MAC` puede usar [orbstack](https://orbstack.dev) como entorno de ejecución para `docker`).

Entonces, acceda al directorio `docker` y ejecute `./up.sh`, y luego visite [https://127.0.0.1](https://127.0.0.1) para obtener una previsualización local.

<img src="//p.3ti.site/1721104238.avif" style="width:360px">

### Publicación de contenido

`i18n.site` utiliza una arquitectura [de aplicación de una página](https://developer.mozilla.org/docs/Glossary/SPA), y la página de entrada del sitio web y el contenido del sitio web se implementan de manera independiente.
...
Para la publicación oficial, se copiarán los archivos modificados en el directorio del nuevo número de versión.
...
`ol` es la abreviatura de `online`, que indica el entorno en línea, utilizado para la publicación oficial. También es el archivo de configuración predeterminado cuando se utiliza el parámetro de línea de comando `-n` para la publicación a `npm`.
...
Luego, configure el contenido del directorio `md/out/ol/v` con la ruta del prefijo URL establecida en `v:`.
...
El sitio web se puede desplegar en cualquier lugar, [github page](https://pages.github.com) y [cloudflare page](https://pages.cloudflare.com) son buenas opciones.

Dado que el sitio web utiliza una arquitectura [de aplicación de una página](https://developer.mozilla.org/docs/Glossary/SPA), recuerde reescribir las rutas URL que no contienen `. ` a `index.html`.

La página de entrada del sitio web solo debe desplegarse una vez y no es necesario volver a desplegarla para las actualizaciones de contenido posteriores.
...
#### Desplegar en cloudflare page

[cloudflare page](//pages.cloudflare.com) es más amigable para China continental y se recomienda su uso en comparación con `github page`.
...
Después de vincular el nombre de dominio personalizado, vaya al nombre de dominio para configurar la reescritura de ruta de la aplicación de una página, como se muestra a continuación:
...
#### Optimización del despliegue de sitios web en China continental
...
Luego, utilice el almacenamiento `out/ol/htm` y el `CDN` de los proveedores de nube en China continental para desplegar el siguiente contenido.

Puede usar la informática de punta para reescribir la ruta y adaptarse a aplicaciones de una página. Por ejemplo, [Baidu Smart Cloud `CDN`](//cloud.baidu.com/product/cdn.html) se puede configurar de esta manera:
...
Dado que el registro `MX` y el registro `CNAME` no pueden coexistir, si desea recibir correos electrónicos de nombres de dominio al mismo tiempo, debe cooperar con el script [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten) para reducir el nivel `CNAME` al registro `A`.

Además, debido a que las tarifas de tráfico extranjero de los proveedores de nube en China continental son relativamente costosas, si desea optimizar los costos, puede usar [la resolución geográfica gratuita de Huawei DNS](https://support.huaweicloud.com/usermanual-dns/dns_usermanual_0041.html) y el nombre de dominio personalizado [Cloudflare for SaaS](https://developers.cloudflare.com/cloudflare-for-platforms/cloudflare-for-saas) (como se muestra a continuación) para lograrlo. Desvío de tráfico ── El tráfico en China continental se dirige a Baidu Cloud `CDN`, y el tráfico internacional se dirige a Cloudflare.

![](https://p.3ti.site/1721119788.avif)

Estas soluciones de optimización del despliegue son más complejas y se presentarán en capítulos separados en el futuro.

#### Redirección de nombres de dominio genéricos

Si utiliza `i18n.site` para generar un sitio web como su sitio web principal, generalmente necesita configurar la redirección de nombres de dominio genéricos, es decir, redirigir el acceso a `*.xxx.com` (incluido `www.xxx.com`) a `xxx.com`.
...
Por ejemplo, la configuración de redirección de nombres de dominio genérico de `*.i18n.site` en la imagen de arriba es la siguiente:
