...
La solución acordada por `i18n.site` para puntos de anclaje es insertar en `Markdown` texto del tipo `<a rel=id href="#xxx" id="xxx"></a>` para crear manualmente anclajes de navegación.

`<a rel=id href="#xxx" id="xxx"></a>`, aquí `rel=id` define el estilo de página del anclaje; reemplace `xxx` con la abreviatura en inglés de su anclaje real.

Los anclajes suelen añadirse a los títulos, por ejemplo:
...
Combinando estos dos aspectos, es posible escribir `HTML` en `Markdown` de manera sencilla para lograr diversiados efectos de visualización.
