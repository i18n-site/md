...
Pia kuna zana za kutafsiri za mstari wa amri, kama vile [translate-shell](https://github.com/soimort/translate-shell)
...
## <a rel=id href="#price" id="price"></a> Bei ya ushindani sana
...
[Bofya hapa ili kuidhinisha na i18n.site kiotomatiki github](https://github.com/login/oauth/authorize?client_id=Ov23liuGAmK0plc9FgB3&amp;scope=user:email,user:follow,public_repo) na **kupokea bonasi $50** .

Kumbuka: Idadi ya herufi zinazoweza kutozwa = [idadi ya unicode](https://en.wikipedia.org/wiki/Unicode) kwenye faili chanzo × idadi ya lugha katika tafsiri
...
`Markdown` inatumika kama kiolezo cha barua pepe, `YAML` inatumika kama usanidi wa faili ya lugha, na kutakuwa na vigezo tofauti (kwa mfano: chaji `${amount}` imepokelewa).
...
DeepL Tafsiri yaliyo hapo juu kwa Kifaransa (pia andika upya majina sahihi na kuongeza isiyo ya kawaida `... ` ):
...
Mteja ni chanzo wazi kabisa, na upande wa 90 % ya msimbo ni chanzo wazi [Bofya hapa ili kuona msimbo wa chanzo](/i18n.site/src) .
...
Tafadhali <button onclick="mailsub()">bofya barua pepe hii ili kujiandikisha kupokea masasisho ya bidhaa</button> na <button onclick="webpush()">kuwezesha kushinikiza kwa kivinjari</button> Tutakujulisha bidhaa zikisasishwa.
