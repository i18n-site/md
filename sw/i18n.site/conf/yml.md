...
Miongoni mwao, kipengee cha usanidi `upload` hadi `ext:` kinamaanisha kuwa `.md` pekee ndiyo kitapakiwa wakati wa uchapishaji.
...
`use: Toc` inamaanisha kutoa kwa kutumia kiolezo `Toc` , ambacho kinatoa kiolezo kimoja `Markdown` .
...
### `use: Md` , Kiolezo Cha Faili Moja Bila Muhtasari
...
### `use: Doc` , Kiolezo Cha Hati Zenye Faili Nyingi
...
Njia ya kuandika hapo juu ni sawa na `url: xxx` , na faili yake sambamba ni `en/xxx/TOC` .

#### Jedwali La TOC La Index Ya Yaliyomo

`i18n.site` itatumia programu- `.i18n/hook/after.tran/TOC.js` `js` kwenye ghala la onyesho ili kusoma faili `TOC` za faharasa ya saraka inayolingana na usanidi wa violezo `doc` ili kutoa muhtasari wa saraka `json` .
...
Kwa mfano, ikiwa `/test` imefikiwa, na `nav:` imesanidiwa bila kiambishi awali cha njia hii, na lugha ya sasa ya kuvinjari ni Kiingereza (msimbo wa `en` ), `/en/test.md` itapakiwa kwa chaguo-msingi na itatolewa kwa kutumia kiolezo cha `Md` .
