...
Katika usanidi, tafadhali badilisha `i18n.site` hadi thamani ya `host:` katika `.i18n/htm/ol.yml` , hifadhi nyingi za vitu zinaweza kusanidiwa chini ya `s3` , na sehemu `region` ni ya hiari (duka nyingi za vitu hazihitaji kuweka uga huu).
...
`URL redirect` Chagua uelekezaji upya unaobadilika, tafadhali rekebisha `/en` katika njia ya `concat("/en",http.request.uri.path,".htm")` ya kuelekeza upya hadi lugha chaguo-msingi unayotaka injini tafuti zijumuishe.
...
Kisha unda hati katika [EdgeJS ya makali](//console.bce.baidu.com/cdn/#/cdn/ejs/list) kama ifuatavyo
...
Kwa mfano, [DNS la Huawei](https://www.huaweicloud.com) hutoa uchanganuzi wa kikanda bila malipo, ambao trafiki ya China bara inaweza kupitia Baidu Smart Cloud, na trafiki ya kimataifa inaweza kupitia `cloudflare` .
...
Kwa sababu `R2` `cloudflare` kufikiwa kwa jina la kikoa lililogeuzwa kukufaa, hifadhi ya kitu cha wahusika wengine inahitaji kutumiwa kuweka faili tuli.
...
Ongeza sheria ya ubadilishaji kama inavyoonyeshwa hapa chini, iweke kwanza (ya kwanza ina kipaumbele cha chini):
