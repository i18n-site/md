...
Овај чланак ће поделити имплементацију `i18n.site` чисте фронт-енд технологије претраге. Посетите [i18n.site](//i18n.site)

[Интерактивни интерфејс](//github.com/i18n-site/plugin/tree/main/qy) [за претрагу](//github.com/i18n-site/ie/tree/main/qy) кода /

## Преглед Решења За Претрагу Пуног Текста Без Сервера

За мале веб-сајтове као што су документи/лични блогови који су чисто статични, несумњиво је превише тешко да сами направите позадину за претрагу целог текста, а претрага целог текста без услуга је несумњиво боља тежина.

Постојећа решења за претрагу пуног текста без сервера спадају у две широке категорије.

Један је добављач услуге претраге треће стране сличан [algolia.com](//algolia.com) који обезбеђује компоненте за претрагу пуног текста на фронт-енду.

Такве услуге захтевају плаћање и нису доступне корисницима у континенталној Кини због проблема са усклађеношћу веб локација.
...
Познатије чисте фронт-енд претраге целог текста укључују [lunrjs](https://lunrjs.com) [ ElasticLunr.js ] [https://github.com/weixsong/elasticlunr.js](%E5%9F%BA%E4%BA%8E%60lunrjs%60%E4%BA%8C%E6%AC%A1%E5%BC%80%E5%8F%91) .

`lunrjs` Постоје два начина за прављење индекса, али оба имају своје проблеме.
...
2. Учитавајте документе и правите индексе у ходу
   ...
   Изградња индекса је рачунарски интензиван задатак. Поновна израда индекса сваки пут када му приступите довешће до очигледних кашњења и лошег корисничког искуства.

Поред `lunrjs` , постоје још нека решења за претрагу целог текста, као што је :

[fusejs](https://www.fusejs.io) израчунајте сличност између низова за претрагу.

Перформансе овог решења су изузетно лоше и не могу се користити за претрагу целог текста (погледајте [Fuse.js Дуг упит траје више од 10 секунди, како га оптимизовати?](https://stackoverflow.com/questions/70984437/fuse-js-takes-10-seconds-with-semi-long-queries) ).

[TinySearch](https://github.com/tinysearch/tinysearch) користите Блум филтер за претрагу, не може се користити за претрагу префикса (на пример, унесите `goo` , претражите `good` , `google` ) и не може постићи сличан ефекат аутоматског завршетка.

Због незадовољства недостацима постојећих решења, `i18n.site` је развио ново чисто фронт-енд решење за претрагу целог текста, које има следеће карактеристике :
...
* `/\p{P}/` је регуларни израз који `! " # $ % & ' ( ) * + , - . / : ; < = > ? @ [ \ ] ^ _ ` знаковима интерпункције { | } . `.</p><ul><li> `split('.')` је зато што `Firefox` сегментирање речи претраживача не сегментира `.` .</li>
...
У фронт-енд интеракцији, трака [напретка](https://dev.to/i18n-site/a-single-progress-uses-pure-css-to-achieve-animation-effects-2oo) [учитавања](https://juejin.cn/post/7413586285954154522) индекса може да се прикаже да би се / кашњење при учитавању по css progress + .

### ИндекедДБ Високо Истовремено Писање

Пројекат је [idb](https://www.npmjs.com/package/idb) на основу асинхроне енкапсулације IndexedDB
...
Преокренута претрага целог текста имплементирана искључиво на предњем крају, са брзим одговором и без потребе за сервером.

Веома је погодан за мале и средње веб странице као што су документи и лични блогови.