...
[i18n.site/payBill](//i18n.site/payBill) , потребно је да повежете кредитну картицу за плаћање (није потребна допуна, веб локација ће аутоматски одбити накнаде према употреби, [погледајте почетну страницу за цене](/#price) ).

## Користити

### Демо Пројекат

Молимо погледајте демо пројекат да научите конфигурацију `i18` превода [github.com/i18n-site/demo.i18](//github.com/i18n-site/demo.i18)
...
Различити `fromTo` могу се конфигурисати за различите поддиректорије / Демонстрација је написана на следећи начин :
...
У овој конфигурационој табели, изворни језик за превод директоријума `blog` је `zh` , а изворни језик за превод директоријума `blog/your_file_name.md` је `ja` .

### Игнорисати Датотеку

Подразумевано, све датотеке које почињу са `.md` и `.yml` у директоријуму изворног језика биће преведене.

Ако желите да занемарите одређене датотеке и да их не преводите (као што су недовршени нацрти), можете користити конфигурацију поља `ignore` .
