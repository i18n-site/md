...
Подржава превод HTML Markdown , садржај у таговима `<pre>` и `<code>` од HTML ембеддед у `MarkDown` није преведен
...
Подржава конфигурацију [HOGO](https://github.com/gohugoio/hugo) статичног блога, који преводи само поља `title` , `summary` и `description` .

## Немојте Преводити Имена Променљивих

`Markdown` се користи као шаблон е-поште, `YAML` се користи као конфигурација језичке датотеке и биће променљиви параметри (на пример: примљена је допуна `${amount}` ).
...
`ファルコンがラマを上回る？ハグ顔ランキングが論争を巻き起こす` на јапански :

![](//p.3ti.site/1720199391.avif)

`Le faucon surpasse le lama ? Le classement Hugging Face suscite la polémique` на француски :
...
`ファルコンがラマに勝利、ハグ顔ランキングが物議を醸す` на јапански :
