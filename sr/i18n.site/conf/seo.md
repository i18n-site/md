...
Узмите конфигурациону датотеку `.i18n/htm/ol.yml` у демо пројекту као пример
...
Прво измените вредност `host:` изнад у име вашег домена, као што је `i18n.site` .
...
У конфигурацији, промените `i18n.site` у вредност `host:` у `.i18n/htm/ol.yml` , више складишта објеката се може конфигурисати под `s3` , а поље `region` је опционо (многа складишта објеката не морају да постављају ово поље).
...
Затим креирајте скрипту у [EdgeJS Едге](//console.bce.baidu.com/cdn/#/cdn/ejs/list) на следећи начин
...
Пошто `R2` `cloudflare` приступити преко прилагођеног имена домена, потребно је користити складиште објеката треће стране за постављање статичких датотека.

`cloudflare` узимамо [backblaze.com](https://www.backblaze.com)
