...
### `use: Toc` , Шаблон За Једну Датотеку Са Обрисом
...
### `use: Doc` , Шаблон Документа Са Више Датотека
...
Горњи метод писања је еквивалентан `url: xxx` , а његова одговарајућа датотека је `en/xxx/TOC` .

#### Индекс Садржаја ТОЦ-А

`i18n.site` ће извршити `js` додатак `.i18n/hook/after.tran/TOC.js` у демо складишту да би прочитао индексну датотеку `TOC` директоријума која одговара конфигурацији шаблона `doc` да би генерисала скицу `json` директоријума.
...
На пример, ако се приступи `/test` , а `nav:` је конфигурисан без префикса ове путање, а тренутни језик прегледања је енглески (код `en` ), `/en/test.md` ће се подразумевано учитати и приказати помоћу шаблона `Md` .
