...
Узимајући демо пројекат као пример, `.i18n/htm/foot.pug` у директоријуму `md` дефинише подножје веб странице.
...
**Немојте писати `css` и `js` у `foot.pug`** , иначе ће доћи до грешке.

Молимо погледајте следеће, стил је уписан у одговарајућу `.css` , а интеракција се постиже креирањем веб компоненти.
...
Додавање `class="a"` линку спречава конвертовање везе у `MarkDown` Видите :
 [➔ `YAML` Како спречити да се линк `HTML` конвертује `Markdown` :](/i18/qa#H2) !