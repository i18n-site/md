...
`i18n.site` এর প্রোফাইল হল `.i18n/conf.yml` ।

[`i18`](/i18) , `ignore:` এবং `i18n:` এর সেটিংস ব্যতীত, কনফিগারেশন ফাইলটি নিম্নরূপ:
...
`nav:` কনফিগারেশন বিকল্প, হোম পেজের শীর্ষে নেভিগেশন মেনুর সাথে সম্পর্কিত।

<img src="//p.3ti.site/1721051426.avif" style="width:320px">

তাদের মধ্যে, `i18n: home` `en/i18n.yml` এর মধ্যে `home: Home` এর সাথে মিলে যায়।

`en/i18n.yml` একাধিক ভাষায় অনুবাদ করা হবে, যেমন `zh/i18n.yml` ।
...
`url:` `Markdown` এর ফাইল পাথকে প্রতিনিধিত্ব করে ( `/` রুট ডিরেক্টরি `/README.md` এর সাথে মিলে যায়, এই ফাইলের নামের জন্য একটি বড় হাতের উপসর্গ এবং একটি ছোট হাতের প্রত্যয় প্রয়োজন)।
...
যদি একটি নির্দিষ্ট পাথ অ্যাক্সেস করা হয় এবং এর পাথ উপসর্গটি `nav:` তে কনফিগার করা না হয়, তবে পাথের সাথে সম্পর্কিত `MarkDown` ফাইল ডিফল্টরূপে লোড হবে এবং `Md` টেমপ্লেট ব্যবহার করে রেন্ডার করা হবে।

উদাহরণস্বরূপ, যদি `/test` অ্যাক্সেস করা হয়, এবং `nav:` এই পথ ছাড়াই কনফিগার করা হয়, এবং পৃষ্ঠার ভাষা ইংরেজি (কোড `en` ), `/en/test.md` ডিফল্টরূপে লোড হবে এবং টেমপ্লেট `Md` ব্যবহার করে রেন্ডার করা হবে।
...
উপরের `blog` হল `Doc` এর একক-আইটেম মোড।
...
উপরের লেখার পদ্ধতিটি `url: blog` এর সমতুল্য, এবং এর সংশ্লিষ্ট ফাইলটি `en/blog/TOC` ।

#### একাধিক প্রকল্প

`.i18n/conf.yml` এর মধ্যে `i18n:doc` এর কনফিগারেশন হল মাল্টি-প্রজেক্ট মোড।
...
দ্রষ্টব্য : ** `demo1,demo2` এর মধ্যে `,` কমা এর আগে এবং পরে কোন স্পেস ** উচিত নয়।
...
`json` ডিরেক্টরী আউটলাইন তৈরি করতে `TOC` টেমপ্লেট কনফিগারেশনের সাথে সম্পর্কিত `doc` ডিরেক্টরি সূচক ফাইলটি পড়ার জন্য `i18n.site` ডেমো গুদামে `js` প্লাগ-ইন `.i18n/hook/after.tran/TOC.js` চালাবে।

আপনি যদি `doc` টেমপ্লেট ব্যবহার করেন তবে আপনার অবশ্যই এই প্লাগ-ইন থাকতে হবে।

আপনি যদি একটি খালি ফোল্ডার থেকে প্রকল্প `i18n.site` শুরু করেন, তাহলে আপনার ডিরেক্টরিতে ডেমো প্রকল্প `.i18n` অনুলিপি করতে ভুলবেন না।

`Doc` টেমপ্লেট উত্পন্ন `json` এর উপর ভিত্তি করে বিষয়বস্তুর সারণী রেন্ডার করবে।
...
`news/README.md` `News` সাথে মিলে যায়,
`news/begin.md` `Our Product is Online !` এর সাথে মিলে যায়

`TOC` ফাইলগুলি আউটলাইনের অনুক্রমিক সম্পর্ক নির্দেশ করতে ইন্ডেন্ট করা হয় এবং মাল্টি-লেভেল ইন্ডেন্টেশন সমর্থন করে।
...
এটি `en/demo2/README.md` প্রথম সারির সাথে মিলে যায় :
