<h1 style="display:flex;justify-content:space-between">⋅ Uluslararası çözümler i18n.site<img src="//p.3ti.site/logo.svg" style="user-select:none;margin-top:-1px;width:42px"></h1>
...
## <a rel=id href="#project" id="project"></a> Proje Açıklaması
...
`Markdown` ve `YAML` birden çok dilde bir komut satırı aracına ( [kaynak kodu](https://github.com/i18n-site/rust/tree/main/i18) ) çevirin.

Formatını mükemmel bir şekilde koruyabilir `Markdown` Dosya değişikliklerini tanımlayabilir ve yalnızca değiştirilen dosyaları çevirebilir.
...
Her Şey Bir Dosya " `UNIX` " paylaşıyoruz ve karmaşık ve hantal kurumsal düzeyde çözümler sunmadan yüzlerce dildeki çevirileri yönetebiliyoruz.

Çeviriyi `Markdown` için en tanıdık araçları kullanabilirsiniz (ancak paragraf ekleyemez veya silemezsiniz) ve sürüm kontrolü yapmak için en bilinen yöntemi kullanabilirsiniz.

Dil dosyaları için açık kaynak olarak bir kod tabanı oluşturulabilir ve `Pull Request` 'nin sürecinin yardımıyla küresel kullanıcılar çevirilerin sürekli optimizasyonuna katılabilir. Kusursuz bağlantı [github](//github.com) Ve diğer açık kaynak toplulukları.
...
Tamamen statik, okuma deneyimi için optimize edilmiş ve [i18](#i18) çevirisiyle entegre edilmiş bir proje belge sitesi oluşturmak için en iyi seçimdir.
...
Bu web sitesi bu çerçeve temel alınarak geliştirilmiştir ve kullanıcı, ödeme ve diğer işlevleri ( [kaynak kodu](/i18n.site/src) ) içermektedir. Daha sonra ayrıntılı bir eğitim yazılacaktır.

[→ Kullanım kılavuzu için lütfen proje belgelerini okuyun](/i18n.site) .
