...
Karışık HTML Markdown çevirisini destekler, `MarkDown` gömülü HTML ın `<pre>` ve `<code>` etiketlerindeki içerik çevrilmez
...
`i18` , `MarkDown` başlığı tanıyabilir ve büyük/küçük harfe duyarlı bir dile çeviri yaparken ilk harfi otomatik olarak büyük harfe çevirir.
...
Çince, Japonca ve Korece dillerinin makine çevirisi İngilizce dışında bir dil haline gelmiştir ve aşağıdaki Çince cümle gibi terimler sıklıkla birlikte çevrilmektedir:
...
DeepL Yukarıdakileri Fransızcaya çevirin (ayrıca özel adları yeniden yazıp tek rakamı `... ` ekleyerek):
