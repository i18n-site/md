...
`i18n.site` yerleşik `i18` çeviri aracına sahiptir. [Erişim belirtecini yapılandırmak üzere `i18` belgesine başvurmak için lütfen buraya tıklayın](/i18/use) .
...
Çalıştırdıktan sonra çeviri ve önbellek dosyaları oluşturulacaktır. Lütfen `git add . ` `md` dizinindeki depoya eklemeyi unutmayın.
...
`S3` sunucusunda oturum açmak için [rclone](https://rclone.org) 'yi kullanabilir, ardından aşağıdaki komut dosyasına başvurabilir ve değiştirebilirsiniz ve artımlı değişiklikleri yalnızca her sürüm için `S3` kopyalayabilirsiniz.
...
[Bir organizasyon oluşturmak için öncelikle burayı github](https://github.com/account/organizations/new?plan=free) . Örnek olarak aşağıdaki organizasyon adı `i18n-demo` .
...
Alan adını [Alibaba CDN](https://cdn.console.aliyun.com/domain/list) ekleyin ve Alibaba Cloud `CDN` alan adını `*.xxx.com` ile `CNAME` arasında işaretleyin.
...
Web sitesinin statik dosyaları, örneğin `favicon.ico`, `robots.txt` vb.

Buradaki simge dosyaları [realfavicongenerator.net](https://realfavicongenerator.net) ile oluşturulabilir.

#### `.i18n`

`.i18n` dizini altında `i18n.site` yapılandırma dosyaları ve çeviri önbelleği gibi unsurlar bulunur. Daha fazla bilgi için bir sonraki bölüm olan ["Yapılandırma"](/i18n.site/conf)'ya bakabilirsiniz.

#### `en`

`.i18n/conf.yml` yapılandırma dosyasında belirtilen `fromTo` değeri `en`'e karşılık gelen kaynak dil dizinidir
...
Çeviri yapılandırması hakkında bilgi için lütfen [i18](/i18/use) sayfasına bakın.