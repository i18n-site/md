...
Statik sayfalarda, bu sayfanın : gibi farklı dil sürümlerine olan bağlantıları belirtmek için `link` kullanın.
...
Örnek olarak demo projedeki `.i18n/htm/ol.yml` konfigürasyon dosyasını alın
...
Lütfen öncelikle yukarıdaki `host:` değerini alan adınızla değiştirin ( `i18n.site` gibi).
...
Yapılandırmada, lütfen `i18n.site` `host:` `.i18n/htm/ol.yml` değeri olarak değiştirin, `s3` altında birden fazla nesne deposu yapılandırılabilir ve `region` alanı isteğe bağlıdır (birçok nesne deposunun bu alanı ayarlamasına gerek yoktur).
