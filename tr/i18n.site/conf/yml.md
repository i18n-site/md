...
`i18n.site` profili `.i18n/conf.yml` dosyasındadır.

[`i18`](/i18) , `ignore:` ve `i18n:` ayarları dışında, konfigürasyon dosyası aşağıdaki gibi olacaktır:
...
`upload` ayarındaki `ext:` seçeneği, yayınlama sırasında sadece `.md` dosyalarının yüklenmesini ifade eder.
...
Bunlar arasında `i18n: home` , `en/i18n.yml` dosyasındaki `home: Home` karşılık gelir.

`en/i18n.yml` gibi birden fazla dile çevrilecektir, örneğin `zh/i18n.yml`.

<img src="//p.3ti.site/1721051689.avif" style="width:320px">

Çeviri tamamlandıktan sonra, çevirinin `yml` değerlerini değiştirebilirsiniz, ancak çevirinin `yml` anahtarlarını eklemeyin veya silmeyin.
...
`use: Toc` , tek bir `Markdown` dosyasını oluşturan `Toc` şablonunu kullanarak oluşturma anlamına gelir.

`TOC` , `Table of Contents` kısaltmasıdır. Bu şablon oluşturulduğunda, bu `Markdown` dosyanın içeriği kenar çubuğunda görüntülenecektir.
...
Belirli bir yola erişilirse ve bu yol `nav:` yapılandırılmamışsa, yola karşılık gelen `MarkDown` dosyası varsayılan olarak yüklenecek ve `Md` şablonu kullanılarak oluşturulacaktır.

Örneğin, `/test` erişilirse ve `nav:` bu yol için yapılandırılmamışsa ve sayfa dili İngilizce ise (kod `en` ), varsayılan olarak `/en/test.md` yüklenecek ve şablon `Md` kullanılarak oluşturulacaktır.
...
`Doc` şablonu, tek veya birden fazla proje için belge ana hatları oluşturmak üzere birden fazla `MarkDown` dosyasını entegre etmeyi destekler.

#### Tek Proje (Birden Fazla Dosya)

Yukarıdaki `blog` örneği, `Doc` tek öğe modudur.
...
Yukarıdaki yazma yöntemi `url: blog` ile eşdeğerdir ve karşılık gelen dosyası `en/blog/TOC` olacaktır.

#### Birden Fazla Proje

`.i18n/conf.yml` dosyasındaki `i18n:doc` ayarı çoklu proje modudur.
...
`i18n.site` `json` dizin taslağını oluşturmak için `TOC` şablon yapılandırmasına karşılık gelen `doc` dizin indeks dosyasını okumak için demo deposunda `js` eklenti `.i18n/hook/after.tran/TOC.js` çalıştıracaktır.

`doc` şablon kullanıyorsanız bu eklentiye sahip olmanız gerekir.

Proje `i18n.site` boş bir klasörden başlatırsanız demo proje `.i18n` dizininizi kopyalamayı unutmayın.
...
`TOC` dosyaları, anahattın hiyerarşik ilişkisini belirtmek ve çok düzeyli girintiyi desteklemek için girintilidir.
...
İçerik `en/demo2/README.md` gibi `README.md` maddesine yazılabilir.
