...
Tampilan Model CPU `AMD EPYC 9224 24-Core Processor` skor kinerja inti tunggal 1285
...
Pangguna anyar kudu ngirim kertu ID kanggo otentikasi jeneng asli nalika tuku.

## Konfigurasi Login Tanpa Sandhi

Sawise sawetara menit, server kabeh diaktifake.

Sing pertama yaiku nyunting mesin lokal `~/.ssh/config` Setel alias kanggo IP banjur gunakake `ssh-copy-id root@c2` kanggo ngatur login tanpa sandhi.
...
Banjur upgrade sistem menyang ubuntu 24 kanthi siji klik lan nginstal piranti lunak sing umum digunakake.
...
Langkah iki mbutuhake wektu suwe Sawise ngenteni setengah jam, upgrade lan instalasi piranti lunak sing umum digunakake bakal sukses. Sampeyan bisa konfirmasi kanthi printah ing ngisor iki:
...
Jaringan saka Eropa menyang China ora stabil Disaranake `ssh` [mosh](//mosh.org)

Gawe skrip `c0` ing `~/.bin` mesin lokal (utamane ditambahake ing variabel lingkungan `PATH` ) kaya ing ngisor iki:
...
Sawise nggawe, wenehi ijin sing bisa dieksekusi lan tautan alus `c1` `c2` .
...
Ing mangsa ngarep, sampeyan bisa langsung ngetik `c0` , `c1` lan sampeyan bisa nggunakake `mosh` dhuwur-kacepetan langsung sambungan lan ngetik [tmux](https://www.ruanyifeng.com/blog/2019/10/tmux.html)
...
Owah-owahan sidik driji bakal kedadeyan nalika ssh , `ssh-keygen -R ip` kanggo ngresiki
...
Bakal kanthi otomatis ngompres kanthi standar Printah kanggo mriksa rasio kompresi yaiku:
...
Gunakake `snapper list-configs` kanggo ndeleng subvolume sing wis disetel.

<img alt="" src="https://i-01.eu.org/1718302689.avif">

Gunakake `snapper -c mnt list` kanggo ndeleng snapshot saka subvolume

`/etc/snapper/configs/mnt` Sampeyan bisa ndeleng setelan serep khusus saka subvolume iki
...
Elinga yen operasi ing ndhuwur ditindakake ing sistem ngluwari, mula owah-owahan kasebut mung bakal ditrapake yen sistem urip maneh lan ora ana maneh ing mode ngluwari.

Iki tegese sampeyan ora bakal langsung ndeleng owah-owahan ing log sing ana dadi terus-terusan, nanging kabeh log sing digawe sawise iki bakal tetep.
...
Aku nggunakake aksi github kanggo nggawe kompilasi otomatis lan skrip panyebaran sadurunge, nanging nemokake yen layanan penyebaran ora bisa ssh.

Mlebet sistem ngluwari lan ndeleng log minangka masalah ijin:

`Missing privilege separation directory: /run/sshd`

Sampeyan bisa nggawe direktori iki nganggo printah ing ngisor iki
...
Sawise diselidiki bola-bali, amarga pangguna sing ngunggah tar `uid` `1001` github action

tar Unpacking kudu rampung tanpa paramèter `--no-same-owner` iku bakal decompressed menyang `uid` kanggo `1001` pangguna
