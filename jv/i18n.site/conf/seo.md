...
Njupuk file konfigurasi `.i18n/htm/ol.yml` ing proyek demo minangka conto
...
Mangga ngowahi nilai `host:` ing ndhuwur dadi jeneng domain sampeyan, kayata `i18n.site` .
...
Ing konfigurasi, mangga ngganti `i18n.site` kanggo nilai `host:` ing `.i18n/htm/ol.yml` , macem-macem toko obyek bisa diatur ing `s3` , lan lapangan `region` opsional (akeh toko obyek ora perlu nyetel lapangan iki).
...
`URL redirect` Pilih redirection dinamis, mangga ngowahi `/en` ing path redirection `concat("/en",http.request.uri.path,".htm")` menyang basa standar sing pengin dilebokake mesin telusur.
...
Banjur gawe skrip ing [EdgeJS pinggiran](//console.bce.baidu.com/cdn/#/cdn/ejs/list) kaya ing ngisor iki
...
Contone, [Huawei Cloud DNS](https://www.huaweicloud.com) nyedhiyakake analisa regional gratis, kanthi lalu lintas Cina daratan bisa ngliwati Baidu Smart Cloud, lan lalu lintas internasional bisa ngliwati `cloudflare` .
...
Amarga `R2` `cloudflare` diakses kanthi jeneng domain khusus, panyimpenan obyek pihak katelu kudu digunakake kanggo nyelehake file statis.
