...
Amarga rekaman `MX` lan rekaman `CNAME` ora bisa urip `CNAME` , yen sampeyan pengin `A` email jeneng domain ing wektu sing padha, sampeyan kudu kerja sama karo skrip [cname_flatten](https://github.com/i18n-site/lib/tree/main/cname_flatten)
...
Yen sampeyan nggunakake `i18n.site` kanggo ngasilake situs web minangka situs web utama, sampeyan biasane kudu ngonfigurasi pangalihan pan-domain, yaiku, pangalihan akses menyang `*.xxx.com` (kalebu `www.xxx.com` ) menyang `xxx.com` .
...
Mangga tambahake konfigurasi sing padha karo ing ngisor `/root/i18n/md/out/ol/htm` ing paragraf `server` saka nginx `out/ol/htm`
